...
* **merge\_entity()：**透過將新的屬性值合併到現有實體來更新現有實體。
* **insert\_or\_merge\_entity()：**透過取代現有實體來進行更新。如果實體不存在，將會插入新的實體：
* **insert\_or\_replace\_entity()：**透過將新的屬性值合併到現有實體來更新現有實體。如果實體不存在，將會插入新的實體。
