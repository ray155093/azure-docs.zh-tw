...
1. 若要編譯及建置應用程式，您必須先在電腦上安裝 [Visual Studio 2015](https://www.visualstudio.com/) 或更新版本。
2. 安裝最新版的 [Azure SDK for .NET](https://azure.microsoft.com/downloads/)。 此 SDK 包含 Azure 快速入門範例專案、Azure 儲存體模擬器和 [Azure Storage Client Library for .NET](https://msdn.microsoft.com/library/azure/dn261237.aspx)。
...
1. 在 Visual Studio 中，選取 [檢視] 功能表上的 [方案總管]。 開啟 **App.config** 檔案，並將 Azure 儲存體模擬器的連接字串標記為註解：

   `<!--<add key="StorageConnectionString" value = "UseDevelopmentStorage=true;"/>-->`

2. 在 App.config 檔案中，取消註解 Azure 儲存體服務的連接字串，並使用您的帳戶認證來取代 `[AccountName]` 和 `[AccountKey]`：
