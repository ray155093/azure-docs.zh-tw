...
#### 方案
若要解決缺少次要索引的問題，您可以為每個實體儲存多個複本，且每個複本分別使用不同 **RowKey** 值。如果您使用如下所示的結構來儲存實體，就能夠根據電子郵件地址或員工識別碼，有效率地擷取員工實體。前置詞值 **RowKey**、"empid\_" 和 "email\_" 可讓您使用一組電子郵件地址或員工識別碼，來查詢單一員工或一組員工。
...
#### 方案  
若要解決缺少次要索引的問題，您可以為每個實體儲存多個複本，且每個複本分別使用不同的 **PartitionKey** 和 **RowKey** 值。如果您使用如下所示的結構來儲存實體，就能夠根據電子郵件地址或員工識別碼，有效率地擷取員工實體。前置詞值 **PartitionKey**、"empid\_" 和 "email\_" 可讓您識別您想要用於查詢的索引。
...
如需搭配使用 SAS 權杖與表格服務的詳細資訊，請參閱[共用存取簽章第 1 部分：了解 SAS 模型](../storage-dotnet-shared-access-signature-part-1/)。
