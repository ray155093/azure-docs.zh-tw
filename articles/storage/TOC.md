# Overview

## [Introduction](storage-introduction.md) 
## [Get started in five minutes ](storage-getting-started-guide.md)

# Get Started

## [Create a storage account](storage-create-storage-account.md)

## Blob Storage

### [.NET](storage-dotnet-how-to-use-blobs.md)
### [Java](storage-java-how-to-use-blob-storage.md)
### [Node.js](storage-nodejs-how-to-use-blob-storage.md)
### [C++](storage-c-plus-plus-how-to-use-blobs.md)
### [Python](storage-python-how-to-use-blob-storage.md)
### [PHP](storage-php-how-to-use-blobs.md)
### [Ruby](storage-ruby-how-to-use-blob-storage.md)
### [iOS](storage-ios-how-to-use-blob-storage.md)
### [Xamarin](storage-xamarin-blob-storage.md)

## File Storage
### [Windows, .NET, PowerShell](storage-dotnet-how-to-use-files.md)
### [Linux](storage-how-to-use-files-linux.md)
### [Java](storage-java-how-to-use-file-storage.md)
### [C++](storage-c-plus-plus-how-to-use-files.md)
### [Python](storage-python-how-to-use-file-storage.md)

## Queue Storage
### [.NET](storage-dotnet-how-to-use-queues.md)
### [Java](storage-java-how-to-use-queue-storage.md)
### [Node.js](storage-nodejs-how-to-use-queues.md)
### [C++](storage-c-plus-plus-how-to-use-queues.md)
### [Python](storage-python-how-to-use-queue-storage.md)
### [PHP](storage-php-how-to-use-queues.md)
### [Ruby](storage-ruby-how-to-use-queue-storage.md)

## Table Storage
### [.NET](storage-dotnet-how-to-use-tables.md)
### [Java](storage-java-how-to-use-table-storage.md)
### [Node.js](storage-nodejs-how-to-use-table-storage.md)
### [C++](storage-c-plus-plus-how-to-use-tables.md)
### [Python](storage-python-how-to-use-table-storage.md)
### [PHP](storage-php-how-to-use-table-storage.md)
### [Ruby](storage-ruby-how-to-use-table-storage.md)

# How To

## Premium Storage

### [High-Performance Storage for VM Workloads](storage-premium-storage.md)
### [Migrating to Premium Storage](storage-migration-to-premium-storage.md)
### [Design for High Performance](storage-premium-storage-performance.md)

## Develop with Storage
### [Azure Storage Samples](storage-samples.md)
### [Configure Connection Strings](storage-configure-connection-string.md)
### [Set and Retrieve Properties and Metadata ](storage-properties-metadata.md)
### [List Azure Storage Resources in C++](storage-c-plus-plus-enumeration.md)
### [Use the storage emulator](storage-use-emulator.md)
### [Hudson Continuous Integration solution](storage-java-hudson-continuous-integration-solution.md)
### [Jenkins Continuous Integration solution](storage-java-jenkins-continuous-integration-solution.md)
### [Use Azure Storage in Windows Store apps](storage-use-store-apps.md)
### Azure Storage Explorer
#### [Storage Explorer (Preview)](../vs-azure-tools-storage-manage-with-storage-explorer.md)
#### [Manage blobs with Storage Explorer (Preview)](../vs-azure-tools-storage-explorer-blobs.md)
### [Azure Storage Client Tools](storage-explorers.md)

## Manage Storage
### [Configure a custom domain name for your Blob storage endpoint](storage-custom-domain-name.md)
### [Using Azure PowerShell](storage-powershell-guide-full.md)
### [Using the Azure CLI](storage-azure-cli.md)
### [Using Azure Automation](automation-manage-storage.md)

## Performance & Availability
### [Replication](storage-redundancy.md)
### [Storage Analytics](storage-analytics.md)
### [Table Design Guide](storage-table-design-guide.md)
### [Scalability and Performance Targets](storage-scalability-targets.md)
### [Performance and Scalability Checklist](storage-performance-checklist.md)
### [Managing Concurrency](storage-concurrency.md)
### [What to do if an Azure Storage outage occurs](storage-disaster-recovery-guidance.md)
### [Monitor a storage account](storage-monitor-storage-account.md)
### [Monitor, diagnose, and troubleshoot](storage-monitoring-diagnosing-troubleshooting.md)
### [End-to-end troubleshooting tutorial](storage-e2e-troubleshooting.md)
### [Enabling and viewing metrics data](storage-enable-and-view-metrics.md)
### [Troubleshoot errors deleting storage accounts, containers, or VHDs in a Resource Manager deployment](storage-resource-manager-cannot-delete-storage-account-container-vhd.md)
### [Troubleshoot errors deleting Azure storage accounts, containers, or VHDs in a classic deployment](storage-cannot-delete-storage-account-container-vhd.md)
### [Troubleshooting Azure File storage problems](storage-troubleshoot-file-connection-problems.md)

## Blob Storage

### [Hot and cool storage tiers](storage-blob-storage-tiers.md)
### [Create a blob snapshot](storage-blob-snapshots.md)
### [Cross-Origin Resource Sharing (CORS)](storage-cors-support.md)
### [On-premises application](storage-java-use-blob-storage-on-premises-app.md)

## Disks for IaaS VMs
### [Back up Azure VM disks with incremental snapshots](storage-incremental-snapshots.md)

## Security

### [Storage security guide](storage-security-guide.md)
### [Azure Storage Service Encryption for Data at Rest](storage-service-encryption.md)
### [Manage anonymous read access to containers and blobs](storage-manage-access-to-resources.md)
### [Shared access signatures (SAS)](storage-dotnet-shared-access-signature-part-1.md)
### [Using shared access signatures with Blob storage](storage-dotnet-shared-access-signature-part-2.md)
### Client-Side Encryption
#### [With Azure Key Vault](storage-client-side-encryption.md)
#### [Java](storage-client-side-encryption-java.md)
#### [Python](storage-client-side-encryption-python.md)
#### [Tutorial: Encrypt and decrypt blobs using Azure Key Vault](storage-encrypt-decrypt-blobs-key-vault.md)

## Transfer Data to Storage
### [Moving data to and from Storage](storage-moving-data.md)
### [Transfer data with the AzCopy Command-Line Utility](storage-use-azcopy.md)
### [Import/Export Service for Blob storage](storage-import-export-service.md)

## Classic Portal
### [Using the Classic Portal](storage-create-storage-account-classic-portal.md)
### [Monitor, diagnose, and troubleshoot using the Classic Portal](storage-monitoring-diagnosing-troubleshooting-classic-portal.md)
### [End-to-End Troubleshooting using Metrics and Logging, AzCopy, and Message Analyzer using Classic Portal](storage-e2e-troubleshooting-classic-portal.md)
### [Enabling Azure Storage metrics and viewing metrics data using the Classic Portal](storage-enable-and-view-metrics-classic-portal.md)

