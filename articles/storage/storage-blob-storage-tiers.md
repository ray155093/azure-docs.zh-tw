...
上述每個資料存取案例均受惠於針對特定存取模式最佳化的差異化儲存層。Azure Blob 儲存體現可透過各種價格模式，滿足差異化儲存層的這項需求。
...
4. 選取 [Resource Manager] 做為部署模型。
	...
	分層式儲存體僅適用於 Resource Manager 儲存體帳戶；這是新資源的建議部署模型。如需詳細資訊，請參閱 [Azure Resource Manager 概觀](../resource-group-overview.md)。
...
7. 選取存取層︰[非經常性存取] 或 [經常性存取]。預設值為 [經常性存取]。
