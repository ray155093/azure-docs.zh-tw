<properties
   pageTitle="在新的或現有的應用程式閘道上設定 Web 應用程式防火牆 | Microsoft Azure"
...
設定 Web 應用程式防火牆 SKU。可用大小為 **WAF\_Large** 和 **WAF\_Medium**。使用 Web 應用程式時，層級必須是 **WAF**。
...
> [AZURE.NOTE] 應用程式的子網路至少應有 28 個遮罩位元。此值會讓應用程式閘道執行個體的子網路中留有 10 個可用位址。若使用較小的子網路，您可能無法在日後為應用程式閘道新增更多執行個體。
...
使用[建立資源群組](#create-the-resource-group)步驟所建立之資源群組中的上述子網路建立虛擬網路
...
>[AZURE.NOTE]  您可以選擇 **WAF\_Medium** 或 **WAF\_Large**，使用 WAF 時的層級永遠是 **WAF**。容量是介於 1 到 10 之間的任何數字。
