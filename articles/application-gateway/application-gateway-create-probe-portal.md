...
瀏覽到 http://portal.azure.com，然後選取現有的應用程式閘道。
...
* **主機** - 這是用於探查的主機名稱。 只有當應用程式閘道上設定多站台時適用，否則請使用 '127.0.0.1'。 這與 VM 主機名稱不同。
...
> [!IMPORTANT]
> 主機名稱不是伺服器名稱。 這是在應用程式伺服器上執行的虛擬主機名稱。 探查會傳送到 http://(host name):(port from httpsetting)/urlPath
> 
> 
