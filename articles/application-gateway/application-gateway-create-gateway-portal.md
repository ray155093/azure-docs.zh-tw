<properties
   pageTitle="使用入口網站建立應用程式閘道 | Microsoft Azure"
...
- **名稱** - 應用程式閘道的名稱。
- **SKU 大小** - 這是應用程式閘道的大小，可用的選項為 (小型、中型及大型)。
- **執行個體計數** - 執行個體數目，此值應該是介於 2 到 10 之間的數字。
- **資源群組** - 保存應用程式閘道的資源群組，這可以是現有的資源群組或新的資源群組。
- **位置** - 應用程式閘道的區域，這是與資源群組所在相同的位置。這一點相當重要，因為虛擬網路和公用 IP 必須與閘道位於相同位置。
...
定義基本設定之後，下一步是定義要使用的虛擬網路。虛擬網路將裝載應用程式閘道為其進行負載平衡的應用程式。
...
建立應用程式閘道時，所要設定的最後一項設定就是接聽程式組態。如果使用的是 **http**，則沒有其他要設定的剩餘項目，您可以按一下 [確定]。若要利用 **https**，則需要設定進一步的組態。

若要使用 **https**，必須提供憑證。需要有憑證的私密金鑰，因此必須提供憑證的 .pfx 匯出及檔案的密碼。


![[設定] 刀鋒視窗上空白的 [接聽程式組態] 區段][8]
...
這會建立一個具有接聽程式、後端集區、後端 http 設定及規則之預設設定的基本應用程式閘道。佈建成功之後，您可以依據您的部署需求修改這些設定。
