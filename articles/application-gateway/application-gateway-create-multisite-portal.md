...
這個案例會將多網站支援新增至現有應用程式閘道。 若要完成此案例，現有應用程式閘道必須可以設定。 請造訪[使用入口網站建立應用程式閘道](application-gateway-create-gateway-portal.md)，以了解如何在入口網站中建立基本應用程式閘道。
...
1. 建立要用於每個網站的後端集區。
2. 建立每個網站應用程式閘道都會支援的新接聽程式。
3. 建立規則，將每個接聽程式與適當的後端對應。

## <a name="create-back-end-pools-for-each-site"></a>建立每個網站的後端集區
需要應用程式閘道支援的每個網站的後端集區，在此案例中會建立 2 個，一個用於 contoso11.com，另一個用於 fabrikam11.com。
...
如此即完成透過 Azure 入口網站設定具有多網站支援的現有應用程式閘道。
