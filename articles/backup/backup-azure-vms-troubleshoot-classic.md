...
| 備份 | 備份因為內部錯誤而失敗 - 請在幾分鐘內重試此作業。如果問題持續發生，請連絡 Microsoft 支援服務。 | 請檢查存取 VM 儲存體時是否有暫時性的問題。請查看 [Azure Status (Azure 狀態)](https://azure.microsoft.com/zh-TW/status/)，了解該區域目前是否有與計算/儲存體/網路相關的問題。當備份後續問題緩和後，請重試一次。 |
