...
## 效能與資源使用率
如同內部部署的備份軟體一樣，在 Azure 中備份 VM 也需要規劃容量和資源使用率。[Azure 儲存體限制](azure-subscription-service-limits.md#storage-limits)會定義如何建構 VM 部署，以發揮最大效能並盡可能不影響執行中的工作負載。
...
### 儲存體帳戶限制
從客戶儲存體帳戶中複製備份資料時，這些資料會計入儲存體帳戶的每秒輸入/輸出作業 (IOPS) 和輸出 (儲存體輸送量) 度量內。同時，虛擬機器也會執行並耗用 IOPS 和輸送量。目標是要確保總流量 (備份與虛擬機器) 不超過儲存體帳戶限制。
...
## 最佳作法
在設定虛擬機器備份時，我們建議您遵循最佳做法。

- 請避免將超過 4 個來自相同雲端服務的傳統 VM 排程在同一時間內進行備份。對於多個來自相同雲端服務的 VM，如果您想要設定其 VM，建議您錯開備份排程一小時。 
- 請避免將超過 40 個 Resource Manager VM 排程在同一時間內進行備份。
- 請將 VM 備份排程在非尖峰時段，讓備份服務能獲得將資料從客戶儲存體帳戶傳輸到備份保存庫的 IOPS。 
- 請利用原則確保 VM 分散在不同的儲存體帳戶中。如果儲存在單一儲存體帳戶中的 VM 磁碟總數超過 20 個，我們建議您將 VM 分散在不同的備份排程，以便在備份的傳輸階段獲得所需的 IOPS。
- 我們不建議將在進階儲存體上執行的 VM 還原到同一個儲存體帳戶，因為這項操作會影響備份作業，降低備份的可用 IOPS 數目。 
- 建議您將每部進階 VM 保存在不同的進階儲存體帳戶上，以期維持良好的時間效能。 
