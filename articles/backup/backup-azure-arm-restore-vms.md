...
5. 從清單中選取 VM，以開啟儀表板。
    ...
    ![保存庫中的 VM 清單](./media/backup-azure-arm-restore-vms/list-of-vms-in-vault-selected.png)

    VM 儀表板會開啟至包含 [復原點] 圖格的 [監視] 區域。
...
7. 在 [還原] 刀鋒視窗上，按一下 [復原點] 以開啟 [選取復原點] 刀鋒視窗。

    ![還原刀鋒視窗](./media/backup-azure-arm-restore-vms/recovery-point-selector.png)

    根據預設，對話方塊會顯示過去 30 天內的所有復原點。依您的喜好編輯 [期間]、[年] 和 [還原點一致性] 清單。如需每種還原點類型的詳細資訊，請參閱[資料一致性](./backup-azure-vms-introduction.md#data-consistency)的說明。
    - [期間] 是當年的月份，或過去 30 天。
    - [年] 是特定的年份。
    - [還原點一致性] 選擇以下清單︰
        - 當機時保持一致還原點，
...
8. 選擇復原點，然後按一下 [確定]。
    ...
    ![選擇復原點](./media/backup-azure-arm-restore-vms/select-recovery-point.png)

    [還原] 刀鋒視窗會顯示已設定復原點。

    ![已設定復原點](./media/backup-azure-arm-restore-vms/recovery-point-set.png)

9. 在 [還原] 刀鋒視窗上，按一下 [復原組態] 以開啟其刀鋒視窗。

    ![已設定復原組態精靈](./media/backup-azure-arm-restore-vms/recovery-configuration-wizard.png)

## 選擇 VM 復原組態

您現在已選取復原點，請選擇儲存體帳戶以及用於設定您的復原 VM 的方法。設定復原 VM 的選項包括使用︰Azure 入口網站或 PowerShell。在選擇儲存體帳戶時，您必須選擇與復原服務保存庫共用相同位置的帳戶。不支援區域備援的儲存體帳戶。如果沒有與復原服務保存庫相同位置的儲存體帳戶，您必須在開始還原作業之前，建立一個儲存體帳戶。

1. 如果您尚未到達該位置，請移至 [還原] 刀鋒視窗。確定已選取 [復原點]，然後按一下 [復原組態] 以開啟 [復原組態] 刀鋒視窗。

    ![已設定復原組態精靈](./media/backup-azure-arm-restore-vms/recovery-configuration-wizard.png)

2. 在 [復原組態] 刀鋒視窗上，按一下 [儲存體帳戶] 以開啟與復原服務保存庫位於相同位置的儲存體帳戶清單。

3. 從清單中選擇儲存體帳戶，然後按一下 [確定]。

    ![儲存體帳戶清單](./media/backup-azure-arm-restore-vms/list-of-storage-accounts.png)

    在 [復原組態] 刀鋒視窗上，選擇 [儲存體帳戶] 對話方塊中顯示的儲存體帳戶。

    ![選擇儲存體帳戶](./media/backup-azure-arm-restore-vms/selected-storage-account.png)

4. 在入口網站中還原 VM 時，您可以設定基本 ARM 或傳統 VM。如果您想要還原複雜的組態 (例如下列清單中的其中一個選項)，您必須使用 PowerShell 從磁碟設定您的 VM。如果您未設定複雜的組態，請跳至步驟 5。

    ![複雜的 VM 組態清單](./media/backup-azure-arm-restore-vms/complex-vm-configurations.png)

    如果還原組態被視為很複雜，則在 [復原組態] 刀鋒視窗上按一下 [確定]。[復原組態] 刀鋒視窗隨即關閉，且 [復原組態] 旁邊會出現核取記號。按一下 [還原] 開始進行磁碟還原作業。不會繼續進行步驟 5，而是繼續[還原具有特殊網路組態的 VM](#restoring-vms-with-special-network-configurations)。

    ![已設定還原](./media/backup-azure-arm-restore-vms/restore-configured.png)

5. 在 [復原組態] 刀鋒視窗上，按一下 [虛擬機器組態] 以開啟 [建立 VM] 刀鋒視窗。

    ![開啟 [建立 VM] 刀鋒視窗](./media/backup-azure-arm-restore-vms/recovery-configuration-create-vm.png)

6. 在 [建立 VM] 刀鋒視窗上，輸入或選取下列各欄位的值︰

    - **虛擬機器名稱** - 提供 VM 的名稱。對資源群組 (適用於 ARM VM) 或雲端服務 (適用於傳統 VM) 來說，此名稱必須是唯一的。如果您使用相同的名稱取代現有的 VM，請先刪除現有的 VM 和資料磁碟，然後從 Azure 備份還原資料。
    ...
    - **虛擬網路** -建立 VM 時，請選取虛擬網路 (VNET)。此欄位提供與訂用帳戶相關聯的所有 VNET。

    > [AZURE.NOTE] 您必須在還原 ARM 架構的 VM 時選取 VNET。對傳統 VM 而言，VNET 是選擇性的。

    - **子網路** - 如果 VNET 有子網路，預設會選取第一個子網路。如果有其他子網路，請選取所需的子網路。

7. 在 [建立 VM] 刀鋒視窗上，按一下 [確定] 完成設定。

    [建立 VM] 刀鋒視窗隨即關閉，而 [復原組態] 刀鋒視窗會顯示新 VM 的名稱。

    ![已完成復原設定](./media/backup-azure-arm-restore-vms/recovery-configuration-complete.png)

8. 在 [復原組態] 刀鋒視窗上，按一下 [確定] 完成還原設定。

9. 在 [還原] 刀鋒視窗上，按一下 [還原] 以觸發還原作業。

    ![已完成復原設定](./media/backup-azure-arm-restore-vms/trigger-restore-operation.png)

## 追蹤還原作業

一旦觸發還原作業，備份服務會建立一個作業以便追蹤還原作業。備份服務也會建立並暫時顯示通知。

![已觸發還原](./media/backup-azure-arm-restore-vms/restore-triggered.png)

如果看不到通知，您隨時可以按一下 [通知] 圖示來檢視您的通知。

![已觸發還原](./media/backup-azure-arm-restore-vms/restore-notification.png)

若要檢視正在處理的作業，或檢視完成的作業，請開啟 [備份作業] 清單。

1. 在 Azure 功能表上按一下 [瀏覽]，然後在服務清單中輸入**還原服務**。服務清單會調整成您輸入的內容。當您看到 [復原服務保存庫] 時，請選取它。
...
2. 從這份清單，選取與您還原的 VM 相關聯的保存庫。當您按一下保存庫時，其儀表板隨即開啟。
    ...
    ![復原服務保存庫清單](./media/backup-azure-arm-restore-vms/select-vault-open-vault-jobs.png)
...
>[AZURE.IMPORTANT] 建立 VM 的特殊網路組態時，您必須使用 PowerShell 從磁碟還原。


為了在還原至磁碟後，完全重新建立虛擬機器，請依照以下步驟執行：

1. 使用 [Azure 備份 PowerShell](../backup-azure-vms-automation.md#restore-an-azure-vm) 從復原服務保存庫還原磁碟
