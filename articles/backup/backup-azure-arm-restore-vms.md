...
於定義的間隔進行資料快照，來保護您的資料。 這些快照稱為復原點，而且儲存在復原服務保存庫中。 如果或需要修復或重新建立 VM，您可以從任何已儲存的復原點還原 VM。 當您還原復原點時，您會使 VM 回到或恢復到取得復原點時的狀態。 本文說明如何使用還原 VM。
...
> [!NOTE]
> 入口網站提供還原 VM 專用的「快速建立」選項。 如果您想要自訂即將還原之 VM 的 VM 組態，請使用 PowerShell 或入口網站來還原備份的磁碟，並使用 PowerShell 命令將它們附加至所選的 VM 組態。 請參閱 [使用特殊網路組態還原 VM](#restoring-vms-with-special-network-configurations)。
>
>
...
2. 在 [還原組態] 刀鋒視窗上有兩個選擇︰
   ...
   * 還原備份的磁碟
   ...
   ### <a name="restore-full-virtual-machine"></a>還原完整虛擬機器
   在 [還原組態]  刀鋒視窗上，輸入或選取下列各欄位的值︰
   ...
   * **儲存體帳戶** - 此功能表會列出與復原服務保存庫位於相同位置的儲存體帳戶。 選擇儲存體帳戶時，請選取與「復原服務」保存庫共用相同位置的帳戶。 不支援區域備援的儲存體帳戶。 如果沒有與復原服務保存庫相同位置的儲存體帳戶，您必須在開始還原作業之前，建立一個儲存體帳戶。 儲存體帳戶的複寫類型會使用括號註明。
   ...
   如果您想要自訂您從備份的磁碟 (而非還原組態刀鋒視窗中顯示的磁碟) 建立的虛擬機器，請選取 [還原磁碟] 做為 [還原類型] 值。 此選擇會要求一個儲存體帳戶 (相同的限制適用於選取還原完整虛擬機器儲存體帳戶時)，以便將備份中的磁碟複製到其中。 您可以[使用已還原的磁碟來附加至現有的虛擬機器](../virtual-machines/virtual-machines-windows-attach-disk-portal.md)或[從還原的磁碟建立新的虛擬機器。](./backup-azure-vms-automation.md#restore-an-azure-vm)
...
* 如果您使用 cloud-init 型 Linux 散發套件 (例如 Ubuntu)，基於安全理由，還原後會封鎖密碼。 請在還原的 VM 上使用 VMAccess 擴充功能 [重設密碼](../virtual-machines/virtual-machines-linux-classic-reset-access.md)。 建議您在這些散發套件上使用 SSH 金鑰，以避免在還原後重設密碼。
...
* 還原的 VM 不會有可用性設定值組。 建議您使用還原的磁碟從 PowerShell 建立 VM 時，使用還原磁碟選項和[加入可用性設定組](../virtual-machines/virtual-machines-windows-create-availability-set.md#use-powershell-to-create-an-availability-set)。 
