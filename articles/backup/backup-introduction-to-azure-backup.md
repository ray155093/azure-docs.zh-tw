...
Azure 備份可讓您還原具有受控磁碟的完整 VM﹐或將受控磁碟還原到 Resource Manager 儲存體帳戶。 Azure 會在還原程序期間管理受控磁碟。 您 (客戶) 管理在還原程序期間建立的儲存體帳戶。 如需還原受管理的已加密 VM，VM 的金鑰和祕密應在還原之前已經存在於 Key Vault 中。
