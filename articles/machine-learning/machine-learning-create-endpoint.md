<properties 
    pageTitle="在 Machine Learning 中建立 Web 服務端點 | Microsoft Azure" 
...
建立端點之後，您就可以透過同步 API、批次 API 以及 Excel 工作表來取用該端點。 除了透過此 UI 新增端點之外，您也可以使用端點管理 API，以程式設計方式加入端點。 
 ...
 >[AZURE.NOTE] 如果您已在 Web 服務中新增額外的端點，就無法刪除預設端點。
...
[如何使用已發佈的 Azure Machine Learning Web 服務](machine-learning-consume-Web-services.md)。 
