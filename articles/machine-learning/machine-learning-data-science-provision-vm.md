...
>[AZURE.NOTE] 佈建大約 10-20 分鐘。在 Azure 入口網站中會顯示佈建的狀態。
...
>[AZURE.NOTE] 您可能會收到訊息，表示您的評估期間已過期。請輸入 Microsoft 帳戶認證或建立新的免費帳戶，以取得 Visual Studio Community 版本的存取權。
...
>[AZURE.NOTE] SQL Server 2016 Developer Edition 只能用於進行開發和測試。您需要授權，才能在生產環境中執行它。
...
>[AZURE.NOTE] 在 1.5.0 以前建立的 Microsoft 資料科學虛擬機器執行個體 (2016 年 9 月 3 日以前)，使用的目錄結構與上表指定的稍有不同。
