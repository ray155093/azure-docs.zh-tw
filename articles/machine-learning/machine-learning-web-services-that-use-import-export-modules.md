<properties
	pageTitle="部署使用資料匯入和資料匯出模組的 Azure ML Web 服務 | Microsoft Azure"
...
# 部署使用資料匯入和資料匯出模組的 Azure ML Web 服務 
...
「匯入資料」和「匯出資料」模組可以讀取和寫入許多資料位置，例如透過 HTTP 的 Web URL、Hive 查詢、Azure SQL Database、Azure 表格儲存體、Azure Blob 儲存體、資料摘要提供者，或內部部署的 SQL 資料庫。

本主題使用「範例 5︰二進位分類的訓練、測試、評估︰成人資料集」範例，並假設資料集已載入名為 censusdata 的 Azure SQL 資料表。

## 建立訓練實驗 
 
當您開啟「範例 5︰二進位分類的訓練、測試、評估︰成人資料集」範例時，它會使用範例「成人收入普查二進位分類」資料集。畫布中的實驗看起來會類似下列影像。
...
4.	連接「匯入資料」模組的輸出與「清除遺漏資料」模組的輸入。
5.	在屬性窗格中，在 [資料來源] 下拉式清單中選取 [Azure SQL Database]。
