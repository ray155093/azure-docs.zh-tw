<properties
	pageTitle="Machine Learning Studio 中的簡易實驗 | Microsoft Azure"
...
# 機器學習服務教學課程：在 Azure Machine Learning Studio 中建立您的第一個資料科學實驗
...
4. 在 [開始於] 下，按一下 [無資料行]，然後在篩選資料列中選取 [包含] 和 [資料行名稱]。輸入資料行名稱的清單。這會引導模組僅通過我們指定的資料行。
	...
	> [AZURE.TIP] 藉由執行實驗，我們確定資料的資料行定義已從資料集通過 [清除遺漏的資料][][clean-missing-data] 模組。這表示您連接的其他模組也會具有資料集的資訊。
