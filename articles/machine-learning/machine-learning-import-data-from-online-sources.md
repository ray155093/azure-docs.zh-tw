...
您可以使用[匯入資料][import-data]模組，在實驗進行時，從 Azure Machine Learning Studio 內，自數個線上資料來源其中之一存取資料：
...
> [AZURE.NOTE] 本文提供有關[匯入資料][import-data]模組的一般資訊。如需有關您可以存取之資料類型、格式、參數及常見問題解答的詳細資訊，請參閱[匯入資料][import-data]模組的模組參考主題。

> [AZURE.NOTE] 這項訓練資料會在您的實驗執行時存取，因此只有在該實驗中才可供使用。相較之下，已儲存在資料集模組中的資料則可供工作區中的任何實驗使用。


## 支援的線上資料來源
Azure Machine Learning 的「匯入資料」模組支援下列資料來源：
