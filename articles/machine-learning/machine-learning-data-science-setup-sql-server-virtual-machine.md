...
若要連接其他資料磁碟，請依照「[如何將資料磁碟連接至 Windows 虛擬機器](../virtual-machines/virtual-machines-windows-classic-attach-disk.md)」中所述的步驟執行，這將引導您完成：
...
1. 在 SQL Server Management Studio 物件總管中，以滑鼠右鍵按一下 SQL Server 執行個體的名稱 (虛擬機器名稱)，然後按一下 [屬性]。
...
4. 在物件總管中，以滑鼠右鍵按一下伺服器，然後按一下 [重新啟動]。(如果 SQL Server Agent 處於執行狀態，您也必須將其重新啟動。)
