...
您需使用 [編輯中繼資料][][edit-metadata] 模組來變更與資料集關聯的中繼資料。 在此案例中，我們利用它來提供資料行標題更易記的名稱。 
...
實驗的下一個步驟是將資料集分割成兩個不同的資料集。 我們會將其中一個用於訓練模型，另一個用於測試。
...
如先前所述，將高信用風險誤判為低風險的成本，會比將低信用風險誤判為高風險的成本多出五倍。 為了說明這一點，我們將產生新資料集以反映此成本函式。 在此新資料集中，每個高風險範例都會複寫五次，而每個低風險範例則不複寫。   
...
我們必須為 [資料分割][][split] 模組的每個輸出執行此一相同複寫作業，使訓練和測試資料具有相同的成本調整。 做法是複製剛才的 [執行 R 指令碼][][execute-r-script] 模組，並將其連接至 [分割資料][][split] 模組的其他輸出連接埠。
