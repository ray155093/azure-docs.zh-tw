...
1. 在 [內容] 對話方塊中，展開左窗格中的 [Azure]，按一下 [遠端存取]。

1. 在 [遠端存取] 對話方塊中，確定已核取 [讓所有角色接受使用這些登入認證的遠端桌面連線]。

1. 指定遠端桌面連線的使用者名稱。

1. 指定並確認使用者的密碼。當您建立遠端桌面連線時，將使用在此對話方塊中設定的使用者名稱和密碼值。(請注意，這個密碼不同於您的 PFX 密碼)。

1. 指定使用者帳戶的到期日。

1. 按一下 [新增] 來建立新的自我簽署憑證。(或者，您可以透過 [工作區] 或 [檔案系統] 按鈕，分別從您的工作區或檔案系統選取憑證，但基於本教學課程目的，我們將建立新的憑證。)
