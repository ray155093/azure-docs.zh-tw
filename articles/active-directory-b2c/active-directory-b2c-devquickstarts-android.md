...
在 Azure AD B2C 中，每個使用者經驗皆由 [原則](active-directory-b2c-reference-policies.md)所定義。 此應用程式包含三種身分識別體驗：註冊、登入，以及使用 Facebook 登入。  您必須為每個類型建立一個原則，如 [原則參考文章](active-directory-b2c-reference-policies.md#create-a-sign-up-policy)所述。 建立您的三個原則時，請務必：
...
完整的應用程式也[提供 .zip 檔案格式](https://github.com/AzureADQuickStarts/B2C-NativeClient-Android/archive/complete.zip)，或放在相同儲存機制的 `complete` 分支中。
...
## <a name="download-the-android-adal-and-add-it-to-your-android-studio-workspace"></a>下載 Android ADAL 並將它加入您的 Android Studio 工作區
您有多個選項可供選擇，以在 Android 專案中使用此程式庫：

* 您可以使用原始程式碼將此程式庫匯入到 Eclipse，並連結至您的應用程式。
* 如果您使用 Android Studio，可以使用 AAR 封裝格式並參考二進位檔。

### <a name="option-1-binaries-via-gradle-recommended"></a>選項 1：透過 Gradle 取得二進位檔 (建議選項)
...
### <a name="option-2-aar-via-maven"></a>選項 2：透過 Maven 取得 AAR
...
### <a name="option-3-source-via-git-last-resort"></a>選項 3：透過 Git 取得原始檔 (最後手段)
...
## <a name="add-references-to-android-adal-to-your-project"></a>將 Android ADAL 參考加入至您的專案
> [!NOTE]
...
## <a name="create-the-sign-in-activity"></a>建立 sign-in 活動
...
## <a name="create-the-settings-activity"></a>建立 Settings 活動
這是一個會填入設定 UI 的活動。
...
## <a name="create-the-add-task-activity"></a>建立 add-task 活動
...
## <a name="create-the-to-do-list-activity"></a>建立 to-do list 活動
...
> [!NOTE]
> Microsoft Intune 公司入口網站應用程式提供訊息代理程式元件，其可安裝在使用者的裝置上。 應用程式會在裝置上提供所有應用程式的單一登入 (SSO) 存取。 開發人員應做好準備以便使用 Intune。 如果驗證器中已建立一個使用者帳戶，ADAL for Android 會使用訊息代理程式帳戶。 如果要使用訊息代理程式，開發人員必須註冊特殊的 `redirectUri` 供訊息代理程式使用。 `redirectUri` 採用 msauth://packagename/Base64UrlencodedSignature 的格式。 您可以使用指令碼 `brokerRedirectPrint.ps1` 或使用 API 呼叫 `mContext.getBrokerRedirectUri()`，以取得應用程式的 `redirectUri`。 簽章與您從 Google Play 商店取得的簽署憑證有關。
>
>
...
此外，新增可將 `AuthenticationResult` (其中包含您的權杖)「設定」及「連接」至全域 `Constants` 的方法。 即使 `ToDoActivity.java` 在其流程中使用 `sResult`，您還是需要新增這些方法。 如果您未這麼做，其他活動將無法存取權杖來執行工作 (例如在 `AddTaskActivity.java`中新增工作)。
...
```
## <a name="create-a-method-to-return-a-user-identifier"></a>建立方法以傳回使用者識別碼
...
### <a name="write-helper-methods"></a>撰寫協助程式方法
...
### <a name="create-an-endpoint-url-generator"></a>建立端點 URL 產生器
...
## <a name="write-some-ux-methods"></a>撰寫一些 UX 方法
...
### <a name="session-cookies-in-web-view"></a>Web 檢視中的工作階段 Cookie
