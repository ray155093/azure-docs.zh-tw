...
---
# <a name="azure-ad-b2c-build-a-net-web-app"></a>Azure AD B2C：建置 .NET Web 應用程式
...
在 Azure AD B2C 中，每個使用者經驗皆由 [原則](active-directory-b2c-reference-policies.md)所定義。 此程式碼範例包含三種身分識別體驗：註冊、登入和編輯設定檔。 您必須為每個類型建立一個原則，如 [原則參考文章](active-directory-b2c-reference-policies.md#create-a-sign-up-policy)所述。
...
完整的範例也[以 .zip 檔案格式提供](https://github.com/AzureADQuickStarts/B2C-WebApp-OpenIdConnect-DotNet/archive/complete.zip)，或從相同儲存機制的 `complete` 分支取得。
...
當使用者在 Web 應用程式中選取 [註冊]、[登入]**** 或 [編輯設定檔]**`Controllers\AccountController.cs` 時，** 中會叫用相關聯的動作。 在每個案例中，您可以使用內建的 OWIN 方法來觸發正確的原則：
...
您也可以在控制器中使用 `[Authorize]` 標籤，要求如果使用者未登入，則執行特定的原則。 開啟 `Controllers\HomeController.cs`，將 `[Authorize]` 標籤新增至宣告控制器。  在叫用授權標籤時，OWIN 將會選取所設定的最後一個要執行的原則。
