...
3. 按一下 [Create TodoItems table] 以建立儲存應用程式資料的資料表。
   ...
   > [!NOTE]
   > 請勿從 Azure 傳統入口網站下載 HTML 應用程式。反之，我們會在以下小節中手動建立 Sencha Touch 應用程式。
   > 
   > 
4. 記下 Azure 傳統入口網站中的 **appKey** 和 **appUrl**。您會在本教學指南的其他小節用到它們。
...
### 以 Sencha 套件形式安裝
> [!NOTE]
> 唯有當您已使用 <code>sencha generate app</code> 命令產生應用程式後才能使用此方法。
> 
> 
