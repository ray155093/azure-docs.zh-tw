---
title: 在原始檔控制中儲存 JavaScript 後端專案程式碼 | Microsoft Docs
...
1. 瀏覽至本機 Git 儲存機制的 `.\service` 資料夾、在文字編輯器中開啟 package.json 檔案，然後將下列欄位加入 **dependencies** 物件：
   
        "node-uuid": "~1.4.3"
   
   > [!NOTE]
   > 此項 package.json 檔案更新會導致在推送認可後重新啟動您的行動服務。
   > 
   > 
2. 現在，請瀏覽到 .\\service\\table 子資料夾，開啟並修改 todoitem.insert.js 檔案，如下所示：
