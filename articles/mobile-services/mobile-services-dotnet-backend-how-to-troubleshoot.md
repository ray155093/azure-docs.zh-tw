---
title: 行動服務 .NET 後端的疑難排解 | Microsoft Docs
...
2. 點按 **F5**，以建置並執行專案。在開始頁面上，選取 [立即試用]。
   ...
   > [!NOTE]
   > 如果是在本機主控服務，則在按一下連結後將會導向至下一頁。但如果是在雲端中主控，則系統會提示您提供一組認證。這是為了確保讓未經授權的使用者無法存取您 API 和裝載的相關資訊。若要檢視頁面，您必須使用**空白使用者名稱**，並以您的**應用程式金鑰**作為密碼，來進行登入。您可以導覽至行動服務的 [儀表板] 索引標籤，並選取 [管理金鑰]，在 [Azure 傳統入口網站] 中取得應用程式金鑰。
   > 
...
相同的記錄也可從行動服務之 [記錄] 索引標籤上的 [Azure 傳統入口網站] 中取得。
...
> [!NOTE]
> 行動服務目前只有與 ASP.NET 5.1 相容；目前不支援 ASP.NET 5.2。在部署之後，將您的 ASP.NET NuGet 封裝升級至 5.2.* 可能會產生錯誤。
> 
> 
