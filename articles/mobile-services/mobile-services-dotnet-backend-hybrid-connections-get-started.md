---
title: 使用混合式連線從 .NET 後端行動服務連線至內部佈署 SQL Server | Microsoft Docs
...
5. 修改 **DbContext** 執行個體建構函式，以將 `OnPremisesDBConnection` 值傳遞至類似下列程式碼片段的基底 **DbContext** 建構函式：
...
1. 按下 [**儲存**]，以儲存您剛剛建立的混合式連線和連接字串。
2. 使用 Visual Studio，將更新過的行動服務專案發佈至 Azure。
   
    服務啟動頁面隨即顯示。
3. 和之前一樣使用啟動頁面上的 [立即試用] 按鈕，或使用連線到行動服務的用戶端應用程式，叫用可產生資料庫變更的某些作業。
   
   > [!NOTE]
   > 當您使用 [**立即試用**] 按鈕來啟動說明 API 頁面時，請記得提供您的應用程式金鑰來做為密碼 (使用者名稱則為空白)。
   > 
   > 
