---
title: 開始使用行動服務中的離線資料同步 (iOS) | Microsoft Docs
...
> [!NOTE]
> 若要完成此教學課程，您需要 Azure 帳戶。如果您沒有帳戶，可以註冊 Azure 試用版並取得[免費的行動服務，即使在試用期結束之後仍可繼續使用這些服務](https://azure.microsoft.com/pricing/details/mobile-services/)。如需詳細資訊，請參閱 [Azure 免費試用](https://azure.microsoft.com/pricing/free-trial/?WT.mc_id=AE564AB28 target="_blank")。
> 
> 
...
> [!NOTE]
> 若要從裝置本機存放區中移除已在您行動服務資料庫中刪除的記錄，請啟用 [虛刪除][]。否則，您的應用程式應定期呼叫 `MSSyncTable.purgeWithQuery` 才能清除本機存放區。
> 
> 
...
> [!NOTE]
> 行動服務 SDK 會保留以 "**`ms_`**" 開頭的資料行名稱。請不要在系統資料行以外的項目使用此前置詞。否則，您的資料行名稱會在使用遠端服務時被修改。
> 
> 
