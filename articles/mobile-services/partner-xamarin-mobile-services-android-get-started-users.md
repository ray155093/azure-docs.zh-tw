---
title: 開始使用驗證 (Xamarin.Android) - 行動服務
...
若要完成本教學課程，需有 Windows 的 Visual Studio 與 Xamarin 或 Mac OS X 的 Xamarin Studio。完整的安裝指示位於[設定和安裝 Visual Studio 和 Xamarin](https://msdn.microsoft.com/library/mt613162.aspx)。
...
1. 在 Xamarin Studio 中，開啟您完成[開始使用行動服務] \(英文) 教學課程時所建立的專案。
2. 從 [執行] 功能表，按一下 [開始偵錯] 來啟動應用程式。確認應用程式啟動之後會引發無法處理的例外狀況，狀態碼為 401 (未授權)。
...
2. 將下列方法加入至 **ToDoActivity** 類別：
   ...
   > [!NOTE]
   > 如果您使用的身分識別提供者不是 Microsoft，請將傳給上述 **login** 方法的值變更為下列其中一個：*Facebook*、*Google*、*Twitter* 或 *WindowsAzureActiveDirectory*。
   > 
   > 
