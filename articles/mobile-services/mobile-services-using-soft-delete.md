---
title: 使用行動服務 (Windows 市集) 中的虛刪除 | Microsoft Docs
...
4. 在 Visual Studio 的 [方案總管] 中，展開您 .NET 後端專案下方的 [控制器] 節點，然後開啟您的控制器來源。例如 *TodoItemController.cs*。
5. 在控制器的 `Initialize()` 方法中，將 `enableSoftDelete: true` 參數傳至 EntityDomainManager 建構函式。
