---
title: 註冊 Microsoft 驗證 | Microsoft Docs
...
> [!NOTE]
> Windows Phone 8、Windows Phone 8.1 Silverlight 以及非 Windows 的應用程式可跳過本節。
> 
> 
...
2. 在 Visual Studio 開啟您的 Windows App 專案，然後在方案總管中以滑鼠右鍵按一下 Windows 市集 App 專案，按一下 [市集] > [將應用程式與市集建立關聯]。
   
      ![](./media/mobile-services-how-to-register-microsoft-authentication/mobile-services-store-association.png)
3. 在精靈中，按一下 [登入]，然後以您的 Microsoft 帳戶登入，選取您保留的應用程式名稱，按 [下一步] > [關聯]。
4. (選擇性) 若為 Windows 8.1 通用 App，請針對 Windows Phone 市集專案重複執行步驟 4 與 5。
5. 回到新 App 的 Windows 開發人員中心頁面，按一下 [服務] > [推播通知]。
6. 在 [推播通知] 頁面上，按一下 [Windows 推播通知服務 (WNS) 和 Microsoft Azure 行動服務] 下的 [線上服務網站]。
   
    這會顯示您的應用程式的 Microsoft 帳戶應用程式設定頁面。
7. 記下 [封裝 SID] 值。您可以將這個 SID 儲存在 Azure 入口網站中，讓您的 Windows 應用程式同時啟用單一登入和推播通知。
...
2. 在 App 的 Microsoft 帳戶頁面上，按一下 [API 設定]，啟用 [行動或桌面用戶端應用程式]，將行動服務 URL 設為 [目標網域]，然後在 [重新導向 URL] 中提供下列 URL 格式之一，然後按一下 [儲存]：
   ...
   * **JavaScript 後端**：`https://<mobile_service>.azure-mobile.net/login/microsoftaccount`
     ...
     > [!NOTE]
     > 請確定針對您的行動服務後端類型使用正確的重新導向 URL 路徑格式。若這不正確，驗證也將不會成功。[根網域] 應該會自動填入。
     > 
     > 
...
3. 按一下 [應用程式設定]，記下 [用戶端識別碼]、[用戶端密碼] 以及 [套件 SID] 中的值。
...
1. 在 [Azure 傳統入口網站]中，按一下行動服務的 [身分識別] 索引標籤，輸入取自身分識別提供者的用戶端識別碼、用戶端密碼以及套件 SID，然後按一下 [儲存]。
   
   > [!NOTE]
   > 若是 Windows Phone 8、Windows Phone Store 8.1 Silverlight 或非 Windows 應用程式，則不需提供套件 SID 值。
   > 
   > 
