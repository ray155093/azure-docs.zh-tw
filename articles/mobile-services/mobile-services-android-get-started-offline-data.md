---
title: 在 Android 行動服務應用程式中加入離線資料同步 | Microsoft Docs
...
在本教學課程中，您將從[行動服務快速啟動教學課程]更新應用程式，以啟用離線同步處理，然後離線加入資料、將這些項目同步處理到線上資料庫，並在 Azure 傳統入口網站中驗證變更，藉此測試應用程式。
...
10. 在 `onOptionsItemSelected` 方法中，以此程式碼取代 `if` 區塊的內容：
    ...
    此程式碼會在您按右上角的 [重新整理] 按鈕時執行。除了在啟動時同步處理以外，這是您將本機存放區同步至 Azure 的主要方式。這促使了同步變更的批次處理且具有效率 (有鑑於從 Azure 提取是相當耗成本的作業)。您也可以將 `syncAsync` 呼叫新增至 `addItem` 和 `checkItem` 方法，將此應用程式設計成在每次變更時同步處理。
