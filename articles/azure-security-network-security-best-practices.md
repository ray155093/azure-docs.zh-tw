...
若要深入了解 DMZ 及其在 Azure 中的部署方式，請參閱 [Microsoft 雲端服務和網路安全性](best-practices-network-security.md)一文。
...
若要深入了解 Azure 內部負載平衡器的運作方式和部署方式，請閱讀[開始使用 PowerShell 在 Resource Manager 中建立網際網路面向的負載平衡器](./load-balancer/load-balancer-get-started-internet-arm-ps.md#update-an-existing-load-balancer)。
