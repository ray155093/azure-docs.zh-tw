...
如果您想要對範圍中 `Failed` 的 *For each* 動作執行動作，您可以配對 `@result()` 與**[篩選陣列](../connectors/connectors-native-query.md)**動作和 **[ForEach](app-service-logic-loops-and-scopes.md)** 迴圈。  這可讓您篩選出結果陣列中已失敗的動作。  您可以取得篩選後的結果陣列，並使用 **ForEach** 迴圈對每個失敗執行動作。  我們在下面提供了範例，並會接著提供詳細說明。  這個範例會傳送 HTTP POST 要求，其中含有任何在範圍 `My_Scope`內失敗之動作的回應本文。
