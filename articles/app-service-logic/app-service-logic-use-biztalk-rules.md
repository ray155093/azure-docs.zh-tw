---
title: 了解和在邏輯應用程式中建立 BizTalk 規則 API 應用程式 | Microsoft Docs
...
## 規則概念
## 詞彙
_詞彙_是由規則條件和動作中使用之計算物件的易記名稱所組成的定義集合。詞彙定義讓規則更易於給特定商務領域中的人員閱讀、了解與共用。

設計的詞彙用來消弭商務語意與實作之間的差距。例如，核准狀態的資料繫結可能指向特定資料庫中某個資料列的特定資料行，以 SQL 查詢的形式表示。與其在規則中插入這種複雜的表示法，您可能會選擇以「狀態」的易記名稱建立與該資料繫結相關聯的詞彙定義。 接下來，您可以在任何數量的規則包含「狀態」，且規則引擎可從資料表中擷取對應的資料。

## 規則
商務規則是管理商務程序執行的宣告式陳述式。規則由條件和動作組成。條件會受到評估，如果評估為 true，規則引擎即會指出一或多個動作。
...
此規則會將商務邏輯 (以比較兩個貨幣值的形式) 套用到資料或事實 (以交易數量與可用資金的形式)，以判斷是否要進行交易。
您可使用「商務規則」來建立、修改及部署商務規則。或者，您可以透過程式設計方式來執行前述工作。

### 條件
條件是一個 True/False (布林值) 運算式，其中包含一或多個述詞。
在我們的範例中，述詞小於或等於會套用至金額和可用資金。此條件將一律評估為 true 或 false。
述詞可以結合邏輯運算子 AND、OR 及 NOT，來形成邏輯運算式，這個運算式可能相當大，但一律會被評估為 True 或 False。

### 動作
動作是條件評估的功能結果。如果符合某個規則條件，會起始對應的一或多個動作。
在我們的範例中，「執行交易」和「列印收據」就是在條件 (在此案例中為「若金額小於或等於可用資金」) 為 True 時執行的動作，而且也只有在此時才會執行。動作藉由對 XML 文件執行設定作業，顯示於商務規則架構中。
...
## 建立規則 API 應用程式
1. 登入 Azure 入口網站
2. 選取 [新增] -> [Marketplace]，然後搜尋 *BizTalk 規則*
3. 從結果清單中選取 BizTalk 規則[BizTalk 規則] 刀鋒視窗開啟
4. 選取 [建立] 按鈕  
   ![替代文字][3]
5. 在開啟的新分頁中，輸入下列資訊：  
   1. 名稱 - 提供規則 API 應用程式的名稱
   2. App Service 方案 - 選取或建立新的 App Service 方案
   3. 定價層 - 選擇您要讓此應用程式屬於哪個定價層
   4. 資源群組 – 選取或建立應用程式應所屬的資源群組
   5. 訂用帳戶 - 選取您要使用的訂用帳戶
   6. 位置 – 選擇您要部署應用程式的地理位置。
6. 選取 [*建立*]。您的 BizTalk 規則 API 應用程式在幾分鐘內就會建立。
...
1. 按一下 [新增] 後，新的 [新增定義] 分頁隨即開啟。輸入下列值
   ...
   2. 說明 – 選擇性欄位。
   3. 定義類型 – 有 2 種支援的類型。在此範例中選擇常值
   4. 資料類型 - 這可讓使用者選取定義的資料類型。目前支援 4 種資料類型：
      i.字串 – 這些值必須在雙引號中輸入 (「範例字串」) 
      ii.布林值 – 這可以是 true 或 false 
      iii.數字 – 可以是任何十進位數字 
      iv.日期時間 – 這表示定義屬於日期類型。資料必須以下列格式輸入 – mm/dd/yyyy hh:mm:ss AM\\PM  
   5. 輸入 – 這是您輸入您定義值的位置。在此輸入的值必須符合選擇的資料型。您可以輸入單一值、一組以逗號分隔的值，或是使用關鍵字 *to* 的值範圍。例如，您可以輸入唯一值 1；一組值 1、2、3；或是範圍 1 to 5。請注意，範圍僅適用於數字。
   6. 選取 [確定]。
...
### 大量新增
上述步驟已擷取建立詞彙定義的經驗。在建立之後，建立的詞彙定義會以清單的形式出現。您可以從相同結構描述產生多個定義，而無須每次都重複上述步驟，但這有特定需求。此時，「大量新增」功能正可彰顯其功效。
選取 [大量新增]，會將您帶往新刀鋒視窗。第一個步驟是選取要建立多個定義的結構描述。選取此項後會開啟新刀鋒視窗，讓您從已上傳的結構描述清單中選擇，或允許您上傳新的結構描述。
此時，XPATHS 屬性會解除鎖定。選取此項後會開啟結構描述檢視器，您可以在當中選取多個節點。
建立的多個定義會將其名稱預設為所選節點的名稱。這些名稱一律可在建立後修改。
...
在開發人員建立必要的詞彙後，預期應由商務分析人員透過 Azure 入口網站建立商務原則。  
    ...
    2.    此頁面會顯示此特定規則應用程式具有的原則清單。分析人員只要輸入原則名稱並點擊索引標籤兩次，即可加入原則。多個原則可以位於單一規則 API 應用程式中。  
    3.    選取已建立的原則，會讓使用者進入原則詳細資料頁面，而得以檢視位於原則中的規則。  
    ![替代文字][8]  
...
## 測試原則
您可以在 [測試原則] 刀鋒視窗中，使用「測試原則」命令測試原則。在此分頁中，您可以檢視在原則中使用，且需要使用者輸入的詞彙定義清單。使用者可以針對其測試案例，為這些輸入手動新增值。或者，使用者也可以選擇為輸入匯入測試 XML。在所有輸入皆匯入後，即可執行測試，而每個詞彙定義的輸出都會顯示在輸出資料行中，以便比較。若要檢視商務分析人員容易使用的記錄檔，請按一下 [檢視記錄檔]，以檢視執行記錄檔。若要儲存記錄檔，可以使用 [儲存輸出] 選項來儲存所有測試相關資料，以進行獨立分析。

## 在邏輯應用程式中使用規則
原則已撰寫並經過測試後，此時已可供取用。您可以從入口網站首頁左側選取 Logic Apps，來建立新的邏輯應用程式。建立邏輯應用程式之後，請啟動並選取 [*觸發程序和動作*]。然後您可以選取 [*從頭建立*] 範本。請依照步驟將 BizTalk 規則 API 應用程式新增至邏輯應用程式。完成此動作後，會有一個選項供您選擇要以哪些規則 API 應用程式 (動作) 作為目標。動作包括要執行的原則清單。請選擇在其後必須輸入原則的特定原則。使用者可以使用規則 API 應用程式下游的輸出，做出進一步的決策。

## 透過 API 使用規則
您也可以使用一組各種種類的 API 叫用規則 API 應用程式。如此，使用者就不限於只能使用 Logic Apps，而可以藉由 REST 呼叫在任何應用程式中使用規則。按一下 [規則] 儀表板中的 [API 定義] 透鏡，即可檢視確切可用的 REST API。
