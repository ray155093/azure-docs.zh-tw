...
每個項目有一個條件的 foreach 迴圈，其先前的模式可以複寫在有篩選動作的新結構描述中。  升級時應該會自動進行此作業。  條件會在 foreach 迴圈之前變成篩選動作 (以便只傳回符合條件的項目陣列)，而且該陣列會傳遞至 foreach 動作。  您可以在 [本文](app-service-logic-loops-and-scopes.md)
