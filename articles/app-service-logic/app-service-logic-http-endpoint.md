...
## 將觸發程序加入您的定義
第一個步驟是將觸發程序加入您的邏輯應用程式定義，如此您就能收到連入要求。有 3 種類型的觸發程序可以接收要求︰
* manual
* apiConnectionWebhook
* httpWebhook

本文的其餘部分會以 **manual** 為例，但所有的原則同樣適用於其他 2 個類型的觸發程序。將此觸發程序加入您的工作流程定義，會使其看起如下：
...
此外，您可以在定義中指定 JSON 結構描述，讓設計工具產生您可以傳入步驟的權杖。例如，下例會讓設計工具有 `title` 和 `name` 權杖可用︰
