---
title: 使用混合式連線管理員 | Microsoft Docs
...
---
# <a name="connect-to-on-premises-connectors-using-the-hybrid-connection-manager"></a>使用混合式連線管理員連線至內部部署連接器
> [!NOTE]
> 這一版文章適用於邏輯應用程式 2014-12-01-preview 結構描述版本。 Logic Apps 公開上市 (GA) 版使用閘道來進行內部部署連線。 深入了解新的[閘道](app-service-logic-gateway-connection.md)和 [Logic Apps GA](https://azure.microsoft.com/documentation/services/logic-apps/)。
> 
> 
...
> [!NOTE]
> 只有當您連接到防火牆後方的內部部署資源時，才需要混合式連線管理員。 如果您未連接至內部部署系統時，則不需要混合式連線管理員。
> 
> 
...
## <a name="get-the-service-bus-connection-string"></a>取得服務匯流排連接字串
在 Azure 入口網站中，複製服務匯流排根 SAS 連接字串。 此連接字串會將 Azure 連接器連接至內部部署系統。 

1. 在 [Azure 傳統入口網站](http://go.microsoft.com/fwlink/p/?LinkID=213885)中，選取您的服務匯流排命名空間，然後選取 [ **連接資訊**]：
...
1. 在 [Azure 入口網站](http://go.microsoft.com/fwlink/p/?LinkID=525040)中，選取您建立的連接器。 若要開啟它，您可以選取 [瀏覽]，選取 [API Apps]，然後選取您的連接器或 API 應用程式。 
   <br/><br/>
   在 [混合式連線] 中，設定**未完成**：
   <br/>
   ![][2] 
...
4. 在 [ **內部部署混合式連線管理員**] 下，您可以下載混合式連線管理員，或直接從入口網站安裝它。 
   <br/><br/>
   若要直接從入口網站安裝，請移至內部部署 IIS 伺服器，瀏覽至入口網站，然後選取 [ **下載及設定**]。
   <br/><br/>
   若要下載混合式連線管理員，請移至內部部署 IIS 伺服器，並移至 **ClickOnce 應用程式** (http://hybridclickonce.azurewebsites.net/install/Microsoft.Azure.BizTalk.Hybrid.ClickOnce.application)。 將會自動開始安裝讓您執行。
...
現在當您重新瀏覽至連接器時，混合式連線狀態為 [ **已連接**]。 您可能必須關閉連接器，再重新開啟它：
<br/>
![][4] 

> [!NOTE]
> 若要切換至次要連接字串，請重新執行混合式連線設定，並輸入 [ **次要組態字串**]。
> 
> 
...
| 80、443 |如果連接埠 9352 和 5671 無法使用，*則*連接埠 80 和 443 是用於資料傳輸和控制通道的後援連接埠。允許對這些連接埠的輸出連線。<br/><br/>允許對這些連接埠的輸出連線。 |
...
### <a name="on-premises-troubleshooting"></a>內部部署疑難排解
...
3. 在 IIS 伺服器上，確認該連接器已安裝且在執行中： 
   * 已經為您的連接器建立一個網站。 例如，如果您已建立 SQL 連接器，則會有 ***MicrosoftSqlConnector_nnn*** 網站。 在 IIS 管理員 (inetmgr) 中，請確認此網站列出且已啟動。 
...
### <a name="in-the-azure-classic-portal"></a>在 Azure 傳統入口網站中
...
**回答**：這是主要由 Web Apps (先前的網站) 和 Mobile Apps (先前的行動服務) 用來連接至內部部署的[混合式連線](../biztalk-services/integration-hybrid-connection-overview.md)技術。 此混合式連線管理員是其本身的 [設定](../biztalk-services/integration-hybrid-connection-create-manage.md) ，而且使用 Azure BizTalk 服務 (幕後)。 它僅支援 TCP 和 HTTP 通訊協定。
...
**問題**：當我建立自訂的 API 應用程式時，我可以使用 App Service 混合式連線管理員來連接至內部部署嗎？ 

**回答**：這不符合慣例。 您可以使用內建的連接器，設定 App Service 混合式連線管理員來連接至內部部署系統。 然後，使用此連接器搭配您的自訂 API 應用程式，可能是使用邏輯應用程式。 目前，您無法開發或建立自己的混合式 API 應用程式 (例如 SQL 連接器或檔案連接器)。
...
[監視 Logic Apps](app-service-logic-monitor-your-logic-apps.md)<br/>
[服務匯流排定價](https://azure.microsoft.com/pricing/details/service-bus/)
