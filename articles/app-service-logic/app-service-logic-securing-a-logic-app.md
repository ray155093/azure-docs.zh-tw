---
title: "保護 Azure 邏輯應用程式 | Microsoft Docs"
description: "保護 Azure 邏輯應用程式的輸入、存取及控制"
services: logic-apps
...
如果您使用[部署範本](app-service-logic-create-deploy-template.md)來自動化您的部署，您可以在資源範本上設定 IP 範圍。  
...
邏輯應用程式的所有呼叫都來自每個區域一組特定的 IP 位址。  您可以加入其他篩選以僅允許接受來自那些指定 IP 位址的要求。  您可以在[此文章](app-service-logic-limits-and-config.md#configuration)中找到那些 IP 位址的清單。
...
來自邏輯應用程式的許多受管理連接器提供對內部部署系統 (包括檔案系統、SQL、SharePoint、DB2 等等) 的安全連線。  閘道會透過 Azure 服務匯流排利用加密通道轉送內部部署資料，以及所有源自安全外送流量 (來自閘道代理程式) 的所有流量。  如需閘道運作方式的詳細資料，請參閱[此文章](app-service-logic-gateway-install.md#how-the-gateway-works)。
...
[建立部署範本](app-service-logic-create-deploy-template.md)  
[例外狀況處理](app-service-logic-exception-handling.md)  
[監視邏輯應用程式](app-service-logic-monitor-your-logic-apps.md)  
[診斷邏輯應用程式失敗和問題](app-service-logic-diagnosing-failures.md)  
