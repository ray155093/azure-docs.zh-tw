<properties 
   pageTitle="為網路安全性群組疑難排解 - 入口網站 | Microsoft Azure"
...
6. [取得有效的安全性規則] 刀鋒視窗隨即會出現，如下圖所示︰
	...
	- **網路介面︰**選取了 *VM1-NIC1*。一個 VM 可以有多個網路介面 (NIC)。每個 NIC 只能有唯一的有效安全性規則。進行疑難排解時，您可能需要檢視每個 NIC 的有效安全性規則。
...
7. 按一下 [Subnet1-NSG] 索引標籤可檢視對子網路套用的 NSG 有效規則，如下圖所示︰
	...
	*denyRDP* 規則是 RDP 連線失敗的原因。移除此規則應該可以解決問題。
	...
	>[AZURE.NOTE]如果與 NIC 相關聯的 VM 不在執行中，或者尚未對 NIC 或子網路套用 NSG，則不會顯示任何規則。

8. 若要編輯 NSG 規則，請在 [相關聯的 NSG] 區段中按一下 [Subnet1-NSG]。這會開啟 [Subnet1-NSG] 刀鋒視窗。您可以按一下 [輸入安全性規則]，直接編輯規則。
...
3. 選取某個網路介面。在下圖中選取了名為 *VM1-NIC1* 的 NIC。
	...
	>[AZURE.NOTE] 如果從網路介面移除 NSG，子網路 NSG 仍然會對指定的 NIC 有效。此時，輸出只會顯示子網路 NSG 的規則。只有當 NIC 附加至 VM 時，規則才會出現。

4. 您可以直接編輯與 NIC 和子網路相關聯之 NSG 的規則。若要了解如何進行，請閱讀＜檢視虛擬機器的有效安全性規則＞一節的步驟 8。

## 檢視網路安全性群組 (NSG) 的有效安全性規則
...
3. 選取某個 NSG。在下圖中選取了名為 VM1-nsg 的 NSG。
	...
	>[AZURE.NOTE] 如果只對空白子網路套用 NSG，則不會列出 VM。如果對與 VM 無關的 NIC 套用 NSG，也不會列出這些 NIC。
...
>[AZURE.NOTE] 雖然子網路和 NIC 各只能套用一個 NSG，但是 NSG 可以與多個 NIC 和多個子網路相關聯。
