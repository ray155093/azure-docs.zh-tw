...
* 只要兩個訂用帳戶之間有一個具有權限的使用者授權對等互連，您就可以對等互連存在於兩個不同訂用帳戶中的虛擬網路，而訂用帳戶會與相同的 Azure Active Directory 租用戶產生關聯。 對於與不同 Active Directory 租用戶相關聯的訂用帳戶，您可以使用 [VPN 閘道](../vpn-gateway/vpn-gateway-about-vpngateways.md?toc=%2fazure%2fvirtual-network%2ftoc.json#V2V)與其中的虛擬網路連線。
