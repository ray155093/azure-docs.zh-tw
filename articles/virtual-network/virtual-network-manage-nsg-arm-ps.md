---
title: 在資源管理員中使用 PowerShell 管理 NSG | Microsoft Docs
...
2. 執行 `Get-AzureRmVirtualNetworkSubnetConfig` Cmdlet 來擷取 **FrontEnd** 子網路，並將其儲存在變數中，如下所示。
        ...
        $subnet = Get-AzureRmVirtualNetworkSubnetConfig -VirtualNetwork $vnet `
            -Name FrontEnd 
3. 執行 `Get-AzureRmNetworkSecurityGroup` Cmdlet 來擷取現有的 NSG，並將其儲存在變數中，如下所示。
   
        $nsg = Get-AzureRmNetworkSecurityGroup -ResourceGroupName RG-NSG `
            -Name NSG-FrontEnd
4. 將 **subnet** 變數的 **NetworkSecurityGroup** 屬性設定為 **$null**，如下所示。
   
        $subnet.NetworkSecurityGroup = $nsg
5. 若要儲存對子網路所進行的變更，請執行 `Set-AzureRmVirtualNetwork` Cmdlet，如下所示。
