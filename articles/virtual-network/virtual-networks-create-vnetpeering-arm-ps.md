---
title: "使用 Powershell Cmdlet 建立 VNet 對等互連 | Microsoft Docs"
...
---
# <a name="create-vnet-peering-using-powershell-cmdlets"></a>使用 Powershell Cmdlet 建立 VNet 對等互連
...
2. 若要建立 VNet 對等互連，您必須建立兩個連結，每個方向各一。 下列步驟會先建立 VNet1 至 VNet2 的 VNet 對等互連連結︰
...
3. 此步驟會建立 VNet2 至 VNet1 的 VNet 對等互連連結：
    ...
    輸出會顯示︰
        ...
        RemoteGateways        : null
        RemoteVirtualNetworkAddressSpace : null
4. 一旦建立 VNet 對等互連連結後，您可以看到以下連結狀態︰
...
1. 讀取 **VNET1** 的虛擬網路物件，Azure Resource Manager 虛擬網路如下所示：
   
        $vnet1 = Get-AzureRmVirtualNetwork -ResourceGroupName vnet101 -Name vnet1
2. 若要在此案例中建立 VNet 對等互連，只需要一個連結，特別是從 **VNET1** 至 **VNET2** 的連結。 此步驟需要知道您的傳統 VNet 的資源識別碼。 資源群組識別碼的格式如下所示︰
