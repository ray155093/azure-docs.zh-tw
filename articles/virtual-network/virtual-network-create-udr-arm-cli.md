---
title: 在資源管理員中使用 Azure CLI 控制路由和使用虛擬應用裝置 | Microsoft Docs
...
本文涵蓋之內容包括資源管理員部署模型。您也可以[在傳統部署模型中建立 UDR](virtual-network-create-udr-classic-cli.md)。
...
1. 執行 **`azure network route-table create`** 命令，建立前端子網路的路由表。
...
2. 執行 **`azure network route-table route create`** 命令，在上方建立的路由表中建立路由，將目的地為後端子網路 (192.168.2.0/24) 的所有流量傳送到 **FW1** VM (192.168.0.4)。
...
3. 執行 **`azure network vnet subnet set`** 命令，將上方建立的路由表關聯至 **FrontEnd** 子網路。
...
2. 執行 **`azure network route-table route create`** 命令，在上方建立的路由表中建立路由，將目的地為前端子網路 (192.168.1.0/24) 的所有流量傳送到 **FW1** VM (192.168.0.4)。
   
        azure network route-table route create -g TestRG -r UDR-BackEnd -n RouteToFrontEnd -a 192.168.1.0/24 -y VirtualAppliance -p 192.168.0.4
3. 執行 **`azure network vnet subnet set`** 命令，將上方建立的路由表關聯至 **BackEnd** 子網路。
