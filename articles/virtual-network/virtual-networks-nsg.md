...
---
# <a name="what-is-a-network-security-group-nsg"></a>什麼是網路安全性群組 (NSG)？
...
| **通訊協定** |規則要符合的通訊協定 |TCP、UDP 或 \* |使用 \* 做為通訊協定包含 ICMP (僅東西流量)，以及 UDP 和 TCP 且可能會降低您需要的規則數目<br/>同時，使用 \* 可能會是過於廣泛的方法，因此請確定您只在真的有必要時才使用 |
| **來源連接埠範圍** |規則要符合的來源連接埠範圍 |1 到 65535 的單一連接埠號碼、連接埠範圍 (亦即 1-65635)，或 \* (所有連接埠) |來源連接埠可以是暫時的。 除非您的用戶端程式是使用特定連接埠，否則在大部分情況下請使用「*」。<br/>請嘗試儘可能使用連接埠範圍以避免需要多個規則<br/>多個連接埠或連接埠範圍不可使用逗號分組 |
...
* **VIRTUAL_NETWORK：**這個預設標籤代表所有網路位址空間。 其包含虛擬網路位址空間 (在 Azure 中定義的 CIDR 範圍)，以及所有已連接的內部部署位址空間和已連接的 Azure VNet (區域網路)。
* **AZURE_LOADBALANCER：**這個預設標籤代表 Azure 的基礎結構的負載平衡器。 這會轉譯成作為 Azure 健康狀態探查來源的 Azure 資料中心 IP。
...
| Azure 入口網站 |![是](./media/virtual-network-nsg-overview/green.png) |[![Yes][green]](virtual-networks-create-nsg-arm-pportal.md) |
...
| ARM 範本 |![否](./media/virtual-network-nsg-overview/red.png) |[![Yes][green]](virtual-networks-create-nsg-arm-template.md) |
...
[黃色]: ./media/virtual-network-nsg-overview/yellow.png
[紅色]: ./media/virtual-network-nsg-overview/red.png
