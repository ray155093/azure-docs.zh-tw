...
## <a name="a-namecreateacreate-the-vm"></a><a name="create"></a>建立 VM
...
## <a name="a-name--validateavalidate-vm-creation-and-nics"></a><a name = "validate"></a>驗證 VM 建立和 NIC
...
如果您建立資源群組只是為了完成本文中的步驟，您可以使用 `az group delete --name Multi-NIC-VM` 命令刪除資源群組以移除所有資源。

>[!WARNING]
>請先確認資源群組中除了本文指令碼所建立的資源外，再也沒有其他資源，然後才刪除資源群組。 執行 `az resource list --resource-group Multi-NIC-VM` 命令來檢視資源群組中的資源。
