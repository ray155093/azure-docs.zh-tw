...
3. [建立儲存體帳戶](../virtual-machines/virtual-machines-linux-create-cli-complete.md?toc=%2fazure%2fvirtual-network%2ftoc.json#create-a-storage-account)以用於 VM。

4. 建立 NIC 和您要指派給 NIC 的 IP 設定。 您可以視需要新增、移除或變更組態。 案例中描述下列組態︰
...
5. [建立 Linux VM](../virtual-machines/virtual-machines-linux-create-cli-complete.md?toc=%2fazure%2fvirtual-network%2ftoc.json#create-the-linux-vms) 文章。 請務必移除 ```  --availset-name myAvailabilitySet \ ``` 屬性，此案例不需要它。 根據您的案例使用適當的位置。 
...
6. 輸入下列命令，以檢視 NIC 和相關聯的 IP 設定︰
    ...
    --name myNic1
    ```
7. 完成本文的[將 IP 位址新增至 VM 作業系統](#os-config)一節中適用於您的作業系統的步驟，將私人 IP 位址新增至 VM 作業系統。
...
2. 根據您的需求，完成下列其中一個章節中的步驟︰
    ...
    - **新增公用 IP 位址**
        ...
        - **將資源與現有的 IP 組態產生關聯**
            ...
            在傳回的輸出中，尋找類似下列一行中的 IPConfig-3： <br>
