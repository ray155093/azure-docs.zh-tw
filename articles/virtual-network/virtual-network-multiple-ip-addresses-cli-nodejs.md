...
1. 依照[安裝和設定 Azure CLI](../xplat-cli-install.md) 文章中的步驟來安裝和設定 Azure CLI 1.0，並使用 `azure-login` 命令登入 Azure 帳戶。

2. 登入並選取適當的訂用帳戶後，在 PowerShell (您無法使用 CLI 進行註冊) 中執行下列命令來註冊預覽︰
