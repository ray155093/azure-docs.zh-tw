---
title: 在傳統部署模型中使用 Azure CLI 控制路由和使用虛擬應用裝置 | Microsoft Docs
...
2. 執行 **`azure network route-table create`** 命令，建立前端子網路的路由表。
...
3. 執行 **`azure network route-table route set`** 命令，在上方建立的路由表中建立路由，將目的地為後端子網路 (192.168.2.0/24) 的所有流量傳送到 **FW1** VM (192.168.0.4)。
...
4. 執行 **`azure network vnet subnet route-table add`** 命令，將上方建立的路由表關聯至 **FrontEnd** 子網路。
...
2. 執行 **`azure network route-table route set`** 命令，在上方建立的路由表中建立路由，將目的地為前端子網路 (192.168.1.0/24) 的所有流量傳送到 **FW1** VM (192.168.0.4)。
   
        azure network route-table route set -r UDR-BackEnd -n RouteToFrontEnd -a 192.168.1.0/24 -t VirtualAppliance -p 192.168.0.4
3. 執行 **`azure network vnet subnet route-table add`** 命令，將上方建立的路由表關聯至 **BackEnd** 子網路。
