---
title: DMZ 範例 – 建置具有 NSG 的簡單 DMZ | Microsoft Docs
...
2. 此範例中的第一個規則會允許所有內部網路之間的 DNS 流量流往 Backend 子網路上的 DNS 伺服器。此規則有一些重要參數：
   ...
   * “Action” 指出受此規則影響的流量是要封鎖或允許。
         ...
         Get-AzureNetworkSecurityGroup -Name $NSGName | `
             ...
             -Protocol *
3. 此規則會允許 RDP 流量從網際網路流往 VNET 中任一子網路上任何伺服器的 RDP 連接埠。此規則使用兩種特殊位址前置詞：“VIRTUAL\_NETWORK” 和 “INTERNET”。其可輕易處理較大類別的位址前置詞。
...
## 參考
### 主要的指令碼和網路組態
將完整指令碼儲存在 PowerShell 指令碼檔案中。將網路組態儲存到名為 “NetworkConf1.xml” 的檔案。視需要修改使用者定義的變數。執行指令碼，然後依照上面的＜範例 1＞一節中所含的防火牆規則設定指示進行。
