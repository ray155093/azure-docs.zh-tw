...
VPN 閘道是用來在虛擬網路和內部部署位置之間傳送網路流量的資源集合。閘道可用於網站間、點對站和 ExpressRoute 連線。VPN 閘道也可用來傳送 Azure 內多個虛擬網路之間的流量 (VNet 對 VNet)。

若要建立連線，您可將虛擬網路閘道新增至 VNet，並設定額外的 VPN 閘道資源及其設定。對於每種閘道類型，每個虛擬網路只能有一個虛擬網路閘道。例如，您可以有一個使用 -GatewayType Vpn 的虛擬網路閘道，以及一個使用 -GatewayType ExpressRoute 的虛擬網路閘道。
...
因為 VPN 閘道是資源集合，所以您可以使用一個工具設定部份資源，然後切換到另一個工具來進行不同的資源設定。您目前無法在 Azure 入口網站中進行每一項 VPN 閘道資源設定。文章中各組態的指示會指定是否需要特定工具。如果您使用傳統部署模型，您此時可以在傳統入口網站中作業或使用 PowerShell。如需可用的個別設定的詳細資訊，請參閱[關於 VPN 閘道設定](vpn-gateway-about-vpn-gateway-settings.md)。
...
您可以將網站間 VPN 設定為 ExpressRoute 的安全容錯移轉路徑，或使用網站間 VPN 來連線至不在您網路中但透過 ExpressRoute 連接的網站。對於相同的虛擬網路，此組態需要兩個虛擬網路閘道，一個使用 -GatewayType Vpn，而另一個使用 -GatewayType ExpressRoute。
