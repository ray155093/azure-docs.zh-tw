...
- 名稱：**VNet1**，使用位址空間 **192.168.0.0/16** 和 **10.254.0.0/16**。 請注意，您可以針對 VNet 使用一個以上的位址空間。
- 子網路名稱：**FrontEnd** **192.168.1.0/24**
...
## <a name="<a-name="declare"></a>part-1---log-in-and-set-variables"></a><a name="declare"></a>第 1 部分 - 登入和設定變數
...
## <a name="part-2---configure-a-vnet"></a>第 2 部分 - 設定 VNet 
...
## <a name="part-3---add-trusted-certificates"></a>第 3 部分 - 新增受信任的憑證
...
您最多可新增 20 個受信任的憑證至 Azure。 若要取得公開金鑰，將憑證匯出為 Base64 編碼的 X.509 (.cer) 檔案。 匯出成 .cer 的其中一種方法 是開啟 **certmgr.msc**，在個人/憑證中找出憑證。 以滑鼠右鍵按一下並匯出為 "Base-64 encoded X.509(.CER)" (不含私密金鑰)。 請記下匯出為 .cer 檔案的檔案路徑。 這是取得憑證的 Base64 字串表示法的範例。 
...
## <a name="part-4---create-the-vpn-gateway"></a>第 4 部分 – 建立 VPN 閘道
...
## <a name="part-5---download-the-vpn-client-configuration-package"></a>第 5 部分 - 下載 VPN 用戶端組態封裝
...
## <a name="<a-name="cc"></a>part-6---generate-the-client-certificate"></a><a name="cc"></a>第 6 部分 - 產生用戶端憑證
...
## <a name="part-7---install-the-client-certificate"></a>第 7 部分 - 安裝用戶端憑證
...
## <a name="part-8---connect-to-azure"></a>第 8 部分 - 連接到 Azure
...
## <a name="part-9---verify-your-connection"></a>第 9 部分 - 驗證您的連線
...
## <a name="to-add-or-remove-a-trusted-root-certificate"></a>新增或移除受信任的根憑證
...
## <a name="to-manage-the-list-of-revoked-client-certificates"></a>管理撤銷的用戶端憑證清單
