<properties
   pageTitle="使用 VPN 閘道和 PowerShell 連接 Azure VNet | Microsoft Azure"
...
4. 建立 TestVNet1 的子網路設定
	...
	下列範例會使用您先前設定的變數。在此範例中，閘道子網路使用 /27。雖然您可以使用小至 /29 的子網路來建立閘道子網路，但是不建議您這麼做。我們建議使用較大的子網路，例如 /27 或 /26。這麼做可讓您利用現有或未來可能需要較大閘道子網路的組態。
