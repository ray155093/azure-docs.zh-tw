...
2. 一旦封裝安裝完成後，您就可以啟動 VPN 連線。設定封裝並非由 Microsoft 所簽署。您可能想要使用組織的簽署服務來簽署封裝，或是自行使用 [SignTool](http://go.microsoft.com/fwlink/p/?LinkId=699327) 來簽署。即使沒有簽署，還是可以使用封裝。不過，如果封裝未簽署，當您安裝封裝時將會出現警告。
