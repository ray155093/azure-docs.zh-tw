...
了解建構 Azure Resource Manager 範本如何讓人卻步。幸運的是，Resource Manager 可協助您進行這項工作，因為您可以從您的訂用帳戶中現有的資源匯出範本。您可以使用產生的範本了解範本語法，或視需要自動重新部署解決方案。
...
4. Resource Manager 會為您擷取下列 5 個檔案：
   ...
   - 用於定義解決方案之基礎結構的範本。當您透過入口網站建立儲存體帳戶時，Resource Manager 會使用範本來部署它，並且儲存該範本供日後參考。

   - 您可以在部署期間用來傳入值的參數檔案。它包含您在第一次部署時提供的值，但是您可以在重新部署範本時變更這些值。

   - 您可以為了部署範本而使用的 Azure PowerShell 指令碼檔案。

   - 您可以為了部署範本而使用的 Azure 令列介面 (CLI) 指令碼檔案。

   - 您可以為了部署範本而使用的 .NET 類別。
     ...
     請注意，範本會定義儲存體帳戶名稱、類型和位置的參數。參數也會指出是否已啟用加密，且預設值為 **false**。在 [資源] 區段中，您會看到要部署之儲存體帳戶的定義。

方括弧會包含在部署期間評估的運算式。範本中加上方括弧的運算式是用來在部署期間取得參數值。您可以使用更多的運算式，而且您稍後會在本文中看到其他運算式的範例。如需完整清單，請參閱 [Azure Resource Manager 範本函數](resource-group-template-functions.md)。
...
2. 您會再次看到可用來重新部署解決方案的 5 個檔案，但是這次的範本有點不同。此範本只包含 2 個參數：一個用於儲存體帳戶名稱，一個用於虛擬網路名稱。
     ...
     Resource Manager 未擷取在部署期間使用的範本。而是根據資源的目前組態產生新的範本。Resource Manager 不知道您想要傳入做為參數的值，因此它根據資源群組中的值，硬式編碼大部分的值。例如，儲存體帳戶位置和複寫值設為：
