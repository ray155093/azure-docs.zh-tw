...
在微服務的世界裡，每個邏輯作業都需要在服務的各種元件中完成工作。 UI 元件會與驗證提供者元件進行通訊，以驗證使用者認證和 API 元件，取得視覺效果的資料。 使用 API 元件時可以查詢其他服務中的資料，並使用快取提供者元件及向計費元件通知這個呼叫。 Application Insights 支援分散式遙測相互關聯。 它允許偵測負責 UI 失敗或效能衰退的元件。

這篇文章說明 Application Insights 所使用的資料模型。 它涵蓋內容傳播技巧和通訊協定。 它還包括實作不同語言和平台上的相互關聯概念。

## <a name="telemetry-correlation-data-model"></a>遙測相互關聯資料模型

Application Insights 會定義分散遙測相互關聯的[資料模型](/application-insights-data-model.md)。 若要將遙測與邏輯作業相關聯 - 每個遙測項目都有稱為 `operation_Id` 的內容欄位。 每個遙測項目會在分散式追蹤內共用這個識別碼。 因此即使單一層中遺失遙測，您仍然可以將其他元件所報告的遙測相關聯。
...
Application Insights 資料模型會定義兩個欄位 - `request.source` 和 `dependency.target` 來解決這個問題。 第一個欄位會定義起始要求的元件，而第二個 - 元件傳回相依性呼叫的回應。


## <a name="example"></a>範例

讓我們以 STOCK PRICES 這個應用程式為例，它會使用稱為 STOCKS API 的外部 API 來顯示目前股票的市場價格。 STOCK PRICES 應用程式具有 `Stock page` 頁面，會向處理這個 AJAX 呼叫的伺服器發出 AJAX 呼叫 `GET /Home/Stock`。 若要使用 http 呼叫 `GET /api/stock/value`傳回結果應用程式查詢 STOCK API。
