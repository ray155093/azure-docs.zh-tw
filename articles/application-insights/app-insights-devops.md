<properties
	pageTitle="使用 Application Insights 深入診斷 Web 應用程式s | Microsoft Azure"
...
Application Insights 會監視您正在執行的 Web 應用程式。它會告訴您有關失敗和效能問題，並協助您分析客戶如何使用您的應用程式。它適用於在許多平台 (ASP.NET、J2EE、Node.js ...) 上執行的應用程式，並可裝載於雲端或內部部署。

![](./media/app-insights-devops/010.png)

請務必在執行現代應用程式時加以監視。最重要的是，您想要在大部分客戶之前偵測到失敗。您也會想要找出並修正效能問題，儘管這並不嚴重，但或許有某些動作速度變慢，或對您的使用者造成些許不便。當您滿意系統的執行時，會想要知道使用者正利用它來執行哪些動作︰他們正在使用最新功能嗎？ 是否已成功使用它？

現代 Web 應用程式是以持續傳遞週期來開發︰發行新功能或改進功能；觀察其對於使用者的適用程度；根據該知識來規劃下一個增量開發。這其中的關鍵部分是觀察階段。Application Insights 提供工具來監視 Web 應用程式的效能和使用方式。

這其中最重要觀點是診斷及判斷。如果應用程式失敗，則會造成商務損失。因此，監視架構的主要角色是以可靠的方式偵測失敗、立即通知您，並為您呈現診斷問題所需的資訊。這就是 Application Insights 的確實功用。

### Bug 來自何處？

Web 系統中的失敗通常是由組態問題或其許多元件之間的互動不良所引起。因此，處理即時網站事件時的首要任務就是找出問題所在︰原因出自於哪些元件或關聯性？

我們之中有些人 (灰白頭髮的那些人) 可能會記得一個電腦程式在一部電腦中執行的單純年代。開發人員可以在交付之前非常徹底地測試該程式；但交付之後，就很少會再看見或聽到任何有關該程式的事。使用者必須忍受殘留的 Bug 長達數年之久。
...
典型的 Web 應用程式會有許多即時元件。除了用戶端 (在瀏覽器或裝置應用程式中) 和 Web 伺服器，可能還會有後續的後端處理 - 可能是元件管線，或共同作業項目的鬆散集合。此外，它們其中有許多都不在您的控制內 - 它們是您所需的外部服務。
...
解決方案[是一個 Microsoft Azure 上應用程式和服務的系統](https://www.microsoft.com/zh-TW/enterprise/microsoftcloud/realmadrid.aspx)。延展性是關鍵需求︰流量非常多變，而且可在比賽期間和接近比賽時達到非常高的量。

對於皇家馬德里而言，監視系統效能是非常重要的。Visual Studio Application Insights 能夠提供整個系統的完整檢視，以確保提供可靠且高等級的服務。

俱樂部也能對球迷進行深入了解︰他們的所在位置 (只有 3% 在西班牙)、他們對於球員、歷史記錄和即將舉行的比賽感興趣的程度為何，以及他們如何回應每場比賽結果。

這個遙測資料中大多數都會自動收集而不需要新增任何程式碼，可大幅簡化解決方案並降低操作複雜度。對於皇家馬德里而言，Application Insights 每個月可處理 38 億個遙測點。
...
[智慧型診斷](app-insights-nrt-proactive-diagnostics.md)是一項最新功能。您不需提供任何特殊組態，Application Insights 就能自動偵測並警示您，關於您應用程式中異常提高的失敗率。它的智慧型功能足以忽略偶爾失敗的背景，而且也只會依要求中的增加比率按比例提高。例如，如果您依存的其中一個服務中發生失敗，或者如果您剛部署的新組建並沒有運作得很好，則當您查看電子郵件之後立即就能了解相關資訊(而且會提供 Webhook，讓您能夠觸發其他應用程式)。

這項功能的另一個層面會針對遙測執行每日的深入分析，以找出很難發現的異常效能模式。例如，它會找到與特定地理區域或特定瀏覽器版本相關聯的慢速效能。

在這兩種情況下，警示不只會通知您它發現的徵兆，而且還會為您提供有助於診斷問題所需的資料，例如相關的例外狀況報告。

![](./media/app-insights-devops/030.png)

客戶 Samtec 說：「在最近的功能快速轉換期間，我們發現低於調整規模的資料庫已到達它的資源限制並導致逾時。主動偵測警示會在我們分類問題時逐字顯現，非常接近即時公告。這個與 Azure 平台相結合的警示，幾乎可協助我們即時修正問題。總停機時間 < 10 minutes.zs」
...
應用程式對應會自動探索您的應用程式拓撲 (在其上方放置效能資訊)，讓您能夠輕易地在整個分散式環境中識別出效能瓶頸和有問題的流程。它可讓您探索 Azure 服務上的應用程式相依性。您可以藉由了解問題是否與程式碼相關或與相依性相關，並從單一位置向內切入到相關診斷體驗，來將問題分類。例如，您的應用程式可能會因為 SQL 層的效能降低而失敗。利用應用程式對應，您就能夠以視覺化方式看見它並向內切入到 SQL 索引建議程式或 Query Insights 體驗。
...
Visual Studio 和 Eclipse 提供工具，可為您正在開發的專案設定正確的 SDK 封裝。當您建立新專案時，您會獲得新增 Application Insights 的選項。

如果您需要使用追蹤記錄架構 (例如 Log4N、NLog 或 System.Diagnostics.Trace)，則您可以選擇將記錄傳送到 Application Insights 以及另一個遙測，因此，可輕鬆地將追蹤與要求、相依性呼叫及例外狀況相互關聯。
...
偵錯期間，您可以選擇將遙測保留於開發電腦中，在 Visual Studio 中加以檢視，但不會將它傳送到入口網站。這可避免與生產環境的遙測偵錯混合在一起。
...
開始使用 Application Insights 的方法很簡單。主要選項包括：
...
* 效能 - 影響很低。遙測會進行批次處理。
* [支援](app-insights-get-dev-support.md) - 您可以利用 Azure 支援方案；目前有熱烈參與的論壇 (您可以從中取得由我們的開發人員所提供的解答)，而最後的辦法是，我們可以為您提供個別協助，特別是當我們仍然處於 Beta 版期間。
* [定價](app-insights-pricing.md) - 您可以開始免費使用，當您處於低用量期間可繼續使用。
