---
title: "建立新的 Application Insights 資源 | Microsoft Docs"
...
Azure Application Insights 會在 Microsoft Azure「資源」中顯示您應用程式的相關資料。 因此，建立新的資源屬於[設定 Application Insights 以監視新應用程式][start]的一環。 在許多情況下，這是可由 IDE 自動完成的工作，而且也建議盡量使用這種方式。 但在某些情況下，您必須以手動方式建立資源。
...
* 「應用程式類型」會影響您在 [概觀] 刀鋒視窗上看到的內容，以及[計量瀏覽器][計量]中提供的屬性。 如果沒有看到您的應用程式類型，請選擇 ASP.NET。
...
SDK 包含不需撰寫任何程式碼，即可傳送遙測資料的標準模組。 若要追蹤使用者動作或更詳細地診斷問題，請[使用 API][api] 來傳送您自己的遙測。
