---
title: 在 Application Insights 中設定警示
...
> [!NOTE]
> 此外，您可能會收到來自[主動式偵測](app-insights-proactive-detection.md)的電子郵件，警告您關於您的應用程式效能的異常模式。不同於警示，這些通知會執行而不需要您進行設定。它們的目標是調整您的應用程式效能，而非引發關於立即問題的警示。
> 
> 
...
> [!NOTE]
> 在 [警示] 刀鋒視窗中，您會看到已經設定警示集︰[NRT 主動診斷](app-insights-nrt-proactive-diagnostics.md)。這是會監視要求失敗率這一個特定度量的自動警示。因此除非您決定要停用此功能，否則不需要設定自己的要求失敗率警示。
> 
> 
...
* [使用 Webhook 自動設定警示](../monitoring-and-diagnostics/insights-webhooks-alerts.md)
