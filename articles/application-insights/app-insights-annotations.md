---
title: Application Insights 的版本註解 | Microsoft Docs
...
3. 在另一個瀏覽器視窗中，開啟 (或建立) 可從 Visual Studio Team Services 管理部署的發行範本。
...
4. 將 [APIKey] 欄位設為變數 `$(ApiKey)`。
5. 回到 [API 存取] 刀鋒視窗，建立新的 API 金鑰並複製。
