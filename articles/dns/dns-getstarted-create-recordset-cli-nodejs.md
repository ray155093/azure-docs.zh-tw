---
title: "使用 Azure CLI 1.0 建立 DNS 記錄 | Microsoft Docs"
description: "如何建立 Azure DNS 的主機記錄。 使用 Azure CLI 1.0 建立記錄集和記錄"
services: dns
...
# <a name="create-dns-records-using-the-azure-cli-10"></a>使用 Azure CLI 1.0 建立 DNS 記錄
...
本文將逐步引導您完成使用 Azure CLI 1.0 建立記錄和記錄集的程序。
...
本節說明如何在 Azure DNS 中建立 DNS 記錄。 此範例假設您已經有[安裝 Azure CLI 1.0、登入，並建立 DNS 區域](dns-getstarted-create-dnszone-cli-nodejs.md)。

在此頁面上所有範例都使用 'A' DNS 記錄類型。 如需其他記錄類型與如何管理 DNS 記錄與記錄集的進一步詳細資訊，請參閱[使用 Azure CLI 1.0 來管理 DNS 記錄與記錄集](dns-operations-recordsets-cli-nodejs.md)。
...
如果建立新的記錄集，則會使用預設存留時間 (TTL) 3600。 如需有關如何使用不同 TTL 的指示，請參閱[使用 Azure CLI 1.0 在 Azure DNS 中管理 DNS 記錄](dns-operations-recordsets-cli-nodejs.md)。
...
用來指定記錄資料的參數，根據記錄的類型而所有不同。 例如，對於類型 "A" 的記錄，您可使用參數 `-a <IPv4 address>` 指定 IPv4 位址。 請參閱 `azure network dns record-set add-record -h` 來列出其他記錄類型的參數。 如需每種記錄類型的範例，請參閱[使用 Azure CLI 1.0 管理 DNS 記錄和記錄集](dns-operations-recordsets-cli-nodejs.md)。
...
了解如何[使用 Azure CLI 1.0 管理 DNS 區域](dns-operations-dnszones-cli-nodejs.md)。

了解如何[使用 Azure CLI 1.0 管理 DNS 記錄和記錄集](dns-operations-recordsets-cli-nodejs.md)。
