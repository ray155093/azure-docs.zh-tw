...
若要建立 DNS 記錄，請使用 `az network dns record-set <recordtype> add-record` 命令。 (其中 recordtype 是記錄類型，也就是 A、SRV、TXT 等)。如需協助，請參閱 `az network dns record-set --help`。 A, SRV, TXT, etc) For help, see `az network dns record-set --help`.
...
請使用 `az network dns record-set create` 命令建立記錄集。 如需協助，請參閱 `az network dns record-set create --help`。
...
用來指定記錄資料的參數，根據記錄的類型而所有不同。 例如，對於類型 "A" 的記錄，您可使用參數 `-a <IPv4 address>` 指定 IPv4 位址。 每個記錄類型的參數可以使用 `az network dns record-set <type> add-record --help` 列出。
...
若要擷取現有的記錄集，使用 `az network dns record-set show`。 如需協助，請參閱 `az network dns record-set show --help`。
...
您可以使用 `az network dns record-set add-record` 在新的記錄集內建立記錄，或用它將記錄新增至現有記錄集。
...
若要從現有記錄集內移除 DNS 記錄，請使用 `az network dns record-set ? remove-record` (其中 ? 是記錄類型)。 is the record type). 如需協助，請參閱 `az network dns record-set -h`。
...
若要修改 CNAME 記錄，請使用 `az network dns record-set update` 並搭配 --set 參數來新增記錄值。 不同於其他記錄類型，CNAME 記錄集只能包含單一記錄。
...
使用 `az network dns record-set soa update` 修改指定 DNS 區域的 SOA。 如需協助，請參閱 `az network dns record soa update --help`。
...
[記錄集中繼資料](dns-zones-records.md#tags-and-metadata)可用來將應用程式特定資料與每一個資料集產生關聯 (以索引鍵值組的形式)。 若要修改現有記錄集的中繼資料，請使用 `az network dns record-set update`。 如需協助，請參閱 `az network dns record-set update --help`。
