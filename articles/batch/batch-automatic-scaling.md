...
> [!NOTE]
> 自動調整公式由 [Batch REST][rest_api] API 變數、類型、作業和函式所組成。甚至在使用 [Batch .NET][net_api] 程式庫時，您會在公式字串中使用這些。
> 
> 
...
> [!IMPORTANT]
> 如果您使用上述其中一項技術建立已啟用自動調整的集區，則**不**得指定集區的 targetDedicated 參數。也請注意，如果您想要對已啟用自動調整的集區手動調整大小 (例如使用 [BatchClient.PoolOperations.ResizePool][net_poolops_resizepool])，則必須先在集區**停用**自動調整，然後調整其大小。
> 
> 
...
> [!NOTE]
> 若要評估自動調整公式，您必須先使用有效的公式在集區啟用自動調整。
> 
> 
