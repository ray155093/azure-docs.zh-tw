...
您應該會看到「使用者/公用」資料夾中的目錄清單。
如果未顯示任何檔案，而且「/c/使用者/公用」資料夾不是空的，則未正確地設定磁碟區對應。 
...
3. 關閉方案，並從 .xproj 檔案中移除下列幾行︰
    ...
    ```
4. 重新開啟方案
5. 從 Properties\launchSettings.json 檔案移除下列幾行︰
    ...
    ```
6. 在 publishOptions 中從 project.json 移除下列與 Docker 相關的檔案︰
    ...
    ```
7. 解除安裝舊版本並安裝 Docker 工具 0.40，然後從 ASP.Net Core Web 或主控台應用程式的內容功能表再次[新增] -> [Docker 支援]。 這會將新的必要 Docker 構件新增回您的專案。 
...
2. 開啟 %USERPROFILE%\AppData\Local\Microsoft\VisualStudio\14.0\
3. 刪除下列資料夾
     ```
       ComponentModelCache
       Extensions
