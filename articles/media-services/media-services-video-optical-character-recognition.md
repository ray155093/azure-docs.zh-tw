<properties
	pageTitle="使用 Azure 媒體分析以將視訊檔案中的文字內容轉換為數位文字 | Microsoft Azure"
...
#使用 Azure 媒體分析以將視訊檔案中的文字內容轉換為數位文字 
...
TimeInterval|(選擇性) 說明取樣率。預設值是每 1/2 秒。<br/>JSON 格式 – HH:mm:ss.SSS (預設值 00:00:00.500)<br/>XML 格式 – W3C XSD 持續時間基礎 (預設值 PT0.5)
DetectRegions|(選擇性) DetectRegion 物件的陣列，可指定視訊畫面內要偵測其中文字的區域。<br/>DetectRegion 物件是由下列四個整數值所組成︰<br/>Left - 從左邊界起算的像素<br/>Top - 從上邊界起算的像素<br/>Width – 以像素為單位的區域寬度<br/>Height – 以像素為單位的區域高度
