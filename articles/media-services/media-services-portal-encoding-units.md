...
此頁面提供如何調整媒體處理的概觀，並示範如何使用 Azure 傳統入口網站來進行此作業。您也可以使用其他技術完成這項工作：
...
媒體服務帳戶是與可決定媒體處理工作速度的保留單元類型相關聯。您可以選擇下列的保留單元類型：**S1**、**S2** 或 **S3**。例如，在執行相同編碼作業的前提下，使用 **S2** 保留單元類型的速度會比 **S1** 類型快。如需詳細資訊，請參閱[保留單元類型](https://azure.microsoft.com/blog/high-speed-encoding-with-azure-media-services/)。
...
|**S1**|**S2**|**S3**|
----------|------------|----------|------------
預定的使用案例| 單一位元速率編碼。<br/>在 SD 或下列解決方法的檔案、對時間不敏感、低成本。|單一位元速率和多重位元速率編碼。<br/>SD 和 HD 編碼的一般使用方式。 |單一位元速率和多重位元速率編碼。<br/>Full HD 和 4K 解析度影片。對時間敏感、周轉時間更快的編碼。 
