...
- 在 AMS 金鑰傳遞端必須設定下列各項︰
	...
	- **應用程式憑證密碼** - 客戶建立 .pfx 檔案用的密碼。
	- **應用程式憑證的密碼識別碼** - 客戶必須上傳密碼，就類似於他們上傳其他 AMS 金鑰的方式，並使用 **ContentKeyType.FairPlayPfxPassword** 列舉值。在結果中，他們會得到 AMS 識別碼，這是他們要在金鑰傳遞原則選項內使用所需之物。
	- **IV** - 16 個位元組的隨機值，必須符合資產傳遞原則中的IV。客戶會產生 IV，並將它放在兩個位置︰資產傳遞原則和金鑰傳遞原則選項。 
	- **ASK** - 當您使用 Apple 開發人員入口網站產生憑證時，會收到 ASK (應用程式密碼金鑰)。每個開發小組都會收到一個唯一的 ASK。請儲存一份 ASK，並將它存放在安全的地方。您之後必須將 ASK 設定為 Azure 媒體服務的 FairPlayAsk。 
	-  **ASK 識別碼** -由 Apple 提供。客戶必須上傳 ASk，就類似於他們上傳其他 AMS 金鑰的方式，並使用 **ContentKeyType.FairPlayASk** 列舉值。在結果中，他們會得到 WAMS 識別碼，這是他們要在金鑰傳遞原則選項內使用所需之物。

- FPS 用戶端必須設定下列各項︰
 	- **應用程式憑證 (AC)** - 包含作業系統用來加密某些裝載之公開金鑰的 .cer/.der 檔案。AMS 必須了解它，因為播放程式需要它。金鑰傳遞服務會使用對應的私密金鑰來解密。
...
- 支援使用 **AES-128 CBC** 加密之 HLS 的用戶端︰在 OS X、Apple 電視、iOS 上的 Safari。
...
1. 設定內容金鑰的授權原則。在建立內容金鑰授權原則時，您需要指定下列各項︰ 
	...
	- FairPlay 原則選項組態。如需如何設定 FairPlay 的詳細資訊，請參閱底下的 ConfigureFairPlayPolicyOptions() 方法。
	- 限制 (開放或權杖) 
