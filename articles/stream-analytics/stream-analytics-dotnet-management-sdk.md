---
title: "管理 .NET SDK：串流分析的管理 .NET SDK | Microsoft Docs"
...
* 設定要使用的輸入來源和輸出目標。 如需進一步的指示，請參閱[新增輸入](stream-analytics-add-inputs.md)來設定範例輸入，以及參閱[新增輸出](stream-analytics-add-outputs.md)來設定範例輸出。
