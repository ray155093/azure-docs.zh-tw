...
這個範例應用程式會產生事件並推送至事件中樞執行個體 (簡稱為事件中樞)。 服務匯流排事件中樞是串流分析慣用的事件擷取方法。 請參閱[服務匯流排文件](/documentation/services/service-bus/)中的事件中樞文件。
...
3. 將 TwitterClient.exe.config 中的 EventHubConnectionString 和 EventHubName 值換成事件中樞的連接字串和名稱。 您稍早複製的連接字串會提供事件中樞的連接字串和名稱，因此請務必加以區隔，並將每個項目放在正確的欄位中。 例如，請考慮下列連接字串：
   ...
   請注意，EventHubName 值中__不會__出現 "EntityPath=" 文字。
