...
## <a name="configuring-streaming-units"></a>設定串流處理單位
...
## <a name="embarrassingly-parallel-job"></a>窘迫平行作業
...
這個查詢是簡單的篩選，我們不必擔心要分割我們傳送至事件中樞的輸入。 您會注意到查詢的 Partition By 是 PartitionId，因此我們滿足了上述的需求 #2。 對於輸出，我們必須設定作業中的事件中樞輸出，以便把 PartitionKey 欄位設定為 PartitionId。 再檢查一次，輸入資料分割 == 輸出資料分割。 這個拓撲的確是窘迫平行。

### <a name="query-with-grouping-key"></a>利用群組索引鍵的查詢
...
這個查詢有群組索引鍵，且同一個查詢執行個體必須處理相同的索引鍵。 這代表我們必須以資料分割的方式，把事件傳送至事件中樞。 我們該關心哪個索引鍵？ **PartitionId** 是作業邏輯概念，我們真正關心的索引鍵是 **TollBoothId**。 這代表我們應該要把我們傳送到事件中樞之事件資料的 **PartitionKey**，設定為事件的 **TollBoothId**。 查詢的 Partition By 是 PartitionId，因此沒問題了。 至於輸出，因為它是 blob，所以我們不需要擔心設定 **PartitionKey** 的問題。 而對於需求 #4，同樣地，因為這是 Blob，所以我們不需要擔心。 這個拓撲的確是窘迫平行。

### <a name="multi-step-query-with-grouping-key"></a>利用群組索引鍵的多重步驟查詢
輸入 – 有 8 個資料分割輸出的事件中樞 – 有 8 個資料分割的事件中樞
...
### <a name="not-using-event-hubs-or-blobs-as-output"></a>不把事件中樞或 Blob 當做輸出來使用
輸入 – 有 8 個資料分割輸出的事件中樞 – PowerBI

PowerBI 輸出目前並不支援資料分割。

### <a name="multi-step-query-with-different-partition-by-values"></a>有不同 Partition By 值的多重步驟查詢
輸入 – 有 8 個資料分割輸出的事件中樞 – 有 8 個資料分割的事件中樞
...
### <a name="examples-of-scale"></a>調整的範例
...
> [!NOTE]
> 如果您要聯結兩個串流，請確定串流是由執行聯結之資料行的分割索引鍵進行分割的，且兩個串流中有相同數目的資料分割。
> 
> 

## <a name="configure-stream-analytics-job-partition"></a>設定資料流分析工作資料分割
**若要調整工作的串流處理單元**
...
## <a name="stream-analytics-throughput-at-scale---raspberry-pi-scenario"></a>大規模串流分析輸送量：Raspberry Pi 案例
