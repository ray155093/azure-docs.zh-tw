...
這個範例應用程式會產生事件並推送至事件中樞執行個體以供即時處理。 服務匯流排事件中樞是 Stream Analytics 慣用的事件擷取方法。 您可以在 [Azure 服務匯流排文件](/documentation/services/service-bus/)中深入了解事件中樞。
...
1.  下載 [TelcoGenerator.zip 檔案](http://download.microsoft.com/download/8/B/D/8BD50991-8D54-4F59-AB83-3354B69C8A7E/TelcoGenerator.zip)，並將其解壓縮至目錄。
    ...
    > [AZURE.NOTE] Windows 可能會封鎖下載的 zip 檔案。 請以滑鼠右鍵按一下檔案，然後選取 [內容]。 如果顯示「這個檔案來自另一部電腦，可能會封鎖以協助保護您的電腦。」訊息，則勾選 [解除封鎖] 方塊，並在 zip 檔案上按一下 [套用]。
