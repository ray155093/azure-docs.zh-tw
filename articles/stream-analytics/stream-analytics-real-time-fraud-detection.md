<properties
	pageTitle="用串流分析：即時詐騙偵測 | Microsoft Azure"
...
- 選擇性：[GitHub](https://aka.ms/azure-stream-analytics-telcogenerator) 中事件產生器的原始程式碼
...
2.	將 **telcodatagen.exe.config** 中的 Microsoft.ServiceBus.ConnectionString 與 EventHubName 值換成事件中樞連接字串與名稱。
    ...
    **注意**︰從 Azure 入口網站複製的連接字串會將連接名稱放在結尾。請務必從 add key= 欄位移除 ";EntityPath=<value>"。
...
3.	在查詢編輯器中按一下 [測試]。
4.	提供一個測試檔案，您使用先前步驟建立的檔案或 [telco.json](https://github.com/Azure/azure-stream-analytics/blob/master/Samples/SampleDataFiles/Telco.json) 均可。
