...
-   **延展性：**串流分析能夠處理高達 1 GB/秒的高事件輸送量。與 [Azure 事件中樞](https://azure.microsoft.com/services/event-hubs/)整合，可讓解決方案每秒擷取數百萬來自連線裝置、點擊資料流及記錄檔等項目的事件。為達到此目的，串流分析運用事件中樞資料分割功能，每個資料分割可產生 1 MB/秒。使用者可將計算分割成查詢定義內的數個邏輯步驟，且每個步驟都能夠進一步分割以增加研展性。
