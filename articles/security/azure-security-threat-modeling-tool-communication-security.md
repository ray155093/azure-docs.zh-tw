...
| 適用的技術 | 泛型 |
| 屬性              | StorageType - 檔案 |
| 參考              | [Azure 檔案儲存體](../storage/storage-files-introduction.md)、[如何在 Windows 上掛接 Azure 檔案共用](../storage/storage-file-how-to-use-files-windows.md) |
| 步驟 | 使用 REST API 時，Azure 檔案儲存體支援 HTTPS，但較常用來作為連線到 VM 的 SMB 檔案共用。 SMB 2.1 不支援加密，因此只允許在 Azure 中的相同區域內連接。 不過，SMB 3.0 支援加密，而且可以搭配 Windows Server 2016、Windows Server 2012 R2、Windows 8.1 和 Windows 10 使用，允許跨區域存取，甚至是桌面上的存取。 |
