...
Azure 磁碟加密是可讓您加密您的 Windows 和 Linux IaaS 虛擬機器磁碟的新功能。 Azure 磁碟加密利用 Windows 的業界標準 [BitLocker](https://technet.microsoft.com/library/cc732774.aspx) 功能和 Linux 的 [DM-Crypt](https://en.wikipedia.org/wiki/Dm-crypt) 功能，為 OS 和資料磁碟提供磁碟區加密。 此解決方案與 [Azure 金鑰](https://azure.microsoft.com/documentation/services/key-vault/)保存庫整合，可幫助您控制和管理您的金鑰保存庫訂用帳戶中的磁碟加密金鑰和密碼，同時確保虛擬機器磁碟中的所有資料會在您的 Azure 儲存體中輕鬆加密。
...
3. Azure 會更新 VM 服務模型，且 IaaS VM 會標示為已解密。 VM 的待用內容不會再加密。
4. 停用加密作業並不會刪除客戶的金鑰保存庫和加密金鑰資料 (適用於 Windows 的 BitLocker 加密金鑰或適用於 Linux 的複雜密碼)。

## <a name="prerequisites"></a>必要條件
以下是針對概觀一節提到支援的案例，在 Azure IaaS VM 上啟用 Azure 磁碟加密的必要條件

* 使用者必須擁有有效的作用中 Azure 訂用帳戶，才能在 Azure 支援的區域中建立資源
* 在下列 Windows 伺服器 SKU 上支援 Azure 磁碟加密 - Windows Server 2008 R2、Windows Server 2012、Windows Server 2012 R2 和 Windows Server 2016。
...
**注意：**[Azure PowerShell SDK 1.1.0 版](https://github.com/Azure/azure-powershell/releases/tag/v1.1.0-January2016)不支援 Azure 磁碟加密。 如果您收到與使用 Azure PowerShell 1.1.0 相關的錯誤，請參閱[與 Azure PowerShell 1.1.0 相關的 Azure 磁碟加密錯誤](http://blogs.msdn.com/b/azuresecurity/archive/2016/02/10/azure-disk-encryption-error-related-to-azure-powershell-1-1-0.aspx)一文。
...
* 若要執行任何 Azure CLI 命令並將它與您的 Azure 訂用帳戶產生關聯，您必須先安裝 Azure CLI 版本：
  ...
  * 搭配使用適用於 Mac、Linux 和 Windows 的 Azure CLI 與 Azure Resource Manager，請參閱[這裡](../virtual-machines/azure-cli-arm-commands.md)
...
#### <a name="setup-the-azure-ad-application-in-azure-active-directory"></a>在 Azure Active Directory 中設定 Azure AD 應用程式
...
##### <a name="create-a-new-azure-ad-app-using-azure-powershell"></a>使用 Azure PowerShell 建立新 Azure AD 應用程式
...
##### <a name="provisioning-the-azure-ad-client-id-and-secret-from-the-azure-classic-deployment-model-portal"></a>從 Azure 傳統部署模型入口網站佈建 Azure AD 用戶端識別碼和密碼
Azure AD 用戶端識別碼和密碼也可使用位於 https://manage.windowsazure.com 的 Azure 傳統部署模型入口網站來佈建，請遵循下列步驟來執行這項工作：
...
##### <a name="use-an-existing-app"></a>使用現有的應用程式
...
> [!NOTE]
> Linux VM 目前不支援 AAD 憑證型驗證。
> 
> 

下列各節含有為 Azure AD 設定憑證型驗證的必要步驟。

##### <a name="create-a-new-azure-ad-app"></a>建立新 Azure AD 應用程式
...
##### <a name="use-an-existing-azure-ad-app"></a>使用現有的 Azure AD 應用程式
...
##### <a name="upload-a-pfx-file-to-key-vault"></a>上傳 PFX 檔案至金鑰保存庫
...
##### <a name="deploy-a-certificate-in-key-vault-to-an-existing-vm"></a>將金鑰保存庫中的憑證部署到現有 VM
...
#### <a name="setting-key-vault-access-policy-for-the-azure-ad-application"></a>設定 Azure AD 應用程式的金鑰保存庫存取原則
...
### <a name="setting-and-configuring-azure-key-vault-for-azure-disk-encryption-usage"></a>針對 Azure 磁碟加密的使用方式設定與配置 Azure 金鑰保存庫
...
#### <a name="provisioning-a-key-encryption-key-optional"></a>佈建金鑰加密金鑰 (選擇性)
...
#### <a name="set-key-vault-permissions-to-allow-the-azure-platform-access-to-the-keys-and-secrets"></a>設定金鑰保存庫權限以允許 Azure 平台存取金鑰和密碼
...
### <a name="enable-encryption-on-new-iaas-vms-created-from-the-azure-gallery"></a>對透過 Azure 資源庫建立的新 IaaS VM 啟用加密
...
### <a name="enable-encryption-on-new-iaas-vms-created-from-customer-encrypted-vhd-and-encryption-keys"></a>對透過客戶加密的 VHD 和加密金鑰建立的新 IaaS VM 啟用加密
...
#### <a name="using-resource-manager-template"></a>使用 Resource Manager 範本
...
### <a name="enable-encryption-on-existing-or-running-iaas-linux-vm-in-azure"></a>在 Azure 中現有或執行中的 IaaS Linux VM 上啟用加密
...
### <a name="get-encryption-status-of-an-encrypted-iaas-vm"></a>取得加密的 IaaS VM 的加密狀態
...
#### <a name="get-encryption-status-of-an-encrypted-windows-vm-using-azure-resource-manager-portal"></a>使用 Azure Resource Manager 入口網站取得加密之 Windows VM 的加密狀態
...
#### <a name="get-encryption-status-of-an-encrypted-windowslinux-iaas-vm-using-disk-encryption-ps-cmdlet"></a>使用磁碟加密 PS Cmdlet 取得加密之 (Windows/Linux) IaaS VM 的加密狀態
...
#### <a name="get-encryption-status-of-the-iaas-vm-from-disk-encryption-cli-command"></a>透過磁碟加密 CLI 命令取得 IaaS VM 的加密狀態。
...
##### <a name="disable-encryption-on-existingrunning-iaas-vm-in-azure-using-resource-manager-template"></a>使用 Resource Manager 範本在 Azure 中的現有/執行中 IaaS VM 上停用加密
...
若為 Linux VM，則可以使用[這個](https://aka.ms/decrypt-linuxvm)範本來停用加密。
...
##### <a name="disable-encryption-on-existingrunning-iaas-vm-in-azure-using-ps-cmdlet"></a>使用 PS Cmdlet 在 Azure 中的現有/執行中 IaaS VM 上停用加密
...
### <a name="preparing-a-pre-encrypted-windows-vhd"></a>準備已預先加密的 Windows VHD
...
#### <a name="prepare-os-volume-for-bitlocker-using-bdehdcfg"></a>使用 `bdehdcfg` 準備用於 BitLocker 的 OS 磁碟區
執行下面的命令來壓縮作業系統磁碟分割及為 BitLocker 準備電腦。

    bdehdcfg -target c: shrink -quiet

#### <a name="using-bitlocker-to-protect-the-os-volume"></a>使用 BitLocker 保護作業系統磁碟區
...
#### <a name="encrypting-os-drive-on-a-running-linux-vm"></a>在執行中的 Linux VM 上加密 OS 磁碟機
...
* (適用於 RHEL 和 CentOS) VM 上必須[已停用](https://access.redhat.com/documentation/en-US/Red_Hat_Enterprise_Linux/7/html/SELinux_Users_and_Administrators_Guide/sect-Security-Enhanced_Linux-Working_with_SELinux-Changing_SELinux_Modes.html#sect-Security-Enhanced_Linux-Enabling_and_Disabling_SELinux-Disabling_SELinux) SELinux。 停用 SELinux 之後，至少必須將 VM 重新開機一次。
...
> [!NOTE]
> 您必須使用 UUID=... 來指定 /etc/fstab 中的資料磁碟機，而不是指定區塊裝置名稱 (例如 /dev/sdb1)。 加密期間，VM 上的磁碟機順序會變更。 如果 VM 依賴特定的區塊裝置順序，則無法在加密後掛接裝置。
> 
> 
...
> [!NOTE]
> 未以 `systemd` 服務的形式執行的所有使用者空間程序皆應使用 `SIGKILL` 來刪除。 VM 應重新開機。 在執行中 VM 上啟用 OS 磁碟加密時，請規劃 VM 的停機時間。
> 
> 
...
建議您在重新開機「之前」，先儲存 VM 的[開機診斷](https://azure.microsoft.com/en-us/blog/boot-diagnostics-for-virtual-machines-v2/)。
...
2. 查看[開機診斷](https://azure.microsoft.com/en-us/blog/boot-diagnostics-for-virtual-machines-v2/)。 來自 ADE 擴充功能的訊息應該會在前面加上 `[AzureDiskEncryption]`。
...
#### <a name="preparing-a-pre-encrypted-linux-vhd"></a>準備已預先加密的 Linux VHD
##### <a name="ubuntu-16"></a>Ubuntu 16
###### <a name="configure-encryption-during-distro-install"></a>在安裝散發套件期間設定加密
1. 在分割磁碟時選取 [設定加密的磁碟區]。
...
###### <a name="configure-encryption-to-work-with-azure"></a>設定加密來與 Azure 搭配運作
...
3. 如果您要在 Windows 中編輯 *azure_crypt_key.sh* 並將它複製到 Linux，請記得執行 *dos2unix /usr/local/sbin/azure_crypt_key.sh*。
...
3. 使用[這些指示](https://azure.microsoft.com/en-us/documentation/articles/virtual-machines-linux-suse-create-upload-vhd/#prepare-opensuse-131)準備要上傳到 Azure 的 VM。 還不要執行最後一個步驟 (解除佈建 VM)。
...
5. 使用[這些指示](https://azure.microsoft.com/en-us/documentation/articles/virtual-machines-linux-create-upload-centos/#centos-70)準備要上傳到 Azure 的 VM。 還不要執行最後一個步驟 (解除佈建 VM)。
...
### <a name="upload-disk-encryption-secret-for-the-pre-encrypted-vm-to-key-vault"></a>上傳已預先加密 VM 的磁碟加密密碼至金鑰保存庫
...
### <a name="specify-secret-url-when-attaching-os-disk"></a>在連接 OS 磁碟時指定密碼 URL
