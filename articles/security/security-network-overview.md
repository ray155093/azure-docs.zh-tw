...
閱讀[虛擬網路概觀](../virtual-network/virtual-networks-overview.md)一文，即可深入了解 Azure 虛擬網路。
...
閱讀[網路安全性群組](../virtual-network/virtual-networks-nsg.md)一文，即可深入了解**網路安全性群組**。
...
若要深入了解**使用者定義的路由**，請參閱[什麼是使用者定義的路由和 IP 轉送](../virtual-network/virtual-networks-udr-overview.md)一文。
...
## 安全遠端存取和跨單位連線
需要遠端完成 Azure 資源的安裝、組態和管理。此外，您可能想要部署內部部署和 Azure 公用雲端中含有元件的[混合式 IT](http://social.technet.microsoft.com/wiki/contents/articles/18120.hybrid-cloud-infrastructure-design-considerations.aspx) 方案。這些案例都需要安全遠端存取。
...
若要深入了解**點對站 VPN** 連接，請參閱[使用 PowerShell 設定虛擬網路的點對站連接](../vpn-gateway/vpn-gateway-howto-point-to-site-rm-ps.md)一文。
...
若要深入了解 **Azure 的站對站 VPN**，請參閱[使用 Azure 入口網站建立具有站對站 VPN 連接的 Resource Manager VNet](../vpn-gateway/vpn-gateway-howto-site-to-site-resource-manager-portal.md) 一文。

若要深入了解如何規劃 **VPN 跨單位連接**，請參閱[規劃與設計 VPN 閘道](../vpn-gateway/vpn-gateway-plan-design.md)。
...
若要深入了解內部部署位置與 Azure 虛擬網路之間的**專用 WAN 連結**，請閱讀 [ExpressRoute 技術概觀](../expressroute/expressroute-introduction.md)一文。
...
如需 Azure 虛擬網路到 Azure 虛擬網路站對站 VPN 的詳細資訊，請參閱[使用 Azure Resource Manager 和 PowerShell 設定 VNet 對 VNet 連接](../vpn-gateway/vpn-gateway-vnet-vnet-rm-ps.md)一文。
...
如需 Azure 應用程式閘道的詳細資訊，請閱讀[應用程式閘道概觀](../application-gateway/application-gateway-introduction.md)一文。

### 網路層級負載平衡
相較於 HTTP 負載平衡，網路層級負載平衡可根據 IP 位址和連接埠 (TCP 或 UDP) 號碼進行負載平衡決策。您可以使用 Azure Load Balancer，獲取 Azure 中網路層級負載平衡的優點。Azure Load Balancer 的一些重要特性包括︰
...
若要深入了解外部 Azure Load Balancer，請閱讀[多個虛擬機器或服務之間的網際網路面向負載平衡器](../load-balancer/load-balancer-internet-overview.md)一文。

若要深入了解內部 Azure Load Balancer，請閱讀[內部負載平衡器概觀](../load-balancer/load-balancer-internal-overview.md)一文。
...
若要深入了解使用 Azure 流量管理員的全域負載平衡，請參閱[什麼是流量管理員？](../traffic-manager/traffic-manager-overview.md)一文。
...
若要深入了解網路安全性群組記錄，請閱讀[網路安全性群組 (NSG) 的記錄檔分析](../virtual-network/virtual-network-nsg-manage-log.md)一文。
...
若要深入了解 **Azure 虛擬網路 DNS 伺服器**，請閱讀[虛擬網路概觀](../virtual-network/virtual-networks-overview.md)一文。

若要深入了解在 Azure 虛擬網路上使用**專屬 DNS 伺服器**，請閱讀[管理虛擬網路 (VNet) 所使用的 DNS 伺服器](../virtual-network/virtual-networks-manage-dns-in-vnet.md)一文。
...
若要深入了解 Azure DNS，請閱讀 [Azure DNS 概觀](../dns/dns-overview.md)。
...
若要深入了解 Azure 中的 DMZ 設計，請閱讀 [Microsoft 雲端服務和網路安全性](../best-practices-network-security.md)一文。
...
若要深入了解 Azure 資訊安全中心，請參閱 [Azure 資訊安全中心簡介](../security-center/security-center-intro.md)一文。
