...
1. 擴充 *javax.mail.Authenticator* 類別，以及在 *getPasswordAuthentication* 方法的實作中，傳回 SendGrid 使用者名稱和密碼。  
...
3. 建立郵件並指派 [收件者]、[寄件者]、[主旨] 
   和內容值。這顯示在[如何：建立電子郵件](#bkmk_HowToCreateEmail)一節中。
4. 透過 *javax.mail.Transport* 物件傳送郵件。這顯示在 [如何：傳送電子郵件](如何：傳送電子郵件.md)一節中。
...
## 如何：使用篩選器來啟用頁尾、追蹤和分析
SendGrid 運用篩選器提供其他電子郵件功能。這些設定可新增到電子郵件以啟用特定功能，例如啟用點擊追蹤、Google 分析、訂閱追蹤等。如需完整的篩選器清單，請參閱[篩選器設定][篩選器設定] \(英文)。
