...
本文說明使用自訂版本的 Microsoft 監視代理程式 (MMA) 將內部部署基礎結構中的 Windows 電腦直接連接到 OMS 的步驟。您必須安裝並連接所有您想要上架至 OMS 之電腦的代理程式，以便讓它們將資料傳送給 OMS，以及在 OMS 入口網站中檢視及處理此資料。
...
- 每一部 Windows 電腦都必須能夠連線到網際網路。此連線可以是直接連線、透過 Proxy 連線、或透過 OMS Log Analytics 轉寄站連線。
...
5. 在 [代理程式安裝程式選項] 頁面中，您可以選擇將代理程式連接至 Operational Insights (OMS)、Operations Manager，或者，如果您想要稍後再設定代理程式可以選擇保留空白。按 [下一步]。   
    - 如果您選擇連接至 Operational Insights (OMS)，請將您在上一個程序複製到 [記事本] 的內容貼到 [工作區識別碼] 和 [工作區索引鍵 (主索引鍵)]，然後按一下 [下一步]。![貼上工作區識別碼和主索引鍵](./media/log-analytics-windows-agents/oms-mma-aoi-setup.png)
...
7. 在 [設定成功] 頁面中，按一下 [完成]。
8. 完成時，[Microsoft 監視代理程式] 會出現在 [控制台] 中。您可以檢閱您的設定，並確認代理程式已連接到 Operational Insights (OMS)。當連接到 OMS，代理程式會顯示訊息︰**Microsoft 監視代理程式已成功連接到 Azure Operational Insights (OMS) 服務。**
...
## 手動設定代理程式
如果您已安裝代理程式但尚未設定，或是您做了設定變更，您可以使用下列資訊來啟用或重新設定它們。設定代理程式之後，它會註冊代理程式服務，並將獲得所需的組態資訊及包含解決方案資訊的管理組件。

1. 安裝 Microsoft 監視代理程式之後，開啟 [控制台]。
2. 開啟 [Microsoft Monitoring Agent]，然後按一下 [連接至 Azure Operational Insights (OMS)]。   
3. 將您在上一個程序複製到 [記事本] 的內容貼到 [工作區識別碼] 和 [工作區索引鍵 (主索引鍵)]，然後按一下 [確定]。![設定 Operational Insights](./media/log-analytics-windows-agents/oms-mma-aoi.png)
...
### 停用代理程式
1. 安裝代理程式之後，開啟 [**控制台**]。
2. 開啟 Microsoft Monitoring Agent，然後按一下 [Azure Operational Insights (OMS)] 索引標籤。
3. 清除**至 Azure Operational Insights 的連線**。

## 使用命令列設定代理程式

- 您可以參考下列範例來使用 Windows PowerShell。
    ```
...
1.	在安裝代理程式的電腦上，開啟 [控制台]。
2.	開啟 [Microsoft 監視代理程式]，然後按一下 [Operations Manager] 索引標籤。![Microsoft 監視代理程式 Operations Manager 索引標籤](./media/log-analytics-windows-agents/oms-mma-om01.png)
3.	如果 Operations Manager 伺服器已與 Active Directory 整合，請按一下 [自動更新來自 AD DS 的管理群組指派]。
4.	按一下 [新增] 以開啟 [新增管理群組] 對話方塊。![Microsoft 監視代理程式新增管理群組](./media/log-analytics-windows-agents/oms-mma-om02.png)
5.	在[管理群組名稱] 方塊中，輸入管理群組的名稱。
6.	在 [主要管理伺服器] 方塊中，輸入主要管理伺服器的電腦名稱。
7.	在 [管理伺服器連接埠] 方塊中，輸入 TCP 連接埠號碼。
8.	在 [代理程式動作帳戶] 頁面下，選擇本機系統帳戶或本機網域帳戶。
9.	按一下 [確定] 關閉 [新增管理群組] 對話方塊，然後按一下 [確定] 關閉 [Microsoft 監視代理程式屬性] 對話方塊。
...
## (選擇性) 設定 Proxy 和防火牆設定
如果您環境中的 Proxy 伺服器或防火牆有網際網路存取上的限制，請參閱[在 Log Analytics 中設定 Proxy 和防火牆設定](log-analytics-proxy-firewall.md)以讓代理程式能與 OMS 服務通訊。
