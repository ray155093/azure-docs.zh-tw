...
# <a name="connect-windows-computers-to-log-analytics"></a>將 Windows 電腦連接到 Log Analytics
...
>[AZURE.NOTE] 若需要在 Azure 中執行的虛擬機器，可使用[虛擬機器擴充功能](log-analytics-azure-vm-extension.md)簡化安裝。
...
2. 在 [直接附加電腦] 下，按一下適用於電腦處理器類型的 [下載 Windows 代理程式] 來下載安裝檔案。
3. 按一下 [工作區識別碼] 右邊的複製圖示，然後將識別碼貼入記事本中。
4. 按一下 [主索引鍵] 右邊的複製圖示，然後將識別碼貼入記事本中。     
    ![複製工作區識別碼和主索引鍵](./media/log-analytics-windows-agents/oms-direct-agent-primary-key.png)
...
## <a name="optionally,-configure-agents-to-use-the-oms-log-analytics-forwarder"></a>(選擇性) 將代理程式設定為使用 OMS Log Analytics 轉寄站
