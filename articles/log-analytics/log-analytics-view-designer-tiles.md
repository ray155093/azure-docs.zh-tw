<properties
    pageTitle="Log Analytics 檢視設計工具圖格參考 | Microsoft Azure"
...
| **時間間隔** |
| Duration | 從目前日期到使用查詢的時間間隔之間的持續時間。  例如，如果指定 **7 天**，則查詢僅限於 7 天前至目前日期內建立的記錄。 |
| 結束資料位移 | 選擇性的位移，是從目前日期到使用主查詢的時間間隔之間的持續時間。  例如，如果指定 **-1 天**的 [結束資料位移] 以及 **7天**的 [持續時間]，則查詢僅限於 8 天前至昨天建立的記錄。 |
...
| **折線圖** | **> 圖說文字** |
| 圖說文字 | 圖格   顯示在圖說值之上的文字。 |
| 系列名稱 | 將用於圖說文字值之系列的屬性值。  如未提供任何系列，會使用查詢中的所有記錄。 |
