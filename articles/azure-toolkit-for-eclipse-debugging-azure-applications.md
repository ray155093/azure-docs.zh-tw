...
---
# <a name="debugging-azure-applications-in-eclipse"></a>在 Eclipse 中偵錯 Azure 應用程式
...
## <a name="debugging-a-specific-role-instance-in-a-multi-instance-deployment"></a>在多個執行個體的部署中偵錯特定的角色執行個體
...
例如，假設您計劃在您的部署中執行最多 5 個角色執行個體。 使用角色屬性對話方塊中的 [端點] 屬性頁，建立一個執行個體輸入端點，並為其指派一個公用連接埠範圍，而不是單一連接埠號碼。 例如，在 [公用連接埠] 輸入方塊中，指定 **81-85**。
...
## <a name="windows-only-to-debug-your-application-while-running-in-the-compute-emulator"></a>僅限 Windows：在計算模擬器中執行應用程式時偵錯
> [!NOTE]
> Azure 模擬器只在 Windows 中提供。 如果您使用 Windows 以外的作業系統，請略過本節。 
> 
> 

1. 建置要在模擬器中測試的專案：在 Eclipse 的專案總管中，以滑鼠右鍵按一下 [MyAzureProject]，依序按一下 [內容]、[Azure]，將 [建置目的]設為 [在模擬器中測試]。
2. 重建您的專案：從 Eclipse 功能表按一下 [專案]，然後按一下 [全部建置]。
...
12. 在計算模擬器指出應用程式正在執行後，在您的瀏覽器中執行 **http://localhost:8080/MyHelloWorld**。
     如果 [確認檢視參數] 對話方塊提示，按一下 [是]。
     您的偵錯工作階段現在應該執行到設定中斷點的程式碼。

這會告訴您如何在計算模擬器中偵錯；下一節說明如何偵錯部署至 Azure 的應用程式。

## <a name="debugging-notes"></a>偵錯注意事項
