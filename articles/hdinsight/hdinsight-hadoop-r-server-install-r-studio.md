...
> [!NOTE]
> 本文中描述的程序僅適用於在佈建叢集時未選擇安裝 RStudio Server 社群版的情況。  如果您已在佈建期間新增，則可以按一下 Azure 入口網站中叢集的 [R 伺服器儀表版] 圖格，然後在 [R Studio Server] 圖格上存取它。 
...
7. 建立到叢集的 SSH 通道，方法是將 HDInsight 叢集上的 `ssh -L localhost:8787:localhost:8787 USERNAME@CLUSTERNAME-ed-ssh.azurehdinsight.net` 對應至用戶端電腦。 您必須在開啟新的瀏覽器工作階段之前，建立 SSH 通道。
