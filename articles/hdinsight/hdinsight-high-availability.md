...
## <a name="configure-head-node-size"></a>設定前端節點大小
...
SDK 的情況十分類似。 使用 SDK 建立與佈建叢集的說明已記錄在 [使用 HDInsight .NET SDK](hdinsight-provision-clusters.md)。 超大型前端節點的組態要求將 `HeadNodeSize = NodeVMSize.ExtraLarge` 參數加入此程式碼中所使用的 `ClusterCreateParameters()` 方法。
...
* [使用 RDP 連接到 HDInsight 叢集](hdinsight-administer-use-management-portal.md#connect-to-clusters-using-rdp)
