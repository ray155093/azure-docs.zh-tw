...
description: "說明如何搭配使用 Notebook 和 Apache Spark 以根據未經處理資料建立結構描述、將它們儲存成資料表，然後針對資料表使用 BI 工具來分析資料等作業的逐步指示"
services: hdinsight
documentationcenter: 
author: nitinme
manager: jhubbard
...
## <a name="hivetable"></a>將未經處理資料儲存為資料表
...
將資料儲存為資料表後，下一節我們將使用 BI 工具連線資料表並執行進一步視覺化。
...
3. 建立 Notebook。 按一下 [新增]，然後按一下 [PySpark]。
    ...
    ![建立 Jupyter Notebook](./media/hdinsight-apache-spark-use-bi-tools/hdispark.note.jupyter.createnotebook.png "建立 Jupyter Notebook")
...
## <a name="tableau"></a>使用 Tableau Desktop 來分析資料表中的資料
