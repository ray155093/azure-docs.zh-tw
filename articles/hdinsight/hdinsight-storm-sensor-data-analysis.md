...
1. 開啟新的命令提示字元、殼層或終端機，將目錄變更至 **hdinsight-eventhub-example/TemperatureMonitor**，然後使用下列命令啟動拓撲︰
   ...
   > `storm jar target/TemperatureMonitor-1.0-SNAPSHOT.jar org.apache.storm.flux.Flux --local -R /no-hbase.yaml --filter dev.properties`
    ...
    此命令會以本機模式啟動 **no-hbase.yaml** 檔案中定義的拓撲。 **dev.properties** 檔案中包含的值可提供事件中樞的連線資訊。 啟動之後，拓撲會從事件中樞讀取項目，並將它們傳送到在本機電腦上執行的儀表板。 您應該會看見 Web 儀表板中出現數個線條，如下圖所示：
...
> [!IMPORTANT]
> 請注意，HDInsight 叢集的名稱是 **storm-BASENAME** 和 **hbase-BASENAME**，其中 BASENAME 是您提供給範本的名稱。 連接到叢集時，您會在稍後步驟中使用這些名稱。 另請注意，儀表板網站的名稱是 **basename-dashboard**。 您會在檢視儀表板時使用此名稱。
...
2. 使用 scp 將 **TemperatureMonitor-1.0-SNAPSHOT.jar** 檔案上傳至您的 Storm 叢集。 在下列範例中，以建立叢集時所提供的 SSH 使用者取代 **USERNAME**，並以您稍早提供的基底名稱取代 **BASENAME**。 出現提示時，請輸入 SSH 使用者的密碼。
   ...
   > 檔案可能需要幾分鐘的時間才能上傳完畢。
    ...
    使用 scp 來上傳 **dev.properties** 檔案，因為此檔案包含用來連接到事件中樞和儀表板的資訊。
