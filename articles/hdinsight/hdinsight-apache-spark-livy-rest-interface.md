...
# 使用 Apache Spark REST API 將遠端作業提交至 HDInsight Spark 叢集
...
您可以使用 Livy 執行互動式 Spark 殼層，或提交要在 Spark 上執行的批次作業。 本文將討論如何使用 Livy 提交批次作業。 下列語法會使用 Curl 向 Livy Spark 端點發出 REST API 呼叫。
...
## 請舉例說明
在本節中，我們將透過範例了解如何使用 Livy Spark 來提交批次作業、監視作業的進度，然後將作業刪除。 我們在此範例中使用的應用程式，就是 [建立獨立 Scala 應用程式，並在 HDInsight Spark 叢集上執行](hdinsight-apache-spark-create-standalone-application.md)一文中所開發的應用程式。 下列步驟假設：
...
1. 我們先確認 Livy Spark 正在叢集上執行。 我們可以取得執行中的批次清單，加以確認。 如果這是您第一次使用 Livy 執行作業，則應會傳回零。
...
## 在 HDInsight 3.5 叢集上使用 Livy Spark
...
**問題︰**如果您正在執行 Livy Spark 作業，並參考與叢集相關聯的其他儲存體之外部 jar，則作業將會失敗。
