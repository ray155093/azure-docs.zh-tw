...
> [AZURE.NOTE] 如需 HDInsight 叢集上支援的 Oozie 版本，請參閱 [HDInsight 所提供 Hadoop 叢集版本的新功能][hdinsight-versions]。
...
3. 使用下列命令將 **useooziewf.hql** 複製到 **wasbs:///tutorials/useoozie/useooziewf.hql**：
...
3. 使用以下命令，將 **workflow.xml** 檔案複製到 **wasbs:///tutorials/useoozie/workflow.xml**：

        hdfs dfs -copyFromLocal workflow.xml /tutorials/useoozie/workflow.xml

##<a name="create-the-database"></a>建立資料庫

依照 [建立 SQL Database](../sql-database/sql-database-get-started.md) 文件中的步驟，建立新的資料庫。 建立資料庫時，請使用 __oozietest__ 做為資料庫名稱。 並記下用於資料庫伺服器的名稱，因為下一節中需要用到該名稱。
...
8. Enter `exit` at the `1>` 以結束 tsql 公用程式。
...
2. 開啟 nano 編輯器後，請使用下列項目做為檔案的內容：
    ...
    * 將 **wasbs://mycontainer@mystorageaccount.blob.core.windows.net** 的所有執行個體替換為您之前收到的值。
    ...
    > [AZURE.WARNING] 您必須使用完整的 WASB 路徑，其中包含容器和儲存體帳戶做為路徑的一部分。 使用簡短格式 (wasbs:///) 會在作業開始時導致 RunHiveScript 動作失敗。
