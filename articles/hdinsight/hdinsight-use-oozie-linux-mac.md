---
title: 在以 Linux 為基礎的 HDInsight 上使用 Hadoop Oozie 工作流程 | Microsoft Docs
...
---
# <a name="use-oozie-with-hadoop-to-define-and-run-a-workflow-on-linux-based-hdinsight"></a>在以 Linux 為基礎的 HDInsight 上搭配 Hadoop 使用 Oozie 來定義並執行工作流程
...
> [!NOTE]
> 還有另一個選項可以定義與 HDInsight 搭配的工作流程，那就是 Azure Data Factory。 若要進一步了解 Azure Data Factory，請參閱[搭配 Data Factory 使用 Pig 和 Hive][azure-data-factory-pig-hive]。
> 
> 
...
> [!NOTE]
> `-p` 參數會使系統建立路徑中的所有目錄 (如果這些目錄尚未存在)。 **data** 目錄將用來保存 **useooziewf.hql** 指令碼所使用的資料。
> 
> 
...
4. 使用以下命令，將 **workflow.xml** 檔案複製到 **wasbs:///tutorials/useoozie/workflow.xml**：
...
1. 使用以下命令取得預設儲存體的完整 WASB 位址。 這稍後會在組態檔中用到：
    ...
    儲存 **wasbs://mycontainer@mystorageaccount.blob.core.windows.net** 值，後續的步驟中會使用該值。
...
4. 開啟 nano 編輯器後，請使用下列項目做為檔案的內容：
   ...
   * 將 **wasbs://mycontainer@mystorageaccount.blob.core.windows.net** 的所有執行個體替換為您之前收到的值。
     
     > [!WARNING]
     > 您必須使用完整的 WASB 路徑，其中包含容器和儲存體帳戶做為路徑的一部分。 使用簡短格式 (wasbs:///) 會在作業開始時導致 RunHiveScript 動作失敗。
     > 
     > 
...
6. 您可以在 [Job Info](工作資訊.md) 索引標籤中看到基本的工作資訊，以及工作內的個別動作。 使用上方的索引標籤，即可檢視 Job Definition (工作定義)、Job Configuration (工作組態)，以及存取 Job Log (工作記錄)，或檢視工作的定向非循環圖 (DAG)。
...
7. 在 [作業資訊] 索引標籤中選取其中一個動作，便會出現此動作的資訊。 例如，選取 **RunHiveScript** 動作。
...
2. 如果發生錯誤或特定動作失敗，請選取該動作，以查看 [Error Message](錯誤訊息.md)  欄位是否有提供失敗的詳細資訊。
...
### <a name="ja002:-oozie-is-not-allowed-to-impersonate-&lt;user>"></a>JA002：不允許 Oozie 模擬 &lt;USER>
**徵兆**：作業狀態會變更為 **SUSPENDED**。 作業的詳細資料會將 RunHiveScript 狀態顯示為 **START_MANUAL**。 選取該動作會顯示下列錯誤訊息：
...
* [在 HDInsight 上將 Hadoop 作業的資料上傳][hdinsight-upload-data]
* [在 HDInsight 中搭配 Hadoop 使用 Sqoop][hdinsight-use-sqoop]
