---
title: "在以 Linux 為基礎的 HDInsight 上安裝 GNU R | Microsoft Docs"
...
---
# <a name="install-and-use-gnu-r-on-linux-based-hdinsight-hadoop-clusters"></a>在以 Linux 為基礎的 HDInsight Hadoop 叢集上安裝和使用 GNU R

您可以使用 **指令碼動作** 叢集自訂，在 HDInsight 上 Hadoop 中的任何一種叢集上安裝 GNU R。 資料科學家和分析師可以使用 R 來處理部署在 HDInsight 中的 Hadoop 叢集上大量的資料。

> [!WARNING]
> 本文件提供在 HDInsight 上安裝 GNU R 作為自訂動作的的指示。 它__不__會提供與 __HDInsight 上的 Microsoft R Server__ 相同的功能。 我們建議您在 HDInsight 上使用 R 伺服器，而不是這份文件中的步驟。
>
> 如需如何在 HDInsight 上使用 R 伺服器的資訊，請參閱[開始在 HDInsight 上使用 R 伺服器](hdinsight-hadoop-r-server-get-started.md)。 
...
1. 使用[佈建以 Linux 為基礎的 HDInsight 叢集](hdinsight-hadoop-provision-linux-clusters.md)中的步驟開始佈建叢集，但是不完成佈建。

2. 在 [選用組態] 刀鋒視窗中，選取 [指令碼動作]，並使用下列資訊完成表單：
...
4. 繼續如[佈建以 Linux 為基礎的 HDInsight 叢集](hdinsight-hadoop-provision-linux-clusters.md)中所述佈建叢集。
...
3. 輸入下列 R 程式。 此程式碼會產生數字 1 到 100，然後將它們乘以 2。
...
4. 接下來，請使用下列項目查看儲存 MapReduce 輸出的目標暫存路徑：
    ...
    此命令會傳回如下值：`/tmp/file5f615d870ad2`。 若要檢視實際輸出，請使用下列命令：
