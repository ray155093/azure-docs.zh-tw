...
[!INCLUDE [僅限 Windows](../../includes/hdinsight-windows-only.md)]
...
* Apache Storm on HDInsight 叢集：請參閱[開始使用 Apache Storm on HDInsight](hdinsight-apache-storm-tutorial-get-started.md) 以取得建立叢集的步驟。
  ...
  > [!NOTE]
  > HDInsight Tools for Visual Studio 目前不支援提交 Storm 拓撲到 Linux 架構的叢集。
  > 
  > 
...
3. 使用 **Windows 檔案總管**找到包含您專案的目錄，例如 **C:\\Users<your\_user\_name>\\Documents\\Visual Studio 2013\\Projects\\WordCount\\WordCount**。在此目錄中，開啟 [Bin]，然後按一下 [偵錯]。您應該會看到執行測試時所產生的文字檔：sentences.txt、counter.txt 和 splitter.txt。開啟每個文字檔，並檢查資料。
