---
title: "使用 Azure Data Factory 在 HDInsight 中建立以 Linux 為基礎的隨選 Handooop 叢集 | Microsoft Docs"
...
> [!NOTE]
> 目前，您只可以從 Azure Data Factory 建立以 Linux 為基礎的 HDInsight 叢集 3.2 版。
>
>
...
> [!IMPORTANT]
> 記下您的指令碼中使用的資源群組名稱、儲存體帳戶名稱和儲存體帳戶金鑰。  您會在下一節中用到這些資料。
>
>
...
容器名稱為 adfgetstarted 。  讓它保持原狀。 否則，您必須更新 Resource Manager 範本。
...
備妥儲存體帳戶、輸入資料和 HiveQL 指令碼，您就準備好建立 Azure Data Factory。 有數種方法可建立 Data Factory。 在本教學課程中，您將使用 Azure 入口網站呼叫 Resource Manager 範本。 您也可以從 [Azure CLI](../azure-resource-manager/resource-group-template-deploy-cli.md) 和 [Azure PowerShell](../azure-resource-manager/resource-group-template-deploy.md#deploy) 呼叫 Resource Manager 範本。 如需其他 Data Factory 建立方法，請參閱 [教學課程︰建立您的第一個 Data Factory](../data-factory/data-factory-build-your-first-pipeline.md)。

最上層的 Resource Manager 範本包含︰
...
1. 按一下以下影像，在 Azure 入口網站中登入 Azure 並開啟 Resource Manager 範本。 範本是位於 https://hditutorialdata.blob.core.windows.net/adfhiveactivity/data-factory-hdinsight-on-demand.json。
...
* 將下列程式碼以及 Resource Manager 範本中的 Microsoft.DataFactory/datafactories 資源加入最上層資源群組。 會建立新的儲存體帳戶：
