...
* 對於 __以 Linux 為基礎__ 的 HDInsight 叢集，在建立叢集之後使用 Ambari 變更組態。
...
* 對於 __以 Windows 為基礎__ 的 HDInsight 叢集，使用遠端桌面連接到叢集。 如需詳細資訊，請參閱 [使用 RDP 連接到 HDInsight](hdinsight-administer-use-management-portal.md#connect-to-clusters-using-rdp) 。

    連接之後，請使用桌面上的 [Hadoop 命令列]  圖示來開啟命令提示字元。

* 對於 __以 Linux 為基礎__ 的 HDInsight 叢集，使用 SSH 連接到叢集。 如需有關搭配使用 SSH 與以 Linux 為基礎的叢集的詳細資訊，請參閱下列其中一份文件：
