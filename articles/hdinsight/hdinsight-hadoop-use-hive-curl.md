...
---
# <a name="run-hive-queries-with-hadoop-in-hdinsight-with-curl"></a>使用 Curl 在 HDInsight 中以 Hadoop 執行 Hive 查詢
...
2. 使用下列命令建立新資料表 **log4jLogs**：
   ...
   * **SELECT** - 選擇其資料欄 **t4** 包含值 **[ERROR]** 的所有資料列計數。 這應該會傳回值 **3** ，因為有&3; 個資料列包含此值。
     ...
     > 
     > 
   * **INPUT__FILE__NAME LIKE '%25.log'** - 這會限制該搜尋僅能使用其檔名以 .log 結尾的檔案。 如果不存在，Hive 便會嘗試搜尋這個目錄和子目錄中的所有檔案，包括不符合這份資料表定義之資料行結構描述的檔案。
     
     > [!NOTE]
     > 請注意 %25 是 % 的 URL 編碼格式，因此實際的條件是 `like '%.log'`。 % 必須為 URL 編碼，因為會將其視為在 URL 中的特殊字元。
     > 
     > 
