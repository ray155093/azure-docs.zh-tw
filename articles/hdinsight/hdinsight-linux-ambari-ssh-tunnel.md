...
4. 選取 [確定] 以新增 Proxy 並關閉 [Proxy 設定]。
...
1. 在瀏覽器中，移至 http://headnodehost:8080。 `headnodehost` 位址會透過通道傳送到叢集，並解析為執行 Ambari 的前端節點。 出現提示時，請輸入您叢集的管理員使用者名稱 (admin) 和密碼。 Ambari Web UI 可能會出現第二次的提示。 若是如此，請重新輸入資訊。
    ...
    > [AZURE.NOTE] 當您使用 http://headnodehost:8080 位址連接到叢集時，會使用 HTTP 並透過通道直接連接到執行 Ambari 的前端節點，並使用 SSH 通道來保護通訊安全。 若是不使用通道，而是透過網際網路連接時，就會使用 HTTPS 來保護通訊安全。 若要透過 HTTPS 與網際網路連接，請使用 https://CLUSTERNAME.azurehdinsight.net，其中 __CLUSTERNAME__ 是叢集的名稱。
...
4. 您應該會看到類似如下的頁面：
    ...
    > [AZURE.NOTE] 請注意此頁面的 URL，它應該會類似於 __http://hn1-CLUSTERNAME.randomcharacters.cx.internal.cloudapp.net:8088/cluster__。 這使用了節點的內部完整網域名稱 (FQDN)，而且必須使用 SSH 通道才能存取。
