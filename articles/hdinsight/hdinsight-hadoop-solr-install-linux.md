...
> [!NOTE]
> 此文件中的步驟需要以 Linux 為基礎的 HDInsight 叢集。如需搭配以 Windows 為基礎的叢集使用 Solr 的詳細資訊，請參閱[在 HDInsight Hadoop 叢集上安裝和使用 Solr (Windows)](hdinsight-hadoop-solr-install.md)。
> 
> 
...
1. 決定主要前端節點的主機名稱：
   ...
   2. 使用下列命令以取得完整主機名稱︰
       ...
       這是應該用於下列步驟中的主機名稱。
2. 在瀏覽器中，連接到 **http://HOSTNAME:8983/solr/#/**，其中 *_HOSTNAME\*\_ 是您在先前步驟中決定的名稱。
...
3. 接下來，將目錄變更為 **/usr/hdp/current/solr/example/solr**。在這裡每個集合會有子目錄。每個集合目錄包含__資料__目錄，這是該集合的快照所在的位置。
