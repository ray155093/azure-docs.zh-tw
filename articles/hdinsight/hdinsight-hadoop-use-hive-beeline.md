...
3. 若要儲存檔案，請使用 __Ctrl__+__X__，然後輸入 __Y__，最後按 __Enter__。
...
如果您有 Beeline 安裝在您的叢集外的用戶端上，您可以使用下列命令連接。 將 __CLUSTERNAME__ 取代為 HDInsight 叢集的名稱。 將 __PASSWORD__ 取代為管理員 (HTTP 登入) 帳戶的密碼。
