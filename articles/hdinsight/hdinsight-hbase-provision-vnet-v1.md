---
title: 在虛擬網路上佈建 HBase 叢集 | Microsoft Docs
...
> 
> 如需這份文件的「使用 Azure 入口網站搭配使用 Azure Resource Manager (ARM) 的 PowerShell 程式碼片段」版本，請參閱[在 Azure 虛擬網路上佈建 HBase 叢集](hdinsight-hbase-provision-vnet.md)。
> 
> 
...
> [!NOTE]
> HDInsight 叢集使用 Azure Blob 儲存體來儲存資料。如需詳細資訊，請參閱[在 HDInsight 中搭配使用 Azure Blob 儲存體和 Hadoop](hdinsight-hadoop-use-blob-storage.md) (英文)。您將會需要儲存體帳戶和 Blob 儲存體容器。儲存體帳戶的位置必須符合虛擬網路位置和叢集位置。
> 
> 
...
> [!NOTE]
> 如需使用 Azure PowerShell 佈建新 HBase 叢集的相關資訊，請參閱[使用 Azure PowerShell 佈建 HBase 叢集](#powershell)。
> 
> 
...
1. 在 [設定叢集] 頁面上，輸入或選取下列內容：
   ...
   > 
    ...
    按一下向右按鈕。
2. 輸入此叢集的 Hadoop 使用者名稱及密碼，然後按一下右鍵。
...
3. 在 [儲存體帳戶] 頁面上，提供下列值：
    ...
    按一下向右箭頭。
4. 在 [指令碼動作] 頁面上，選取右下角的核取記號。請不要點選 [加入指令碼動作] 按鈕，因為本教學課程不需要自訂叢集設定。
   
    ![設定指令碼動作以自訂 HDInsight HBase 叢集][img-provision-cluster-page5]
   
   > [!NOTE]
   > 設定期間可使用此頁面自訂叢集。如需詳細資訊，請參閱[使用指令碼動作自訂 HDInsight 叢集](hdinsight-hadoop-customize-cluster.md)。
   > 
   > 
...
2. 使用 Java 應用程式從遠端連接到 HBase 時，您必須使用完整網域名稱 (FQDN)。若要決定此名稱，您必須取得 HBase 叢集的連線特定 DNS 尾碼。請使用 Curl 以查詢 Ambari，或使用遠端桌面連接到叢集。
   ...
   * **Azure PowerShell** - 使用下列 Azure PowerShell 指令碼以註冊 **Get-ClusterDetail** 函數，並可使用該函數傳回 DNS 尾碼：
   ...
   > [!NOTE]
   > 您也可以使用遠端桌面連接到 HBase 叢集 (將連接到前端節點)，再從命令提示字元執行 **ipconfig** 以取得 DNS 尾碼。如需啟用遠端桌面通訊協定 (RDP) 並使用 RDP 連接到叢集的指示，請參閱[使用 Azure 傳統入口網站在 HDInsight 中管理 Hadoop 叢集][hdinsight-admin-portal]。
   > 
...
> [!NOTE]
> 如需 Azure 虛擬網路中的名稱解析的詳細資訊，包括如何使用您自己的 DNS 伺服器，請參閱[名稱解析 (DNS)](../virtual-network/virtual-networks-name-resolution-for-vms-and-role-instances.md)。
> 
> 
