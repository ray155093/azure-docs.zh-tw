---
title: "從 Windows 上的 PuTTY 搭配 HDInsight 叢集使用 SSH | Microsoft Docs"
description: "了解如何從以 Windows 為基礎的用戶端使用 PuTTY SSH 用戶端來建立和使用 SSH 金鑰，驗證以 Linux 為基礎的 HDInsight 叢集。"
services: hdinsight
documentationcenter: 
author: Blackmist
manager: jhubbard
...
---
# <a name="use-ssh-with-hdinsight-hadoop-from-putty-on-windows"></a>從 Windows 上的 PuTTY 搭配 HDInsight (Hadoop) 使用 SSH
...
> [!NOTE]
> 本文中的步驟假設您使用以 Windows 為基礎的用戶端，並搭配 PuTTY SSH 用戶端。 如果您使用提供 `ssh` 命令的 Linux、Unix、OS X 或 Windows 系統，請參閱[從 Linux、Unix 或 OS X 在 HDInsight 上搭配使用 SSH 與以 Linux 為基礎的 Hadoop](hdinsight-hadoop-linux-use-ssh-unix.md)。

## <a name="prerequisites"></a>必要條件

* 適用於以 Windows 為基礎的用戶端的 **PuTTY** 和 **PuTTYGen**。 這些公用程式可從下列位置取得：[http://www.chiark.greenend.org.uk/~sgtatham/putty/download.html](http://www.chiark.greenend.org.uk/~sgtatham/putty/download.html)。
* 支援 HTML5 的新式網頁瀏覽器。
...
根據記錄，Windows 尚未提供 SSH 用戶端。 PuTTY 是可安裝在 Windows 上的圖形 SSH 用戶端。
...
5. 按一下 [儲存私密金鑰] 將金鑰儲存到 **.ppk** 檔案。 此金鑰會用來驗證以 Linux 為基礎的 HDInsight 叢集。
   ...
   > [!NOTE]
   > 此金鑰可用來存取以 Linux 為基礎的 HDInsight 叢集，因此請將它儲存在安全的位置。

6. 按一下 [儲存公開金鑰] 將金鑰儲存為 **.txt** 檔案。 這可讓您在日後建立其他以 Linux 為基礎的 HDInsight 叢集時重複使用此公開金鑰。
   
   > [!NOTE]
   > PuTTYGen 的頂端也會顯示此公開金鑰。 當您使用 Azure 入口網站建立叢集時，可以用滑鼠右鍵按一下此欄位，複製值並將其貼入表單。

## <a name="create-a-linux-based-hdinsight-cluster"></a>建立以 Linux 為基礎的 HDInsight 叢集
...
如果您選取 [SSH 公開金鑰]，則可以將公開金鑰 (在 PuttyGen 會顯示於 [貼到 OpenSSH 授權金鑰檔案的公開金鑰] 欄位) 貼入 [SSH 公開金鑰] 欄位，或選取 [選取檔案] 以瀏覽和選取包含公開金鑰的檔案。\_
...
## <a name="connect-to-a-linux-based-hdinsight-cluster"></a>連線至以 Linux 為基礎的 HDInsight 叢集
...
3. 在 [類別] 中，選取 [工作階段]。 在 [PuTTY 工作階段的基本選項] 畫面上，將您的 HDInsight 伺服器的 SSH 位址輸入到 [主機名稱 (或 IP 位址)] 欄位。 連接至叢集時，您可以使用兩個可能的 SSH 位址：
    ...
    * **前端節點位址**︰若要連接到叢集的前端節點，請使用您的叢集名稱加上 **-ssh.azurehdinsight.net**。 例如， **mycluster-ssh.azurehdinsight.net**。
   
    * **邊緣節點位址**︰如果您要連接到 HDInsight 叢集上的 R Server，您可以使用位址 **RServer.CLUSTERNAME.ssh.azurehdinsight.net** (其中 CLUSTERNAME 是叢集名稱) 連接到 R Server 邊緣節點。 例如，**RServer.mycluster.ssh.azurehdinsight.net**。
...
5. 按一下 [ **開啟** ] 來連線到叢集。
   ...
   > [!NOTE]
   > 如果這是您第一次連線到叢集，您會收到安全性警示。 這是正常現象。 請選取 [ **是** ] 快取伺服器的 RSA2 金鑰以繼續進行。

6. 出現提示時，輸入您建立叢集時所輸入的使用者。 如果您對使用者提供密碼，系統會提示您一併輸入密碼。

> [!NOTE]
> 上述步驟假設您使用連接埠 22，這將會連接到 HDInsight 叢集上的主要前端節點。 如果您使用連接埠 23，您將會連接到次要前端節點。 如需前端節點的詳細資訊，請參閱 [HDInsight 上 Hadoop 叢集的可用性和可靠性](hdinsight-high-availability-linux.md)。
...
3. 當瀏覽對話方塊顯示時，請選取包含金鑰的 .ppk 檔案，然後按一下 [ **開啟**]。 這樣會將金鑰新增至 Pageant，在連接至叢集時將金鑰提供給 PuTTY。
   ...
   > [!IMPORTANT]
   > 如果您使用 SSH 金鑰來保護您的帳戶的安全，您必須先完成上述步驟，才能連接至背景工作節點。
...
7. 建立連接之後，請使用下列項目以擷取您的叢集中的節點清單。 將 *ADMINPASSWORD* 取代為您的叢集系統管理帳戶的密碼。 將 *CLUSTERNAME* 取代為您叢集的名稱。
