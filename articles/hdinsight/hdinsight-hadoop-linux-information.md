...
2. 使用 [Azure CLI](../xplat-cli-install.md) 取得儲存體帳戶的資源群組。在下列命令中，將 **ACCOUNTNAME** 取代為從 Ambari 擷取的儲存體帳戶名稱：
   ...
   > [!NOTE]
   > 如果此命令未傳回任何項目，您需要將 Azure CLI 變更為 Azure 資源管理員模式，然後再重新執行命令。若要切換至 Azure 資源管理員模式，請使用下列命令。
   > 
   > `azure config mode arm`
   > 
   > 
3. 取得儲存體帳戶的金鑰。將 **GROUPNAME** 取代為上一個步驟的資源群組。將 **ACCOUNTNAME** 取代為儲存體帳戶名稱：
