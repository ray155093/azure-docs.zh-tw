...
* Apache Storm on HDInsight 叢集。 使用下列其中一個使用者入門文章以建立叢集：
    ...
    > [AZURE.NOTE] 本文件中的步驟是以在 HDInsight 叢集 3.3 或 3.4 使用 Storm 為基礎。 這些叢集提供 Storm 0.10.0 和 Hadoop 2.7，可減少為了讓此範例運作所需進行的步驟數目。
    >
...
* 如果您是在 HDInsight 叢集上使用 __以 Linux 為基礎的__ Storm，請參閱 [部署和管理以 Linux 為基礎的 HDInsight 上的 Apache Storm 拓撲](hdinsight-storm-deploy-monitor-topology-linux.md)

* 如果您是在 HDInsight 叢集上使用 __以 Windows 為基礎的__ Storm，請參閱 [部署和管理以 Windows 為基礎的 HDInsight 上的 Apache Storm 拓撲](hdinsight-storm-deploy-monitor-topology-linux.md)
