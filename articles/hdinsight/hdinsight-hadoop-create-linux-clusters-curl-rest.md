...
Azure Resource Manager 範本是描述**資源群組**與其中所有資源 (例如 HDInsight) 的 JSON 文件。這個以範本為基礎的方法可讓您在一個範本中定義 HDInsight 所需的所有資源，並透過可將變更套用至群組的**部署**，整體管理群組的變更。
...
> [!NOTE]
> 以下步驟是*使用 Azure CLI 建立用來存取資源的服務主體*文件中[使用密碼建立服務主體](../azure-resource-manager/resource-group-authenticate-service-principal-cli.md#create-service-principal-with-password)一節所提供之資訊的簡易版。 這些步驟可建立新的服務主體，用來驗證用於建立 Azure 資源 (例如 HDInsight 叢集) 的 REST API 要求。
...
2. 使用以下命令在 Azure Active Directory 中建立新的應用程式。
...
4. 使用 **Object ID** 值將 **Owner** 角色指派給服務主體。 您也必須使用稍早取得的 **訂用帳戶 ID** 。
