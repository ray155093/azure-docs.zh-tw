...
> 
> 如需這份文件的「使用 Azure 入口網站搭配使用 Azure Resource Manager (ARM) 的 PowerShell 程式碼片段」版本，請參閱[在 HDInsight 叢集上安裝 Giraph](hdinsight-hadoop-giraph-install.md)。
> 
> 
...
2. 執行 SimpleShortestPathsComputation 範例。使用 tiny\_graph.txt 檔案做為輸入，即可使用下列 Azure PowerShell Cmdlet 來執行此範例。您必須已安裝和設定 [Azure PowerShell][powershell-install]。
...
3. 檢視結果。一旦工作完成，結果會儲存在 **wasbs:///example/out/shotestpaths** 資料夾中的兩個輸出檔中。這些檔案稱為 **part-m-00001** 和 *_part-m-00002\*\_。執行下列步驟來下載和檢視輸出：
