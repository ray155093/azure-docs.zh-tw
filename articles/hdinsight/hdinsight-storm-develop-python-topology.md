...
1. 建置 uber jar：
    ...
    這樣會在此專案的 `/target` 目錄中建立名為 __WordCount--1.0-SNAPSHOT.jar__ 的檔案。
...
* __resources/resources__：Leiningen 建立預設的 `resources` 目錄，不過，這裡儲存的檔案似乎會新增至根據這個專案所建立的 jar 檔案的根目錄，但 Storm 需要這些檔案位於名為 `resources` 的子目錄中。 因此加入一個子目錄，Python 檔案就儲存在 `resources/resources`中。 在執行階段，這當成存取 Python 元件時的根目錄 (/)。
