---
title: "設定兩個 Azure 虛擬網路之間的 DNS | Microsoft Docs"
...
---
# <a name="configure-dns-between-two-azure-virtual-networks"></a>設定兩個 Azure 虛擬網路之間的 DNS
...
* **具備 VPN 連線的兩個 Azure 虛擬網路**。  如需指示，請參閱[設定兩個 Azure 虛擬網路之間的 VPN 連線][hdinsight-hbase-geo-replication-vnet]。
...
## <a name="create-azure-virtual-machines-to-be-used-as-dns-servers"></a>建立要做為 DNS 伺服器使用的 Azure 虛擬機器
...
## <a name="add-the-dns-server-role-the-two-virtual-machines"></a>新增兩個虛擬機器的 DNS 伺服器角色
...
5. 在 RDP 工作階段中，按一下左下角的 [Windows] 按鈕以開啟 [開始] 畫面。
6. 按一下 [ **伺服器管理員** ] 磚。
7. 按一下 [ **新增角色與功能**]。
8. 依序按一下 [ **下一步**
9. 選取 [角色型或功能型安裝]，然後按一下 [下一步]。
10. 選取您的 DNS 虛擬機器 (它應該已反白顯示)，然後按 [ **下一步**]。
11. 核取 [ **DNS 伺服器**]。
12. 按一下 [新增功能]，然後按一下 [繼續]。
13. 按三次 [下一步]，然後按一下 [安裝]。
...
1. 從 Azure 傳統入口網站，依序按一下 [新增]、[網絡服務]、[虛擬網路]、[註冊 DNS 伺服器]。
2. 輸入：
...
## <a name="test-the-name-resolution-across-the-virtual-networks"></a>測試虛擬網路之間的名稱解析
