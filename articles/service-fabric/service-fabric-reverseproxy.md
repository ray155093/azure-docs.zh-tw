---
title: "Service Fabric 反向 Proxy | Microsoft Docs"
...
---
# <a name="service-fabric-reverse-proxy"></a>Service Fabric 反向 Proxy
...
如需有關本主題的詳細資訊，請參閱 [與服務進行通訊](service-fabric-connect-and-communicate-with-services.md)。

### <a name="communicating-via-sf-reverse-proxy"></a>透過 SF 反向 Proxy 進行通訊
...
### <a name="reaching-microservices-via-the-sf-reverse-proxy-from-outside-the-cluster"></a>從叢集外部透過 SF 反向 Proxy 連線到微服務
您不需要在 Azure Load Balancer 設定個別服務的連接埠，只需在 Azure 負載平衡器設定 SF 反向 Proxy 連接埠。 這樣可允許叢集外部的用戶端透過反向 Proxy 連線到叢集內部的服務，而不需要額外設定。

![外部通訊][0]

> [!WARNING]
> 在負載平衡器上設定反向 Proxy 的連接埠，將叢集中公開 http 端點的所有微服務設為可從叢集外部尋址。
> 
> 

## <a name="uri-format-for-addressing-services-via-the-reverse-proxy"></a>透過反向 Proxy 定址服務的 URI 格式
...
* **http(s)：** 反向 Proxy 可設定為接受 HTTP 或 HTTPS 流量。 如果是 HTTPS 流量，SSL 終止會發生在反向 Proxy。 反向 Proxy 是透過 HTTP 轉送要求到叢集中的服務。 **請注意，目前不支援 HTTPS 服務。**
* **叢集 FQDN | 內部 IP：**如果是外部用戶端，可以設定反向 Proxy 讓其可透過叢集網域設定 (例如 mycluster.eastus.cloudapp.azure.com)。 依預設，反向 Proxy 會在每個節點上執行，因此可以在 localhost 或任何內部節點 IP (例如 10.0.0.1) 上達到內部流量。
* **連接埠︰** 為反向 Proxy 指定的連接埠。 例如 19008。
* **ServiceInstanceName：** 這是您嘗試連線到 "fabric: /" 配置的 SANS 服務的完整部署服務執行個體名稱 。 例如，若要連線到 fabric:/myapp/myservice/ 服務，可以使用 myapp/myservice。
...
1. 在範本的 [參數區段](../azure-resource-manager/resource-group-authoring-templates.md) 定義反向 Proxy 的連接埠。
    ...
    ```
2. 在 **叢集** [resources type 區段](../azure-resource-manager/resource-group-authoring-templates.md)
   
    在 '2016-09-01' 之前的 apiVersion，連接埠由是參數名稱 ***httpApplicationGatewayEndpointPort*** 識別
    ...
    若 apiVersion 在 '2016-09-01' 當天或之後，連接埠由是參數名稱 ***reverseProxyEndpointPort*** 識別
...
3. 若要從 Azure 叢集外部定址反向 Proxy，請為步驟 1 指定的連接埠設定 **Azure Load Balancer 規則** 。
    ...
    ```
4. 若要在連接埠上設定反向 Proxy 的 SSL 憑證，請將憑證新增至 **叢集** [resources type 區段](../azure-resource-manager/resource-group-authoring-templates.md)
   
    若 apiVersion 在 '2016-09-01' 之前，憑證由是參數名稱 ***httpApplicationGatewayCertificate*** 識別
    ...
    ```
    若 apiVersion 在 '2016-09-01' 當天或之後，憑證由是參數名稱 ***reverseProxyCertificate*** 識別
