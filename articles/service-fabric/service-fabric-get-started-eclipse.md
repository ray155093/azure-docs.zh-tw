...
1. 確定已安裝最新版 Eclipse **Neon** 和最新的 Buildship 版本 (1.0.17 或更新版本)。 您可以選擇 ``Help => Installation Details``，檢查已安裝的元件版本。 您可以使用[這裡][buildship-update]的指示更新 Buildship。 若要檢查 Eclipse Neon 是否為最新版本並加以更新，您可以移至 ``Help => Check for Updates``。

2. 若要安裝 Service Fabric 外掛程式，請選擇 ``Help => Install New Software...``。
...
> [!NOTE]
> 如果在 Eclipse 上安裝或更新 Service Fabric Eclipse 外掛程式時需要很久的時間，這是因為 Eclipse 每次都會嘗試擷取對 Eclipse 執行個體註冊之所有更新網站所發生的所有新變更的中繼資料。 因此，若要加快程序，您可以使用這個小秘訣 - 您可以移至 `Available Software Sites`，並取消核取指向 Service Fabric 外掛程式位置 `http://dl.windowsazure.com/eclipse/servicefabric` 之項目以外的所有項目。
>

## <a name="create-service-fabric-application-using-eclipse"></a>使用 Eclipse 建立 Service Fabric 應用程式

1. 移至 ``File => New => Other``。 選取 ``Service Fabric Project``。 按一下 ``Next``。
...
3. 從範本 (執行者、無狀態、容器或來賓可執行檔) 的可用集合中選取服務範本。 按一下 ``Next``。

    ![Service Fabric 新專案第 3 頁][create-application/p3]

4. 在此頁面上輸入服務名稱和/或相關的服務詳細資料，然後按一下 ``Finish``。

    ![Service Fabric 新專案第 4 頁][create-application/p4]

5. 當您建立第一個 Service Fabric 專案時，系統會詢問您是否要設定 Service Fabric 檢視方塊，請選取 ``yes`` 以繼續。
...
* 以滑鼠右鍵按一下您剛才在上面建立的 Service Fabric 應用程式。 在內容功能表中選取選項 ``Service Fabric``。 這會帶出有多個選項的子功能表。 看起來像這樣 -
  ...
  - ``Build Application`` 會建置應用程式而不進行清除
  - ``Rebuild Application`` 會對應用程式執行清除建置
  - ``Clean Application`` 會清除所建置構件的應用程式


* 您也可以從這個功能表選擇部署、解除部署和發佈應用程式。
  - ``Deploy Application`` 會部署到您的本機叢集
  - ``Publish Application...`` 會詢問您要選取 ``Local.json`` 還是 ``Cloud.json`` 發佈設定檔。 這些 JSON 檔案會用來儲存連線到本機或雲端 (Azure) 叢集所需的資訊 (例如連線端點和安全性資訊)。

  ![Service Fabric 快顯功能表][publish/Publish]

* 另一種方式是使用 Eclipse 執行組態來部署 Service Fabric 應用程式。

  1. 選擇 ``Run => Run Configurations``。 在 ``Grade Project`` 底下選取 ``ServiceFabricDeployer`` 執行組態。
  2. 在右窗格的 ``Arguments`` 索引標籤底下，指定 [本機] 或 [雲端] 來做為 ``publishProfile``。 預設設定是 [本機]。 若是部署至遠端/雲端叢集，請選取 [雲端]。
  3. 視情況編輯 `Local.json` 或 `Cloud.json`，以確保發佈設定檔中已填入適當的資訊，以及端點詳細資訊和安全性認證 (如果有的話)。
  4. 確定右窗格上位於 ``Grade Project`` 底下的 ``Working Directory`` 指向您要部署的應用程式。 如果不是，請按一下 ``Workspace...`` 按鈕，然後選取您要的應用程式。
  5. 依序按一下 [套用] 及 [執行]。
...
2. 選取 `Add ServiceFabric Service` 選項，讓它引導您完成下一組步驟，以將服務新增至專案。
...
1. 選擇 ``Run => Run Configurations``。 按一下左窗格中 ``Grade Project`` 左邊的小箭號。
2. 以滑鼠右鍵按一下 ``ServiceFabricDeployer``，然後選取 ``Duplicate``。 為此組態指定新名稱，例如 ``ServiceFabricUpgrader``。
3. 在右窗格的 ``Arguments`` 索引標籤底下，將 ``-Pconfig='deploy'`` 變更為 ``-Pconfig=upgrade``，然後按一下 ``Apply``。
4. 現在，您已建立並儲存用來升級應用程式的執行組態，您可以在想要時 ``Run``。 這會處理從應用程式資訊清單檔中取得最新版更新應用程式類型版本的程序。
