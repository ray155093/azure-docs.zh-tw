...
## <a name="stateless-and-stateful-micrososervices-for-service-fabric"></a>無狀態與具狀態的 Service Fabric 微服務
...
* 開發可大幅調整且自我修復的應用程式。
* 使用 Service Fabric 程式設計模型，可開發由微服務所組成的應用程式。 或者，可以只是裝載您選擇的來賓執行檔和其他應用程式架構，例如 ASP.NET Core 1 或 Node.js。
* 開發高度可靠的無狀態與可設定狀態微服務。
