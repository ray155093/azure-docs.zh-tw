...
> [!NOTE]
> 預設的 ActorStateProvider (即 **KvsActorStateProvider**) **不會**自行清除備份資料夾 (位於透過 ICodePackageActivationContext.WorkDirectory 取得的應用程式工作資料夾下方)。 這樣可能會填滿您的工作資料夾。 將備份移到外部儲存體之後，應該明確地清除備份回呼中的備份資料夾。
> 
> 
