---
title: "在 Azure 微服務中儲存應用程式狀態 |Microsoft Docs"
...
可靠的集合具有現成的增強式一致性保證，可讓您更輕鬆地推論應用程式的狀態。
...
這可讓可靠的集合只在記憶體中套用作業。
由於記錄檔會保存，即使複本失敗而必須重新啟動，可靠的狀態管理員在其記錄檔中仍具有足夠的資訊，以重新執行複本已遺失的所有作業。
...
如此一來，當複本必須重新啟動時，可靠的集合將會復原其檢查點的狀態，而可靠的狀態管理員則會復原並播放在該檢查點之後發生的所有狀態變更。

> [!NOTE]
> 檢查點加入另一個值可改善一般情況下的復原效能。
> 這是因為檢查點只包含最新的版本。
> 
> 
