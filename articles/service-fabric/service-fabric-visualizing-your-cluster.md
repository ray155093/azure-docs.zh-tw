...
>[AZURE.TIP] 按一下每個元素旁邊的省略符號，可以從樹狀檢視執行相同的動作。
...
| 節點 | 啟動 | 啟動節點。 |
|| 停用 (暫停) | 在其目前的狀態中暫停節點。服務將會繼續執行，但 Service Fabric 並不會主動在節點上移入或移出任何項目，除非有需要這樣做來避免服務中斷或資料不一致的問題。這個動作通常用來在特定節點上啟用偵錯服務，以確保它們不會在檢查期間移動。 |
