...
- 如果執行時間超過預期時間，執行工作的次要服務就會回報。它會回報 **PendingTasks** 屬性上的服務執行個體。這樣會指出服務執行個體有問題，但它不會擷取執行個體停止的情況。因為報告在該時間已清除完畢。它可以回報次要服務。如果次要服務完成工作，則次要執行個體會從存放區清除報告。如此並不會擷取到認可訊息遺失的情況，而從主要服務觀點看來，工作並未完成。
