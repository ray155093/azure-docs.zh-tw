...
同質性對於微服務環境而言，乍看之下似乎不具太大意義。同質性是一個控制項，主要提供來協助簡化將先前已存在且較大型的單體式應用程式轉換到雲端和微服務世界的程序。也就是說，在某些情況下它也可當做提升服務效能的合法最佳化，雖然並非沒有副作用。
...
3.	一切都沒問題，但事實上，這兩個元件具有非常多對話/效能敏感的內容，所以當我將它們移到個別服務時，我的效能會遭到重挫，且整個應用程式無法使用。
...
### 盡力而為的期望狀態
在同質性與單體式架構之間有一些差異。它們幾乎全部都可歸結為下列事實：同質性關聯性就是盡力而為 – 例如，由於它們基本上是不同的服務，因此可單獨失敗。有其他因素會導致同質性之服務的不同複本或執行個體分隔開來，例如容量限制。在這些情況下，即使有同質關聯性，由於其他限制，它暫時不會強制執行。如果稍後可以強制執行所有其他條件約束和同質性，它會自動更正。

### 鏈結與星形的比較
我們目前無法建立同質關聯性鏈結的模型。這表示是，如果有一個服務是某一個同質關聯性中的子系，則該服務不能是另一個同質關聯性中的父系。如果您想要建立這種關聯性的模型，您必須有效地將它的模型建立為星形，而不是鏈結，方法是改為將最下層子系的父系設定為「中間」子系的父系。根據您的服務的排列方式，這可能需要建立「預留位置」服務做為多個子系的父系。
...
- 使用者嘗試使用同質性的許多原因，例如限制服務在小的機器集合上執行，以及嘗試彙總服務集合的負載，實際上透過應用程式群組能獲得更佳的支援。請參閱[應用程式群組](service-fabric-cluster-resource-manager-application-groups.md)
