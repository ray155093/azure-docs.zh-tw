...
## <a name="create-and-run-the-web-performance-and-load-test-project"></a>建立及執行 Web 效能和負載測試專案
...
1. 在您的 Web 效能和負載測試專案的捷徑功能表上選擇 [新增]  > 取得一個免費帳戶。 在 [負載測試] 精靈中，選擇 [下一步] 按鈕以設定測試設定。
...
4. 在 [回合設定]  區段中，指定負載測試持續期間。
   ...
   > [!NOTE]
   > [測試反覆項目] 選項只有當您使用 Visual Studio 在本機執行負載測試時才可以使用。
   >
   >
5. 在 [回合設定] 的 [位置] 區段中，指定產生負載測試要求的位置。 此精靈可能會提示您登入您的 Team Services 帳戶。 登入，然後選擇一個地理位置。 完成時，選擇 [完成]  按鈕。
6. 建立負載測試之後，開啟 取得一個免費帳戶。loadtest 專案並選擇目前回合設定，例如 [回合設定]  > **1 [Active]**取得一個免費帳戶。 這會在 [屬性]  視窗中開啟回合設定。
...
### <a name="run-the-load-test-by-using-visual-studio-team-services"></a>使用 Visual Studio Team Services 執行負載測試
選擇 [執行負載測試]  命令來啟動測試回合。
