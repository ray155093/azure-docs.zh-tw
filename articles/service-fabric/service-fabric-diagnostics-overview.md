...
---
# <a name="monitoring-and-diagnostics-in-azure-service-fabric"></a>在 Azure Service Fabric 中監視與診斷

監視與診斷對於在生產環境中開發、測試並成功部署而言，極為重要。 若您一開始就實作監視與診斷，Service Fabric 解決方案效果最佳，因為這有助於確保服務在本機開發環境與實際生產叢集安裝中能順暢地運作。
...
監視與診斷很重要，它能確保所有一切都能如預期運作，並讓您在回應狀況時服務中斷的時間最短。 

監視與診斷的整體工作流程包含三個步驟： 
1. **事件產生**：這包含基礎結構 (叢集) 和應用程式/服務層級中的事件 (記錄、追蹤、自訂事件) 
2. **事件彙總**：產生的事件必須經過收集與彙總後，才能夠顯示。 這可透過由 Windows Azure 診斷設定的儲存體資料表，或藉由建立 EventFlow 管線完成
3. **分析**︰事件必須已視覺化並可藉由某種形式存取，才能視需要進行分析及顯示。 您可以選擇如 ApplicationInsights、Operations Management Suite、Kibana 等工具，以及數種其他工具

雖然有很多產品已涵蓋所有這三個領域，但許多客戶會針對每個層面選擇不同的技術。 重要的是每項技術能夠互相配合，為應用程式提供端對端的監視解決方案。

根據您的需求，您可以進一步擴充診斷和監視，以包含自動化的警示與緩和功能，這些功能通常內建到您選擇使用的分析工具。 

## <a name="event-generation"></a>事件產生

事件、記錄和追蹤會從基礎結構層 (來自叢集、機器或 Service Fabric 動作的所有一切) 或從應用程式層 (任何新增至部署到叢集之應用程式與服務的檢測) 產生。

### <a name="infrastructure-monitoring-the-cluster"></a>基礎結構：監視叢集

Service Fabric 有助於在基礎結構失敗時讓應用程式繼續執行，但您仍須了解錯誤是發生於應用程式，還是發生於基礎結構。 您也需要監視基礎結構來規劃容量，以了解何時該新增或移除基礎結構。 重點是監視組成 Service Fabric 部署的基礎結構和應用程式，並進行疑難排解。 即使應用程式可供客戶使用，基礎結構仍可能是潛在的問題來源。 為了確保叢集能如預期運作，Service Fabric 會設定您有五個不同現成的記錄通道：

1. 操作通道：由 Service Fabric 與叢集執行的高階作業，包括即將進行的節點事件、正在部署的新應用程式，或 SF 升級復原等等。 
2. 客戶資訊通道：健康情況報告與負載平衡決策
3. Reliable Services 事件：程式設計模型特定的事件
4. Reliable Actors 事件︰程式設計模型的特定事件和效能計數器
5. 支援的記錄：由 Service Fabric 產生的系統記錄僅供我們在提供支援時使用

強烈建議在建立叢集時啟用「診斷」。 這可在入口網站中完成，如下所示，或使用包含診斷的 Azure Resource Manager 範本完成。 

![在診斷啟用的狀況下建立 Azure 入口網站叢集](./media/service-fabric-diagnostics-overview/azure-enable-diagnostics.png)

如上所示，也有選擇性欄位可新增 Application Insights (AppInsights) 檢測索引鍵。 如果您選擇使用 AppInsights 進行任何事件分析 (AppInsights 是其中一個建議的解決方案)，請在此包含 AppInsights 資源 instrumentationKey (GUID)。
...
### <a name="application-instrumenting-code-for-custom-events"></a>應用程式：自訂事件檢測程式碼
...
## <a name="event-aggregation-and-collection"></a>事件彙總與集合
...
[Microsoft Diagnostics EventFlow](https://github.com/Azure/diagnostics-eventflow) 可以將節點的事件路由傳送至一個或多個監視目的地。 由於是以 NuGet 套件形式加入服務專案中，EventFlow 程式碼和組態會隨著服務一起傳送，不會發生稍後提及有關 Azure 診斷的個別節點組態問題。 EventFlow 在您的服務處理序中執行，並直接連線到已設定的輸出。 因為是直接連線，所以 EventFlow 適用於 Azure、容器和內部部署的服務部署。 如果您在高密度情況下執行 EventFlow，例如在容器中，請務必小心，因為每個 EventFlow 管線來建立外部連線。 如果您裝載許多處理序，將會產生大量輸出連線！ 這對 Service Fabric 應用程式來說不會有什麼問題，因為 `ServiceType` 的所有複本都在同一處理序中執行，這會限制輸出連線的數量。 EventFlow 也提供事件篩選，因此只會傳送符合指定篩選的事件。 如需有關搭配 Service Fabric 使用 EventFlow 的詳細資訊，請參閱[直接從 Azure Service Fabric 服務處理序收集記錄](service-fabric-diagnostic-collect-logs-without-an-agent.md)。
...
監視的最後一個部分是視覺化事件資料流、報告服務效能以及在偵測到問題時發出警示。 針對監視的這個部分，您可以使用不同的解決方案。 AppInsights 與 Operations Management Suite (OMS) 會根據事件資料流進行警示。 您可以使用 Microsoft Power BI 或協力廠商解決方案 (例如 [Kibana](https://www.elastic.co/products/kibana) 或 [Splunk](https://www.splunk.com/)) 將資料視覺化。

### <a name="appinsights"></a>AppInsights

AppInsights 是建議用來監視應用程式和服務的工具之一。 更新後的 AI.SDK 可以完善地進行 Service Fabric 事件，除了提供良好的資料視覺效果與查詢工具 (透過 AppInsights Analytics) 外，還能夠建立一個精確的 AppMap 來幫助您追蹤應用程式或叢集中處理序之間的相依性。

透過在 Azure Marketplace 中搜尋「Application Insights」設定 AppInsights 資源。 在建立一個資源後，移至 [屬性] 尋找 AI 檢測索引鍵 (採用 GUID 格式)。 這用於︰
* 整合 AppInsights 以直接透過 Azure Resource Manager 範本，或在建立叢集時 (假設診斷已啟用) 經由 Azure 入口網站，從 Service Fabric 叢集接收基礎結構層級事件
* 設定 EventFlow (eventFlowConfig.json) 將資料輸出至 Application Insights，如上節所述

### <a name="oms"></a>OMS

OMS 是另一個建議用來診斷與監視 Service Fabric 叢集的工具。 Service Fabric 解決方案可新增至任何工作區，且具備儀表板可顯示不同類型的 Service Fabric 事件。 OMS 工作區在 Log Analytics 中也有強大的記錄查詢工具。

若要設定 OMS 工作區，請確認已為您的叢集啟用診斷。 從 Azure Marketplace 將 [Service Fabric 分析] 新增至現有的 OMS 工作區，或新增一個工作區。 設定工作區的資料來源，以連接到您的叢集正在寫入事件的 Azure 儲存體資料表。 

為了讓 OMS 挑選自訂事件，您也必須確定您新增至應用程式的檢測也會寫入至相同的儲存體資料表，或任何其他也設定成工作區來源的資料表。 

由於您可以將容器解決方案新增至能夠搭配 Service Fabric 協調的容器順暢運作的工作區，因此如果您需要監視與診斷容器，OMS 目前也是針對資料視覺效果與分析建議採取的途徑。 若要檢視簡短的設定指南，請移至[這裡](service-fabric-diagnostics-containers-windowsserver.md)。
...
監視程式是個別的服務，可以觀察服務之間的健全狀況和負載，並報告健全狀況模型階層中任何項目的健全狀況。 如此可避免在單一服務檢視中無法偵測到的錯誤。 監視程式也適合裝載能夠針對已知狀況執行修復動作的程式碼，而無需使用者互動。 您可以在[這裡](https://github.com/Azure-Samples/service-fabric-watchdog-service)找到範例監視程式服務實作。
