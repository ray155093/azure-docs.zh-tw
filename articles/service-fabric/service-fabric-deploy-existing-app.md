...
只有一個 SetupEntrypoint，所以如果應用程式的 setup/config 需要多個指令碼，則必須在單一批次檔上搭配 setup/config 指令碼。如同 Entrypoint 元素，SetupEntrypoint 可以執行任何類型的檔案：可執行檔、批次檔和 PowerShell Cmdlet。在上述範例中，SetupEntrypoint 是以位於 Code 目錄的 `scripts` 子目錄中的批次檔 LaunchConfig.cmd 為基礎 (假設 WorkingDirectory 元素已設為 Code)。
...
- `WorkingDirectory` 適合用來設定正確的工作目錄，以便應用程式或初始化指令碼使用相對路徑。
...
記錄檔會儲存在服務的其中一個工作目錄中。若要判斷檔案的位置，您必須使用 Service Fabric 總管來判斷服務執行所在的節點以及目前使用的工作目錄。本文稍後會說明此程序。
