...
## <a name="to-publish-an-application-by-using-the-publish-service-fabric-application-dialog-box"></a>使用 [發佈 Service Fabric 應用程式] 對話方塊發佈應用程式
...
2. 在 [連接端點]  區段中，指定本機或遠端 Service Fabric 叢集的發佈端點。 若要新增或變更連接端點，請按一下 [連接端點]  下拉式清單。 此清單會根據您的 Azure 訂用帳戶，顯示您可以對其發佈的可用 Service Fabric 叢集連接端點。 請注意，如果您尚未登入 Visual Studio，系統將會提示您登入。
    ...
    >[AZURE.NOTE] 如果您想要發佈至任意端點 (例如合作對象叢集)，請參閱下面的＜發佈到任意叢集端點＞一節。
