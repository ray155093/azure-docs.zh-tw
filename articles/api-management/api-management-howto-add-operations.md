---
title: "如何將作業新增至 Azure API 管理中的 API | Microsoft Docs"
...
按一下 [加入作業]  以加入新的作業。 將顯示 [新增作業]，且預設會選取 [簽章] 索引標籤。
...
[顯示名稱] 和 [說明] 會提供作業說明，並用來提供文件給在開發人員入口網站中使用此 API 的開發人員。
...
若要輕鬆又快速地啟用作業的快取，請選取 [快取] 索引標籤並核取 [啟用] 核取方塊。

![快取][api-management-caching-tab]

 指定操作回應保留在快取中的時間週期。 預設值為 3600 秒或 1 小時。
...
若要新增回應，請按一下 [新增]，開始輸入所需的狀態碼。 在此範例中，狀態碼為 [200 確定]。 當代碼出現在下拉式清單時，請選取它，回應碼就會建立並加入至您的操作。
