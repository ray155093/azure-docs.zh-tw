...
如需原則陳述式及其設定的完整清單，請參閱[原則參考文件][原則參考文件]。
...
設定原則時，可以針對全域進行設定，或是以[產品][產品]、[API][API] 或[操作][操作]為範圍來設定。 若要設定原則，請瀏覽至發佈者入口網站的原則編輯器。
...
> [!NOTE]
> 如果未啟用您想要新增的原則，請確定您是在該原則的正確範圍內。 每個原則陳述式都是針對在特定範圍和原則區段中使用所設計。 若要檢閱原則的原則區段和範圍，請查看[原則參考文件][原則參考文件]中該原則的**使用方式**一節。
> 
> 

[原則參考文件][原則參考文件]中提供了原則陳述式及其設定的完整清單。
