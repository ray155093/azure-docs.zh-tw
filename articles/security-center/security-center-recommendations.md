...
1. 按一下 [資訊安全中心] 刀鋒視窗上的 [建議] 圖格。將會開啟 [建議] 刀鋒視窗。
...
|[啟用子網路/網路介面上的網路安全性群組](security-center-enable-network-security-groups.md)|建議您啟用子網路和網路介面上的網路安全性群組 (NSG)。|
...
1. 在 [建議] 刀鋒視窗中，選取 [啟用反惡意程式碼]。![選取啟用反惡意程式碼][5]
...
5. 在 [加入擴充功能] 刀鋒視窗上輸入必要的組態設定，然後選取 [確定]。![安裝反惡意程式碼][6]
