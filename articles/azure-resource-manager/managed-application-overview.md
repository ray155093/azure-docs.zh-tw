...
本文提供這兩種體驗的概觀。 首先，我們來了解受管理應用程式的運作方式。 
...
如需廠商體驗的深入資訊，請參閱[建立及發佈 Azure 受管理的應用程式](managed-application-publishing.md)。

如需取用者體驗的深入資訊，請參閱[使用 Azure 受管理的應用程式](managed-application-consumption.md)。
