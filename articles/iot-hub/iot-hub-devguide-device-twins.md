<properties
 pageTitle="開發人員指南 - 了解裝置對應項 | Microsoft Azure"
...
> [AZURE.NOTE] 上述程式碼片段舉例說明了可能用來編碼裝置組態和其狀態的方式，為了方便閱讀，其內容已經過最佳化。 IoT 中樞不會對裝置對應項中的所需屬性和報告屬性強制實行特定結構描述。
...
> [AZURE.NOTE] 目前，裝置對應項只能從使用 MQTT 通訊協定連線至 IoT 中樞的裝置存取。
...
> [AZURE.NOTE] [Azure IoT 裝置 SDK][lnk-sdks] 中已實作此邏輯。 只有當裝置應用程式無法使用任何 Azure IoT 裝置 SDK，因而必須直接為 MQTT 介面編寫程式時，此說明才有用。
