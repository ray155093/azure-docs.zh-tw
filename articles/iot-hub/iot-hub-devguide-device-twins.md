...
---
# <a name="understand-device-twins"></a>了解攣生裝置
...
在上述範例中，後端和裝置應用程式會使用 `telemetryConfig` 裝置對應項所需屬性和報告屬性來同步處理此裝置的遙測組態。 例如：
...
2. 裝置應用程式會在已連線或第一次重新連線時，立即收到變更通知。 裝置應用程式接著會報告更新的組態 (或使用 `status` 屬性的錯誤狀況)。 以下是報告屬性的部分︰
        ...
        ...
3. 透過[查詢][lnk-query]裝置對應項，應用程式後端可以跨裝置持續追蹤組態作業的結果。
...
後端會使用下列不可部分完成的作業 (透過 HTTP 公開) 來操作裝置對應項︰

1. **依識別碼擷取裝置對應項**。 此作業會傳回裝置對應項文件的內容，包括標籤以及所需屬性、報告屬性和系統屬性。
2. **部份更新裝置對應項**。 此作業可讓後端部分更新裝置對應項的標籤或所需屬性。 部分更新會以 JSON 文件的形式來表示，以新增或更新上述任何屬性。 設定為 `null` 的屬性會遭到移除。 例如，下列程式碼會以 `{"newProperty": "newValue"}` 值建立新的所需屬性、以 `"otherNewValue"` 覆寫 `existingProperty` 的現有值，並完全移除 `otherOldProperty`。 其他現有的所需屬性或標籤不會發生任何變更︰
...
除了這些作業外，後端還可以使用類似 SQL 的 [IoT 中樞查詢語言][lnk-query]來查詢裝置對應項，並使用[作業][lnk-jobs]對大量的裝置對應項執行作業。
...
裝置對應項所需屬性和報告屬性沒有 Etag，但是有一定會遞增的 `$version` 值。 與 Etag 類似，更新端 (例如報告屬性的裝置應用程式或所需屬性的後端) 可以使用版本來強制保有一致的更新。
