...
3. 修改 **main** 方法來建立 **MessageCallback** 執行個體，並先呼叫 **setMessageCallback** 方法再開啟用戶端，如下所示︰
   ...
   > [!NOTE]
   > 如果您使用 HTTP/1 而非 AMQP 做為傳輸，**DeviceClient** 執行個體不會經常檢查 IoT 中樞是否傳來訊息 (每隔 25 分鐘以上)。如需有關 AMQP 與 HTTP/1 支援間的差異，以及 IoT 中樞節流的詳細資訊，請參閱 [IoT 中樞開發人員指南][IoT Hub Developer Guide - C2D]。
   > 
   > 
