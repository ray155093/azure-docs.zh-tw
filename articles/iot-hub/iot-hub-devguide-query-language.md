...
會擷取 **location.region** 標籤設為 **US**的對應項。
布林運算子和算術比較也受到支援，例如︰
...
會擷取所有位於 US、且設定為遙測傳送頻率比每分鐘還低的對應項。 為了方便起見，您也可以搭配使用陣列常數與 **IN** 和 **NIN** (不在) 運算子。 例如，

        SELECT * FROM devices
        WHERE property.reported.connectivity IN ['wired', 'wifi']

會擷取所有報告使用 wifi 或有線連線的對應項。 請參閱 [WHERE 子句][lnk-query-where]一節，以取得篩選功能的完整參考。
...
會針對在 2016 年 9 月之後所建立的裝置 **myDeviceId** 擷取所有已完成的對應項更新作業。
...
其中 **attribute_name** 指的是 FROM 集合中 JSON 文件的任何屬性。 您可以在[開始使用對應項查詢][lnk-query-getstarted]一節中找到一些 SELECT 子句範例。
...
條件是評估為布林值的運算式，也就是說，任何不同於布林值 **true** 的常數會被視為 **false** (包括 **null**、**undefined**、任何物件或陣列執行個體、任何字串，以及很顯然地布林值 **false**)。
