---
title: "將 Raspberry Pi (C) 連接到 Azure IoT - 開始使用 | Microsoft Docs"
description: "開始使用 Raspberry Pi 3、建立 Azure IoT 中樞，以及將 Pi 與 IoT 中樞連線。"
services: iot-hub
documentationcenter: 
author: shizn
manager: timtl
...
---
# <a name="connect-your-raspberry-pi-3-device-to-your-iot-hub-using-c"></a>使用 C 將 Raspberry Pi 3 裝置連接到 IoT 中樞
...
在本課程中，您將以一種作業系統來設定 Raspberry Pi 3 裝置、設定開發環境，並將應用程式部署至 Pi。
...
前往[設定裝置](iot-hub-raspberry-pi-kit-c-lesson1-configure-your-device.md)。
...
前往[取得工具](iot-hub-raspberry-pi-kit-c-lesson1-get-the-tools-win32.md)。

### <a name="create-and-deploy-the-blink-application"></a>建立並部署閃爍應用程式
從 GitHub 複製範例 C 閃爍應用程式，並使用 gulp 將此應用程式部署至您的 Raspberry Pi 3 面板。 此範例應用程式會讓與面板連接的 LED 每兩秒閃爍一次。

*預估完成時間：5 分鐘*

前往[建立並部署閃爍應用程式](iot-hub-raspberry-pi-kit-c-lesson1-deploy-blink-app.md)。
...
前往[取得 Azure 工具](iot-hub-raspberry-pi-kit-c-lesson2-get-azure-tools-win32.md)。
...
前往[建立 IoT 中樞並登錄 Raspberry Pi 3](iot-hub-raspberry-pi-kit-c-lesson2-prepare-azure-iot-hub.md)。
...
移至[建立 Azure 函式應用程式和 Azure 儲存體帳戶](iot-hub-raspberry-pi-kit-c-lesson3-deploy-resource-manager-template.md)。
...
前往[執行範例應用程式以傳送裝置到雲端訊息](iot-hub-raspberry-pi-kit-c-lesson3-run-azure-blink.md)。
...
前往[讀取保存在 Azure 儲存體中的訊息](iot-hub-raspberry-pi-kit-c-lesson3-read-table-storage.md)。
...
前往[執行範例應用程式以接收雲端到裝置訊息](iot-hub-raspberry-pi-kit-c-lesson4-send-cloud-to-device-messages.md)。
...
前往[選讀區段：變更 LED 的開啟與關閉行為](iot-hub-raspberry-pi-kit-c-lesson4-change-led-behavior.md)。

## <a name="troubleshooting"></a>疑難排解
如果在課程期間遇到任何問題，您可以在[疑難排解](iot-hub-raspberry-pi-kit-c-troubleshooting.md)一文中尋求解決方案。
