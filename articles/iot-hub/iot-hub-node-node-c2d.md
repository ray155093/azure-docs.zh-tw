...
# <a name="tutorial:-how-to-send-cloud-to-device-messages-with-iot-hub-and-node.js"></a>教學課程：如何使用 IoT 中樞和 Node.js 傳送雲端到裝置訊息
...
您可以在 [IoT 中心開發人員指南][IoT 中樞開發人員指南 - C2D]中，找到有關雲端到裝置訊息的詳細資訊。
...
## <a name="receive-messages-on-the-simulated-device"></a>在模擬裝置上接收訊息
...
2. 修改 **connectCallback** 函式來處理從 IoT 中樞傳送的訊息。 在此範例中，裝置永遠會叫用 **完整** 函式，目的是通知 IoT 中樞訊息已處理完畢。 新版的 **connectCallback** 函式會看起來像這樣︰
    ...
    > [AZURE.NOTE] 如果您使用 HTTP/1 而非 AMQP 或 MQTT 做為傳輸，**DeviceClient** 執行個體不會經常檢查 IoT 中樞是否傳來訊息 (每隔 25 分鐘以上)。 如需有關 AMQP、MQTT 和 HTTP/1 支援間的差異，以及 IoT 中樞節流的詳細資訊，請參閱 [IoT 中樞開發人員指南][IoT 中樞開發人員指南 - C2D]。
...
[IoT 中樞開發人員指南 - C2D]: iot-hub-devguide-messaging.md
