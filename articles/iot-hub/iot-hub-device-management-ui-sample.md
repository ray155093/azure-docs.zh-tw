<properties
 pageTitle="使用 IoT 中樞裝置管理 UI | Microsoft Azure"
...
與範例裝置管理 UI 互動，有助於您鞏固涵蓋於 Azure IoT 中樞裝置管理[概觀][lnk-dm-overview]與[開始使用][lnk-get-started]文章中的概念與功能。本文章將逐步引導您了解三個主要裝置管理概念：裝置對應項、裝置查詢和裝置工作，如範例裝置管理 Web UI 中所示。
...
裝置管理範例 UI 的預設主畫面是 [裝置] 檢視，其中包含下列 5 個元件：
...
[裝置管理概觀][lnk-dm-overview]介紹「裝置對應項」概念，它代表 Azure IoT 中樞中實體 (或模擬) 的裝置。在裝置方格中，您可以從裝置清單選取任何已註冊的裝置，來檢視和編輯該裝置的裝置對應項。

在您的第一個模擬裝置 **Device11-7ce4a850** 上進入此詳細檢視，方法是選取對應的裝置資料列，然後按一下 [編輯] 按鈕 (您也可以按兩下資料列，或在搜尋方塊中輸入裝置識別碼)。

您現在正在檢視完整的裝置對應項元件表示，其中您可以更新可寫入屬性和執行其他的裝置作業，詳述如下：

![][2]

1.  **編輯裝置對應項**︰這包括裝置的 [啟用/停用] 切換。
...
當您了解裝置管理概觀後，裝置工作可讓您在一或多個實體裝置上協調簡單或複雜的動作。在本節中，您將在範例 UI 中建立裝置工作，來在所有具有 "bacon" 標籤的模擬裝置上執行重新開機作業：

1.  從 [Only Bacon] 裝置查詢清單中，在每個裝置資料行上按一下，將它標記以進行重新開機工作作業：
...
在一段時間後重新整理此畫面，父工作與三個子工作狀態會變更為 [完成]，表示重新開機作業已經成功，並由模擬裝置確認。使用 [裝置識別碼] 資料行判斷哪項工作與哪個裝置關聯。


> [AZURE.NOTE] 如果子工作傳回狀態「失敗」，請檢查您的模擬裝置是否仍在測試系統中執行。如果沒有，請再次執行 simulate.bat 或 simulate.sh 指令碼，並重複上述的重新開機裝置工作步驟。
