...
您可以從 3 個層中做選擇：**免費**、**標準 1** 和**標準 2**。免費層只允許 500 個裝置連接至 IoT 中樞，每天最多 8,000 封訊息。

**S1 (低頻率)**：IoT 中樞 S1 (低頻率) 版本的設計對象為具有大量裝置，但每個裝置產生的資料量卻非常少的 IoT 解決方案。S1 (低頻率) 版本的每個單位可讓您跨所有連接的裝置，每天傳輸高達 400,000 封訊息。

**S2 (高頻率)**：IoT 中樞 S2 (高頻率) 版本的設計對象是裝置會在其中產生大量資料的 IoT 解決方案。S2 (高頻率) 版本的每個單位可讓您跨所有連接的裝置，每天傳輸高達 6 百萬封訊息。
...
位置選項提供可在其中使用 IoT 中樞的區域清單。IoT 中樞可於下列位置部署：美國東部、美國西部、北歐、西歐、東亞和東南亞。
...
只有在當天傳送的訊息數目不會衝突時，才允許從較高的層 (S2) 移至較低層 (S1)。例如，如果每天的訊息數目都超過 400,000，則 IoT 中樞無法從 S2 的層變更為 S1。
