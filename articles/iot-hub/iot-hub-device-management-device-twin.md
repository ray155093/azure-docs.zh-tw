---
title: IoT 中樞裝置管理對應項 | Microsoft Docs
...
### 啟動範例
若要啟動範例，您需要執行 **DeviceTwin.exe** 程序。這會從裝置對應項和實體裝置讀取裝置屬性。它也會變更實體裝置上的裝置屬性。遵循下列步驟來啟動範例：

1. 從您複製 **azure-iot-sdks** 儲存機制的根資料夾，瀏覽至 **azure-iot-sdks\\csharp\\service\\samples\\bin** 資料夾。
2. 執行 `DeviceTwin.exe <IoT Hub Connection String>`。

您應該會在命令列視窗中看到顯示裝置對應項使用情形的輸出。本範例將逐步引導您完成下列程序：

1. 印出裝置對應項的所有裝置屬性。
2. 深層讀取：從實體裝置讀取電池電量裝置屬性 (3 次)。
3. 深層寫入：將 **Timezone** 裝置屬性寫入實體裝置中。
4. 深層讀取：讀取實體裝置中的 **Timezone** 裝置屬性，以查看它是否已經變更。
