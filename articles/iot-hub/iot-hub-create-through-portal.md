...
按一下 [端點] 以顯示正在修改之 IoT 中樞的端點清單。 端點有兩個主要類型︰IoT 中樞內建的端點，以及 IoT 中樞建立後新增的端點。

### <a name="built-in-endpoints"></a>內建端點
內建端點有兩個主要類型︰**雲端到裝置意見反應**和**事件**。

* **雲端到裝置意見反應**設定：此設定有&2; 個子設定：訊息的**雲端到裝置 TTL** (存留時間) 和**保留時間**。 第一次建立 IoT 中樞時，這些設定會在預設值&1; 小時內建立。 若要調整這些值，可使用滑桿或輸入值。
* **事件**設定：有數個子設定，其中有一些會在建立 IoT 中樞時命名/指派，並且只能複製到其他可自訂的子設定。 這些子設定都會在下一節列出。

**分割**：這個值會在 IoT 中樞建立時設定，並且可以透過這項設定變更。

**事件中樞相容的名稱和端點**：建立 IoT 中樞時，事件中樞會在內部建立，而您在某些情況下可能需要其存取權。 此事件中樞相容名稱和端點無法自訂，但可透過 [複製] 按鈕使用。

**保留時間**：預設為&1; 天，但可以使用下拉式清單自訂為其他值。 這個值對於「裝置到雲端」而言是以天為單位，而不是像「雲端到裝置」那樣以小時為單位進行設定。
...
您可以透過入口網站將自訂端點新增至您的 IoT 中樞。 從端點刀鋒視窗，按一下刀鋒視窗頂端的 [新增] 以開啟 [新增端點] 刀鋒視窗。 在刀鋒視窗中輸入所需的資訊，然後按一下 [確定]。 您的自訂端點接著會出現在主要端點刀鋒視窗中。
...
## <a name="routes"></a>路由
按一下 [路由] 以管理 IoT 中樞將您的裝置分派至雲端訊息的方式。

![][14]

您可以將其他路由新增至 IoT 中樞。按一下刀鋒視窗頂端的 [新增] 並在刀鋒視窗中輸入所需的資訊並按一下 [確定]。 您的路由接著會出現在主要端點刀鋒視窗中。 您可以編輯路由，其做法是按一下路由清單中的路由，然後加以修改。 若要啟用路由，按一下清單中的路由並將啟用/停用切換為 [關閉]。 按一下刀鋒視窗底部的 [確定] 以儲存變更。
