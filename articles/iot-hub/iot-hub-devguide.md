...
您可以使用 [IoT 中樞資源提供者端點](#endpoints)上的非同步作業，將裝置身分識別大量匯入到 IoT 中樞的身分識別登錄。匯入是長時間執行的作業，其使用客戶提供的 Blob 容器中的資料，將裝置身分識別資料寫入至裝置身分識別登錄。
...
如需如何建構和使用安全性權杖的詳細資訊，請參閱 [IoT 中樞安全性權杖][lnk-sas-tokens]文章。
...
使用受限制的資源 URI 建立權杖，可以設定中心層級安全性原則的範圍。例如，要從裝置傳送裝置到雲端訊息的端點是 **/devices/{裝置識別碼}/messages/events**。您也可以使用中樞層級的共用存取原則搭配 **DeviceConnect** 權限，來簽署 resourceURI 為 **/devices/{裝置識別碼}** 的權杖，建立只能用來代表裝置 **deviceId** 傳送訊息的權杖。

這個機制類似於[事件中樞發行者原則][lnk-event-hubs-publisher-policy]，並讓您可實作自訂驗證方法 (如[設計您的解決方案][lnk-guidance-security]的安全性一節中所述)。

## 訊息

IoT 中樞提供通訊的傳訊基礎︰

- [雲端到裝置](#c2d)︰從應用程式後端 (*service* 或 *cloud*)。
...
* 一組*應用程式屬性*。這是應用程式可以定義的字串屬性字典，而不需將訊息本文還原序列化即可加以存取。IoT 中樞不會修改這些屬性。
* 不透明的二進位主體。
...
* **現場閘道器**。使用 HTTP/1 和 MQTT 時，您無法使用相同的 TLS 連線來連線到多個裝置 (各有自己的每一裝置認證)。可見在實作[現場閘道案例][lnk-azure-gateway-guidance]時，這些並不是最理想的通訊協定，因為對於每個連線至現場閘道的裝置，這些通訊協定需要一個現場閘道和 IoT 中樞之間的 TLS 連線。
...
IoT 中樞會實作 MQTT v3.1.1 通訊協定，但其具有下列限制和特定行為：
  ...
  * **不支援 QoS 2**：若裝置用戶端使用 **QoS 2** 發佈訊息，IoT 中樞就會關閉網路連接。當裝置用戶端訂閱具有 **QoS 2** 的主題時，IoT 中樞會在 **SUBACK** 封包中授與最大 QoS 層級 1。
  * **保留**：如果裝置用戶端發佈 RETAIN 旗標設為 1 的訊息，IoT 中樞會在訊息中新增 **x-opt-retain** 應用程式屬性。這表示 IoT 中樞不會持續保留訊息，而是改為將它傳遞至後端應用程式。
  
如需進一步詳細資訊，請參閱 [IoT 中樞的 MQTT 支援][lnk-mqtt-support]一文。

最後請務必檢閱 [Azure IoT 通訊協定閘道][lnk-azure-protocol-gateway]，它可讓您部署直接與 IoT 中樞互動的高效能自訂通訊協定閘道。Azure IoT 通訊協定閘道器可讓您自訂裝置通訊協定，以順應要重建的 MQTT 部署或其他自訂通訊協定。這個方法的代價是需要自我裝載和運作自訂的通訊協定閘道器。

### 裝置到雲端 <a id="d2c"></a>

如[端點](#endpoints)一節中所詳述，裝置到雲端訊息是透過裝置面向端點 (**/devices/{裝置識別碼 }/messages/events**) 傳送，並透過與[事件中樞][lnk-event-hubs]相容的服務面向端點 (**/messages/events**) 來接收。因此，您可以使用標準事件中樞整合和 SDK 來接收裝置到雲端的訊息。
...
* 裝置到雲端的訊息會分割存放到建立時所設定的一組固定分割區中 (請參閱[裝置到雲端的組態選項](#d2cconfiguration))。
...
* IoT 中樞不允許使用 **PartitionKey** 進行任意資料分割。裝置到雲端訊息是根據其來源的 **deviceId** 進行分割。
...
如[端點](#endpoints)一節中所詳述，您可以透過服務對應端點 (**/messages/devicebound**) 傳送雲端到裝置訊息，而裝置則可透過裝置特定端點 (**/devices/{裝置識別碼}/messages/devicebound**) 來接收它們。

每個雲端到裝置訊息都以單一裝置為目標，其中會將 **to** 屬性設定為 **/devices/{裝置識別碼}/messages/devicebound**。
...
執行緒可能無法處理訊息，且不會通知 IoT 中樞。在此情況下，訊息會在過了 [可見性 (或鎖定) 逾時] 的一分鐘預設值之後，自動從 [不可見] 狀態轉換回 [已加入佇列] 狀態。訊息可以在 [已加入佇列] 與 [不可見] 狀態之間轉換的次數，以 IoT 中樞上 [最大傳遞計數] 屬性中指定的次數為限。達到該轉換次數之後，IoT 中樞就會將訊息的狀態設定為 [寄不出]。同樣地，IoT 中樞也會在訊息的到期時間過後 (請參閱[存留時間](#ttl))，將訊息的狀態設定為 [寄不出]。
...
> [AZURE.NOTE] 利用訊息到期的常見方式是，設定較短的存留時間值，以避免將訊息傳送至已中斷連線的裝置。這可達到與維護裝置連線狀態相同的效果，同時更具效率。它也能夠藉由要求訊息收條，收到 IoT 中樞的通知，告知哪些裝置能夠收到訊息，而哪些裝置不在線上或故障。

#### 訊息意見反應 <a id="feedback"></a>

當您傳送雲端到裝置的訊息時，服務可以要求傳遞每則訊息的意見反應 (關於該訊息的最終狀態)。

- 如果您將 **Ack** 屬性設定為 **positive**，則只有在雲端到裝置訊息達到 [已完成] 狀態時，IoT 中樞才會產生回饋訊息。
- 如果您將 **Ack** 屬性設定為 **negative**，則只有在雲端到裝置訊息達到 [寄不出] 狀態時，IoT 中樞才會產生回饋訊息。
- 如果您將 **Ack** 屬性設定為 **full**，則 IoT 中樞在上述任一情況下都會產生回饋訊息。

> [AZURE.NOTE] 當 **Ack** 是 **full** 時，如果未收到任何回饋訊息，即表示回饋訊息已過期，服務無法得知原始訊息發生什麼狀況。實際上，服務應該確保它可以在回饋到期之前對其進行處理。最長到期時間是兩天，因此當發生失敗時，應該有相當充裕的時間可以讓服務啟動並正常執行。

如[端點](#endpoints)中所述，IoT 中樞會透過服務對應端點 (**/messages/servicebound/feedback**) 以訊息傳遞回饋。回饋的接收語意與雲端到裝置訊息的接收語意相同，並且具有相同的 [訊息生命週期](#訊息生命週期)。可能的話，訊息意見反應會放入單一訊息中，其格式如下。
...
[IoT Hub throttling and you (IoT 中樞節流和您)][lnk-throttle-blog] 部落格文章深入討論 IoT 中樞節流行為。
