...
使用者入口網站是一個 IIS 網站，可讓使用者註冊 Azure Multi-Factor Authentication 及維護自己的帳戶。 使用者可以變更電話號碼、變更 PIN，或選擇在下次登入時略過雙步驟驗證。
...
1. 在 Azure Multi-Factor Authentication Server 中，按一下左側功能表中的 [使用者入口網站] 圖示，然後按一下 [安裝使用者入口網站]。
2. 按 [下一步] 。
3. 按 [下一步] 。
...
如果 Azure Multi-Factor Authentication Server 執行所在的伺服器不是網際網路對向，您應該在個別的網際網路對向伺服器上安裝使用者入口網站。 
...
* 使用 IIS 7.x 或更高版本時，需要 ASP.NET 和 IIS 6 Metabase 相容性角色服務。
* 使用 SSL 憑證保護使用者入口網站。
...
* 確保使用者入口網站可以使用 "PhoneFactor Admins" 安全性群組中服務帳戶的認證來驗證 Azure Multi-Factor Authentication Web 服務 SDK。 如果 Azure Multi-Factor Authentication Server 在加入網域的伺服器上執行，此服務帳戶和群組也會存在於 Active Directory 中。 如果伺服器未加入網域，此服務帳戶和群組會存在於 Azure Multi-Factor Authentication Server 本機。

若要將使用者入口網站安裝在 Azure Multi-Factor Authentication Server 以外的伺服器上，您必須進行以下三個步驟：

1. 在與 Multi-Factor Authentication Server 相同的伺服器上安裝 Web 服務 SDK
2. 安裝使用者入口網站 
...
1. 在具有 Azure MFA Server 的伺服器上開啟「Windows 檔案總管」，然後瀏覽至在 Azure Multi-Factor Authentication Server 安裝所在的資料夾 (例如 C:\Program Files\Multi-Factor Authentication Server)。 將 32 位元或 64 位元版本的 MultiFactorAuthenticationUserPortalSetup 安裝檔案複製到網際網路對向伺服器。
2. 在網際網路對向 Web 伺服器上，以系統管理員權限執行此檔案。 若要這樣做，最簡單的方式是以系統管理員身分開啟命令提示字元，再瀏覽至複製安裝檔案的位置。
...
5. 找出 USE_WEB_SERVICE_SDK 機碼，並將從 **false** 值變更為 **true**。 
6. 找出 WEB_SERVICE_SDK_AUTHENTICATION_USERNAME 機碼，並將值設定為 PhoneFactor Admins 安全性群組中服務帳戶的使用者名稱。 使用合格的使用者名稱，例如 domain\username 或 machine\username。 
7. 找出 WEB_SERVICE_SDK_AUTHENTICATION_PASSWORD 機碼，並將值設定為 PhoneFactor Admins 安全性群組中服務帳戶的密碼。
8. 找出 pfup_pfwssdk_PfWsSdk 設定，並將值從 “http://localhost:4898/PfWsSdk.asmx” 變更為在 Azure Multi-Factor Authentication Server 上執行之 Web 服務 SDK 的 URL (如 https://computer1.domain.local/MultiFactorAuthWebServiceSdk/PfWsSdk.asmx)。 此連線使用 SSL，因此請以伺服器名稱參閱 Web 服務 SDK，而不要以 IP 位址，因為 SSL 憑證是針對伺服器名稱所核發。 如果伺服器名稱無法解析為網際網路對向伺服器的 IP 位址，請在該伺服器上的主機檔案新增項目，讓 Azure Multi-Factor Authentication Server 的名稱能與其 IP 位址相對應。 完成變更後，儲存 web.config 檔案。
...
使用 [系統管理員] 索引標籤，新增將擁有系統管理權限的使用者。  加入系統管理員時，您可以微調他們所得到的權限。 按一下 [新增] 按鈕，選取使用者及其權限，然後按一下 [新增]。

![使用者入口網站管理員](./media/multi-factor-authentication-get-started-portal/admin.png)

## <a name="security-questions"></a>安全性問題
如果選取 [使用遞補用的安全性問題] 選項，請使用 [安全性問題] 標籤來指定使用者必須回答的安全性問題。  Azure Multi-Factor Authentication Server 提供預設問題供您使用。 您可以變更順序，或加入您自己的問題。  當加入您自己的問題時，您也可以指定用來顯示這些問題的語言。

![使用者入口網站安全性問題](./media/multi-factor-authentication-get-started-portal/secquestion.png)

## <a name="saml"></a>SAML
使用 [SAML] 索引標籤來設定使用者入口網站，以使用 SAML 接受來自識別提供者的宣告。  您可以指定逾時工作階段、驗證憑證和登出重新導向 URL。

![SAML](./media/multi-factor-authentication-get-started-portal/saml.png)

## <a name="trusted-ips"></a>信任的 IP
使用 [信任的 IP] 索引標籤來指定可供新增的單一 IP 位址或 IP 位址範圍，以便讓從這些 IP 位址其中之一登入的使用者不必完成雙步驟驗證。
