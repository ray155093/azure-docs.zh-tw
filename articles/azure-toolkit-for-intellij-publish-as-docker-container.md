...
# <a name="how-to-publish-a-web-app-as-a-docker-container-using-the-azure-toolkit-for-intellij"></a>如何使用適用於 IntelliJ 的 Azure 工具組，將 Web 應用程式發佈作為 Docker 容器

Docker 容器是部署 Web 應用程式的廣泛使用方法，在這些 Web 應用程式中，開發人員可將所有的專案檔和相依性合併至單一套件，以供部署至伺服器。 適用於 IntelliJ 的 Azure 工具組可為 Java 開發人員簡化此程序，方法為新增 發佈作為 Docker 容器功能以部署到 Microsoft Azure，本文中的步驟會逐步引導您將應用程式發佈至 Azure 作為 Docker 容器所需的步驟。
...
> [!NOTE]
>
> 若要發佈 web 應用程式，您必須建立部署就緒的成品。 如需詳細資訊，請參閱本文稍後的[建立成品的其他資訊](#artifacts)。
>
> 此外，至少完成部署精靈一次之後，您在本逐步解說中指定的大部分設定，都會在您重新執行精靈時用作預設值。
>
...
1. 使用下列方法之一來啟動 [發佈作為 Docker 容器] 精靈︰
   ...
   * 在 [專案]工具視窗中，以滑鼠右鍵按一下您的專案，然後依序按一下 [Azure][發佈作為 Docker 容器]： ![發佈作為 Docker 容器][PUB01]

   * 在 IntelliJ 工具列中按一下 [發佈群組]，然後按一下 [發佈作為 Docker 容器]： ![發佈作為 Docker 容器][PUB02]
...
1. 若要建立新的 Docker 主機，請按一下 綠色的加號 (「**+**」)；這將會啟動 [建立 Docker 主機] 對話方塊。
...
Azure 工具組會開始在 Docker 容器中將 web 應用程式部署至 Azure，除非您已將 IntelliJ 設定為在背景中部署，否則會出現顯示部署進度的對話方塊。 
![部署進度][PUB09]
...
1. 按一下綠色的加號 (「**+**」) 來新增成品，然後按一下 [Web 應用程式︰成品]。
   ![新增構件][ART02]

1. 將成品命名並同時確保不會新增 ".war" 延伸模組，然後按一下 [確定]。
   ![成品屬性][ART03]
