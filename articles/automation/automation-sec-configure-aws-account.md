...
建立並複製 AWS 安全性金鑰後，您必須使用 Azure 自動化帳戶建立認證資產以安全地儲存金鑰，並讓金鑰與 Runbook 參照。按照 [Azure 自動化中的認證資產](automation-certificates.md###使用 Azure 入口網站建立新認證) 一文中**建立新認證**一節的步驟來進行，並輸入下列資訊︰

1. 在 [名稱] 方塊中，輸入 **AWScred** 或遵循您的命名標準的適當值。
2. 在 [使用者名稱] 方塊中輸入您的**存取識別碼**，並在 [密碼] 和 [確認密碼] 方塊中輸入您的**密碼存取金鑰**。

## 後續步驟
* 檢閱[在 Amazon Web Services 中自動部署 VM](automation-scenario-aws-deployment.md) 解決方案文章以了解如何建立 Runbook 以在 AWS 中自動執行工作。
