...
1.	按一下 [測試窗格] 來開啟 [測試] 刀鋒視窗。<br> ![測試窗格](media/automation-first-runbook-graphical/runbook-toolbar-test.png)
...
1.	按一下 [發佈] 來發佈 Runbook，然後出現提示時按一下 [是]。<br> ![Publish](media/automation-first-runbook-graphical/runbook-toolbar-publish.png)
...
4.	我們只想要啟動 Runbook，因此按一下 [開始] 然後出現提示時按一下 [是]。<br> ![啟動 Runbook](media/automation-first-runbook-graphical/runbook-controls-start.png)
...
我們已經測試並發行我們 Runbook，但是到目前為止，它似乎並不實用。我們想要讓它管理 Azure 資源。在設定驗證之前，我們會建立變數來保存訂用帳戶識別碼，並在設定要在下面的步驟 6 進行驗證的活動時參考此變數。在繼續之前，請從 [導覽] 窗格上的 [訂用帳戶] 選項複製您的訂用帳戶識別碼。
...
6.	在畫布上選取 [Get-AutomationVariable]，然後在 [組態] 控制項窗格的 [標籤] 文字方塊中輸入 [取得訂用帳戶識別碼]。  
7.	在 [程式庫] 控制項中，於搜尋文字方塊中輸入 **Get-AutomationPS**。
8.	將 [Get-AutomationPSCredential] 加入至畫布。<br>![搜尋 Get-AutomationPSCred Cmdlet](media/automation-first-runbook-graphical/search-get-automationpscredential-cmdlet.png)
9.	在畫布上選取 [Get-AutomationPSCredential]，然後在 [組態] 控制項窗格的 [標籤] 文字方塊中輸入 [取得 Azure 認證]。
10.	按一下 [參數]，隨即會顯示 [活動參數組態] 刀鋒視窗。 
11. 在 [活動參數組態] 刀鋒視窗中按一下 [名稱]，隨即會顯示 [參數值] 刀鋒視窗。  
12.	 在 [參數值] 刀鋒視窗中，為 [資料來源] 選取 [Runbook 輸入]，接著在清單中選取 [AzureCredentialAssetName]，然後按兩次 [確定]。   
13.	將滑鼠停留在 [取得訂用帳戶識別碼]，直到圖形的底端出現圓形。按一下圓形，並將箭頭拖曳到 [取得 Azure 認證]。您剛才建立的箭號是*連結*。Runbook 將會從 [取得訂用帳戶識別碼] 來開始，然後執行 [取得 Azure 認證]。<br> ![建立活動之間的連結](media/automation-first-runbook-graphical/runbook-link-getsubscription-getcredential.png)
14.	在 [程式庫] 控制項中，於搜尋文字方塊中輸入 **Add-AzureRm**。
15. 將 [Add-AzureRmAccount] 加入至畫布。<br> ![Add-AzureRMAccount](media/automation-first-runbook-graphical/search-powershell-cmdlet-addazurermaccount.png)
16.	將滑鼠停留在 [取得 Azure 認證]，直到圖形的底端出現圓形。按一下圓形，並將箭頭拖曳到 [Add-AzureRmAccount]。
17.	選取 [Add-AzureRmAccount]，然後按一下 [組態] 控制項窗格中的 [參數]。
18.	[Add-AzureRmAccount] 有多個參數集，因此我們必須先選取一個參數，我們才可以提供參數值。按一下 [參數集]，然後選取 [SubscriptionId] 參數集。
19.	一旦您選取參數集，參數會在 [活動參數組態] 刀鋒視窗中顯示。按一下 [認證]。<br> ![加入 Azure RM 帳戶參數](media/automation-first-runbook-graphical/add-azurermaccount-parameters.png)
20.	在 [參數值] 刀鋒視窗中，為 [資料來源] 選取 [活動輸出]，接著在清單中選取 [取得 Azure 認證]，然後按一下 [確定]。   
21.	按一下 [SubscriptionId]，接著在 [參數值] 刀鋒視窗中，選取清單中的 [取得訂用帳戶識別碼]，然後按兩次 [確定] 以返回畫布。
22.	在 [組態] 控制項窗格的 [標籤] 文字方塊中輸入 [連接到 Azure]。

此時您的 Runbook 看起來應該像下面這樣︰<br>![Runbook 驗證組態](media/automation-first-runbook-graphical/runbook-authentication-config.png)
...
2. 將 [Start-AzureRmVM] 加入至畫布，然後對它按一下並拖曳到 [連接到 Azure] 底下。
3. 將滑鼠停留在 [連接到 Azure]，直到圖形的底端出現圓形。按一下圓形，並將箭頭拖曳到 [Start-AzureRmVM]。 
...
此時您的 Runbook 看起來應該像下面這樣︰<br>![Runbook 驗證組態](media/automation-first-runbook-graphical/runbook-startazurermvm.png)
...
2. 移除 [連接到 Azure] 和 [Start-AzureRmVM] 之間的連結，方法是按一下該連結，然後按 [Delete] 鍵。
...
9. 建立從 [連接到 Azure] 至 [Get-AzureRmVM] 的連結。
...
23. 在第二個 [Write-Output] 控制項中按一下 [參數]，然後將 [標籤] 值變更為 [通知 VM 啟動失敗]。
24. 在 [InputObject] 中將 [資料來源] 變更為 [PowerShell 運算式]，然後輸入運算式「$VMName could not start.」。
25. 建立從 [Start-AzureRmVM] 至 [通知 VM 已啟動] 和 [通知 VM 啟動失敗] 的連結。
26. 選取連往 [通知 VM 已啟動] 的連結並將 [套用條件] 變更為 [True]。
27. 在 [條件運算式] 中輸入「$ActivityOutput['Start-AzureRmVM'].IsSuccessStatusCode -eq $true」。此 Write-Output 控制項現在只會在虛擬機器已成功啟動時執行。
28. 選取連往 [通知 VM 啟動失敗] 的連結並將 [套用條件] 變更為 [True]。
29. 在 [條件運算式] 中輸入「$ActivityOutput['Start-AzureRmVM'].IsSuccessStatusCode -ne $true」。此 Write-Output 控制項現在只會在虛擬機器未成功啟動時執行。
30.	儲存 Runbook 並開啟 [測試] 窗格。
31.	在虛擬機器停止下啟動 Runbook，它應該會啟動。
