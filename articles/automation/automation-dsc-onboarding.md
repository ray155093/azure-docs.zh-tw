...
2. 在本機複製下列指令碼。 此指令碼包含用來建立中繼設定的 PowerShell DSC 設定，以及開始執行中繼設定建立作業的命令。
   ...
   # <a name="the-dsc-configuration-that-will-generate-metaconfigurations"></a>將產生中繼設定的 DSC 設定
...
2. 如果您希望電腦向 Azure 自動化 DSC 報告 DSC 狀態資訊，但不提取設定或 PowerShell 模組，請將 **ReportOnly** 參數設定為 true。
3. 執行指令碼。 您現在工作目錄中應該有一個名為 **DscMetaConfigs** 的資料夾，其中包含要上架之電腦的 PowerShell DSC 中繼設定。
...
3. 從您要上架節點的目標自動化帳戶下載您想要上架之電腦的 PowerShell DSC 中繼設定：
   ...
   # <a name="define-the-parameters-for-get-azurermautomationdsconboardingmetaconfig-using-powershell-splatting"></a>使用 PowerShell 展開來定義 Get-AzureRmAutomationDscOnboardingMetaconfig 的參數
     $Params = @{
   ...
   # <a name="use-powershell-splatting-to-pass-parameters-to-the-azure-automation-cmdlet-being-invoked"></a>使用 PowerShell 展開來將參數傳遞給要叫用的 Azure 自動化 Cmdlet
   # <a name="for-more-info-about-splatting-run-get-help--name-aboutsplatting"></a>如需展開的詳細資訊，請執行︰Get-Help -Name about_Splatting
