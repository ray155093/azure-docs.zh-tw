...
1. 在 **MainActivity** 類別的 **OnCreate** 方法中，在呼叫 `LoadApplication()` 之前加入下列程式碼：
...
1. 在 **MainPage** 類別的建構函式中，在呼叫 `LoadApplication()` 之前加入下面這行程式碼：
...
2. (選擇性) 在 **WinPhone81** 專案中，開啟 App.xaml.cs，然後加入下列 `using` 陳述式：
   
        using Microsoft.WindowsAzure.MobileServices;
        using <your_Portable_Class_Library_namespace>;
   
    以您的可攜式類別庫的命名空間取代 `<your_Portable_Class_Library_namespace>`。
3. 將下列 **OnActivated** 方法覆寫加入至 **App** 類別：
