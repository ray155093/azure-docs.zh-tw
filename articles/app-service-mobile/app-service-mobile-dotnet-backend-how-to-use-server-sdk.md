...
3. 如果系統要求您選取發佈目標，請按一下 [Microsoft Azure App Service] > [下一步]，然後視需要使用您的 Azure 認證來登入。Visual Studio 會直接從 Azure 下載並安全地儲存您的發佈設定。
...
5. 在 App\_Start/Startup.MobileApp.cs 檔案中，加入對 **MapApiControllers** 擴充方法的呼叫，如下列範例所示：
...
在上述範例中，您應該使用 HTTPS 配置，將 Web.config 檔案中的 *authAudience* 和 *authIssuer* 應用程式設定，設定為每個應用程式根目錄的 URL。同樣地，您應該將 *authSigningKey* 設定為您應用程式的簽署金鑰值。若要取得簽署金鑰：
