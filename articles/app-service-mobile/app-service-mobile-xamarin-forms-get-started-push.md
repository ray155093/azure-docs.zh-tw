...
6. 初始化 **OnCreate** 方法開頭的 `instance` 變數，如下所示。
        ...
        instance = this;
7. 在名為 `GcmService.cs` 的 **Droid** 專案中，加入新的類別檔案，並確定下列 **using** 陳述式出現在檔案頂端︰
        ...
        using System.Text;
8. 在檔案頂端的 **using** 陳述式和 **namespace** 宣告之間，加入下列權限要求。
...
11. 在 **GcmService** 類別中加入下列程式碼，以覆寫 **OnRegistered** 事件處理常式並實作 **Register** 方法。
...
4. 按 [執行] 按鈕以建置專案，並在 Android 裝置或模擬器上啟動應用程式。
...
3. 此外，也在 **AppDelegate** 中新增 **DidReceivedRemoteNotification** 事件處理常式的下列覆寫：
