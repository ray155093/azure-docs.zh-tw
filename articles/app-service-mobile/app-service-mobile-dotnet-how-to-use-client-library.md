...
本指南將示範如何在 Windows 和 Xamarin 應用程式中針對 Azure App Service Mobile Apps 使用受管理的用戶端程式庫，來執行一般案例。 如果您不熟悉 Mobile Apps，應考慮先完成 [Azure Mobile Apps 快速入門][1] 教學課程。 在本指南中，我們會著重於用戶端受管理的 SDK。 若要深入了解 Mobile Apps 的伺服器端 SDK，請參閱 [.NET 伺服器 SDK][2] 或 [Node.js 伺服器 SDK][3] 的文件。

## <a name="reference-documentation"></a>參考文件

用戶端 SDK 的參考文件位於此處：[Azure Mobile Apps .NET 用戶端參考資料][4]。
您也可以在 [Azure-Samples GitHub 存放庫][5]中找到數個用戶端範例。
...
我們假設您已建立並發佈您的行動應用程式後端專案 (至少包含一個資料表)。  在本主題使用的程式碼中，資料表的名稱為 `TodoItem`，且其具有下列資料行：`Id`、`Text` 和 `Complete`。 此資料表就是當您完成 [Azure Mobile Apps quickstart]時所建立的相同資料表。
...
[JsonPropertyAttribute][6] 是用來定義用戶端類型與資料表之間的 *PropertyName* 對應。

若要了解如何在 Mobile Apps 後端建立資料表，請參閱 [.NET 伺服器 SDK 主題][7]或 [Node.js 伺服器 SDK 主題][8]中的資訊。 如果您已使用＜快速入門＞在 Azure 入口網站中建立行動應用程式後端，也可以使用 **Azure 入口網站** 中的 [簡易表]設定。

###<a name="how-to:-install-the-managed-client-sdk-package"></a>做法︰安裝受管理的用戶端 SDK 封裝

使用下列其中一種方法，從 [NuGet][9 ]針對 Mobile Apps 安裝受管理的用戶端 SDK 套件：
...
###<a name="<a-name="symbolsource"></a>how-to:-work-with-debug-symbols-in-visual-studio"></a><a name="symbolsource"></a>做法︰使用 Visual Studio 中的偵錯符號

您可以在 [SymbolSource][10] 上取得適用於 Microsoft.Azure.Mobile 命名空間的符號。  若要整合 SymbolSource 與 Visual Studio，請參閱 [SymbolSource 指示][11]。

##<a name="<a-name="create-client"></a>create-the-mobile-apps-client"></a><a name="create-client"></a>建立 Mobile Apps 用戶端
...
###<a name="<a-name="instantiating"></a>how-to:-create-a-table-reference"></a><a name="instantiating"></a>作法：建立資料表參考
...
###<a name="<a-name="querying"></a>how-to:-query-data-from-your-mobile-app"></a><a name="querying"></a>如何：查詢行動應用程式中的資料
...
>[AZURE.NOTE] 系統會強制使用伺服器控制的頁面大小，以防止傳回所有資料列。  分頁可避免大型資料集的預設要求對服務造成負面影響。  若要傳回 50 筆以上的資料列，請依照以分頁方式傳回資料的說明，使用 `Skip` 和 `Take` 方法。
...
這兩種方法是相同的，而且可以交換使用。  舊有選項&mdash;在一個查詢中串連多個述語&mdash;較為精簡，也是我們給您的建議。
...
###<a name="<a-name="sorting"></a>how-to:-sort-returned-data"></a><a name="sorting"></a>作法：排序傳回的資料
...
###<a name="<a-name="paging"></a>how-to:-return-data-in-pages"></a><a name="paging"></a>作法：以分頁方式傳回資料
...
[IncludeTotalCount] 方法會要求已傳回之 _all_ 記錄的總數，忽略指定的任何分頁/限制子句：

    query = query.IncludeTotalCount();

在實際的應用程式中，您可以搭配頁面巡覽區控制項或類似的 UI 來使用類似上述範例的查詢，以導覽各個頁面。

>[AZURE.NOTE]若要覆寫行動應用程式後端中的 50 個資料列限制，您也必須將 [EnableQueryAttribute] 套用到公用的 GET 方法並指定分頁行為。 套用到方法時，下列設定最多傳回 1000 個資料列：
>
>    [EnableQuery(MaxTop=1000)]

### <a name="<a-name="selecting"></a>how-to:-select-specific-columns"></a><a name="selecting"></a>作法：選取特定資料欄
...
### <a name="<a-name="lookingup"></a>how-to:-look-up-data-by-id"></a><a name="lookingup"></a>作法：按識別碼查詢資料
...
### <a name="<a-name="untypedqueries"></a>how-to:-execute-untyped-queries"></a><a name="untypedqueries"></a>如何：執行不具類型的查詢
...
### <a name="<a-name="inserting"></a>how-to:-insert-data-into-a-mobile-app-backend"></a><a name="inserting"></a>如何：將資料插入行動應用程式後端
...
###<a name="<a-name="modifying"></a>how-to:-modify-data-in-a-mobile-app-backend"></a><a name="modifying"></a>如何：修改行動應用程式後端中的資料
...
###<a name="<a-name="deleting"></a>how-to:-delete-data-in-a-mobile-app-backend"></a><a name="deleting"></a>如何：刪除行動應用程式後端中的資料
...
###<a name="<a-name="optimisticconcurrency"></a>how-to:-use-optimistic-concurrency-for-conflict-resolution"></a><a name="optimisticconcurrency"></a>做法：使用開放式並行存取來解決衝突
...
###<a name="<a-name="binding"></a>how-to:-bind-mobile-apps-data-to-a-windows-user-interface"></a><a name="binding"></a>如何：將 Mobile Apps 資料繫結至 Windows 使用者介面
...
###<a name="<a-name="pagesize"></a>change-the-page-size"></a><a name="pagesize"></a>變更頁面大小
...
##<a name="<a-name="#offlinesync"></a>work-with-offline-tables"></a><a name="#offlinesync"></a>使用離線資料表
...
2. (選擇性) 若要支援 Windows 裝置，請安裝下列其中一個 SQLite 執行階段封裝︰
    ...
    * **通用 Windows 平台：**安裝 [SQLite for the Universal Windows Universal][5]。
...
###<a name="<a-name="syncoffline"></a>syncing-an-offline-table"></a><a name="syncoffline"></a>同步處理離線資料表
...
##<a name="<a-name="#customapi"></a>work-with-a-custom-api"></a><a name="#customapi"></a>使用自訂 API
...
##<a name="<a-name="authentication"></a>authenticate-users"></a><a name="authentication"></a>驗證使用者
...
支援兩種驗證流程：_用戶端管理_和_伺服器管理_流程。 由於伺服器管理流程採用提供者的 Web 驗證介面，因此所提供的驗證體驗也最為簡單。 因為用戶端管理流程採用提供者特定的裝置特定 SDK，因此可允許與裝置特定功能的深入整合。
...
###<a name="<a-name="clientflow"></a>client-managed-authentication"></a><a name="clientflow"></a>用戶端管理的驗證
...
#### <a name="<a-name="adal"></a>authenticate-users-with-the-active-directory-authentication-library"></a><a name="adal"></a>使用 Active Directory Authentication Library 驗證使用者
...
3. 根據您使用的平台，將下列程式碼新增至您的應用程式。 在每個程式碼中，進行下列取代：
    ...
    * 使用 HTTPS 配置，以您網站的 **/.auth/login/done** 端點取代 _INSERT-REDIRECT-URI-HERE_ 。 此值應與 _https://contoso.azurewebsites.net/.auth/login/done_ 類似。
...
####<a name="<a-name="client-facebook"></a>single-sign-on-using-a-token-from-facebook-or-google"></a><a name="client-facebook"></a>使用來自 Facebook 或 Google 的權杖單一登入
...
####<a name="<a-name="client-livesdk"></a>single-sign-in-using-microsoft-account-with-the-live-sdk"></a><a name="client-livesdk"></a>使用含有 Live SDK 的 Microsoft 帳戶單一登入
...
###<a name="<a-name="serverflow"></a>server-managed-authentication"></a><a name="serverflow"></a>伺服器管理的驗證
...
###<a name="<a-name="caching"></a>caching-the-authentication-token"></a><a name="caching"></a>快取驗證權杖
...
Xamarin 應用程式會使用 [Xamarin.Auth] API，將憑證安全地儲存在 [帳戶] 物件中。 如需使用這些 API 的範例，請參閱 [ContosoMoments 照片分享範例]中的 [AuthStore.cs] 程式碼檔案。
...
##<a name="<a-name="pushnotifications"></a>push-notifications"></a><a name="pushnotifications"></a>推播通知
...
###<a name="<a-name="register-for-push"></a>how-to:-register-for-push-notifications"></a><a name="register-for-push"></a>做法：註冊推播通知
...
###<a name="<a-name="package-sid"></a>how-to:-obtain-a-windows-store-package-sid"></a><a name="package-sid"></a>如何：取得 Windows 市集封裝 SID
...
許多使用套件 SID 的情況會將其視為 URI，在這種情況下，您必須使用 _ms-app://_ 作為配置。 記下您封裝 SID 的版本，封裝 SID 是由串連這個值作為首碼所形成。
...
###<a name="<a-name="register-xplat"></a>how-to:-register-push-templates-to-send-cross-platform-notifications"></a><a name="register-xplat"></a>作法：註冊推送範本以傳送跨平台通知
...
##<a name="<a-name="misc"></a>miscellaneous-topics"></a><a name="misc"></a>其他主題
...
如需另一個處理錯誤狀況的範例，請造訪 [Mobile Apps 檔案範例]。 [LoggingHandler] 範例能提供記錄委派處理常式 (下述)，以將提出的要求記錄在後端。

###<a name="<a-name="headers"></a>how-to:-customize-request-headers"></a><a name="headers"></a>作法：自訂要求標頭
...
[如何針對 Active Directory 登入設定 App Service]: app-service-mobile-how-to-configure-active-directory-authentication.md
