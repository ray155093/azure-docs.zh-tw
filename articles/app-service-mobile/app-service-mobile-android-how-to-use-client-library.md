...
# <a name="how-to-use-the-android-client-library-for-mobile-apps"></a>如何使用 Mobile Apps 的 Android 用戶端程式庫
...
###<a name="<a-name="gradle-build"></a>update-the-gradle-build-file"></a><a name="gradle-build"></a>更新 Gradle 組建檔案
...
###<a name="<a-name="enable-internet"></a>enable-internet-permission"></a><a name="enable-internet"></a>啟用網際網路權限

若要存取 Azure，您的應用程式必須啟用網際網路權限。 如果尚未啟用，請將下列這一行程式碼加入至 AndroidManifest.xml  檔案：

    <uses-permission android:name="android.permission.INTERNET" />

## <a name="the-basics-deep-dive"></a>基本概念的深入探討

本節將探討快速入門應用程式中與使用 Azure Mobile Apps 有關的一些程式碼。  

###<a name="<a-name="data-object"></a>define-client-data-classes"></a><a name="data-object"></a>定義用戶端資料類別
...
###<a name="<a-name="create-client"></a>how-to:-create-the-client-context"></a><a name="create-client"></a>做法：建立用戶端內容
...
###<a name="<a-name="binding"></a>how-to:-bind-data-to-the-user-interface"></a><a name="binding"></a>作法：將資料繫結到使用者介面
...
#### <a name="<a-name="layout"></a>how-to:-define-the-layout"></a><a name="layout"></a>作法：定義配置
...
#### <a name="<a-name="adapter"></a>how-to:-define-the-adapter"></a><a name="adapter"></a>作法：定義配接器
...
### <a name="<a-name="api"></a>the-api-structure"></a><a name="api"></a>API 結構
...
#### <a name="<a-name="use-adapter"></a>how-to:-use-the-adapter"></a><a name="use-adapter"></a>作法：使用配接器
...
##<a name="<a-name="querying"></a>how-to:-query-data-from-your-mobile-app-backend"></a><a name="querying"></a>做法：查詢行動應用程式後端中的資料
...
### <a name="<a-name="showall"></a>how-to:-return-all-items-from-a-table"></a><a name="showAll"></a>作法：從資料表傳回所有項目
...
### <a name="<a-name="chaining"></a>how-to:-concatenate-query-methods"></a><a name="chaining"></a>作法：串連查詢方法

用來查詢後端資料表的方法是可以串連的。 鏈結查詢方法可讓您從排序和分頁的篩選資料列中選取特定資料行。 您可以建立複雜的邏輯篩選器。
每個查詢方法都會傳回 Query 物件。 若要結束這一系列的方法並實際執行查詢，請呼叫 **execute** 方法。 例如：
...
##<a name="<a-name="inserting"></a>how-to:-insert-data-into-the-backend"></a><a name="inserting"></a>做法：將資料插入後端
...
##<a name="<a-name="updating"></a>how-to:-update-data-in-a-mobile-app"></a><a name="updating"></a>做法︰更新行動應用程式中的資料
...
##<a name="<a-name="deleting"></a>how-to:-delete-data-in-a-mobile-app"></a><a name="deleting"></a>做法：刪除行動應用程式中的資料
...
##<a name="<a-name="lookup"></a>how-to:-look-up-a-specific-item"></a><a name="lookup"></a>作法：查閱特定項目
...
##<a name="<a-name="untyped"></a>how-to:-work-with-untyped-data"></a><a name="untyped"></a>作法：使用不具類型的資料
...
### <a name="<a-name="json_instance"></a>how-to:-create-an-instance-of-an-untyped-table"></a><a name="json_instance"></a>作法：建立不具型別的資料表執行個體
...
### <a name="<a-name="json_insert"></a>how-to:-insert-into-an-untyped-table"></a><a name="json_insert"></a>作法：插入不具型別的資料表中
...
### <a name="<a-name="json_delete"></a>how-to:-delete-from-an-untyped-table"></a><a name="json_delete"></a>作法：在不具型別的資料表中進行刪除
...
### <a name="<a-name="json_get"></a>how-to:-return-all-rows-from-an-untyped-table"></a><a name="json_get"></a>作法：從不具型別的資料表傳回所有資料列
...
##<a name="<a-name="custom-api"></a>how-to:-call-a-custom-api"></a><a name="custom-api"></a>作法：呼叫自訂 API
...
##<a name="<a-name="authentication"></a>how-to:-add-authentication-to-your-app"></a><a name="authentication"></a>做法：將驗證新增至您的應用程式
...
### <a name="<a-name="caching"></a>how-to:-add-authentication-code-to-your-app"></a><a name="caching"></a>作法：將驗證碼新增至您的應用程式
...
### <a name="<a-name="caching"></a>how-to:-cache-authentication-tokens"></a><a name="caching"></a>作法：快取驗證權杖
...
您可以在快取驗證權杖一節中看到如何[快取驗證權杖][7]的完整範例。

當您嘗試使用到期的權杖時，您會收到「401 未授權」  的回應。 您可以使用篩選器處理驗證錯誤。  篩選器會攔截對 App Service 後端提出的要求。 篩選器程式碼會測試 401 的回應、觸發登入程序，然後繼續執行產生 401 的要求。 

## <a name="<a-name="adal"></a>how-to:-authenticate-users-with-the-active-directory-authentication-library"></a><a name="adal"></a>做法：使用 Active Directory Authentication Library 驗證使用者
...
## <a name="how-to:-add-push-notification-to-your-app"></a>如何：將推播通知新增至您的應用程式

您可以[閱讀概觀][6]，它描述 Microsoft Azure 通知中樞如何支援各種推播通知。  在[本教學課程][5] 中，每次插入記錄時，就會傳送推播通知到所有裝置。

## <a name="how-to:-add-offline-sync-to-your-app"></a>如何：將離線同步處理新增至應用程式
...
##<a name="<a-name="customizing"></a>how-to:-customize-the-client"></a><a name="customizing"></a>作法：自訂用戶端
...
### <a name="<a-name="serialization"></a>how-to:-customize-serialization"></a><a name="serialization"></a>作法：自訂序列化
...
### <a name="<a-name="columns"></a>how-to:-map-different-client-and-server-names"></a><a name="columns"></a>作法：對應不同的用戶端和伺服器名稱
...
### <a name="<a-name="table"></a>how-to:-map-different-table-names-between-the-client-and-the-backend"></a><a name="table"></a>做法：在用戶端與後端之間對應不同的資料表名稱

使用 [getTable()][4] 方法的覆寫將用戶端資料表名稱對應至不同的行動服務資料表名稱：

    mToDoTable = mClient.getTable("ToDoItemBackup", ToDoItem.class);

### <a name="<a-name="conversions"></a>how-to:-automate-column-name-mappings"></a><a name="conversions"></a>作法：自動化資料行名稱對應
...
### <a name="<a-name="complex"></a>how-to:-store-an-object-or-array-property-into-a-table"></a><a name="complex"></a>作法：將物件或陣列屬性儲存在資料表中
...
<!-- URLs. -->
[開始使用 Azure Mobile Apps]: app-service-mobile-android-get-started.md
[ASCII 控制碼 C0 和 C1]: http://en.wikipedia.org/wiki/Data_link_escape_character#C1_set
[Android 的行動服務 SDK]: http://go.microsoft.com/fwlink/p/?LinkID=717033
