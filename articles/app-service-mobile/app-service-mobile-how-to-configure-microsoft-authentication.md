...
6. 在 [重新導向 URI] 下，提供應用程式的端點，然後按一下 [儲存]。
   ...
   > [!NOTE]
   > 您的重新導向 URI 是您的應用程式 URL 加上路徑 */.auth/login/microsoftaccount/callback*。例如，`https://contoso.azurewebsites.net/.auth/login/microsoftaccount/callback`。請確實使用 HTTPS 配置。
   > 
   > 
