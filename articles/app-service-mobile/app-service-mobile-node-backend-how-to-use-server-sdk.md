...
# <a name="how-to-use-the-azure-mobile-apps-node.js-sdk"></a>如何使用 Azure Mobile Apps Node.js SDK
...
### <a name="<a-name="howto-cmdline-basicapp"></a>how-to:-create-a-basic-node.js-backend-using-the-command-line"></a><a name="howto-cmdline-basicapp"></a>作法：使用命令列建立基本 Node.js 後端
...
### <a name="<a-name="howto-vs2015-basicapp"></a>how-to:-create-a-node-backend-with-visual-studio-2015"></a><a name="howto-vs2015-basicapp"></a>作法：使用 Visual Studio 2015 建立 Node 後端
...
7. 在搜尋方塊中輸入 _azure-mobile-apps_ 。  按一下 [azure-mobile-apps 2.0.0] 套件，然後按一下 [安裝套件]。
...
### <a name="<a-name="create-node-backend-portal"></a>how-to:-create-a-node.js-backend-using-the-azure-portal"></a><a name="create-node-backend-portal"></a>作法：使用 Azure 入口網站建立 Node.js 後端
...
### <a name="<a-name="download-quickstart"></a>how-to:-download-the-node.js-backend-quickstart-code-project-using-git"></a><a name="download-quickstart"></a>作法：使用 Git 下載 Node.js 後端快速入門程式碼專案
...
### <a name="<a-name="howto-publish-to-azure"></a>how-to:-publish-your-node.js-backend-to-azure"></a><a name="howto-publish-to-azure"></a>作法：將您的 Node.js 後端發佈至 Azure
...
### <a name="<a-name="howto-enable-homepage"></a>how-to:-enable-a-home-page-for-your-application"></a><a name="howto-enable-homepage"></a>作法：啟用應用程式的首頁
...
## <a name="<a-name="tableoperations"></a>table-operations"></a><a name="TableOperations"></a>資料表作業 
...
| POST /tables/_tablename_ | 在資料表中建立記錄 |
| PATCH /tables/_tablename_/:id | 更新資料表中的記錄 |
| DELETE /tables/_tablename_/:id | 刪除資料表中的記錄 |

此 WebAPI 支援 [OData]，而且擴充資料表結構描述以支援[離線資料同步]。

### <a name="<a-name="howto-dynamicschema"></a>how-to:-define-tables-using-a-dynamic-schema"></a><a name="howto-dynamicschema"></a>作法：使用動態結構描述定義資料表
...
### <a name="<a-name="howto-staticschema"></a>how-to:-define-tables-using-a-static-schema"></a><a name="howto-staticschema"></a>作法：使用靜態結構描述定義資料表
...
### <a name="<a-name="howto-sqlexpress-setup"></a>how-to:-use-sql-express-as-a-development-data-store-on-your-local-machine"></a><a name="howto-sqlexpress-setup"></a>作法：以 SQL Express 作為本機電腦上的開發資料存放區
...
### <a name="<a-name="howto-config-localdev"></a>how-to:-configure-your-project-for-local-development"></a><a name="howto-config-localdev"></a>作法：設定專案以在本機上進行開發

Azure Mobile Apps 會從本機檔案系統讀取名為 _azureMobile.js_ 的 JavaScript 檔案。  請勿使用此檔案在生產環境中設定 Azure Mobile Apps SDK，請改用 [Azure 入口網站] 中的 [應用程式設定]。  _azureMobile.js_ 檔案應會匯出組態物件。  最常見的設定如下：
...
### <a name="<a-name="howto-appsettings"></a>how:-configure-app-settings-for-your-mobile-app"></a><a name="howto-appsettings"></a>作法：設定行動應用程式的應用程式設定
...
### <a name="<a-name="howto-use-sqlazure"></a>how-to:-use-sql-database-as-your-production-data-store"></a><a name="howto-use-sqlazure"></a>做法：使用 SQL Database 做為您的實際執行資料存放區
...
> [AZURE.NOTE] 如果在與行動應用程式後端相同的位置中已經有資料庫，您可以改為選取 [使用現有的資料庫]，然後選取該資料庫。 我們不建議您使用位在不同位置的資料庫，因為這會產生更高的延遲。

6. 在新「行動應用程式」後端中，依序按一下 [設定]  >  [行動應用程式]  >  [資料]  >  [+新增]。
...
### <a name="<a-name="howto-tables-auth"></a>how-to:-require-authentication-for-access-to-tables"></a><a name="howto-tables-auth"></a>作法：需經過驗證才能存取資料表
...
### <a name="<a-name="howto-tables-getidentity"></a>how-to:-use-authentication-claims-with-your-tables"></a><a name="howto-tables-getidentity"></a>作法：透過資料表使用驗證宣告
...
### <a name="<a-name="howto-tables-disabled"></a>how-to:-disable-access-to-specific-table-operations"></a><a name="howto-tables-disabled"></a>做法：停用對特定資料表作業的存取權
...
### <a name="<a-name="howto-tables-query"></a>how-to:-adjust-the-query-that-is-used-with-table-operations"></a><a name="howto-tables-query"></a>做法：調整與資料表作業搭配使用的查詢
...
### <a name="<a-name="howto-tables-softdelete"></a>how-to:-configure-soft-delete-on-a-table"></a><a name="howto-tables-softdelete"></a>作法：設定資料表上的虛刪除
...
### <a name="<a-name="howto-tables-seeding"></a>how-to:-seed-your-database-with-data"></a><a name="howto-tables-seeding"></a>做法：在您的資料庫中植入資料
...
### <a name="<a-name="swagger"></a>how-to:-enable-swagger-support"></a><a name="Swagger"></a>作法：啟用 Swagger 支援
...
swagger 端點位於 http://_yoursite_.azurewebsites.net/swagger。  您可以透過 `/swagger/ui` 端點存取 Swagger UI。  如果您選擇需要跨整個應用程式驗證，Swagger 會產生錯誤。  為了獲得最佳結果，請在「Azure App Service 驗證/授權」設定中選擇允許未經驗證的要求通過，然後使用 `table.access` 屬性來控制驗證。

如果您想要只在本機進行開發時才使用 Swagger 支援，您也可以將 Swagger 選項新增到您的 `azureMobile.js` 檔案中。

## <a name="<a-name="push">push-notifications"></a><a name="push">推播通知
...
### <a name="</a><a-name="send-push"></a>how-to:-send-push-notifications"></a></a><a name="send-push"></a>作法：傳送推播通知
...
###<a name="<a-name="push-user"></a>how-to:-send-push-notifications-to-an-authenticated-user-using-tags"></a><a name="push-user"></a>作法：使用標記將推播通知傳送給已驗證的使用者
...
## <a name="<a-name="customapi"></a>-custom-apis"></a><a name="CustomAPI"></a> 自訂 API

###  <a name="<a-name="howto-customapi-basic"></a>how-to:-define-a-custom-api"></a><a name="howto-customapi-basic"></a>作法：定義自訂 API
...
讓我們使用一個會透過 _Date.now()_ 方法傳回伺服器日期的範例 API。  以下是 api/date.js 檔案：
...
### <a name="<a-name="howto-customapi-auth"></a>how-to:-require-authentication-for-access-to-a-custom-api"></a><a name="howto-customapi-auth"></a>做法：需經過驗證才能存取自訂 API
...
### <a name="<a-name="howto-customapi-auth"></a>how-to:-handle-large-file-uploads"></a><a name="howto-customapi-auth"></a>作法：處理大型檔案上傳
...
### <a name="<a-name="howto-customapi-sql"></a>how-to:-execute-custom-sql-statements"></a><a name="howto-customapi-sql"></a>作法：執行自訂 SQL 陳述式
...
## <a name="<a-name="debugging"></a>debugging,-easy-tables,-and-easy-apis"></a><a name="Debugging"></a>偵錯、簡單資料表及簡單的 API

### <a name="<a-name="howto-diagnostic-logs"></a>how-to:-debug,-diagnose,-and-troubleshoot-azure-mobile-apps"></a><a name="howto-diagnostic-logs"></a>作法：為 Azure Mobile Apps 偵錯、診斷和進行疑難排解
...
### <a name="<a-name="in-portal-editing"></a><a-name="work-easy-tables"></a>how-to:-work-with-easy-tables-in-the-azure-portal"></a><a name="in-portal-editing"></a><a name="work-easy-tables"></a>作法：在 Azure 入口網站中使用簡單資料表
...
###<a name="<a-name="work-easy-apis"></a>how-to:-work-with-easy-apis-in-the-azure-portal"></a><a name="work-easy-apis"></a>做法：在 Azure 入口網站中使用簡單 API
...
###<a name="<a-name="online-editor"></a>how-to:-edit-code-in-the-app-service-editor"></a><a name="online-editor"></a>作法：在 App Service 編輯器中編輯程式碼
