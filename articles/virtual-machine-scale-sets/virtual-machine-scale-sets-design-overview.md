...
從 2016-03-30 API 版本開始，「VM 擴展集」預設會「過度佈建」VM。這表示擴展集實際啟動的 VM 數目會超過您要求的數目，然後再刪除最後啟動的額外 VM。這可改善佈建的成功率，因為即使一部 VM 未成功佈建，Azure Resource Manager 都會將整個部署視為「失敗」。這些額外的 VM 不會向您收費，也不計入配額限制。

雖然這的確改善了佈建的成功率，但也可能導致不是設計來處理 VM 的應用程式出現令人困惑的無預警消失行為。若要將過度佈建關閉，請確定範本中有下列字串："overprovision": "false"
