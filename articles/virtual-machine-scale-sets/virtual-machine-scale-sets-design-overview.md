...
## 限制 (平台與自訂)
建置在平台映像上的擴展集限制為 100 部 VM，這種規模建議 5 個儲存體帳戶。不過，建置在自訂映像 (一個由您所建立) 上的擴展集，必須在一個儲存體帳戶內建立所有的 OS 磁碟 VHD。因此，建置在自訂映像上的擴展集 VM 數上限為 20。如果關閉過度佈建，數目上限為 40。


## 大規模

平台映像的擴展集上限為 100 部 VM，自訂映像為 20 (沒有過度佈建則為 40)。如果需要更多，您必須部署多個擴展集。如需如何執行這項操作的範例，請參閱這個範本︰https://github.com/Azure/azure-quickstart-templates/tree/master/301-custom-images-at-scale。
