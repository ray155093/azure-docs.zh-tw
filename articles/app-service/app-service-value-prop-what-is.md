<properties 
	pageTitle="適用於 Web 應用程式和行動應用程式的 Azure App Service | Microsoft Azure" 
...
App Service 是 Microsoft Azure 的[平台即服務](https://en.wikipedia.org/wiki/Platform_as_a_service) (PaaS) 供應項目。建立適用任何平台或裝置的 Web 與行動應用程式。整合您的應用程式與 SaaS 解決方案、與內部部署應用程式連接，並使您的商務程序自動化。Azure 會使用您選擇的共用 VM 資源或專用 VM，在完全受管理的虛擬機器 (VM) 上執行您的應用程式。
...
[App Service 方案](azure-web-sites-web-hosting-plans-in-depth-overview.md)指定應用程式執行所在的計算資源種類。如果您預期較低的流量負載，您可以使用共用虛擬機器 (VM)。對於較高的負載，您可以從數種大小的專用 VM 進行選擇。多個 App Service 應用程式可以共用相同的方案，並隨著方案一起相應增加或相應減少。
