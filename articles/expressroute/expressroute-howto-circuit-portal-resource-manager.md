---
title: 使用 Resource Manager 和 Azure 入口網站建立和修改 ExpressRoute 循環 | Microsoft Docs
...
2. 按一下 [ExpressRoute] 之後，將會看到 [建立 ExpressRoute 線路] 刀鋒視窗。 填寫此刀鋒視窗上的值時，請確定您指定正確的 SKU 層和資料計量。
   ...
   * 決定計費類型。 您可以指定 [已計量] 以採用計量付費數據傳輸方案，選取 [無限制] 以採用無限行動數據方案。 請注意，您可以將計費類型從 [已計量] 變更為 [無限制]，但無法從 [無限制] 變更為 [已計量]。
