...
---
# <a name="create-a-virtual-network-for-expressroute-in-the-classic-portal"></a>在傳統入口網站中建立 ExpressRoute 的虛擬網路
...
4. 在 [DNS Servers and VPN Connectivity]  頁面上，輸入下列資訊，然後按一下右下角的下一步箭頭。 
   ...
   * **區域網路** - 您必須建立 ExpressRoute 的區域網路網站。 但是，在 ExpressRoute 連接的案例中，指定用於區域網路網站的位址前置詞會被忽略。 相反地，透過 ExpressRoute 電路向 Microsoft 通告的位址前置詞將供路由之用。<BR>如果您已為 ExpressRoute 連線建立區域網路，則可以從下拉式清單中選取它。 如果未建立，請選取 [指定新的區域網路]。
