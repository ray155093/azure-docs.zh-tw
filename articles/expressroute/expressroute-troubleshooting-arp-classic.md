<properties 
   ...
   description="此頁面提供取得適用於 ExpressRoute 線路的 ARP 表格的相關指示"
...
#ExpressRoute 疑難排解指南 - 取得傳統部署模型中的 ARP 表格
...
ARP 表格可協助您驗證第 2 層組態，以及疑難排解基本的第 2 層連線問題。
...
下一節提供您如何檢視 ExpressRoute 邊緣路由器所看見的 ARP 表格的相關資訊。
...
進一步了解之前，請確定您符合下列條件
 ...
 - 用來設定對等互連 (Azure 私用、Azure 公用和 Microsoft) 的 IP 位址範圍。檢閱 [ExpressRoute 路由需求頁面](expressroute-routing.md)中的 IP 位址指派範例，以了解如何將 IP 位址對應到您的 aise 和 ExpressRoute 端上的介面。您可以藉由檢閱 [ExpressRoute 對等互連組態頁面](expressroute-howto-routing-classic.md)來取得對等互連組態的詳細資訊。
 - 在可與這些 IP 位址搭配使用的介面的 MAC 位址上，由您的網路小組 / 連線提供者所提供的資訊。
 - 您必須擁有適用於 Azure 的最新 PowerShell 模組 (版本 1.50 或更新版本)。

## 取得適用於 ExpressRoute 線路的 ARP 表格
本節提供如何使用 PowerShell 來檢視每個對等互連的 ARP 表格的相關指示。您或您的連線提供者必須先設定對等互連，才能有進一步的進展。每個線路都有兩個路徑 (主要和次要)。您可以單獨檢查每個路徑的 ARP 表格。
...
## 如何使用此資訊
對等互連的 ARP 表格可用來決定驗證第 2 層組態與連線。本節提供在不同狀況下 ARP 表格呈現方式的概觀。
...
### 當內部部署 / 連線提供者端發生問題時的 ARP 表格
 ...
 - ARP 表格中只出現一個項目。這將會顯示在 Microsoft 端使用的 MAC 位址與 IP 位址之間的對應。 
