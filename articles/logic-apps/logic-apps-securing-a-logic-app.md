...
來自邏輯應用程式的許多受管理連接器提供對內部部署系統 (包括檔案系統、SQL、SharePoint、DB2 等等) 的安全連線。  閘道會透過 Azure 服務匯流排使用加密通道來轉送內部部署資料，以及所有源自安全外送流量 (來自閘道代理程式) 的所有流量。  如需閘道運作方式的詳細資料，請參閱[此文章](logic-apps-gateway-install.md#how-the-gateway-works)。
