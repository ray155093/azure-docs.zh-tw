---
title: "解碼 Azure Logic Apps 中的 EDIFACT 訊息 | Microsoft Docs"
description: "如何在您的邏輯應用程式中使用 Enterprise Integration Pack 的 EDIFACT 解碼器"
services: logic-apps
...
# <a name="get-started-with-decode-edifact-message"></a>開始使用解碼 EDIFACT 訊息
解碼 EDIFACT 連接器會驗證 EDI 和夥伴特定屬性，產生每個交易集的 XML 文件，以及產生已處理交易的確認。

## <a name="prereqs"></a>必要條件
* Azure 帳戶；您可以建立一個 [免費帳戶](https://azure.microsoft.com/free)
* 需要有整合帳戶才能使用解碼 EDIFACT 訊息連接器。 詳細資料請參閱如何建立[整合帳戶](logic-apps-enterprise-integration-create-integration-account.md)、[合作夥伴](logic-apps-enterprise-integration-partners.md)和 [EDIFACT 協議](logic-apps-enterprise-integration-edifact.md)。
...
4. 輸入整合帳戶詳細資料。  具有星號的屬性為必要項目：
   ...
   | 整合帳戶 * |輸入整合帳戶名稱。 請確定您的整合帳戶和邏輯應用程式位於相同的 Azure 位置 |
    ...
    ![整合帳戶已建立](./media/logic-apps-enterprise-integration-edifact-decode/edifactdecodeimage3.png)  
5. 選取 [建立] 。
6. 請注意，已建立連線：
   
    ![整合帳戶連線詳細資料](./media/logic-apps-enterprise-integration-edifact-decode/edifactdecodeimage5.png)  
7. 選取要解碼的 EDIFACT 一般檔案訊息：
   
    ![提供必要欄位](./media/logic-apps-enterprise-integration-edifact-decode/edifactdecodeimage5.png)  

## <a name="edifact-decoder-details"></a>EDIFACT 解碼器詳細資料
解碼 EDIFACT 連接器會執行下列動作︰ 
