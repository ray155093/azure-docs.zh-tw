...
要求觸發程序允許您輸入您希望接收之承載的 JSON 結構描述。 這可讓設計工具產生權杖，以協助您使用要求的內容。 如果您尚未準備好結構描述，請選取 `Use sample payload to generate schema` 以從範例承載中產生 JSON 結構描述。

![結構描述](./media/logic-apps-http-endpoint/manualtrigger.png)

### <a name="use-parse-json-action"></a>使用剖析 JSON 動作
`Parse JSON` 動作可讓您將 JSON 內容剖析為易記權杖，以在邏輯應用程式中使用。 類似於要求觸發程序，它可讓您輸入或產生您想要剖析內容的 JSON 結構描述。 這是很棒的工具，可讓從服務匯流排、DocumentDb 等取用資料更容易。
