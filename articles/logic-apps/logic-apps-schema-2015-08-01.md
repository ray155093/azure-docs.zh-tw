...
經過這些變更，已移除函式 `@accessKeys()`，改為以 `@listCallbackURL()` 函式來取得端點 (如果需要)。 此外，您現在必須在邏輯應用程式中至少定義一個觸發程序。 如果您想要 `/run` 工作流程，您必須有 `manual`、`apiConnectionWebhook` 或 `httpWebhook` 觸發程序。 
