...
---
# <a name="use-the-storsimple-manager-service-to-manage-storage-accounts-for-storsimple-virtual-array"></a>針對 StorSimple Virtual Array 使用 StorSimple Manager 服務管理儲存體帳戶
...
每個 StorSimple Virtual Array 會在相關聯的儲存體帳戶中建立容器 (具有前置詞 hcs)。 此容器具有您的裝置的所有雲端資料。 請勿藉由透過 Azure 儲存體服務存取容器來刪除容器，因為此動作會導致資料遺失。
...
3. 在 [新增/編輯儲存體帳戶]  對話方塊中：
   ...
   3. 您可以選擇重新產生儲存體帳戶的存取金鑰。 如需詳細資訊，請參閱[重新產生儲存體帳戶金鑰](../storage/storage-create-storage-account.md#manage-your-storage-access-keys)。 請提供新的儲存體帳戶金鑰。 而每個 Azure 儲存體帳戶，都有主要存取金鑰。 
...
> [!IMPORTANT]
> 您只能刪除非使用中的儲存體帳戶。 如果您要刪除的儲存體帳戶正在使用中，系統會通知您。
> 
> 
