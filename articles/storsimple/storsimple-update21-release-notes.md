<properties 
   pageTitle="StorSimple 8000 系列 Update 2.1 版本資訊 | Microsoft Azure"
   description="說明 StorSimple 8000 系列 Update 2.1 的新功能、問題及因應措施。"
   services="storsimple"
...
>[AZURE.IMPORTANT]
> 
> - Update 2.1 僅提供軟體更新。安裝此更新，大約需要花費 1.5-2 小時。 
...
Update 2.1 包含以下重要改良功能。

 
- **自動空間回收最佳化** – 在精簡佈建的磁碟區上刪除資料時，需回收未使用的儲存區塊。此版本已經改善雲端空間回收程序，相較於之前的版本，此版本可讓未使用的空間更快變成可用空間。


- **快照效能增強** – Update 2.1 已經改進在某些情況下處理雲端快照集的時間，例如在使用大型磁碟區以及無資料變換最少的情況下。可因這項增強功能獲益的案例是封存磁碟區。


- **強化支援封裝收集** – 此版本已改善收集及上傳支援封裝的方式。


- **更新可靠性改進** – 此版本包含一些錯誤修正程式，可改善更新可靠性。
...
| 7 | Windows Management Instrumentation (WMI) | 在較早版本的軟體中，有幾個 Web Proxy 失敗且包含「<ManagementException> 提供者載入失敗」例外狀況的錯誤情況。這個錯誤屬於 WMI 記憶體流失錯誤，現在已經修正。 | 是 | 否 |
...
## Update 2.1中的控制器和韌體更新

此版本僅提供軟體更新。但是，如果您是從 Update 2 之前的版本更新，您將需要在您的裝置上安裝驅動程式、Storport、Spaceport 及 (在某些情況下) 磁碟韌體更新。
 
如需如何安裝驅動程式、Storport、Spaceport 及磁碟韌體更新的詳細資訊，請參閱在您的 StorSimple 裝置上[安裝 Update 2.1](storsimple-install-update-21.md)。
