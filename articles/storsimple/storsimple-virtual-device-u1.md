...
3. 在 [建立虛擬裝置] 對話方塊中，指定下列詳細資料。
	...
	5. **虛擬裝置建立的儲存體帳戶** - 此儲存體帳戶將在佈建期間用來存放虛擬裝置映像，並將在佈建後裝載虛擬裝置的磁碟。此儲存體帳戶應該與虛擬裝置和虛擬網路位於相同的區域中。不應讓實體裝置或虛擬裝置用它來儲存資料。根據預設，將基於此用途建立新的儲存體帳戶。不過，如果您知道已經擁有適合此用途的儲存體帳戶，則可從清單中選取該帳戶。
