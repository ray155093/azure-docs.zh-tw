<properties 
   pageTitle="使用 Azure Site Recovery 將 StorSimple 上檔案共用的 DR 自動化| Microsoft Azure"
...
1.  透過 Azure Site Recovery 將 iSCSI 目標與您想要保護的內部部署 VM 中斷連線：
	...
	> [AZURE.NOTE] 這將會導致檔案共用暫時無法使用

1.  從 Azure Site Recovery 入口網站為檔案伺服器 VM [啟用虛擬機器保護](../site-recovery/site-recovery-hyper-v-site-to-azure.md##step-6-enable-replication)。

2.  當起始同步處理開始時，您可以再次重新連線目標。移至 iSCSI 啟動器，選取 StorSimple 裝置，然後按一下 [連線]。

3.  當同步完成且 VM 的狀態為 [受保護] 時，請選取 VM，選取 [設定] 索引標籤，然後據以更新 VM 的網路 (這是已容錯移轉的 VM 所屬的網路)。如果網路沒有顯示，表示同步仍在進行中。
...
1.  建立新的自動化帳戶。請將它維持在 StorSimple Cloud Appliance 和儲存體帳戶建立所在的相同地理區域中。

2.  按一下 [新增] &gt; [應用程式服務] &gt; [自動化] &gt; [Runbook] &gt; [從資源庫]，來將所有必要的 Runbook 匯入自動化帳戶。

	![](./media/storsimple-dr-using-asr/image3.png)

1.  從資源庫中的 [災害復原] 窗格新增下列 Runbook：
...
1.  透過選取自動化帳戶中的 Runbook 並移至 [作者] 索引標籤來發佈所有指令碼。在這個步驟之後，[Runbook] 索引標籤看起來會像下面這樣：

	 ![](./media/storsimple-dr-using-asr/image5.png)

1.  在自動化帳戶中，移至 [資產] 索引標籤，按一下 [加入設定] &gt; [加入認證]，然後加入您的 Azure 認證 – 將資產命名為 AzureCredential。
...
1.  在在自動化帳戶中，選取 [資產] 索引標籤，然後按一下 [加入設定] &gt; [加入變數]，然後加入下列變數。您可以選擇將這些資產加密。這些變數都是復原計劃特定變數。如果您的復原計劃 (您將會在下一個步驟中建立) 名稱為 TestPlan，您的變數就應該是 TestPlan-StorSimRegKey、TestPlan-AzureSubscriptionName 等等。
...
1.  在相同的 [群組 1：後續步驟] 區段中，於上面 4 個指令碼之後，加入一個手動動作。這個動作是您可以確認所有項目都正確運作的點。只有在做為測試容錯移轉的一部份時，才需要新增這個動作 (所以只選取 [測試容錯移轉] 核取方塊)。

2.  在執行手動動作之後，請使用您為其他 Runbook 使用的相同程序新增 Cleanup 指令碼。儲存復原計畫

	> [AZURE.NOTE] 執行測試容錯移轉時，您應該在手動動作步驟中確認所有項目，因為已在目標裝置上複製的 StorSimple 磁碟區，將在手動動作完成之後於清除作業中一併刪除。
...
1.  當次要環境啟動時，您就可以執行您的驗證。

2.  驗證完成之後，按一下 [驗證完成]。將會清除測試容錯移轉環境，且將會完成 TFO 作業。
...
2.  按一下 [容錯移轉]，然後選取 [非計劃性容錯移轉]。

	![](./media/storsimple-dr-using-asr/image9.png)

1.  選取目標網路，然後按一下核取圖示 ✓ 來開始容錯移轉程序。
...
2.  按一下 [容錯移轉]，然後選取 [計劃性容錯移轉]。

3.  選取目標網路，然後按一下核取圖示 ✓ 來開始容錯移轉程序。
...
2.  按一下 [容錯移轉] 並選取 [計劃性容錯移轉] 或 [非計劃性容錯移轉]。

3.  按一下 [變更方向]。

4.  選取適當的資料同步處理和 VM 建立選項。
...
- 備份工作逾時：如果備份磁碟區時花費的時間超出每個指令碼的 Azure Site Recovery 限制 (目前為 120 分鐘)，StorSimple 指令碼就會逾時。
	...
	> [AZURE.IMPORTANT] 請從 Azure 入口網站手動執行備份，然後再執行一次復原計劃。
...
- 時間同步處理錯誤：StorSimple 指令碼發生錯誤並指出備份失敗，即使在入口網站中備份成功。發生此問題的可能原因或許是因為 StorSimple 設備的時間可能沒有和時區中目前的時間同步。
	...
	> [AZURE.IMPORTANT] 請將設備時間與時區中目前的時間同步。

- 設備容錯移轉錯誤：如果復原計劃執行時有進行設備容錯移轉，StorSimple 指令碼可能會失敗。
	
	> [AZURE.IMPORTANT] 請在設備容錯移轉完成後重新執行復原計劃。
