...
完成本教學課程之後，您已指派給 AnswerHub 的 Azure AD 使用者將可在您的 AnswerHub 公司網站 (服務提供者起始的登入) 透過單一登入 (SSO) 存取應用程式，或是使用 [存取面板簡介](active-directory-saas-access-panel-introduction.md)。
...
## <a name="enable-the-application-integration-for-answerhub"></a>啟用 AnswerHub 的應用程式整合
