...
10. 在 [IDP 設定] 索引標籤上，執行下列步驟：
    ...
    1.  在 Azure 傳統入口網站的 [在 AnswerHub 設定單一登入] 對話頁面上，複製 [遠端登入 URL] 值，然後將它貼至 [IDP 登入 URL] 文字方塊中。
    2.  在 Azure 傳統入口網站的 [在 AnswerHub 設定單一登入] 對話頁面上，複製 [遠端登出 URL] 值，然後將它貼至 [IDP 登出 URL] 文字方塊中。
    3.  在 Azure 傳統入口網站的 [在 AnswerHub 設定單一登入] 對話頁面上，複製 [名稱識別碼格式] 值，然後將它貼至 [IDP 名稱識別碼格式] 文字方塊中。
    4.  按一下 [金鑰和憑證]。
