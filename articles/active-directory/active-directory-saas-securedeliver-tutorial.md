...
* Azure 訂用帳戶
* 已啟用 SECURE DELIVER 單一登入 (SSO) 的訂用帳戶
...
## <a name="add-secure-deliver-from-the-gallery"></a>從資源庫加入 SECURE DELIVER
...
7. 在結果窗格中，選取 [SECURE DELIVER]，然後按一下 [完成] 以新增應用程式。
...
若要讓 SSO 運作，Azure AD 必須知道 SECURE DELIVER 中與 Azure AD 互相對應的使用者。 換句話說，必須在 Azure AD 使用者與 SECURE DELIVER 中的相關使用者之間，建立連結關聯性。  

建立此連結關聯性的方法，就是將 Azure AD 中 [使用者名稱] 的值指定為 SECURE DELIVER 中 [Username] 的值。

若要使用 SECURE DELIVER 來設定並測試 Azure AD SSO，您需要完成下列建置組塊：
...
本節的目標是在 Azure 傳統入口網站啟用 Azure AD SSO，並在您的 SECURE DELIVER 應用程式中設定單一登入。
...
3. 在 [設定應用程式設定] 對話方塊頁面上，執行下列步驟，然後按 [下一步]：
   ...
   1. 在 [登入 URL] 文字方塊中，使用下列模式輸入使用者用來登入 SECURE DELIVER 應用程式的 URL：**“https://i-securedeliver.jp/sd/\<公司名稱\>/jsf/login/sso”**。
   2. 如果您不知道值，請透過 [iw-sd-support@fujifilm.com](mailto:iw-sd-support@fujifilm.com) 連絡 SECURE DELIVER 支援小組，取得租用戶 URL。
   3. 在 [識別碼] 文字方塊中，輸入租用戶 URL。 
   4. 按 [下一步] 。
...
### <a name="create-a-secure-deliver-test-user"></a>建立 SECURE DELIVER 測試使用者
本節的目標是在 SECURE DELIVER 中建立名為 Britta Simon 的使用者。 請使用 SECURE DELIVER 支援小組在 SECURE DELIVER 帳戶中加入使用者。

>[!NOTE]
>如果您需要手動建立使用者，您需要連絡 SECURE DELIVER 支援小組。
> 
> 

### <a name="assign-the-azure-ad-test-user"></a>指派 Azure AD 測試使用者
本節的目標是將對 SECURE DELIVER 的存取權授與 Britta Simon，使她能夠使用 Azure SSO。
