...
Azure AD 內有許多選項可變更，其中很多是關於自助式密碼重設 (SSPR)，本指南說明可能的作法。
...
1. 如果已指派**密碼管理員**角色，具有此角色的人會收到通知
2. 如果未指派密碼管理員，則具有**使用者管理員**角色的人會收到通知
...
* 登入頁面背景色彩在高延遲連線時使用，必須是 RGB 十六進位格式
...
[已停用讓我保持登入] 選項可讓使用者在關閉又重新開啟瀏覽器視窗時，保持登入狀態，而不影響工作階段存留期。 您可以在 [Azure Active Directory]、[公司商標]、[編輯公司商標] 下找到此設定。
