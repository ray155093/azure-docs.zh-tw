...
在本節中，您會在 Azure 管理入口網站中啟用 Azure AD 單一登入，並在您的 Cezanne HR Software 應用程式中設定單一登入。
...
3. 在 [Cezanne HR Software 網域及 URL] 區段上，執行下列步驟：
    ...
    > [!NOTE] 
    > 請注意這些不是真正的值。 您必須使用實際的「登入 URL」及「識別碼」來更新這些值。 在此建議您在 [識別碼] 中使用唯一的 URL 值。 若要取得這些值，請連絡 [Cezanne HR Software 支援小組](mailto:info@cezannehr.com)。
...
14. 在 [SAML 2.0 身分識別提供者]  區段中執行下列步驟。
    ...
    b. 在 [實體識別碼] 文字方塊中，放入來自 Azure AD 應用程式組態視窗的 [SAML 實體 ID] 值。

    c. 將 [SAML 繫結] 變更為 'POST'。

    d. 在 [Security Token Service 端點] 文字方塊中，放入來自 Azure AD 應用程式組態視窗的 [SAML 單一登入服務 URL] 值。

    e. 在 [使用者 ID 屬性名稱] 文字方塊中，輸入 'http://schemas.xmlsoap.org/ws/2005/05/identity/claims/name'。
