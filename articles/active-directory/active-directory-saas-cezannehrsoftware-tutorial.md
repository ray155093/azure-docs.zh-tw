...
- 您可以讓使用者使用他們的 Azure AD 帳戶自動登入 Cezanne HR Software 單一登入 (SSO)
...
## <a name="add-cezanne-hr-software-from-the-gallery"></a>從資源庫加入 Cezanne HR Software
...
4. 在 [搜尋] 方塊中，輸入 **Cezanne HR Software**。
    ...
    ![建立 Azure AD 測試使用者](./media/active-directory-saas-cezannehrsoftware-tutorial/tutorial_cezannehrsoftware_00001.png)

5. 在結果窗格中，選取 [Cezanne HR Software]，然後按一下 [新增] 按鈕以新增應用程式。

    ![建立 Azure AD 測試使用者](./media/active-directory-saas-cezannehrsoftware-tutorial/tutorial_cezannehrsoftware_0001.png)

##  <a name="configure-and-test-azure-ad-single-sign-on"></a>設定和測試 Azure AD 單一登入
在本節中，您會以名為 "Britta Simon" 的測試使用者為基礎，設定及測試與 Cezanne HR Software 搭配運作的 Azure AD SSO。

若要讓 SSO 能夠運作，Azure AD 必須知道 Cezanne HR Software 中與 Azure AD 互相對應的使用者。 換句話說，Azure AD 使用者和 Cezanne HR Software 中的相關使用者之間必須建立連結關聯性。

建立此連結關聯性的方法，就是將 Azure AD 中**使用者名稱**的值指派為 Cezanne HR Software 中 **Username** 的值。

若要使用 Cezanne HR Software 設定並測試 Azure AD SSO，您需要完成下列建置組塊：
...
在本節中，您會在 Azure 管理入口網站中啟用 Azure AD SSO，並在您的 Cezanne HR Software 應用程式中設定單一登入。

**若要使用 Cezanne HR Software 設定 Azure AD SSO，請執行下列步驟：**

1. 在 Azure 管理入口網站的 [Cezanne HR Software] 應用程式整合頁面上，按一下 [單一登入]。

    ![設定單一登入][4]

2. 在 [單一登入] 對話方塊頁面上，選取 [SAML 型登入] 做為 [模式]，以啟用單一登入。
 
    ![設定單一登入](./media/active-directory-saas-cezannehrsoftware-tutorial/tutorial_cezannehrsoftware_01.png)

3. 在 [Cezanne HR Software 網域及 URL] 區段上，執行下列步驟：

    ![設定單一登入](./media/active-directory-saas-cezannehrsoftware-tutorial/tutorial_cezannehrsoftware_02.png)
  1. 在 [登入 URL] 文字方塊中，以下列模式輸入 URL：`https://w3.cezanneondemand.com/cezannehr/-/<tenant id>`
  2. 在 [識別碼] 文字方塊中，輸入：`https://w3.cezanneondemand.com/CezanneOnDemand/`

     >[!NOTE] 
     > 這些不是真正的值。 您必須使用實際的「登入 URL」及「識別碼」來更新這些值。 在此建議您在 [識別碼] 中使用唯一的 URL 值。 若要取得這些值，請連絡 [Cezanne HR Software 支援小組](mailto:info@cezannehr.com)。
...
5. 在 [建立新憑證] 對話方塊中，按一下行事曆圖示並選取 [到期日]。 然後按一下 [儲存] 按鈕。
    ...
    ![設定單一登入](./media/active-directory-saas-cezannehrsoftware-tutorial/tutorial_general_300.png)
...
9. 在 [Cezanne HR Software 組態] 區段上，按一下 [設定 Cezanne HR Software] 以開啟 [設定登入] 視窗。

    ![設定單一登入](./media/active-directory-saas-cezannehrsoftware-tutorial/tutorial_cezannehrsoftware_06.png) 

    ![設定單一登入](./media/active-directory-saas-cezannehrsoftware-tutorial/tutorial_cezannehrsoftware_07.png)

10. 在不同的網頁瀏覽器視窗中，以系統管理員身分登入您的 Cezanne HR Software 租用戶。

11. 在左側的導覽窗格上，按一下 [系統設定] 。 移至 [安全性設定] 。 然後瀏覽至 [單一登入設定] 。

    ![在應用程式端設定單一登入](./media/active-directory-saas-cezannehrsoftware-tutorial/tutorial_cezannehrsoftware_000.png)

12. 在 [允許使用者使用下列的單一登入 (SSO) 服務來登入] 面板中檢查 [SAML 2.0] 方塊，然後選取 [進階組態] 選項。
...
14. 在 [SAML 2.0 身分識別提供者]  區段中執行下列步驟。

    ![在應用程式端設定單一登入](./media/active-directory-saas-cezannehrsoftware-tutorial/tutorial_cezannehrsoftware_003.png)
 1. 輸入識別提供者名稱做為**顯示名稱**。
 2. 在 [實體識別碼] 文字方塊中，放入來自 Azure AD 應用程式組態視窗的 [SAML 實體 ID] 值。
 3. 將 [SAML 繫結] 變更為 'POST'。
 4. 在 [Security Token Service 端點] 文字方塊中，放入來自 Azure AD 應用程式組態視窗的 [SAML 單一登入服務 URL] 值。
 5. 在 [使用者 ID 屬性名稱] 文字方塊中，輸入 'http://schemas.xmlsoap.org/ws/2005/05/identity/claims/name'。
 6. 按一下 [上傳] 圖示來上傳從 Azure AD 下載的憑證。
 7. 按一下 [確定] 按鈕。 
...
3.  在 [人員詳細資料]  區段中，執行下列步驟︰
 ...
 4. 在 [電子郵件] 文字方塊中，輸入 Britta Simon 帳戶的電子郵件地址。

4.  在 [帳戶資訊]  區段中，執行下列步驟︰

    ![新增使用者](./media/active-directory-saas-cezannehrsoftware-tutorial/tutorial_cezannehrsoftware_007.png "新增使用者")
 1. 在 [使用者名稱] 文字方塊中，輸入 Britta Simon 的電子郵件地址。
 2. 在 [密碼] 文字方塊中，輸入 Britta Simon 帳戶的密碼。
 3. 選取 [HR Professional] 做為 [安全性角色]。
 4. 按一下 [確定] 。
...
在本節中，您會將 Cezanne HR Software 的存取權授與 Britta Simon，讓她能夠使用 Azure SSO。
