...
---
# <a name="integrate-azure-ad-into-a-xamarin-app"></a>將 Azure AD 整合至 Xamarin 應用程式
...
## <a name="0-set-up-your-xamarin-development-environment"></a>*0.設定您的 Xamarin 開發環境*
...
* 設定應用程式的權限 - 在 [設定] 功能表中，選擇 [必要權限] 區段，依序按一下 [新增] 和 [選取 API]，然後選取 [Windows Azure Active Directory] (這是 AADGraph API)。 然後按一下 [選取權限] 並選取 [讀取目錄資料]。 這樣做可讓您的應用程式查詢 Graph API 的使用者。
...
* 請先開啟 `DirectorySearcher.cs`，並在 `SearchByAlias(...)` 方法中新增參數。 `IPlatformParameters` 是個內容參數，其中包含 ADAL 必須執行驗證的平台特定物件。
