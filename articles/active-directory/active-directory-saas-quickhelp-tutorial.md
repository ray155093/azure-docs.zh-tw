...
3. **[建立 QuickHelp 測試使用者](#creating-a-quickhelp-test-user)** - 使 QuickHelp 中對應的 Britta Simon 連結到她在 Azure AD 中的代表項目。
...
3. 在 [設定應用程式設定] 對話方塊頁面上，執行下列步驟：
   ...
   > [!NOTE]
   > 如果您不知道登入 URL 的值，請連絡您的 QuickHelp 支援小組。
   > 
   > 
...
5. 以系統管理員身分登入您的 QuickHelp 公司網站。
6. 在頂端的功能表中，按一下 [系統管理員]。
   
    ![設定單一登入][21]
7. 在 [QuickHelp 系統管理員] 功能表上，按一下 [設定]。
...
9. 在 [驗證設定] 頁面上，執行下列步驟
    ...
    f.在 [動作列] 中，按一下 [儲存]。
10. 在 Azure 傳統入口網站上，選取單一登入設定確認，然後按 [下一步]。
...
### 測試單一登入
本節的目標是要使用「存取面板」來測試您的 Azure AD 單一登入組態。當您在存取面板中按一下 [QuickHelp] 磚時，應該會自動登入您的 QuickHelp 應用程式。
