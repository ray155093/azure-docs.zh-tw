<properties
    pageTitle="教學課程：Azure Active Directory 與 SanSan 整合 | Microsoft Azure"
...
若要讓單一登入運作，Azure AD 必須知道 SanSan 與 Azure AD 中互相對應的使用者。 換句話說，必須建立 Azure AD 使用者和 SanSan 中相關使用者之間的連結關聯性。
...
4. 在 [設定在 SanSan 單一登入]  頁面上，執行下列步驟：
...
5.  若要為您的應用程式設定 SSO，請連絡 SanSan 支援小組，他們將會協助您設定 SSO。 請提供下列資訊給他們︰
...
> [AZURE.NOTE] PC 瀏覽器設定也適用於行動應用程式和行動瀏覽器以及 PC Web。 
...
> [AZURE.NOTE] 如果您需要手動建立使用者或需要建立使用者批次，則必須連絡 SanSan 支援小組。
