...
---
# <a name="securing-angularjs-single-page-apps-with-azure-ad"></a>使用 Azure AD 保護 AngularJS 單一頁面應用程式
...
2. 在頁面右上角按一下您的帳戶來選擇您的 Azure AD 租用戶。
3. 在左瀏覽窗格中，按一下 [Azure Active Directory]。
4. 按一下 [應用程式註冊]，然後按一下 [新增]。
5. 輸入應用程式的易記名稱，例如 "TodoListService"，選取 [Web 應用程式和/或 Web API] 做為 [應用程式類型]。
6. 在 [登入 URL] 中，輸入範例的基礎 URL，例如 `https://localhost:8080`。
7. 完成註冊後，Azure AD 將會為您的應用程式指派一個唯一用戶端識別碼 (應用程式識別碼)。 您在後續章節中將會用到這個值，所以請從應用程式頁面中複製此值。
8. 在 [設定] 頁面中，設定 [回覆 URL]，這是 AAD 將傳回權杖的位置。 此範例的預設位置是 `https://localhost:44326/`

* Adal.js 會使用 OAuth 隱含流程來與 Azure AD 通訊。  您必須為您的應用程式啟用隱含流程，請執行下列動作：
  * 按一下應用程式頁面中的 [資訊清單]，以開啟資訊清單編輯器，然後找出 `oauth2AllowImplicitFlow` 屬性。 將值設為 `true`。
  * 儲存資訊清單。

## <a name="2-install-adal--configure-the-spa"></a>*2.安裝 ADAL 並設定 SPA*
既然您在 Azure AD 中已有應用程式，您可以安裝 adal.js，並撰寫身分識別相關的程式碼。

* 首先，使用 Package Manager Console，將 adal.js 加入至 TodoSPA 專案：
  * 下載 [adal.js](https://raw.githubusercontent.com/AzureAD/azure-activedirectory-library-for-js/master/lib/adal.js) 並將它新增至 `App/Scripts/` 專案目錄。
  * 下載 [adal-angular.js](https://raw.githubusercontent.com/AzureAD/azure-activedirectory-library-for-js/master/lib/adal-angular.js) 並將它新增至 `App/Scripts/` 專案目錄。
...
## <a name="3----use-adal-to-secure-pages-in-the-spa"></a>*3.  使用 ADAL 來保護 SPA 中的頁面*
