...
若要讓 SSO 運作，Azure AD 必須知道 Atlassian Cloud 中與 Azure AD 互相對應的使用者。 換句話說，必須在 Azure AD 使用者與 Atlassian Cloud 中的相關使用者之間建立連結關聯性。
...
在本節中，您會在傳統入口網站中啟用 Azure AD 單一登入，然後在您的 Atlassian Cloud 應用程式中設定 SSO。
...
3. 在 [設定 App 設定]  對話方塊頁面執行下列步驟：
 ...
 1. 在 [登入 URL] 文字方塊中，使用以下模式輸入使用者登入您的 Atlassian Cloud 應用程式時所使用的 URL：`https://<instancename>.atlassian.net`    
 2. 在 [識別碼] 文字方塊中，以下列模式輸入 URL：`https://id.atlassian.com/login`
...
8. 建立新的 SAML 組態並新增識別提供者組態。
  1. 從 Azure AD 複製實體識別碼值，並將它貼在 [識別提供者實體識別碼] 欄位中。
  2. 複製 SAML SSO URL 並將它貼到 [識別提供者 SSO URL] 方塊中。
  3. 在記事本中從 Azure AD 開啟已下載的憑證，複製不含開始和結束行的值，並將它貼在 [公用 X509 憑證] 方塊中。
  4. 儲存設定。
...
9. 更新 Azure AD 設定，確定您已設定正確的識別項 URL。
  1. 從 [SAML] 畫面複製 [SP身分識別碼]，然後將它貼在 Azure AD 中做為 [識別碼] 值。
  2. [登入 URL] 是 Atlassian Cloud 的租用戶 URL。     
...
6.  在 [使用者設定檔]  對話方塊頁面上，執行下列步驟：
    ...
    ![建立 Azure AD 測試使用者](./media/active-directory-saas-atlassian-cloud-tutorial/create_aaduser_06.png) 
...
在本節中，您要在 Atlassian Cloud 中建立名為 Britta Simon 的使用者。 使用者應該先出現在 Atlassian Cloud 中，才能進行 SSO。 

請以系統管理員權限登入您的 Atlassian Cloud 執行個體，並執行下列步驟。
