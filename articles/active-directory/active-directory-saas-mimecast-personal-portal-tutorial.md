...
完成本教學課程之後，您指派給 Mimecast Personal Portal 的 Azure AD 使用者就能夠單一登入您 Mimecast Personal Portal 公司網站 (服務提供者起始登入) 的應用程式，或是使用 [存取面板簡介](active-directory-saas-access-panel-introduction.md)。
...
## <a name="enabling-the-application-integration-for-mimecast-personal-portal"></a>啟用 Mimecast Personal Portal 的應用程式整合
本節的目的是要說明如何啟用 Mimecast Personal Portal 的應用程式整合。

### <a name="to-enable-the-application-integration-for-mimecast-personal-portal-perform-the-following-steps"></a>若要啟用 Mimecast Personal Portal 的應用程式整合，請執行下列步驟：
...
6. 在**搜尋方塊**中，輸入 **Mimecast Personal Portal**。
...
1. 在 Azure 傳統入口網站的 [Mimecast Personal Portal] 應用程式整合頁面上，按一下 [設定單一登入] 來開啟 [設定單一登入] 對話方塊。
...
9. 在 [驗證設定檔]  區段中，執行下列步驟：
   ...
   4. 在 Azure 傳統入口網站中的 [設定在 Mimecast Personal Portal 單一登入] 對話頁面上，複製 [簽發者 URL] 值，然後將它貼至 [簽發者 URL] 文字方塊中。
   5. 在 Azure 傳統入口網站中的 [設定在 Mimecast Personal Portal 單一登入] 對話頁面上，複製 [遠端登入 URL] 值，然後將它貼至 [登入 URL] 文字方塊中。
   6. 在 Azure 傳統入口網站中的 [設定在 Mimecast Personal Portal 單一登入] 對話頁面上，複製 [遠端登入 URL] 值，然後將它貼至 [登出 URL] 文字方塊中。  
      
      > [!NOTE]
      > Mimecast Personal Portal 的登入 URL 值與登出 URL 值是相同的。
      > 
      > 
...
5. 在 [新位址] 對話方塊中，執行下列步驟：
   ...
   ![儲存](./media/active-directory-saas-mimecast-personal-portal-tutorial/IC795006.png "Save")
   
   1. 在相關的文字方塊中，輸入您要佈建之有效 AAD 帳戶的 [電子郵件地址]、[全域名稱]、[密碼]、[確認密碼] 屬性。
   2. 按一下 [儲存] 。

> [!NOTE]
> 您可以使用任何其他的 Mimecast Personal Portal 使用者帳戶建立工具或 Mimecast Personal Portal 提供的 API，佈建 AAD 使用者帳戶。
> 
> 
...
### <a name="to-assign-users-to-mimecast-personal-portal-perform-the-following-steps"></a>若要指派使用者給 Mimecast Personal Portal，請執行下列步驟：
