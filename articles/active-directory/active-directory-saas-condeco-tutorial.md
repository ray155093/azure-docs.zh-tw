...
本教學課程旨在說明如何將 Condeco 與 Azure Active Directory (Azure AD) 整合。

將 Condeco 與 Azure AD 整合提供下列優點：

* 您可以在 Azure AD 中控制可存取 Condeco 的人員
* 您可以讓使用者使用其 Azure AD 帳戶自動登入 Condeco 單一登入 (SSO)
...
## <a name="add-condeco-from-the-gallery"></a>從資源庫新增 Condeco
...
本節的目標是要說明如何以名為 "Britta Simon" 的測試使用者為基礎，設定及測試與 Condeco 搭配運作的 Azure AD SSO。

若要讓 SSO 運作，Azure AD 必須知道 Condeco 與 Azure AD 中互相對應的使用者。 換句話說，必須在 Azure AD 使用者與 Condeco 中的相關使用者之間建立連結關聯性。  

建立此連結關聯性的方法，就是將 Azure AD 中**使用者名稱**的值指派為 Condeco 中 **Username** 的值。

若要設定及測試與 Condeco 搭配運作的 Azure AD SSO，您需要完成下列建置組塊：
...
3. 在 [設定應用程式設定]  對話方塊頁面上，執行下列步驟：
  ...
  1. 在 [登入 URL] 文字方塊中，使用下列模式輸入使用者用來登入 Condeco 應用程式的 URL：**“https://companyname.condecosoftware.com”**。
  2. 按 [下一步] 。
4. 在 [設定在 Condeco 單一登入]  頁面上，執行下列步驟：
...
### <a name="create-a-condeco-test-user"></a>建立 Condeco 測試使用者
