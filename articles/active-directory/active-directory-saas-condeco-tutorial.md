...
若要讓單一登入能夠運作，Azure AD 必須知道 Condeco 與 Azure AD 中互相對應的使用者。 換句話說，必須在 Azure AD 使用者與 Condeco 中的相關使用者之間建立連結關聯性。  
...
3. 在 [設定應用程式設定]  對話方塊頁面上，執行下列步驟：
    ...
    a. 在 [登入 URL] 文字方塊中，使用下列模式輸入使用者用來登入 Condeco 應用程式的 URL：**“https://companyname.condecosoftware.com”**。
