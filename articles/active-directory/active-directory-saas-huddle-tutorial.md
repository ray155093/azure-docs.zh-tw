...
完成本教學課程之後，您指派給 Huddle 的 Azure AD 使用者就能夠單一登入您 Huddle 公司網站 (服務提供者起始登入) 的應用程式，或是使用 [存取面板簡介](active-directory-saas-access-panel-introduction.md)。
...
## <a name="enabling-the-application-integration-for-huddle"></a>啟用 Huddle 的應用程式整合
本節的目的是要說明如何啟用 Huddle 的應用程式整合。

### <a name="to-enable-the-application-integration-for-huddle-perform-the-following-steps"></a>若要啟用 Huddle 的應用程式整合，請執行下列步驟：
...
1. 在 Azure 傳統入口網站的 [Huddle] 應用程式整合頁面上，按一下 [設定單一登入] 來開啟 [設定單一登入] 對話方塊。
...
3. 在 [設定應用程式 URL] 頁面的 [Huddle 登入 URL] 文字方塊中，使用下列模式輸入 Huddle 租用戶的 URL："*http://company.huddle.com*"，然後按一下 [下一步]。
...
> [!NOTE]
> 您可以使用任何其他的 Huddle 使用者帳戶建立工具或 Huddle 提供的 API 來佈建 AAD 使用者帳戶。
> 
> 
...
### <a name="to-assign-users-to-huddle-perform-the-following-steps"></a>若要將使用者指派給 Huddle，請執行下列步驟：
