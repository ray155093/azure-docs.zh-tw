...
Azure AD Connect 可讓使用者使用同一組密碼登入雲端和內部部署資源。 本文說明每個身分識別模型的主要概念，以協助您選擇要用於登入 Azure AD 的身分識別。

如果您已熟悉 Azure AD 身分識別模型，而且想要深入了解特定的方法，只要按下面的適當主題即可。
...
* [同盟 SSO (含 ADFS)](#federation-using-a-new-or-existing-ad-fs-in-windows-server-2012-r2-farm)
...
由於大部分組織只想要讓使用者登入 Office 365、SaaS 應用程式和其他 Azure AD 資源，因此建議使用預設的密碼同步處理選項。 不過，某些組織有特定原因無法使用此選項，且可以選擇同盟的登入選項，例如 AD FS 或傳遞驗證。 下表可協助您做出正確的選擇。 

我需要 | PHS 和 SSO| PTA 與 SSO| AD FS |
 --- | --- | --- | --- |
自動將我的內部部署 Active Directory 中建立的新使用者、連絡人及群組帳戶同步至雲端|x|x|x|
設定 Office 365 混合式案例的租用戶|x|x|x|
讓我的使用者可以使用其內部部署密碼登入及存取雲端服務|x|x|x|
使用公司認證實作單一登入|x|x|x|
請確定雲端中沒有密碼儲存||x*|x|
啟用內部部署 Multi-Factor Authentication 解決方案|||x|
...
此外，您也可以針對公司網路上加入網域的電腦使用者啟用[單一登入 (SSO)](active-directory-aadconnect-sso.md)。 使用單一登入，啟用的使用者只需要輸入使用者名稱即可安全地存取雲端資源。

![雲端](./media/active-directory-aadconnect-user-signin/passwordhash.png)
...
使用通過驗證，使用者的密碼會針對內部部署 Active Directory 控制器進行驗證，且密碼不需要以任何形式在 Azure AD 中呈現。 這可在驗證雲端服務期間評估內部部署原則，例如登入時數限制。 傳遞驗證會利用可接聽密碼驗證要求的內部部署環境中加入 Windows Server 2012 R2 網域的電腦上之簡單代理程式。 此代理程式不需要任何開放給網際網路的輸入連接埠。

此外，您也可以針對公司網路上加入網域的電腦使用者啟用單一登入。 使用單一登入，啟用的使用者只需要輸入使用者名稱即可安全地存取雲端資源。
![傳遞查詢](./media/active-directory-aadconnect-user-signin/pta.png)

[傳遞查詢](active-directory-aadconnect-pass-through-authentication.md)和[單一登入](active-directory-aadconnect-sso.md)的詳細資訊。
...
使用同盟登入，使用者就可以使用內部部署密碼登入 Azure AD 服務；使用公司網路時，無須輸入密碼就可登入服務。  使用 AD FS 的同盟選項可讓您在 Windows Server 2012 R2 伺服器陣列中部署新的或指定現有的 AD FS。  如果您選擇指定現有的伺服器陣列，則 Azure AD Connect 將會在伺服器陣列與 Azure AD 間設定信任，讓使用者能夠登入。
...
[設定使用 AD FS 的 SSO](active-directory-aadconnect-get-started-custom.md#configuring-federation-with-ad-fs)
...
Azure Active Directory 中的每個目錄隨附內建網域名稱，格式為 contoso.onmicrosoft.com，可讓您開始使用 Azure 或其他 Microsoft 服務。 您可以使用自訂網域改善及簡化登入體驗。 如需關於 Azure AD 中的自訂網域名稱以及如何驗證網域的資訊，請閱讀 [將您的自訂網域名稱新增至 Azure Active Directory](../active-directory-add-domain.md#add-a-custom-domain-name-to-your-directory)
...
| State | 說明 | 需要採取動作 |
|:--- |:--- |:--- |
| 已驗證 |Azure AD Connect 在 Azure AD 中找到一個已驗證的相符網域。 此網域的所有使用者均可使用其內部部署認證進行登入 |不需採取動作 |
| 未驗證 |Azure AD Connect 可能在 Azure AD 中找到相符但未驗證的自訂網域。 如果未驗證網域，此網域使用者的 UPN 尾碼不會在同步處理後變更為預設的 .onmicrosoft.com 尾碼。 |驗證 Azure AD 中的自訂網域。 [深入了解](../active-directory-add-domain.md#verify-the-domain-name-with-azure-ad) |
| 未新增 |Azure AD Connect 找不到對應至 UPN 尾碼的自訂網域。 如果未在 Azure 中新增並驗證網域，此網域使用者的 UPN 尾碼將會變更為預設的 .onmicrosoft.com。 |新增並驗證對應至 UPN 尾碼的自訂網域[深入了解](../active-directory-add-domain.md) |
...
> [!NOTE]
> 使用替代 ID 會與所有 Office 365 工作負載和傳遞驗證不相容。 如需詳細資訊，請參閱 [設定替代的登入 ID](https://technet.microsoft.com/library/dn659436.aspx)。
> 
> 
...
您可以在使用精靈完成 Azure AD Connect 的初始設定之後，使用 Azure AD Connect 中的可用工作，將使用者的登入方法從同盟變更為密碼同步處理或傳遞驗證。 請再次執行 Azure AD Connect 精靈，然後您將會看到可以執行的工作清單。 在工作清單中選取 [變更使用者登入]  。
