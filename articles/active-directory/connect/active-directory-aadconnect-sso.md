...
單一登入是可在 Azure Active Directory Connect 中使用[密碼雜湊同步處理](active-directory-aadconnectsync-implement-password-synchronization.md)或[傳遞驗證](active-directory-aadconnect-pass-through-authentication.md)啟用的一個選項。  啟用之後，當使用者在他們的公司電腦上且已經連線至公司網路時，只需要其使用者名稱，不需要輸入密碼就能登入 Azure Active Directory (Azure AD) 或其他雲端服務。
...
如果少了上述任何一個項目，例如電腦已經離開公司網路且 Active Directory 無法使用，將會提示使用者輸入他們的密碼，就像在沒有使用單一登入時一樣。
...
首先，使用者嘗試存取資源，而此資源信任由 Azure AD (例如 SharePoint Online) 簽發的權杖。  接著 SharePoint Online 會將使用者重新導向去向 Azure AD 驗證。 然後，使用者提供他們的使用者名稱，好讓 Azure AD 可以確認他們的組織是否已啟用單一登入。 假設組織已啟用單一登入，會發生下列情況。
...
>[!NOTE]
>Azure AD Connect 需要能夠與 9090 (TCP) 連接埠上的 *.msappproxy.net 通訊，才能設定 SSO。 只有在設定期間有此需要，在使用者驗證期間則無此需要。
