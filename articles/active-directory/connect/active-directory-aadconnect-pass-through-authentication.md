...
使用相同的認證 (使用者名稱與密碼) 存取您的企業資源和雲端服務，可確保使用者不需記住不同的認證。 這能減少他們忘記登入方式的機會，並能夠降低支援人員協助密碼重設事件的次數。

雖然許多組織都願意使用 Azure AD [密碼同步處理](active-directory-aadconnectsync-implement-password-synchronization.md)向使用者提供單一認證以存取內部部署和雲端服務，但也有許多組織不希望讓密碼 (即使是雜湊形式) 傳輸到內部組織的界限之外。

Azure AD 傳遞驗證能為這些客戶提供簡易的解決方案。 它能確保 Azure AD 服務的密碼驗證是針對其內部部署 Active Directory 執行。 密碼可以在無需複雜網路基礎結構，或是內部部署密碼以任何形式從在於雲端上的情況下進行驗證。

當與[單一登入](active-directory-aadconnect-sso.md)選項結合時，使用者不需要輸入密碼便能登入 Azure AD 或其他雲端服務。 此功能可為這類客戶在他們的公司電腦上提供真正的整合體驗。

![傳遞驗證](./media/active-directory-aadconnect-pass-through-authentication/pta1.png)

傳遞驗證可以透過 Azure AD Connect 設定，並且使用會監聽密碼驗證要求的簡單內部部署代理程式。 代理程式可以輕鬆地部署到多部電腦，以提供高可用性和負載平衡。 因為所有通訊都是輸出通訊，所以不需要 DMZ 或在 DMZ 中安裝連接器。 連接器的電腦需求如下：
...
>[!NOTE]
>當已經在 Azure AD Connect 中選取傳遞驗證做為登入選項時，預覽期間會預設啟用密碼同步處理。 您可以在 Azure AD Connect 的 [選項] 頁面上停用此設定。
...
然後內部部署網域控制站會評估要求並向連接器傳回回應，接著會將此回應傳回給 Azure AD。 然後 Azure AD 會評估回應並適當地回應使用者，例如，簽發權杖或要求多重要素驗證。 此圖顯示各種步驟：
...
- 執行 Windows Server 2012 R2 或更新版本，以在其上執行 Azure AD Connect 的伺服器。 這部電腦必須是和受驗證之使用者所在相同樹系的成員。
- 如果您有超過一個樹系包含要使用 Azure AD 進行驗證的使用者，那些樹系之間必須彼此信任。
...
Azure AD 傳遞驗證是透過 Azure AD Connect 啟用。 啟用傳遞驗證會在和 Azure AD Connect 相同的伺服器上部署第一個連接器。 在安裝 Azure AD Connect 時，請選取一個自訂安裝並在登入選項頁面上選取傳遞驗證。 如需詳細資訊，請參閱[自訂 Azure AD Connect 安裝](active-directory-aadconnect-get-started-custom.md)。

![單一登入](./media/active-directory-aadconnect-sso/sso3.png)

依照預設，第一個傳遞驗證連接器會安裝在 Azure AD Connect 伺服器上。 您應該在另一部電腦上部署第二個連接器，以確保您擁有驗證要求的高可用性和負載平衡。
...
針對與連接器有關的錯誤，您可以查看位於 **應用程式及服務記錄檔\Microsoft\AadApplicationProxy\Connector\Admin** 底下的連接器電腦「事件記錄檔」。 如有需要，只要檢視分析和偵錯記錄檔，並啟用連接器工作階段記錄檔，即可取得更詳細的記錄檔。 不建議在預設啟用此記錄檔的情況下執行，而且只有在停用此記錄檔之後，才能看見內容。
