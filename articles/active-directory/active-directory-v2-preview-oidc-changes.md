...
> [!IMPORTANT]
> **您的作業：請確定您的應用程式不依賴 `profile_info` 值是否存在。**
> 
> 
...
> [!IMPORTANT]
> **您的作業：請確定您的應用程式不依賴 `id_token_expires_in` 值是否存在。**
> 
> 
...
> [!IMPORTANT]
> **您的作業：如果您的應用程式需要使用者的相關資訊，新增 `profile` 和 `email` 範圍。** 請注意，ADAL 預設會在要求中同時包含這些權限。
> 
> 
...
4. **如果您的應用程式需要基本使用者資訊，將 `profile` 和 `email` 範圍新增至您的應用程式。**
