...
- 您可以在 Azure AD 中控制可存取 Dropbox for Business 的人員
...
4. 在搜尋方塊中，輸入 **Dropbox for Business**。
...
建立此連結關聯性的方法是將 Azure AD 中**使用者名稱**的值，指定為 Dropbox for Business 中 **Username** 的值。

若要設定及測試與 Dropbox for Business 搭配運作的 Azure AD 單一登入，您需要完成下列構成要素：

1. **[設定 Azure AD 單一登入](#configuring-azure-ad-single-sign-on)** - 讓您的使用者能夠使用此功能。
2. **[建立 Azure AD 測試使用者](#creating-an-azure-ad-test-user)** - 使用 Britta Simon 測試 Azure AD 單一登入。
3. **[建立 Dropbox for Business 測試使用者](#creating-a-dropbox-for-business-test-user)** - 在 Dropbox for Business 中建立 Britta Simon 的對應項目，且該項目與 Azure AD 中代表使用者的項目連結。
...
3. 在 [Dropbox for Business 網域與 URL] 區段上，執行下列步驟：
    ...
    f. 在 Azure 入口網站的 [登入 URL] 文字方塊中，貼上 URL。

    ![設定單一登入](./media/active-directory-saas-dropboxforbusiness-tutorial/tutorial_dropboxforbusiness_url.png)

     在 [登入 URL] 文字方塊中，使用下列模式輸入 URL︰`https://www.dropbox.com/sso/<id>`

    > [!NOTE] 
    > 這不是真實的值。 使用您從他們的單一登入區段取得之實際登入 URL 將值更新。 請連絡 [Dropbox for Business 用戶端支援小組](https://www.dropbox.com/business/contact)以取得此值。 
...
7. 若要在 [Dropbox for Business] 端上設定單一登入，於 [驗證] 頁面的 [單一登入] 區段中，執行下列步驟： 
   
    ![設定單一登入](./media/active-directory-saas-dropboxforbusiness-tutorial/IC769516.png "設定單一登入")
   
    a. 按一下 [必要]。
   
    b.這是另一個 C# 主控台應用程式。 在 Azure 入口網站的 [設定登入] 視窗上，複製 [SAML 單一登入服務 URL] 值，然後將它貼至 [登入 URL] 文字方塊中。
