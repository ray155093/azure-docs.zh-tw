...
完成本教學課程之後，您指派給 Dropbox for Business 的 Azure AD 使用者就能夠單一登入 Dropbox for Business 公司網站 (服務提供者起始登入) 的應用程式，或是使用 [存取面板簡介](active-directory-saas-access-panel-introduction.md)。
...
### <a name="to-enable-the-application-integration-for-dropbox-for-business-perform-the-following-steps"></a>若要啟用 Dropbox for Business 的應用程式整合，請執行下列步驟：
...
5. 在 Dropbox for Business 租用戶上，於 [驗證] 頁面的 [單一登入] 區段中，執行下列步驟： 
    ...
    b. 在 Azure 傳統入口網站 [設定在 Dropbox for Business 單一登入] 對話方塊頁面上，複製 [登入頁面 URL] 值，然後將它貼至 [登入 URL] 文字方塊。
    ...
    d. 按一下 [選擇憑證]，然後瀏覽至您的 **Base-64 編碼的憑證檔案**。

    e. 按一下 [儲存變更] 以完成 DropBox for Business 租用戶設定。
...
### <a name="to-assign-users-to-dropbox-for-business-perform-the-following-steps"></a>若要指派使用者給 Dropbox for Business，請執行下列步驟：
