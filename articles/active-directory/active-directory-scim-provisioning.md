---
title: "使用 SCIM 來啟用將使用者和群組從 Azure Active Directory 自動佈建到應用程式的功能 | Microsoft Docs"
...
![][1]
*圖：透過 Web 服務從 Azure Active Directory 佈建到身分識別存放區*
...
1. 在網頁瀏覽器中啟動 Azure 管理入口網站，網址為 https://manage.windowsazure.com。
...
2. 在 [佈建端點 URL]  欄位中，輸入應用程式 SCIM 端點的 URL。
3. 如果 SCIM 端點需要來自非 Azure AD 簽發者的 OAuth 持有人權杖，那麼便將所需的 OAuth 持有人權杖複製到 [驗證權杖 (選擇性)]  欄位。 如果此欄位保留空白，則 Azure AD 將在每個要求包含從 Azure AD 簽發的 OAuth 持有人權杖。 使用 Azure AD 作為身分識別提供者的應用程式，可以驗證此 Azure AD 簽發的權杖。
4. 按 [下一步]，然後按一下 [開始測試] 按鈕，讓 Azure Active Directory 嘗試連接到 SCIM 端點。 如果嘗試失敗，就會顯示診斷資訊。  
5. 如果嘗試連接到應用程式成功，則請在其餘畫面上按 [下一步]，然後按一下 [完成] 以結束對話方塊。
6. 在產生的畫面中，選取第三個 [指派帳戶]  按鈕。 在產生的使用者和群組區段中，指派您想要佈建到應用程式的使用者或群組。
7. 指派使用者和群組之後，按一下畫面頂端附近的 [設定]  索引標籤。
8. 在 [帳戶佈建] 下，確認狀態設為開啟。 
9. 在 [工具] 底下，按一下 [重新啟動帳戶佈建] 以開始進行佈建程序。
...
**2：**將下列程式碼新增到該方法中，以將對任何服務端點發出的任何要求，都驗證為持有 Azure Active Directory 代表指定租用戶簽發的權杖，以存取 Azure Active Directory 的圖形 Web 服務： 
...
**1：**Azure Active Directory 會查詢服務是否有 externalId 屬性值與 Azure Active Directory 中使用者的 mailNickname 屬性值相符的使用者。  查詢會以類似於此的超文字傳輸通訊協定要求表示，其中，jyoung 是 Azure Active Directory 中使用者的 mailNickname 範例： 
...
**2：**如果向服務查詢是否有 externalId 屬性值與 Azure Active Directory 中使用者的 mailNickname 值相符的使用者時，回應未傳回任何使用者，Azure Active Directory 就會要求服務佈建與 Azure Active Directory 中的使用者對應的使用者。  以下是這類要求的範例： 
...
**4：**如果要更新某個參考屬性，Azure Active Directory 就會查詢服務，以判斷以該服務作為前端之身分識別存放區中參考屬性目前的值，是否已經與 Azure Active Directory 中該屬性的值相符。  如果是使用者，以這種方式可查詢目前值的唯一屬性會是管理員屬性。  要判斷特定使用者物件的管理員屬性目前是否具有某個值之要求的範例如下： 
...
**6：**若要將使用者從前端為 SCIM 服務的身分識別存放區中解除佈建，Azure Active Directory 會傳送像以下的要求： 
