<properties
	pageTitle="教學課程：Azure Active Directory 與 Pluralsight 整合 | Microsoft Azure"
...
# 教學課程：Azure Active Directory 與 Pluralsight 整合
...
7. 在結果窗格中，選取 [Pluralsight]，然後按一下 [完成] 以加入應用程式。
...
4. **[建立 Pluralsight 測試使用者](#creating-a-pluralsight-test-user)** - 在 Pluralsight 中建立一個與 Azure AD 中代表 Britta Simon 的項目連結的 Britta Simon 對應項目。
...
您也可以利用適當的值新增 **"Unique ID"** 屬性，例如 EmployeeID 和其他適合貴組織的值。另請注意，這不是必要的屬性，但您可以加入它以識別唯一的使用者。
...
1. 若要移除多餘的 [SAML Token 屬性]，請執行下列步驟：
...
1. 在 Azure 傳統入口網站的 [Pluralsight] 應用程式整合頁面上，按一下 [設定單一登入] 來開啟 [設定單一登入] 對話方塊。

    ![設定單一登入][6]

2. 在 [要如何讓使用者登入 Pluralsight] 頁面上，選取 [Azure AD 單一登入]，然後按一下 [下一步]。
...
4. 在 [設定在 Pluralsight 單一登入] 頁面上，執行下列步驟：![設定單一登入](./media/active-directory-saas-pluralsight-tutorial/tutorial_pluralsight_05.png)
...
5. 若要為您的應用程式設定 SSO，請連絡 Pluralsight [專業服務](mailTo:professionalservices@pluralsight.com)小組，並提供下載的中繼資料檔案。
