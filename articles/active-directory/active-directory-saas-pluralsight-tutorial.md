...
3. **[建立 Pluralsight 測試使用者](#creating-a-pluralsight-test-user)** - 使 Pluralsight 中對應的 Britta Simon 連結到該使用者在 Azure AD 中的代表項目。
...
3. 在 [Pluralsight 網域與 URL] 區段中，執行下列步驟：

    ![設定單一登入](./media/active-directory-saas-pluralsight-tutorial/tutorial_pluralsight_url.png)

    在 [登入 URL] 文字方塊中，使用下列模式輸入 URL︰`https://<instance name>.pluralsight.com/sso/<company name>`

    > [!NOTE] 
    > 這不是真實的值。 請使用實際的「登入 URL」來更新此值。 請連絡 [Pluralsight 客戶支援小組](mailto:support@pluralsight.com)以取得此值。 
...
當您在存取面板中按一下 Pluralsight 圖格時，應該會自動登入您的 Pluralsight 應用程式。 如需「存取面板」的詳細資訊，請參閱[存取面板簡介](active-directory-saas-access-panel-introduction.md)。
