<properties
	pageTitle="在組織中啟用 Microsoft Windows Hello 企業版 | Microsoft Azure"
...
# 在組織中啟用 Microsoft Windows Hello 企業版

連接已加入網域的 Windows 10 裝置與 Azure Active Directory (Azure AD) 後，請依下列方式在組織中啟用 Microsoft Windows Hello 企業版。

## 部署 System Center Configuration Manager 最新分支 
...
- **發佈金鑰基礎結構 (PKI)**：若要利用使用者憑證啟用 Microsoft Windows Hello 企業版，您必須備妥 PKI。如果您沒有帳戶，或您不想要用於使用者憑證，您可以 ︰
...
## 透過 Active Directory 中的群組原則設定 Microsoft Windows Hello 企業版

 您可以在 Windows Server Active Directory 中使用群組原則來設定已加入網域的 Windows 10 裝置，以在使用者登入 Windows 時佈建 Windows Hello 企業版使用者認證：
...
## 使用 System Center Configuration Manager設定 Windows Hello 企業版

您只可以使用 System Center Configuration Manager 來部署適用於 Windows 10 版本 1607 的 Windows Hello 企業版原則。如果您需要查閱 Windows 10 電腦的版本，請在 Windows 10 電腦上啟動 winver。
...
2. 在 [支援的平台] 對話方塊中，選取將使用此 Windows Hello 企業版設定檔佈建的平台，然後按 [下一步]。
...
## 設定憑證設定檔以在 Configuration Manager 中使用 Windows Hello 企業版註冊憑證
