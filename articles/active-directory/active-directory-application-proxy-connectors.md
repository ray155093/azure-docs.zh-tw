...
若要將連接器設為群組，請[安裝多個連接器](active-directory-application-proxy-enable.md)，然後命名並組合它們。 最後，您必須將它們指派給特定的應用程式。
...
2. 在 [應用程式 Proxy] 底下，按一下 [管理連接器群組]，然後提供一個群組名稱來建立連接器群組。  
