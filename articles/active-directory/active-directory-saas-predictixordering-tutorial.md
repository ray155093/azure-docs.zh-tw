<properties
	pageTitle="教學課程：Azure Active Directory 與 Predictix Ordering 整合 | Microsoft Azure"
...
# 教學課程：Azure Active Directory 與 Predictix Ordering 整合
...
- Azure AD 訂用帳戶
- 啟用 Predictix Ordering 單一登入功能的訂用帳戶
...
在本節中，您會在傳統入口網站中啟用 Azure AD 單一登入，並在您的 Predictix Ordering 應用程式中設定單一登入。
...
3. 在 [設定應用程式設定] 對話方塊頁面上，執行下列步驟：
    ...
    a.在 [登入 URL] 文字方塊中，使用以下模式輸入您使用者用以登入 Predictix Ordering 應用程式的 URL：**https://\<公司名稱定價>.ordering.predictix.com/sso/request**。
...
5. 在 [告訴我們這位使用者] 對話方塊頁面上，執行下列步驟：![建立 Azure AD 測試使用者](./media/active-directory-saas-predictix-ordering-tutorial/create_aaduser_05.png)
...
6.  在 [使用者設定檔] 對話方塊頁面上，執行下列步驟：![建立 Azure AD 測試使用者](./media/active-directory-saas-predictix-ordering-tutorial/create_aaduser_06.png)
