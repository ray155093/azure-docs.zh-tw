...
本教學課程旨在說明如何整合 Aravo 與 Azure Active Directory (Azure AD)。

Aravo 與 Azure AD 整合提供下列優點：

* 您可以在 Azure AD 中控制可存取 Aravo 的人員
* 您可以讓使用者使用其 Azure AD 帳戶自動登入 Aravo 單一登入 (SSO)
...
## <a name="add-aravo-from-the-gallery"></a>從資源庫新增 Aravo
若要設定 Aravo 與 Azure AD 整合，您需要從資源庫將 Aravo 加入受管理的 SaaS 應用程式清單中。
...
6. 在搜尋方塊中，輸入 **Aravo**。
    ...
    ![建立 Azure AD 測試使用者](./media/active-directory-saas-aravo-tutorial/tutorial_aravo_01.png)
7. 在結果窗格中，選取 [Aravo]，然後按一下 [完成] 以新增應用程式。
   
    ![選取資源庫中的應用程式](./media/active-directory-saas-aravo-tutorial/tutorial_aravo_0001.png)

## <a name="configure-and-test-microsoft-azure-ad-sso"></a>設定並測試 Microsoft Azure AD SSO
本節的目標是要說明如何以名為 "Britta Simon" 的測試使用者為基礎，設定及測試與 Aravo 搭配運作的 Microsoft Azure AD SSO。

若要讓 SSO 運作，Azure AD 必須知道 Aravo 與 Azure AD 中互相對應的使用者。 換句話說，必須建立 Azure AD 使用者和 Aravo 中相關使用者之間的連結關聯性。

建立此連結關聯性的方法，就是將 Azure AD 中**使用者名稱**的值指派為 Aravo 中 **Username** 的值。

若要設定及測試對 Aravo 的 Microsoft Azure AD SSO，您需要完成下列建置組塊：
...
3. **[建立 Aravo 測試使用者](#creating-a-aravo-test-user)** - 使 Aravo 中對應的 Britta Simon 連結到她在 Azure AD 中的代表項目。
...
在本節中，您會在傳統入口網站中啟用 Microsoft Azure AD SSO，並在您的 Aravo 應用程式中設定單一登入。

**若要使用 Aravo 設定 Microsoft Azure AD SSO，請執行下列步驟：**

1. 在傳統入口網站的 [Aravo] 應用程式整合頁面上，按一下 [設定單一登入] 來開啟 [設定單一登入] 對話方塊。
   
    ![設定單一登入][6] 
2. 在 [要如何讓使用者登入 Aravo] 頁面上，選取 [Microsoft Azure AD 單一登入]，然後按 [下一步]。
   
    ![設定單一登入](./media/active-directory-saas-aravo-tutorial/tutorial_aravo_03.png) 
3. 在 [設定應用程式設定] 對話方塊頁面上，執行下列步驟，然後按 [下一步]：
   
    ![設定單一登入](./media/active-directory-saas-aravo-tutorial/tutorial_aravo_04.png)
  1. 在 [識別碼] 文字方塊中，使用下列模式輸入 URL：`https://<company name>.aravo.com`
  2. 在 [回覆 URL] 文字方塊中，以下列模式輸入 URL：`https://<company name>.aravo.com/aems/login.do`
  3. 按 [下一步] 。
   
    >[!NOTE]
    >請注意這些不是真正的值。 您必須使用實際的識別碼和回覆 URL 更新這些值。 若要取得這些值，請連絡 Aravo。 
    > 
4. 在 [設定在 Aravo 單一登入] 頁面上，執行下列步驟，然後按 [下一步]：
   
    ![設定單一登入](./media/active-directory-saas-aravo-tutorial/tutorial_aravo_05.png)
...
1. 在 **Azure 傳統入口網站**中，按一下左方瀏覽窗格的 [Active Directory]。
    ...
    ![建立 Azure AD 測試使用者](./media/active-directory-saas-aravo-tutorial/create_aaduser_09.png)
2. 從 [目錄]  清單中，選取要啟用目錄整合的目錄。
...
5. 在 [告訴我們這位使用者]  對話方塊頁面上，執行下列步驟：
    ...
    ![建立 Azure AD 測試使用者](./media/active-directory-saas-aravo-tutorial/create_aaduser_05.png)
  1. 針對 [使用者類型]，選取 [您組織中的新使用者]。
  2. 在 [使用者名稱] 文字方塊中，輸入 **BrittaSimon**。
  3. 按 [下一步] 。
6. 在 [使用者設定檔]  對話方塊頁面上，執行下列步驟：
   
   ![建立 Azure AD 測試使用者](./media/active-directory-saas-aravo-tutorial/create_aaduser_06.png)
...
7. 在 [取得暫時密碼] 對話方塊頁面上，按一下 [建立]。
    ...
    ![建立 Azure AD 測試使用者](./media/active-directory-saas-aravo-tutorial/create_aaduser_07.png)
8. 在 [取得暫時密碼]  對話方塊頁面上，執行下列步驟：
   
    ![建立 Azure AD 測試使用者](./media/active-directory-saas-aravo-tutorial/create_aaduser_08.png)
  1. 記下 [新密碼] 的值。 
  2. 按一下頁面底部的 [新增] 。   

### <a name="create-a-aravo-test-user"></a>建立 Aravo 測試使用者
本節目標是在 Aravo 中建立名為 Britta Simon 的使用者。請與 Aravo 支援小組合作，在 Aravo 帳戶中新增使用者。

### <a name="assign-the-azure-ad-test-user"></a>指派 Azure AD 測試使用者
本節的目標是授與 Britta Simon 對 Aravo 的存取權，使她能夠使用 Azure SSO。

![指派使用者][200]

**若要將 Britta Simon 指派到 Aravo，請執行下列步驟：**
...
2. 在應用程式清單中，選取 [Aravo] 。
    ...
    ![設定單一登入](./media/active-directory-saas-aravo-tutorial/tutorial_aravo_50.png)
3. 在頂端的功能表中，按一下 [使用者] 。
...
當您在存取面板中按一下 Aravo 圖格時，應該會自動登入您的 Aravo 應用程式。
