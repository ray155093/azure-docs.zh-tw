...
* 有效的 Azure 訂閱
* 啟用 TimeOffManager 單一登入 (SSO) 的訂用帳戶

完成本教學課程之後，您指派給 TimeOffManager 的 Azure AD 使用者就能夠在您的 TimeOffManager 公司網站 (服務提供者起始登入) 使用 SSO 來登入應用程式，或使用[存取面板簡介](active-directory-saas-access-panel-introduction.md)來登入。
...
## <a name="enable-the-application-integration-for-timeoffmanager"></a>啟用 TimeOffManager 的應用程式整合
...
5. 在 [欲執行動作] 對話方塊上，按一下 [從資源庫中新增應用程式]。
   ...
   ![從資源庫新增應用程式](./media/active-directory-saas-timeoffmanager-tutorial/IC749322.png "從資源庫新增應用程式")
6. 在 [搜尋方塊] 中，輸入 **TimeOffManager**。
   
   ![應用程式資源庫](./media/active-directory-saas-timeoffmanager-tutorial/IC795910.png "應用程式資源庫")
...
在此程序中，您需要上傳 base-64 編碼憑證到您的 TimeOffManager 租用戶。 如果您不熟悉此程序，請參閱 [如何將二進位憑證轉換成文字檔](http://youtu.be/PlgrzUZ-Y1o)。
...
7. 在 [單一登入設定]  區段中，執行下列步驟：
  ...
  3.  在 Azure 傳統入口網站中的 [在 TimeOffManager 設定單一登入] 對話頁面上，複製 [簽發者 URL] 值，然後將它貼至 [IdP 簽發者] 文字方塊中。
  4.  在 Azure 傳統入口網站的 [在 TimeOffManager 設定單一登入] 對話頁面上，複製 [遠端登入 URL] 值，然後將它貼至 [IdP 端點 URL] 文字方塊中。
  5.  在 [強制 SAML] 選取 [否]。
  6.  在 [自動建立使用者] 選取 [是]。
  7.  在 Azure 傳統入口網站中的 [在 TimeOffManager 設定單一登入] 對話頁面上，複製 [遠端登出 URL] 值，然後將它貼至 [登出 URL] 文字方塊中，並按一下 [儲存變更]。

1. 在 Azure 傳統入口網站的 [在 TimeOffManager 設定單一登入] 對話方塊上，選取單一登入組態確認，然後按一下 [完成]。
   
   ![設定單一登入](./media/active-directory-saas-timeoffmanager-tutorial/IC795919.png "設定單一登入")
2. 在頂端的功能表中，按一下 [屬性] **屬性** to open the **SAML Token 屬性** 對話方塊。
   
   ![屬性](./media/active-directory-saas-timeoffmanager-tutorial/IC795920.png "屬性")
3. 若要加入必要的屬性對應，請執行下列步驟：
   
   ![saml 權杖屬性](./media/active-directory-saas-timeoffmanager-tutorial/123.png "saml 權杖屬性")
...
>[!NOTE]
>您可以使用任何其他的 TimeOffManager 使用者帳戶建立工具或 TimeOffManager 提供的 API 來佈建 AAD 使用者帳戶。
> 
