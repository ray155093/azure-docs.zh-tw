...
- 您可以讓使用者使用他們的 Azure AD 帳戶自動登入 Voyance 單一登入 (SSO)
...
## <a name="add-voyance-from-the-gallery"></a>從資源庫新增 Voyance
...
在本節中，您會以名為 "Britta Simon" 的測試使用者身分，設定及測試與 Voyance 搭配運作的 Azure AD SSO。

若要讓 SSO 運作，Azure AD 必須知道 Voyance 與 Azure AD 中互相對應的使用者。 換句話說，必須建立 Azure AD 使用者和 Voyance 中相關使用者之間的連結關聯性。
...
3. 在 [設定應用程式設定] 對話方塊頁面上，如果您想要以 **IDP 起始模式**設定應用程式，請執行下列步驟，然後按 [下一步]：
  ...
  1. 在 [識別碼] 文字方塊中，以下列模式輸入 URL：`https://<company name>.nyansa.com`。
  2. 在 [回覆 URL] 文字方塊中，以下列模式輸入 URL：`https://<company name>.nyansa.com/saml/create/`。
  3. 按 [下一步] 。
...
4. 如果您想要在 [設定應用程式設定] 對話方塊頁面上以 **SP 起始模式**設定應用程式，則請按一下 [顯示進階設定 (選擇性)]，然後輸入**登入 URL** 並按 [下一步]。
  ...
  1. 在 [登入 URL] 文字方塊中，以下列模式輸入 URL：`https://<company name>.nyansa.com/`。
  2. 按 [下一步] 。

      >[!NOTE]
      >您必須使用實際的「單一登入 URL」、「識別碼」及「回覆 URL」來更新這些值。 若要取得這些值，請連絡 [Voyance 支援小組](emaiLto:support@nyansa.com)。
      >
...
11. 移至 [SAML v2] 區段並執行下列步驟︰
 ...
 1. 選取 [啟用] 。
 2. 在 [IdP 登入 URL] 文字方塊中，輸入取自於 Azure AD 應用程式組態精靈的 [SAML SSO URL] 值。
 3. 在記事本中開啟您的 Base64 編碼的憑證，將其內容複製到剪貼簿，然後貼到 [IdP 憑證] 文字方塊。
 4. 按一下 [儲存] 。
