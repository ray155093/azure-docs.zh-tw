...
完成本教學課程之後，您指派給 Mozy Enterprise 的 Azure AD 使用者就能夠以單一登入 (SSO) 方式登入您 Mozy Enterprise 公司網站 (服務提供者起始登入) 的應用程式，或是使用 [存取面板簡介](active-directory-saas-access-panel-introduction.md)。
...
## <a name="enabling-the-application-integration-for-mozy-enterprise"></a>啟用 Mozy Enterprise 的應用程式整合
...
5. 在 [欲執行動作] 對話方塊上，按一下 [從資源庫中新增應用程式]。
   ...
   ![從資源庫新增應用程式](./media/active-directory-saas-mozy-enterprise-tutorial/IC749322.png "從資源庫新增應用程式")
6. 在**搜尋方塊**中輸入 **mozy enterprise**。
   
   ![應用程式資源庫](./media/active-directory-saas-mozy-enterprise-tutorial/IC777309.png "應用程式資源庫")
...
在此程序中，您需要上傳 base-64 編碼憑證到您的 Mozy Enterprise 租用戶。 如果您不熟悉這個程序，請參閱 [如何將二進位憑證轉換成文字檔](http://youtu.be/PlgrzUZ-Y1o)

**若要設定單一登入，請執行下列步驟：**

1. 在 Azure 傳統入口網站的 [Mozy Enterprise] 應用程式整合頁面上，按一下 [設定單一登入] 來開啟 [設定單一登入] 對話方塊。
...
7. 在 [驗證原則]  區段中，執行下列步驟：
   ...
   2. 選取 [使用 LDAP 推送] 。
   3. 按一下 [SAML 驗證]  索引標籤。
   4. 在 Azure 傳統入口網站上 [設定在 Mozy Enterprise 單一登入] 對話方塊頁面，複製 [驗證要求 URL] 值，然後將它貼至 [驗證 URL] 文字方塊中。
   5. 在 Azure 傳統入口網站上 [設定在 Mozy Enterprise 單一登入] 對話方塊頁面，複製 [識別提供者 ID] 值，然後將它貼至 [SAML 端點] 文字方塊中。
   ...
   9. 按一下 [儲存變更] 。
8. 在 Azure 傳統入口網站的 [設定在 Mozy Enterprise 單一登入] 對話方塊上，選取單一登入設定確認，然後按一下 [完成]。
