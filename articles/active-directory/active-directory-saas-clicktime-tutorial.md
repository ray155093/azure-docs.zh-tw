...
完成本教學課程之後，您指派給 ClickTime 的 Azure AD 使用者就能夠單一登入您 ClickTime 公司網站 (服務提供者起始登入) 的應用程式，或是使用[存取面板簡介](active-directory-saas-access-panel-introduction.md)。
...
6.  在 [單一登入喜好設定] 組態區段中，執行下列步驟：
    ...
    2.  在 Azure 傳統入口網站的 [在 ClickTime 設定單一登入] 對話頁面上，複製 [單一登入服務 URL] 值，然後將它貼至 [識別提供者端點] 文字方塊中。
