...
* 您可以讓使用者使用他們的 Azure AD 帳戶自動登入 IBM Kenexa Survey Enterprise 單一登入 (SSO)
...
* Azure AD 訂用帳戶
* 已啟用 IBM Kenexa Survey Enterprise SSO 的訂用帳戶
...
在本節中，您會以名為 "Britta Simon" 的測試使用者身分，使用 IBM Kenexa Survey Enterprise 設定及測試 Azure AD SSO。

若要讓 SSO 運作，Azure AD 必須知道 IBM Kenexa Survey Enterprise 與 Azure AD 中互相對應的使用者。 換句話說，必須建立 Azure AD 使用者和 IBM Kenexa Survey Enterprise 中相關使用者之間的連結關聯性。

建立此連結關聯性的方法，就是將 Azure AD 中**使用者名稱**的值指派為 IBM Kenexa Survey Enterprise 中 **Username** 的值。

若要設定及測試與 IBM Kenexa Survey Enterprise 搭配運作的 Azure AD SSO，您需要完成下列構成要素：
...
3. 在 [設定 App 設定]  對話方塊頁面執行下列步驟：
  ...
  1. 在 [識別碼] 文字方塊中，使用下列模式輸入 URL：`https://surveys.kenexa.com/<company code>` 
  2. 在 [回覆 URL] 文字方塊中，以下列模式輸入 URL：`https://surveys.kenexa.com/<company code>/tools/sso.asp`
  3. 按 [下一步] 。
   
   >[!NOTE]
   >請注意這些不是真正的值。 您必須使用實際的識別碼和回覆 URL 更新這些值。 請連絡 IBM Kenexa Survey Enterprise 支援小組，以取得這些值。 
   > 
4. 在 [Configure single sign-on at IBM Kenexa Survey Enterprise] 頁面上，按一下 [下載憑證]，然後將該檔案儲存在您的電腦上：
 ...
 * **單一登出服務 URL**
  ...
  >[!NOTE]
  >請注意，回應中的 NameID 宣告值必須符合 Kenexa 系統中設定的 SSO 識別碼。 因此請與 Kenexa 支援小組合作，以將您組織中適當的使用者識別碼對應為 SSO 識別碼。 Azure AD 預設會將 NameIdentifier 設為 UPN 值。 您可以從 [屬性] 索引標籤中變更此值，如以下螢幕擷取畫面所示。 在完成正確對應後，整合才有作用。 
  > 
...
7. 在 [單一登入確認] 頁面上，按一下 [完成]。  
    ...
    ![Azure AD 單一登入][11]
8. 在 Azure 傳統入口網站中的 [IBM Kenexa Survey Enterprise] 應用程式整合頁面上，按一下頂端功能表中的 [屬性]。
   
    ![設定單一登入](./media/active-directory-saas-kenexasurvey-tutorial/tutorial_kenexasurvey_06.png)
9. 在 [SAML Token 屬性]  對話方塊中，執行下列步驟：
 1. 選取 [NameIdentifier] 的屬性，然後按一下 [編輯] 圖示。
 
    ![設定單一登入](./media/active-directory-saas-kenexasurvey-tutorial/tutorial_kenexasurvey_07.png)  
 2. 從 [屬性值] 清單中，輸入在 Kenexa 系統中設定的 SSO ID 屬性值。  
 3. 按一下頁面底部的 [新增] 。
...
您可以與 IBM Kenexa 支援小組合作，以對應所有使用者的 SSO 識別碼。 此 SSO 識別碼值也應該從 Azure AD 對應至 NameIdentifier 值。 您可以在 [屬性] 索引標籤中變更此預設設定。
