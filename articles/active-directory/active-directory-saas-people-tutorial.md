<properties
	pageTitle="教學課程：Azure Active Directory 與 People 整合 | Microsoft Azure"
...
# 教學課程：Azure Active Directory 與 People 整合
...
3. 在 [設定應用程式設定] 對話方塊頁面上，執行下列步驟，然後按 [下一步]：
    ...
    b.如果您不知道您的租用戶 URL，請透過 [customerservices@peoplehr.com](mailto:customerservices@peoplehr.com) 連絡 People 支援小組來取得。
...
本節的目標是要使用「存取面板」來測試您的 Azure AD 單一登入組態。當您在「存取面板」中按一下 [People] 圖格時，應該會自動登入您的 People 應用程式。
