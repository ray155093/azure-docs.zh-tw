...
## 開始存取權檢閱
> [!NOTE]
> 如果您尚未在 Azure 入口網站的儀表板中加入 PIM 應用程式，請參閱[開始使用 Azure AD Privileged Identity Management](active-directory-privileged-identity-management-getting-started.md) 中的步驟。
> 
> 
...
按一下 [檢閱] 按鈕時，隨即會顯示 [開始存取權檢閱] 刀鋒視窗。您將在此刀鋒視窗上為檢閱設定名稱和時間限制、選擇要檢閱的角色，以及決定將由誰執行檢閱。
