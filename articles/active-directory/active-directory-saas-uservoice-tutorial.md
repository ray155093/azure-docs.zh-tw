...
### <a name="to-enable-the-application-integration-for-uservoice-perform-the-following-steps"></a>若要啟用 UserVoice 的應用程式整合，請執行下列步驟：
...
8. 在 [編輯使用者驗證]  對話頁面上執行下列步驟：
    ...
    b.這是另一個 C# 主控台應用程式。 在 Azure 傳統入口網站的 [設定在 UserVoice 單一登入] 對話方塊頁面上，複製 [遠端登入 URL] 值，然後將它貼到 [SSO 遠端登入] 文字方塊中。

    c. 在 Azure 傳統入口網站的 [設定在 UserVoice 單一登入] 對話方塊頁面上，複製 [遠端登出 URL] 值，然後將它貼到 [SSO 遠端登出] 文字方塊中。
 
    d. 從匯出的憑證複製**指紋**值，然後將它貼入 [目前憑證 SHA1 指紋] 文字方塊。  
...
### <a name="to-assign-users-to-uservoice-perform-the-following-steps"></a>若要指派使用者給 UserVoice，請執行下列步驟：
