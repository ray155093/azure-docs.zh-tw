...
>[AZURE.NOTE]使用 SAP HANA 雲端平台上的群組可讓您在 SAP HANA 雲端平台應用程式中以動態方式將一或多個使用者指派給一或多個角色，這是由 SAML 2.0 判斷提示中的屬性值所決定。 例如，如果判斷提示包含屬性「合約＝暫存」，您可能會希望將所有受影響的使用者新增至「暫存」群組。 「*暫存*」群組可能包含來自一或多個部署在 SAP HANA 雲端平台帳戶中之應用程式的一或多個角色。
>  
