...
* 您可以讓使用者使用他們的 Azure AD 帳戶自動登入 eTouches 單一登入 (SSO)
...
在本節中，您會以名為 "Britta Simon" 的測試使用者身分，使用 eTouches 設定及測試 Azure AD SSO。

若要讓 SSO 運作，Azure AD 必須知道 eTouches 與 Azure AD 中互相對應的使用者。 換句話說，必須在 Azure AD 使用者與 eTouches 中的相關使用者之間建立連結關聯性。

建立此連結關聯性的方法，就是將 Azure AD 中**使用者名稱**的值指派為 eTouches 中 **Username** 的值。

若要設定及測試與 eTouches 搭配運作的 Azure AD SSO，您需要完成下列構成要素：
...
**若要設定與 eTouches 搭配運作的 Azure AD SSO，請執行下列步驟：**
...
5. 在 [設定 App 設定]  對話方塊頁面執行下列步驟：
  ...
  1. 在 [登入 URL] 文字方塊中，使用以下模式輸入使用者登入您的 eTouches 應用程式時所使用的 URL：**https://www.eiseverywhere.com/saml/accounts/?sso&accountid=\<accountid\>**。
  2. 按一下 [下一步]。
...
7. 為了設定應用程式的 SSO，請在 eTouches 應用程式中執行下列步驟：  
  1. 使用系統管理員權限登入 **eTouches** 應用程式。 
  2. 移至 [SAML] 組態。
  3. 在 [一般設定] 區段將 Azure AD 同盟中繼資料內容貼入文字方塊中。
  4. 按一下 [儲存並留下] 按鈕。
  5. 按一下 [SAML 中繼資料] 區段中的 [更新中繼資料] 按鈕。 
  6. 這會開啟頁面並執行 SSO。 一旦 SSO 開始運作，您就可以設定使用者名稱。
  7. 在 [使用者名稱] 欄位中選取**電子郵件地址**，如下圖所示。 
  8. 複製 [SSO URL/ACS] 值，並將它放入 Azure AD 應用程式組態精靈的 [登入 URL] 文字方塊。
...
5. 在 [告訴我們這位使用者]  對話方塊頁面上，執行下列步驟：
 ...
 ![建立 Azure AD 測試使用者](./media/active-directory-saas-etouches-tutorial/create_aaduser_05.png) 
  1. 針對 [使用者類型]，選取 [您組織中的新使用者]。
  2. 在 [使用者名稱] 文字方塊中，輸入 **BrittaSimon**。
  3. 按 [下一步] 。
6. 在 [使用者設定檔]  對話方塊頁面上，執行下列步驟：

 ![建立 Azure AD 測試使用者](./media/active-directory-saas-etouches-tutorial/create_aaduser_06.png)  
