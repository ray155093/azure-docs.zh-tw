...
* 您可以讓使用者使用他們的 Azure AD 帳戶自動登入 Litmos 單一登入 (SSO)
...
## <a name="add-litmos-from-the-gallery"></a>從資源庫加入 Litmos
...
8. 在 Azure 傳統入口網站的 [設定應用程式設定]  對話方塊頁面中，執行下列步驟：
    ...
    1. 在 [識別碼] 文字方塊中，輸入您的使用者用來登入 Litmos 應用程式的 URL (例如：*https://azureapptest.litmos.com/account/Login*)。
...
10. 在您的 **Litmos** 應用程式中，執行下列步驟：
    ...
    >如需詳細資訊，請參閱 [如何將二進位憑證轉換成文字檔](http://youtu.be/PlgrzUZ-Y1o)。
     ...
     3. 在記事本中開啟您的 Base-64 編碼憑證、將其內容複製到剪貼簿，然後將它貼到 [SAML X.509 憑證] 文字方塊中。
...
14. 在 [加入使用者屬性]  對話方塊上，執行下列步驟︰ 
 ...
 1. 按一下 [加入使用者屬性] 。    
   ![設定單一登入][15]
 2. 在 [屬性名稱] 文字方塊中，輸入該資料列所顯示的**屬性名稱**。
 3. 選取對該資料列所顯示的**屬性值**。
 4. 按一下 [完成] 以關閉 [加入使用者屬性] 對話方塊。
