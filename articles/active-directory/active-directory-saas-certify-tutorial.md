...
若要讓單一登入能夠運作，Azure AD 必須知道 Certify 與 Azure AD 中互相對應的使用者。 換句話說，必須在 Azure AD 使用者與 Certify 中的相關使用者之間建立連結關聯性。  
...
1. 於 [設定在 Certify 單一登入]  頁面上，執行下列步驟：
    ...
    b. 按 [下一步] 。
2. 若要為您的應用程式設定 SSO，請透過 support@certify.com. 將下載的憑證檔案附加至您的郵件，並與 Certify 小組分享中繼資料 URL (實體識別碼、SSO 登入 URL 和登出 URL)，以在其端設定 SSO。
