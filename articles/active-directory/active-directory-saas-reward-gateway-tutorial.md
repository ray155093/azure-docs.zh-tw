...
* 您可以讓使用者使用他們的 Azure AD 帳戶自動登入 Reward Gateway 單一登入 (SSO)
...
* Azure AD 訂用帳戶
* 已啟用 Reward Gateway SSO 功能的訂用帳戶
...
若要讓 SSO 能夠運作，Azure AD 必須知道 Reward Gateway 與 Azure AD 中互相對應的使用者。 換句話說，必須要建立某位 Azure AD 使用者與 Reward Gateway 中相關使用者之間的連結關聯性。

建立此連結關聯性的方法是將 Azure AD 中**使用者名稱**的值指定為 Reward Gateway 中 **Username** 的值。

若要設定及測試與 Reward Gateway 搭配運作的 Azure AD SSO，您需要完成下列構成要素：
...
在本節中，您會在傳統入口網站中啟用 Azure AD SSO，並在您的 Reward Gateway 應用程式中設定 SSO。
...
3. 在 [設定 App 設定]  對話方塊頁面執行下列步驟：
  ...
  1. 在 [識別碼 URL] 文字方塊中，使用下列模式輸入使用者用來登入 Reward Gateway 應用程式的 URL： 
...
4. 在 [設定在 Reward Gateway 單一登入] 頁面上，執行下列步驟：
   
    ![設定單一登入](./media/active-directory-saas-reward-gateway-tutorial/tutorial_rewardgateway_05.png)
  * 按一下 [下載中繼資料]，然後將檔案儲存在您的電腦上。
5. 若要為您的應用程式設定 SSO，請連絡 Reward Gateway [支援小組](mailTo:clientsupport@rewardgateway.com)，並提供下列資訊：
...
5. 在 [告訴我們這位使用者]  對話方塊頁面上，執行下列步驟：
    ...
    ![建立 Azure AD 測試使用者](./media/active-directory-saas-reward-gateway-tutorial/create_aaduser_05.png)    
  1. 針對 [使用者類型]，選取 [您組織中的新使用者]。
  2. 在 [使用者名稱] 文字方塊中，輸入 **BrittaSimon**。
  3. 按 [下一步] 。
6. 在 [使用者設定檔]  對話方塊頁面上，執行下列步驟：

   ![建立 Azure AD 測試使用者](./media/active-directory-saas-reward-gateway-tutorial/create_aaduser_06.png)    
...
### <a name="create-an-reward-gateway-test-user"></a>建立 Reward Gateway 測試使用者
