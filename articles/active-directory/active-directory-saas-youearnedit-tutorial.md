<properties
	pageTitle="教學課程：Azure Active Directory 與 YouEarnedIt 整合 | Microsoft Azure"
...
# 教學課程：Azure Active Directory 與 YouEarnedIt 整合
...
7. 在結果窗格中，選取 [YouEarnedIt]，然後按一下 [完成] 以加入應用程式。
...
建立此連結關聯性的方法是將 Azure AD 中**使用者名稱**的值指定為 YouEarnedIt 中 **Username** 的值。
...
5. 在 [告訴我們這位使用者] 對話方塊頁面上，執行以下步驟：![建立 Azure AD 測試使用者](./media/active-directory-saas-youearnedit-tutorial/create_aaduser_05.png)
...
### 建立 YouEarnedIt 測試使用者

在本節中，您要在 YouEarnedIt 中建立名為 Britta Simon 的使用者。請與 YouEarnedIt 支援小組合作，在 YouEarnedIt 平台中加入使用者。

> [AZURE.NOTE]  YouEarnedIt 預期識別提供者會在 NameID 屬性中提供 EmailAddress 或 UserName。如果在資料庫中找不到對應的 UserName 或 EmailAddress 或者不完全相符，則驗證會失敗。這會要求在 SSO 整合之前，將帳戶匯入 YouEarnedIt 系統 (通常透過 API 或 CSV 匯入)。
