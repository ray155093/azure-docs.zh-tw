...
建立此連結關聯性的方法，就是指派 Azure AD 中 [使用者名稱] 的值做為 Beeline 中 [Username] \(使用者名稱) 的值。
...
3. **[建立 Beeline 測試使用者](#creating-an-beeline-test-user)** - 在 Beeline 中建立一個與 Azure AD 中代表 Britta Simon 的項目連結的 Britta Simon 對應項目。
...
1. 在 [設定在 Beeline 單一登入]  頁面上，執行下列步驟：
...
2. 若要為您的應用程式設定 SSO，請連絡 Beeline 支援小組，他們將會協助您設定 SSO。 請注意，您必須傳送電子郵件並附加已下載的中繼資料檔案，還必須提供「實體識別碼」和「單一登出服務 URL」。
...
> [!NOTE]
> 如果您需要手動建立使用者或需要建立使用者批次，則必須連絡 Beeline 支援小組。
> 
> 
...
當您在「存取面板」中按一下 [Beeline] 磚時，應該會自動登入您的 Beeline 應用程式。
