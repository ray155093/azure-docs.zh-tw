...
-   [設定應用程式中的 Azure AD 中繼資料值 (登入 URL、簽發者、登出 URL 與憑證)](#configure-single-sign-on-for-an-application-from-the-azure-ad-gallery)
...
13. 按一下 [設定 &lt;應用程式名稱&gt;]，可存取如何設定應用程式使用單一登入的文件。 此外，您會有透過該應用程式設定 SSO 所需的中繼資料 URL 與憑證。
...
8.  在 [使用者屬性] 區段下，從 [使用者識別碼] 下拉式清單中選取您使用者的唯一識別碼。 所選的選項必須符合應用程式中預期的值，才能驗證使用者。
...
9.  若要新增使用者屬性，請按一下 [檢視和編輯所有其他使用者屬性]，以編輯當使用者登入時要以 SAML 權杖傳送至應用程式的屬性。
...
8.  移至 [SAML 簽署憑證] 區段，然後按一下 [下載] 資料行值。 根據應用程式設定單一登入時所需的項目，您會看到可下載中繼資料或 XML 憑證的選項。
...
11. 在 [網域及 URL] 中輸入必要值 這些值應從應用程式廠商處取得。
  ...
  1. 若要將應用程式設定為 IdP 啟始的 SSO，請輸入 [回覆 URL] 與 [識別碼]。

  2. **選擇性：**若要將應用程式設定為服務提供者啟始的單一登入，則 [登入 URL] 為必要值。

12. 在 [使用者屬性] 中，從 [使用者識別碼] 下拉式清單中選取您使用者的唯一識別碼。
