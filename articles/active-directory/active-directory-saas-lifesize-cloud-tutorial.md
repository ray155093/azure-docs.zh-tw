...
* 您可以讓使用者使用他們的 Azure AD 帳戶自動登入 Lifesize Cloud 單一登入 (SSO)
...
* Azure AD 訂用帳戶
* 已啟用 Lifesize Cloud 單一登入 (SSO) 的訂用帳戶
...
* 從資源庫新增 Lifesize Cloud
* 設定並測試 Azure AD SSO

## <a name="add-lifesize-cloud-from-the-gallery"></a>從資源庫新增 Lifesize Cloud
...
在本節中，您會在傳統入口網站中啟用 Azure AD SSO，然後在您的 Lifesize Cloud 應用程式中設定單一登入。
...
3. 在 [設定 App 設定]  對話方塊頁面執行下列步驟：
  ...
  1. 在 [登入 URL] 文字方塊中，使用以下模式輸入您的使用者用來登入 Lifesize Cloud 應用程式的 URL：**https://login.lifesizecloud.com/ls/?acs**。
  2. 按一下 [下一步]。
...
8. 現在設定 SSO 組態 UI 中的下列值。    
  ...
  1. 從 Azure AD 複製簽發者 URL 的值，並貼到 [識別提供者簽發者] 文字方塊。 
  2. 從 Azure AD 複製遠端登入 URL 的值，並貼到 [登入 URL] 文字方塊。   
  3. 在記事本中開啟已下載的憑證並複製憑證的內容 (不包括開頭和結尾憑證的行)，將此內容貼到 [X.509 憑證] 文字方塊。
  4. 在 [名字] 文字方塊的 SAML 屬性對應中，輸入以下格式的值：**http://schemas.xmlsoap.org/ws/2005/05/identity/claims/givenname**
  5. 在 [姓氏] 文字方塊的 SAML 屬性對應中，輸入以下格式的值：**http://schemas.xmlsoap.org/ws/2005/05/identity/claims/surname**
  6. 在 [電子郵件] 文字方塊的 SAML 屬性對應中，輸入以下格式的值：**http://schemas.xmlsoap.org/ws/2005/05/identity/claims/emailaddress**
...
5. 在 [告訴我們這位使用者]  對話方塊頁面上，執行下列步驟：
    ...
    ![建立 Azure AD 測試使用者](./media/active-directory-saas-lifesize-cloud-tutorial/create_aaduser_05.png) 
   1. 針對 [使用者類型]，選取 [您組織中的新使用者]。
   2. 在 [使用者名稱] 文字方塊中，輸入 **BrittaSimon**。
   3. 按 [下一步] 。
6. 在 [使用者設定檔]  對話方塊頁面上，執行下列步驟：

   ![建立 Azure AD 測試使用者](./media/active-directory-saas-lifesize-cloud-tutorial/create_aaduser_06.png) 
...
### <a name="create-an-lifesize-cloud-test-user"></a>建立 Lifesize Cloud 測試使用者
