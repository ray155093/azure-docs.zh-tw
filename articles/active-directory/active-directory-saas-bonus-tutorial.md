---
title: "教學課程：Azure Active Directory 與 Bonus.ly 整合 | Microsoft Docs"
...
---
# <a name="tutorial-azure-active-directory-integration-with-bonusly"></a>教學課程：Azure Active Directory 與 Bonus.ly 整合
...
## <a name="enabling-the-application-integration-for-bonusly"></a>啟用 Bonus.ly 的應用程式整合
本節的目的是要說明如何啟用 Bonus.ly 的應用程式整合。

### <a name="to-enable-the-application-integration-for-bonusly-perform-the-following-steps"></a>若要啟用 Bonus.ly 的應用程式整合，請執行下列步驟：
...
6. 在**搜尋方塊**中，輸入 **Bonus.ly**。
   ...
   ![應用程式庫](./media/active-directory-saas-bonus-tutorial/IC773681.png "Application gallery")
...
1. 在 Azure 傳統入口網站的 [Bonus.ly] 應用程式整合頁面上，按一下 [設定單一登入] 來開啟 [設定單一登入] 對話方塊。
...
3. 在 [設定應用程式 URL] 頁面的 [Bonus.ly 租用戶 URL] 文字方塊中，使用下列模式輸入您的 URL："*https://\<tenant-name\>.Bonus.ly*"，然後按 [下一步]： 
...
8. 在 [SAML]  對話頁面上，執行下列步驟：
   ...
   1. 在 Azure 傳統入口網站的 [在 Bonus.ly 設定單一登入] 對話頁面上，複製 [遠端登入 URL] 值，然後將它貼至 [IdP SSO 目標 URL] 文字方塊中。
   2. 在 Azure 傳統入口網站的 [在 Bonus.ly 設定單一登入] 對話頁面上，複製 [簽發者識別碼] 值，然後將它貼至 [IdP 簽發者] 文字方塊中。
   3. 在 Azure 傳統入口網站的 [在 Bonus.ly 設定單一登入] 對話頁面上，複製 [遠端登入 URL] 值，然後將它貼至 [IdP 登入 URL] 文字方塊中。
   4. 從匯出的憑證複製**指紋**值，然後將它貼入 [憑證指紋] 文字方塊。
      ...
      > 
9. 按一下 [儲存] 。
10. 在 Microsoft Azure 傳統入口網站上，選取設定確認，然後按一下 [完成] 以關閉 [設定單一登入] 對話方塊。
...
6. 在 [加入使用者]  對話方塊上，執行下列步驟：
   ...
   1. 在相關的文字方塊中，針對您想要佈建的有效 AAD 帳戶輸入其 [電子郵件]、[名字]、[姓氏]。
   2. 按一下 [儲存] 。
   
   > [!NOTE]
   > AAD 帳戶的持有者會收到一封包含連結的電子郵件，以在啟用帳戶前進行確認。
   > 
   > 

> [!NOTE]
> 您可以使用任何其他的 Bonus.ly 使用者帳戶建立工具或 Bonus.ly 提供的 API 來佈建 AAD 使用者帳戶。
> 
> 
...
### <a name="to-assign-users-to-bonusly-perform-the-following-steps"></a>若要指派使用者給 Bonus.ly，請執行下列步驟：
