...
本教學課程的目的是要示範 Azure 與 ZScaler ZSCloud 的整合。本教學課程中說明的案例假設您已經具有下列項目：

* 有效的 Azure 訂閱
* 啟用 ZScaler ZSCloud 單一登入的訂用帳戶

完成本教學課程之後，您指派給 ZScaler ZSCloud 的 Azure AD 使用者就能夠單一登入您 ZScaler ZSCloud 公司網站 (服務提供者起始登入) 的應用程式，或是使用[存取面板簡介](active-directory-saas-access-panel-introduction.md)進行單一登入。
...
5. 在 [欲執行動作] 對話方塊中，按一下 [從資源庫加入應用程式]。
   ...
   ![從組件庫新增應用程式](./media/active-directory-saas-zscaler-zscloud-tutorial/IC749322.png "從組件庫新增應用程式")
6. 在**搜尋方塊**中，輸入 **ZScaler ZSCloud**。
   
   ![應用程式庫](./media/active-directory-saas-zscaler-zscloud-tutorial/IC800276.png "應用程式庫")
...
## 設定單一登入
本節的目的是要說明如何依據 SAML 通訊協定來使用同盟，讓使用者能夠用自己在 Azure AD 中的帳戶在 ZScaler ZSCloud 中進行驗證。在此程序中，您需要上傳 base-64 編碼憑證到您的 ZScaler ZSCloud 租用戶。如果您不熟悉這個程序，請參閱[如何將二進位憑證轉換成文字檔](http://youtu.be/PlgrzUZ-Y1o)。

### 若要設定單一登入，請執行下列步驟：
1. 在 Azure 傳統入口網站的 [ZScaler ZSCloud] 應用程式整合頁面上，按一下 [設定單一登入] 來開啟 [設定單一登入] 對話方塊。
...
3. 在 [設定應用程式 URL] 頁面的 [ZScaler ZSCloud 單一登入 URL] 文字方塊中，輸入使用者登入 ZScaler ZSCloud 應用程式所使用的 URL，然後按 [下一步]。
   
   ![設定應用程式 URL](./media/active-directory-saas-zscaler-zscloud-tutorial/IC800280.png "設定應用程式 URL")
   
   > [!NOTE]
   > 如有需要，您可以向 ZScaler ZSCloud 支援小組取得您的環境的實際值。
   > 
   > 
...
6. 在頂端的功能表中，按一下 [系統管理]。
   ...
   ![系統管理](./media/active-directory-saas-zscaler-zscloud-tutorial/IC800206.png "系統管理")
...
9. 在 [設定 SAML 單一登入參數] 對話方塊頁面上，執行下列步驟，然後按一下 [完成]：
   ...
   1. 在 Azure 傳統入口網站的 [設定在 ZScaler ZSCloud 單一登入] 對話方塊頁面上，複製 [驗證要求 URL] 值，然後貼至 [傳送使用者以供驗證的 SAML 入口網站 URL] 文字方塊中。
   2. 在 [包含登入名稱的屬性] 文字方塊中，輸入 NameID。
   3. 若要上傳您下載的憑證，請按一下 Zscaler pem。
   4. 選取 [啟用 SAML 自動佈建]。
10. 在 [設定使用者驗證] 對話方塊頁面上執行下列步驟：
    
    ![系統管理](./media/active-directory-saas-zscaler-zscloud-tutorial/IC800210.png "系統管理")
    
    1. 按一下 [儲存]。
    2. 按一下 [立即啟用]。
11. 在 Azure 傳統入口網站的 [設定在 ZScaler ZSCloud 單一登入] 對話方塊上，選取單一登入組態確認，然後按一下 [完成]。
...
2. 從 [工具] 功能表選取 [網際網路選項] 可開啟 [網際網路選項] 對話方塊。
   ...
   ![，](./media/active-directory-saas-zscaler-zscloud-tutorial/IC769492.png "，")
3. 按一下 [連線] 索引標籤。
...
## 設定使用者佈建
為了讓 Azure AD 使用者登入 ZScaler ZSCloud，他們必須佈建到 ZScaler ZSCloud。ZScaler ZSCloud 需以手動的方式佈建。

### 若要設定使用者佈建，請執行下列步驟：
1. 登入 **Zscaler** 租用戶。
2. 按一下 [系統管理]。
   
   ![系統管理](./media/active-directory-saas-zscaler-zscloud-tutorial/IC781035.png "系統管理")
3. 按一下 [使用者管理]。
   
   ![加](./media/active-directory-saas-zscaler-zscloud-tutorial/IC781037.png "加")
4. 在 [使用者] 索引標籤中，按一下 [新增]。
   
   ![加](./media/active-directory-saas-zscaler-zscloud-tutorial/IC781037.png "新增")
...
2. 在 **ZScaler ZSCloud** 應用程式整合頁面中，按一下 [指派使用者]。
   
   ![指派使用者](./media/active-directory-saas-zscaler-zscloud-tutorial/IC800283.png "指派使用者")
