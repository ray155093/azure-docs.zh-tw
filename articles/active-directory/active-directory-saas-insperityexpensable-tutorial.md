<properties
	pageTitle="教學課程：Azure Active Directory 與 Insperity ExpensAble 整合 | Microsoft Azure"
...
本教學課程旨在說明如何整合 Insperity ExpensAble 與 Azure Active Directory (Azure AD)。Insperity ExpensAble 與 Azure AD 整合提供下列優點：
...
6. 在搜尋方塊中，輸入 **Insperity ExpensAble**。

	![建立 Azure AD 測試使用者](./media/active-directory-saas-insperityexpensable-tutorial/tutorial_insperityexpensable_01.png)

7. 在結果窗格中，選取 [Insperity ExpensAble]，然後按一下 [完成] 以新增應用程式。

	![建立 Azure AD 測試使用者](./media/active-directory-saas-insperityexpensable-tutorial/tutorial_insperityexpensable_02.png)

##  設定並測試 Azure AD 單一登入
本節目標是說明如何以名為 "Britta Simon" 的測試使用者為基礎，使用 Insperity ExpensAble 來設定及測試 Azure AD 單一登入。

若要讓單一登入運作，Azure AD 必須知道 Insperity ExpensAble 與 Azure AD 中互相對應的使用者。換句話說，必須在 Azure AD 使用者和 Insperity ExpensAble 中的相關使用者之間建立連結關聯性。建立此連結關聯性的方法是將 Azure AD 中**使用者名稱**的值指派為 Insperity ExpensAble 中 **Username** 的值。

若要設定及測試與 Insperity ExpensAble 搭配運作的 Azure AD 單一登入，您需要完成下列構成要素：

1. **[設定 Azure AD 單一登入](#configuring-azure-ad-single-single-sign-on)** - 讓您的使用者能夠使用此功能。
2. **[建立 Azure AD 測試使用者](#creating-an-azure-ad-test-user)** - 使用 Britta Simon 測試 Azure AD 單一登入。
4. **[建立 Insperity ExpensAble 測試使用者](#creating-a-insperityexpensable-test-user)** - 在 Insperity ExpensAble 中建立 Britta Simon 的對應項目，且該項目與 Azure AD 中代表 Britta Simon 的項目連結。
5. **[指派 Azure AD 測試使用者](#assigning-the-azure-ad-test-user)** - 讓 Britta Simon 能夠使用 Azure AD 單一登入。
5. **[測試單一登入](#testing-single-sign-on)** - 驗證組態是否能運作。

### 設定 Azure AD 單一登入

本節的目標是要在 Azure 傳統入口網站中啟用 Azure AD 單一登入，並在您的 Insperity ExpensAble 應用程式中設定單一登入。



**若要使用 Insperity ExpensAble 設定 Azure AD 單一登入，請執行下列步驟：**

1. 在 Azure 傳統入口網站的 [Insperity ExpensAble] 應用程式整合頁面上，按一下 [設定單一登入] 來開啟 [設定單一登入] 對話方塊。

	![設定單一登入][6]

2. 在 [要如何讓使用者登入 Insperity ExpensAble] 頁面上，選取 [Azure AD 單一登入]，然後按 [下一步]。
...
3. 在 [設定應用程式設定] 對話方塊頁面上，執行下列步驟：
    ...
    a.在 [登入 URL] 文字方塊中，使用以下模式輸入使用者登入您的 Insperity ExpensAble 應用程式時所使用的 URL：`https://server.expensable.com/esapp/Authenticate?companyId=<company ID>`

	b.按 [下一步]。

4. 在 [設定在 Insperity ExpensAble 單一登入] 頁面上，執行下列步驟：
...
5. 若要為您的應用程式設定 SSO，請連絡您的 Insperity ExpensAble 技術支援小組。一旦指派案例，請以電子郵件寄送下載的憑證檔案。同時也請提供 [簽發者 URL] 及 [單一登入服務 URL]，讓這些項目能針對 SSO 整合來進行設定。
...
### 建立 Insperity ExpensAble 測試使用者

本節的目標是在 Insperity ExpensAble 中建立名為 Britta Simon 的使用者。請與 Insperity ExpensAble 支援小組合作，在 Insperity ExpensAble 帳戶中加入使用者。


> [AZURE.NOTE] 如果您需要手動建立使用者，您需要連絡 Insperity ExpensAble 支援小組。


### 指派 Azure AD 測試使用者

本節目標是授與 Britta Simon 對 Insperity ExpensAble 的存取權，讓她能夠使用 Azure 單一登入。
