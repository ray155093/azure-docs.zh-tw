...
3. **[建立 Insperity ExpensAble 測試使用者](#creating-a-insperityexpensable-test-user)** - 在 Insperity ExpensAble 中建立 Britta Simon 的對應項目，且該項目與 Azure AD 中代表 Britta Simon 的項目連結。
...
1. 在 [設定在 Insperity ExpensAble 單一登入] 頁面上，執行下列步驟：
    ...
    b.按 [下一步]。
2. 若要為您的應用程式設定 SSO，請連絡您的 Insperity ExpensAble 技術支援小組。一旦指派案例，請以電子郵件寄送下載的憑證檔案。同時也請提供 [簽發者 URL] 及 [單一登入服務 URL]，讓這些項目能針對 SSO 整合來進行設定。
...
> [!NOTE]
> 如果您需要手動建立使用者，您需要連絡 Insperity ExpensAble 支援小組。
> 
> 
