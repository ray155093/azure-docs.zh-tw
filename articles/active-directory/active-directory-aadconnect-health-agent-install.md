...
|Azure 服務端點的輸出連線|在安裝期間和執行階段，代理程式需要連線至 Azure AD Connect Health 服務端點。如果輸出連線遭到封鎖，請確定下列端點已新增至允許清單：</br></br><li>&#42;.blob.core.windows.net </li><li>&#42;.queue.core.windows.net</li><li>adhsprodwus.servicebus.windows.net - 連接埠：5671 </li><li>https://management.azure.com </li><li>https://s1.adhybridhealth.azure.com/</li><li>https://policykeyservice.dc.ad.msft.net/</li><li>https://login.windows.net</li><li>https://login.microsoftonline.com</li><li>https://secure.aadcdn.microsoftonline-p.com</li> |
...
|如果啟用 IE 增強式安全性，則允許下列網站| 如果已啟用 IE 增強式安全性，則即將安裝代理程式的伺服器上必須允許下列網站。</br></br><li>https://login.microsoftonline.com</li><li>https://secure.aadcdn.microsoftonline-p.com</li><li>https://login.windows.net</li><li>Azure Active Directory 所信任的組織同盟伺服器。例如：https://sts.contoso.com</li>
...
1. 如需有關在 Windows Server 2008 R2 上安裝 Windows PowerShell 4.0 的詳細資訊，請參閱[這裡](http://social.technet.microsoft.com/wiki/contents/articles/20623.step-by-step-upgrading-the-powershell-version-4-on-2008-r2.aspx)的 Wiki 文章。

### 啟用 AD FS 的稽核

> [AZURE.NOTE] 這一節只適用於 AD FS 同盟伺服器。
...
4. 若要啟用稽核功能，請使用提高的權限開啟命令提示字元，然後執行下列命令：<code>auditpol.exe /set /subcategory:"Application Generated" /failure:enable /success:enable</code>
...
> [AZURE.WARNING] 如果有即將停用 AD FS 稽核的群組原則，則 Azure AD Connect Health 代理程式無法收集資訊。請確定您沒有可能即將停用稽核的群組原則。
...
> [AZURE.NOTE] 請記住，使用 Azure AD Connect Health 需要 Azure AD Premium。如果沒有 Azure AD Premium，您就無法完成 Azure 入口網站中的組態。如需詳細資訊，請參閱[需求頁面](active-directory-aadconnect-health-agent-install.md#requirements)。
...
### 變更 Health 代理程式 Proxy 組態
您有下列選項來設定 Azure AD Connect Health 代理程式使用 HTTP Proxy。

>[AZURE.NOTE] 所有的 Azure AD Connect Health 代理程式服務都必須重新啟動，才會更新 Proxy 設定。執行下列命令：<br> Restart-Service AdHealth*
...
>[AZURE.NOTE]若要使用連線工具，您必須先完成代理程式註冊。如果您無法完成代理程式註冊，請確定您已符合 Azure AD Connect Health 的所有[需求](active-directory-aadconnect-health-agent-install.md#requirements)。這項連線測試預設是在代理程式註冊期間執行。
