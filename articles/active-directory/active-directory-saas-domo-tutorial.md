<properties
	pageTitle="教學課程：Azure Active Directory 與 Domo 整合 | Microsoft Azure"
...
# 教學課程：Azure Active Directory 與 Domo 整合

本教學課程旨在說明如何整合 Domo 與 Azure Active Directory (Azure AD)。Domo 與 Azure AD 整合提供下列優點：
...
7. 在結果窗格中，選取 [Domo]，然後按一下 [完成] 來新增應用程式。

	![建立 Azure AD 測試使用者](./media/active-directory-saas-domo-tutorial/tutorial_domo_02.png)

##  設定並測試 Azure AD 單一登入
本節的目標，在於示範如何以名為「Britta Simon」的測試使用者，來設定及測試搭配 Domo 的 Azure AD 單一登入。

如要讓單一登入順利運作，Azure AD 必須要知道與某位 Azure AD 使用者互相對應的 Domo 使用者。換句話說，必須要建立某位 Azure AD 使用者與 Domo 中相關使用者之間的連結關聯性。而建立的方法，是把 Azure AD 中**使用者名稱**的值指派為 Domo 中 **Username** 的值。

如要設定及測試搭配 Domo 的 Azure AD 單一登入，您需要完成下列構成元素：

1. **[設定 Azure AD 單一登入](#configuring-azure-ad-single-single-sign-on)** - 讓您的使用者能夠使用此功能。
2. **[建立 Azure AD 測試使用者](#creating-an-azure-ad-test-user)** - 使用 Britta Simon 測試 Azure AD 單一登入。
4. **[建立 Domo 測試使用者](#creating-a-domo-test-user)** - 在 Domo 中建立 Britta Simon 的對應項目，且該項目必須與 Azure AD 中代表 Britta Simon 的項目連結。
...
4. 在 [設定 App 設定] 對話方塊頁面執行下列步驟：
    ...
    a.在 [登入 URL] 文字方塊中，使用以下模式輸入使用者登入您的 Domo 應用程式時所使用的 URL：`https://<company name>.domo.com`
...
> [AZURE.NOTE] 如果您需要手動建立使用者，就必須連絡 Domo 支援小組。


### 指派 Azure AD 測試使用者

本節的目標，是要授與 Britta Simon 對 Domo 的存取權，讓她能夠使用 Azure 單一登入。
...
本節的目標是要使用存取面板來測試您的 Azure AD 單一登入組態。當您在存取面板中按一下 [Domo] 磚時，應該會自動登入您的 Domo 應用程式。
