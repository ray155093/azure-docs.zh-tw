...
本教學課程旨在說明如何整合 Workrite 與 Azure Active Directory (Azure AD)。

Workrite 與 Azure AD 整合提供下列優點： 

* 您可以在 Azure AD 中控制可存取 Workrite 的人員 
* 您可以讓使用者使用他們的 Azure AD 帳戶自動登入 Workrite 單一登入 (SSO)
...
5. 在 [欲執行動作] 對話方塊上，按一下 [從資源庫中新增應用程式]。
    ...
    ![應用程式][4]
6. 在搜尋方塊中，輸入 **Workrite**。
   
    ![應用程式][5]
7. 在結果窗格中，選取 [Workrite]，然後按一下 [完成] 以加入應用程式。
   
    ![應用程式][500]

## <a name="configure-and-test-azure-ad-sso"></a>設定並測試 Azure AD SSO
本節的目標是要說明如何以名為 "Britta Simon" 的測試使用者為基礎，設定及測試與 Workrite 搭配運作的 Azure AD SSO。

若要讓 SSO 運作，Azure AD 必須知道 Workrite 與 Azure AD 中互相對應的使用者。 換句話說，必須在 Azure AD 使用者和 Workrite 中的相關使用者之間建立連結關聯性。  

建立此連結關聯性的方法是將 Azure AD 中**使用者名稱**的值指定為 Workrite 中 **Username** 的值。

若要設定及測試與 Workrite 搭配運作的 Azure AD SSO，您需要完成下列構成要素：

1. **[設定 Azure AD 單一登入](#configuring-azure-ad-single-single-sign-on)** - 讓您的使用者能夠使用此功能。
2. **[建立 Azure AD 測試使用者](#creating-an-azure-ad-test-user)** - 使用 Britta Simon 測試 Azure AD 單一登入。
3. **[建立 Workrite 測試使用者](#creating-a-halogen-software-test-user)** - 使 Workrite 中對應的 Britta Simon 連結到她在 Azure AD 中的代表項目。
4. **[指派 Azure AD 測試使用者](#assigning-the-azure-ad-test-user)** - 讓 Britta Simon 能夠使用 Azure AD 單一登入。
5. **[測試單一登入](#testing-single-sign-on)** - 驗證組態是否能運作。

### <a name="configure-azure-ad-sso"></a>設定 Azure AD SSO
本節的目標是要在 Azure 傳統入口網站中啟用 Azure AD SSO，並在您的 Workrite 應用程式中設定 SSO。

**若要設定與 Workrite 搭配運作的 Azure AD SSO，請執行下列步驟：**

1. 在 Azure 傳統入口網站的 [Workrite] 應用程式整合頁面上，按一下 [設定單一登入] 來開啟 [設定單一登入] 對話方塊。
   
    ![設定單一登入][6] 
2. 在 [您希望使用者如何登入 Workrite] 頁面上，選取 [Azure AD 單一登入]，然後按 [下一步]。
   
    ![Azure AD 單一登入][7] 
3. 在 [設定應用程式設定]  對話方塊頁面上，執行下列步驟：
   
    ![Azure AD 單一登入][8] 
  1. 在 [登入 URL] 文字方塊中，輸入您的使用者用於登入 Workrite 網站的 URL (例如︰*https://app.workrite.co.uk/securelogin/samlgateway.aspx?id=1a82b5aa-4dd6-4472-9721-7d0193f59e22*)。

    >[!NOTE]
    >如果您不知道登入 URL 的值，請連絡您的 Workrite 支援小組 [support@workrite.co.uk](mailto:support@workrite.co.uk) 。 
    >   
  2. 按 [下一步] 。
4. 在 [設定在 Workrite 單一登入]  頁面上，執行下列步驟：
   
    ![Azure AD 單一登入][9] 
 1. 按一下 [下載憑證]，然後將檔案儲存在您的電腦上。  
 2. 連絡您的 Workrite 支援小組 [support@workrite.co.uk](mailto:support@workrite.co.uk)，提供他們下載的憑證、**簽發者 URL** (實體識別碼)、**單一登入服務 URL**、**單一登出 URL**，然後要求他們為 Workrite 應用程式設定 SSO。  
 3. 按 [下一步] 。
...
2. 在導覽窗格中，按一下 [系統管理員] 。
    ...
    ![指派使用者][400]
3. 移至快速連結然後再按一下 [建立使用者] 。 
   
    ![指派使用者][401]
4. 在 [建立使用者]  對話方塊中，執行下列步驟：
   
    ![指派使用者][402]
 1. 輸入您想要佈建之有效 Azure AD 使用者的**電子郵件**、**名字**和**姓氏**。  
 2. 對 [選擇角色] 選取 [用戶端系統管理員]。  
 3. 按一下 [儲存] 。   

### <a name="assign-the-azure-ad-test-user"></a>指派 Azure AD 測試使用者
本節的目標是授與 Britta Simon 對 Workrite 的存取權，使她能夠使用 Azure SSO。
