...
1.  在 Azure 傳統入口網站的 [Tinfoil Security] 應用程式整合頁面上，按一下 [設定單一登入] 來開啟 [設定單一登入] 對話方塊。
...
8.  在 [單一登入] 組態頁面上，執行下列步驟：
    ...
    1.  選取 [啟用 SAML]。
    2.  按一下 [手動設定]。
    3.  在 Azure 傳統入口網站的 [在 Tinfoil Security 設定單一登入] 對話頁面上，複製 [SAML SSO URL] 值，然後將它貼至 [SAML 貼文 URL] 文字方塊中。
