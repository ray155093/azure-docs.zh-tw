...
3. 在 [設定應用程式設定] 頁面上，執行下列步驟：
	...
	a. 在 [登入 URL] 文字方塊中，使用以下模式輸入您的 Docusign 租用戶的 URL：`https://<company name>.docusign.net/Member/MemberLogin.aspx?ssoname=<SSO instance name>`
...
6. 在頂端的功能表中，展開使用者的功能表，按一下 [喜好設定]，然後在左邊瀏覽窗格中展開 [帳戶管理]，然後按一下 [功能]。
...
8. 在 [SAML 2.0 設定] 區段中，執行下列步驟：
    ...
    a.在 Azure 傳統入口網站的 [設定在 Docusign 單一登入] 對話方塊頁面上，複製 [簽發者 URL] 值，然後貼至 [識別提供者端點 URL] 文字方塊中。
...
9. 在 Azure 傳統入口網站上，選取**單一登入設定確認**，然後按 [下一步]。
