<properties
	pageTitle="Azure 單一登入 SAML 通訊協定 | Microsoft Azure"
...
| ForceAuthn | 選用 | 如果提供，這應該是 false。其他任何值會造成錯誤。|
| IsPassive | 選用 | 如果提供，這應該是 false。其他任何值會造成錯誤。 |  
...
請勿包含 `SPNameQualifer` 屬性。Azure AD 會忽略 `AllowCreate` 屬性。
...
- **Name 宣告**︰`Name` 屬性值 (`http://schemas.xmlsoap.org/ws/2005/05/identity/claims/name`) 是已驗證使用者的使用者主體名稱，例如 `testuser@managedtenant.com`。
- **ObjectIdentifier 宣告**︰`ObjectIdentifier` 屬性值 (`http://schemas.microsoft.com/identity/claims/objectidentifier`) 是代表 Azure AD 中已驗證使用者的目錄物件的 `ObjectId`。`ObjectId` 是不可變的、全域唯一的，且重複使用已驗證使用者的安全識別碼。
