...
* 有效的 Azure 訂閱
* 啟用 Mindflash 單一登入 (SSO) 的訂用帳戶

完成本教學課程之後，您指派給 Mindflash 的 Azure AD 使用者就能夠單一登入您 Mindflash 公司網站 (服務提供者起始登入) 的應用程式，或是使用 [存取面板簡介](active-directory-saas-access-panel-introduction.md)。
...
## <a name="enabling-the-application-integration-for-mindflash"></a>啟用Mindflash 的應用程式整合
本節的目的是要說明如何啟用 Mindflash 的應用程式整合。

### <a name="to-enable-the-application-integration-for-mindflash-perform-the-following-steps"></a>若要啟用 Mindflash 的應用程式整合，請執行下列步驟：
...
5. 在 [欲執行動作] 對話方塊上，按一下 [從資源庫中新增應用程式]。
   ...
   ![從資源庫新增應用程式](./media/active-directory-saas-mindflash-tutorial/IC749322.png "從資源庫新增應用程式")
6. 在**搜尋方塊**中，輸入 **Mindflash**。
   
   ![應用程式資源庫](./media/active-directory-saas-mindflash-tutorial/IC787133.png "應用程式資源庫")
...
1. 在 Azure 傳統入口網站的 [Mindflash] 應用程式整合頁面上，按一下 [設定單一登入] 來開啟 [設定單一登入] 對話方塊。
...
5. 將中繼資料檔傳送給 Mindflash 支援小組。
   ...
   > [!NOTE]
   > 單一登入設定必須由 Mindflash 支援小組執行。 設定完成後，您將會收到通知。
   > 
   > 
...
### <a name="to-assign-users-to-mindflash-perform-the-following-steps"></a>若要指派使用者給 Mindflash，請執行下列步驟：
