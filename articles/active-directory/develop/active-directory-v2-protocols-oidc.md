...
v2.0 端點目前不支援 OpenID Connect `end_session_endpoint`。 這意謂著應用程式無法向 v2.0 端點傳送要求，來結束使用者工作階段及清除 v2.0 端點所設定的 Cookie。
若要將使用者登出，應用程式只需結束自己與使用者的工作階段，而將使用者與 v2.0 端點的工作階段保持不變即可。 下次使用者嘗試登入時，將會看到 [選擇帳戶] 頁面，而其主動登入帳戶會列在其中。 在該頁面上，使用者可以選擇登出任一帳戶並結束與 v2.0 端點的工作階段。
