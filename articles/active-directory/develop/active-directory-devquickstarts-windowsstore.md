...
7. 從 [設定] 頁面，選擇 [必要權限]，然後選擇 [加入]。  針對 "Azure Active Directory" 應用程式，請選取 [Microsoft Graph] 作為 API，然後在 [委派權限] 底下新增 [以登入使用者的身分存取目錄] 權限。  這樣做可讓您的應用程式查詢 Graph API 的使用者。
...
* 回到「Azure 入口網站」中應用程式的 [設定] 索引標籤上，新增具有此值的 **RedirectUri**。  
