# 概觀
## [與 Azure AD 整合](active-directory-how-to-integrate.md)
## [驗證案例](active-directory-authentication-scenarios.md)
## [Graph API](active-directory-graph-api.md)
## [驗證程式庫](active-directory-authentication-libraries.md)
## [驗證通訊協定](active-directory-authentication-protocols.md)
## [應用程式和服務主體物件](active-directory-application-objects.md)
## [開發人員的指南](active-directory-developers-guide.md)
## [OAuth2 隱含流程](active-directory-dev-understanding-oauth2-implicit-grant.md)
## [Azure AD 中的應用程式](active-directory-how-applications-are-added.md)

# 開始使用
## 行動和 PC 應用程式
### [iOS](active-directory-devquickstarts-ios.md)
### [Android](active-directory-devquickstarts-android.md)
### [Windows Phone](active-directory-devquickstarts-windowsphone.md)
### [.Net](active-directory-devquickstarts-dotnet.md)
### [Windows 市集](active-directory-devquickstarts-windowsstore.md)
### [Xamarin](active-directory-devquickstarts-xamarin.md)
### [Cordova](active-directory-devquickstarts-cordova.md)
### [Oauth](active-directory-protocols-oauth-code.md)
## Web 應用程式
### [.NET](active-directory-devquickstarts-webapp-dotnet.md)
### [Java](active-directory-devquickstarts-webapp-java.md)
### [AngularJS](active-directory-devquickstarts-angular.md)
### [NodeJS](active-directory-devquickstarts-openidconnect-nodejs.md)
### [OpenID Connect](active-directory-protocols-openid-connect-code.md)
### [Java 命令列](active-directory-devquickstarts-headless-java.md)
## Web API
### [Web API 和 .NET](active-directory-devquickstarts-webapi-dotnet.md)
### [NodeJS](active-directory-devquickstarts-webapi-nodejs.md)
## [Graph API](active-directory-graph-api-quickstart.md)

# 作法
## [取得租用戶](active-directory-howto-tenant.md)
## [登入任何 Azure AD 使用者](active-directory-devhowto-multi-tenant-overview.md)
## [整合應用程式](active-directory-integrating-applications.md)
## 啟用 SSO
### [Android](active-directory-sso-android.md)
### [iOS](active-directory-sso-ios.md)
## [讓 AppSource 獲得認證](active-directory-devhowto-appsource-certified.md)
## [在資源庫中列出您的應用程式](active-directory-app-gallery-listing.md)
## [了解了解應用程式資訊清單](active-directory-application-manifest.md)
## [驗證 Web 使用者](active-directory-java-authenticate-users-access-control-eclipse.md)
## 使用 SAML
### [檢視存取控制傳回的 SAML](active-directory-java-view-saml-returned-by-access-control.md)
### [Azure AD 和 SAML](active-directory-saml-protocol-reference.md)
### [自訂宣告](active-directory-saml-claims-customization.md)
### [偵錯 SAML 型 SSO](active-directory-saml-debugging.md)
## [使用簽署金鑰變換](active-directory-signing-key-rollover.md)


## 使用 V2 端點
### [概觀](active-directory-appmodel-v2-overview.md)
### [V2 有何不同？](active-directory-v2-compare.md)
### [驗證程式庫](active-directory-v2-libraries.md)
### [應用程式類型](active-directory-v2-flows.md)
### [限制](active-directory-v2-limitations.md)
### [範圍](active-directory-v2-scopes.md)
### [註冊入口網站](active-directory-v2-registration-portal.md)
### [權杖參考](active-directory-v2-tokens.md)
### 通訊協定
#### [OAuth 2.0 用戶端認證](active-directory-v2-protocols-oauth-client-creds.md)
#### [OAuth 2.0 和 OpenID](active-directory-v2-protocols.md)
#### [最近更新](active-directory-v2-preview-oidc-changes.md)
### 行動應用程式
#### [iOS](active-directory-v2-devquickstarts-ios.md)
#### [Windows Phone](active-directory-v2-devquickstarts-wpf.md)
#### [Android](active-directory-v2-devquickstarts-android.md)
### Web Apps 和 API
#### [AngularJS 與 NodeJS](active-directory-v2-devquickstarts-angular-node.md)
#### [AngularJS 與 .NET](active-directory-v2-devquickstarts-angular-dotnet.md)
#### [.NET Web 應用程式](active-directory-v2-devquickstarts-dotnet-web.md)
#### [NodeJS Web 應用程式](active-directory-v2-devquickstarts-node-web.md)
#### [.NET API](active-directory-v2-devquickstarts-dotnet-api.md)
#### [NodeJS API](active-directory-v2-devquickstarts-node-api.md)
#### [呼叫 Web API](active-directory-v2-devquickstarts-webapp-webapi-dotnet.md)
### 直接整合通訊協定
#### [註冊應用程式](active-directory-v2-app-registration.md)
#### [OpenID Connect](active-directory-v2-protocols-oidc.md)
#### [隱含流程](active-directory-v2-protocols-implicit.md)
#### [OAuth 2.0 授權碼](active-directory-v2-protocols-oauth-code.md)
#### [OAuth 2.0 用戶端認證](active-directory-v2-protocols-oauth-client-creds.md)
#### [服務對服務呼叫](active-directory-protocols-oauth-service-to-service.md)

## 使用 Visual Studio
### [開始使用 .NET](vs-active-directory-dotnet-getting-started.md)
### [我的 MVC 專案發生什麼情形？](vs-active-directory-dotnet-what-happened.md)
### [驗證偵測期間發生錯誤](vs-active-directory-error.md)
### [開始使用 WebApi](vs-active-directory-webapi-getting-started.md)
### [我的 WebApi 專案發生什麼情形？](vs-active-directory-webapi-what-happened.md)


# 參考
## [詞彙](active-directory-dev-glossary.md)
## [商標方針](active-directory-branding-guidelines.md)
## [程式碼範例](active-directory-code-samples.md)
## [同盟中繼資料](active-directory-federation-metadata.md)
## [單一登入](active-directory-single-sign-on-protocol-reference.md)
## [單一登出](active-directory-single-sign-out-protocol-reference.md)
## [權杖和宣告](active-directory-token-and-claims.md)
