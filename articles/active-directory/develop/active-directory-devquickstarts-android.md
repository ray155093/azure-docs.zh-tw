...
> [!TIP]
> 試用新 [開發人員入口網站](https://identity.microsoft.com/Docs/Android) 的預覽版本，這可協助您在短短幾分鐘內啟動並執行 Azure Active Directory！  開發人員入口網站會逐步引導您完成註冊應用程式並將 Azure AD 整合至您的程式碼的程序。  當您完成時，您會有可驗證租用戶中使用者的簡單應用程式，以及可接受權杖並執行驗證的後端。 
> 
> 
...
6. 在 [登入 URL] 中，輸入範例的基礎 URL，預設為 `https://localhost:8080`。
7. 在 [應用程式識別碼 URI] 中，輸入 `https://<your_tenant_name>/TodoListService`，將 `<your_tenant_name>` 換成您的 Azure AD 租用戶名稱。  按一下 [確定] 以完成註冊。
8. 在仍位於 Azure 入口網站中時，移至您的應用程式頁面，找出 [應用程式識別碼] 值並將它複製到別處，稍後設定應用程式時將會需要此值。
...
7. 從應用程式頁面，找出 [應用程式識別碼] 值並將它複製到別處，稍後設定應用程式時將會需要此值。
8. 從 [設定] 頁面，選擇 [必要權限]，然後選擇 [加入]。  找出並選取 [TodoListService]，然後在 [委派的權限] 底下新增 [存取 TodoListService] 權限，接著選擇 [完成]。
...
4. 在您的主要活動建立 AuthenticationContext 的執行個體。 此呼叫的詳細資料超出此讀我檔案的範圍，但您可以查看 [Android 原生用戶端範例](https://github.com/AzureADSamples/NativeClient-Android)，由此開始也很不錯。 以下是一個範例，其中使用 SharedPreferences 作為預設快取，且 Authority 的格式為 `https://login.windows.net/yourtenant.onmicrosoft.com`：
...
1. **Broker**：Microsoft Intune 公司入口網站應用程式將提供 Broker 元件。 如果有一個使用者帳戶在這個驗證器上建立，且開發人員選擇不要略過它，ADAL 會使用 Broker 帳戶。 開發人員可以使用下列方法來略過 Broker 使用者：
   ...
   您的應用程式資訊清單應該具有使用 AccountManager 帳戶的權限：http://developer.android.com/reference/android/accounts/AccountManager.html
...
### <a name="authority-url-and-adfs"></a>授權單位 Url 和 ADFS
...
您可以將記錄傳送至檔案形式 logcat，如下所示：
 ...
 adb 命令的其他相關範例：https://developer.android.com/tools/debugging/debugging-log.html#startingLogcat
...
### <a name="session-cookies-in-webview"></a>Web 檢視中的工作階段 Cookie
