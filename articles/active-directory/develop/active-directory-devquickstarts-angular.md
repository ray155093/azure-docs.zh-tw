...
2. 在頂端列上按一下您的帳戶，然後在 [目錄] 清單底下選擇您要註冊應用程式的 Active Directory 租用戶。
3. 按一下左側導覽中的 [更多服務]，並選擇 [Azure Active Directory]。
...
6. 完成註冊後，AAD 會為您的應用程式指派唯一的**應用程式識別碼**。  您會在後續章節中用到這個值，所以請從應用程式索引標籤中複製此值。
7. Adal.js 會使用 OAuth 隱含流程來與 Azure AD 通訊。  您必須為您的應用程式啟用隱含流程，請執行下列動作：
  * 按一下應用程式，並選擇 [資訊清單]，以開啟內嵌資訊清單編輯器。
  * 找出 `oauth2AllowImplicitFlow` 屬性。 將值設為 `true`。
  * 按一下 [儲存]  以儲存資訊清單。
8. 在您的應用程式租用戶上授予權限。 前往 [設定] -> [屬性] -> [所需的權限]，並按一下頂端列中的 [授予權限] 按鈕。 按一下 [ **是** ] 以確認。 
