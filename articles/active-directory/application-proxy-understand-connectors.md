...
連接器可以安裝在允許它們將要求傳送至應用程式 Proxy 服務和後端應用程式的網路上任何地方。 將它們安裝在公司內、非軍事區 (DMZ) 內，甚至在雲端執行的虛擬機器上，都可以正常運作。 重點是執行連接器的電腦也可存取您的應用程式。
...
* 符合 FIPS 規範的電腦可能需要變更其設定，以允許連接器服務、連接器更新程式服務及其安裝程式產生憑證並加以儲存。
