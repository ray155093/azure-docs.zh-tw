...
---
# <a name="how-password-management-works"></a>密碼管理如何運作
> [!IMPORTANT]
...
* [**密碼管理組態入口網站**](#password-management-configuration-portal) – 系統管理員可以在 [Azure 管理入口網站](https://manage.windowsazure.com)中瀏覽至其目錄的 [設定] 索引標籤，來控制其租用戶中密碼管理方式的不同面向。
...
* [**密碼管理報告**](#password-management-reports) – 系統管理員可以在 [Azure 管理入口網站](https://manage.windowsazure.com)中瀏覽至其目錄之 [報告] 索引標籤中的 [活動報告] 區段，來檢視及分析其租用戶中的密碼重設和註冊活動。
* [**Password Writeback Component of Azure AD Connect**](#password-writeback-component-of-azure-ad-connect) – 安裝 Azure AD Connect 以從雲端啟用同盟或密碼同步處理使用者密碼管理時，系統管理員可以選擇性地啟用密碼回寫功能。

## <a name="password-management-configuration-portal"></a>密碼管理組態入口網站
您可以使用 [Azure 管理入口網站](https://manage.windowsazure.com)，瀏覽至目錄之 [設定] 索引標籤中的 [使用者密碼重設原則] 區段，來設定特定目錄的「密碼管理」原則。  您可以從此組態頁面控制如何在您的組織中管理密碼的許多層面，包括：
...
* 當使用者自己的密碼已重設時，對使用者啟用電子郵件通知 (位於 **Azure 管理入口網站** 的 [ [通知](https://manage.windowsazure.com)] 區段)
* 當其他系統管理員重設自己的密碼時，對使用者啟用電子郵件通知 (位於 **Azure 管理入口網站** 的 [ [Azure 管理入口網站](https://manage.windowsazure.com)
* 以您的組織的標誌和名稱設定使用者密碼重設入口網站和密碼重設電子郵件的商標，方法是使用租用戶商標自訂功能 (位於 **Azure 管理入口網站** 的 [ [Azure 管理入口網站](https://manage.windowsazure.com)
...
如需詳細資訊，請參閱[開始使用：Azure AD 密碼管理](active-directory-passwords-getting-started.md)和[最佳做法：Azure AD 密碼管理](active-directory-passwords-best-practices.md)。
...
藉由瀏覽至 [報告] 索引標籤並在 [活動記錄] 區段底下尋找，您將會看到兩個「密碼管理」報告：[密碼重設活動] 和 [密碼重設登錄活動]。  您可以使用這兩份報告，取得使用者在您的組織中註冊和使用密碼重設的檢視。 以下是這些報告在 [Azure 管理入口網站](https://manage.windowsazure.com)中的外觀：
