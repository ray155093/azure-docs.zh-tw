...
> [!NOTE]
> 設定 Concur 訂用帳戶以透過 SAML 進行同盟 SSO 是獨立的工作，您必須連絡 Concur 執行這項工作。
> 
> 

## <a name="enabling-the-application-integration-for-concur"></a>啟用 Concur 的應用程式整合
本節的目的是要說明如何啟用 Concur 的應用程式整合。

### <a name="to-enable-the-application-integration-for-concur-perform-the-following-steps"></a>若要啟用 Concur 的應用程式整合，請執行下列步驟：
...
4. 於 [在 Concur 設定單一登入]  頁面上，執行下列步驟。
   ...
   3. 選取單一登入設定確認，然後按一下 [完成] 來關閉 [設定單一登入] 對話方塊。  
...
### <a name="to-assign-users-to-concur-perform-the-following-steps"></a>若要將使用者指派給 Concur，請執行下列步驟：
