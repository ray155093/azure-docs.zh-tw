...
---
# <a name="integrate-azure-ad-with-a-windows-store-app"></a>整合 Azure AD 與 Windows 市集應用程式
...
5. 遵照提示進行，並建立新的 **原生用戶端應用程式**。 應用程式的 [ **名稱** ] 將對使用者說明您的應用程式。 「 **重新導向 URI** 」是配置和字串的組合，Azure AD 可用它來傳回權杖回應。  現在請先輸入預留位置值，例如 `http://DirectorySearcher`。 我們稍後將會取代此值。 按一下 [建立] 來建立應用程式。
6. 留在 Azure 入口網站中，選擇您的應用程式，按一下 [設定]，然後選擇 [屬性]。
7. 找到應用程式識別碼值，並將它複製到剪貼簿。
8. 設定應用程式的權限 - 在 [設定] 功能表中，選擇 [必要權限] 區段，依序按一下 [新增] 和 [選取 API]，然後選取 [Windows Azure Active Directory] (這是 AADGraph API)。 然後，按一下 [選取權限]，並選取 [以登入使用者的身分存取目錄]。 這樣做可讓您的應用程式查詢 Graph API 的使用者。
...
* 回到 Azure 入口網站中應用程式的 [設定] 索引標籤上，使用這個值取代 **RedirectUri** 的值。  
