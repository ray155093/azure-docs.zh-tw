...
本教學課程旨在說明如何將 23 Video 與 Azure Active Directory (Azure AD) 整合。將 23 Video 與 Azure AD 整合提供下列優點：
...
## 設定並測試 Azure AD 單一登入
本節的目標是要說明如何以名為 "Britta Simon" 的測試使用者為基礎，設定及測試與 23 Video 搭配運作的 Azure AD 單一登入。

若要讓單一登入能夠運作，Azure AD 必須知道 23 Video 與 Azure AD 中互相對應的使用者。換句話說，必須在 Azure AD 使用者與 23 Video 中的相關使用者之間建立連結關聯性。建立此連結關聯性的方法是將 Azure AD 中**使用者名稱**的值指定為 23 Video 中 **Username** 的值。
...
### 設定 Azure AD 單一登入
本節的目標是要在 Azure 傳統入口網站中啟用 Azure AD 單一登入，並在您的 23 Video 應用程式中設定單一登入。
...
3. 在 [設定應用程式設定] 對話方塊頁面上，執行下列步驟：
    ...
    ![Azure AD 單一登入][8]
     ...
     a.在 [回覆 URL] 文字方塊中，輸入使用者用來登入 23 Video 網站的 URL (例如：*https://britta-simon.23Video.com/saml/login*)。
...
### 指派 Azure AD 測試使用者
本節的目標是要授權 Britta Simon 存取 23 Video，讓她能夠使用 Azure 單一登入。
...
### 測試單一登入
本節的目標是要使用「存取面板」來測試您的 Azure AD 單一登入組態。當您在存取面板中按一下 [23 Video] 磚時，應該會自動登入您的 23 Video 應用程式。
