...
## <a name="enable-the-application-integration-for-freshservice"></a>啟用 FreshService 的應用程式整合
...
5. 在 [欲執行動作] 對話方塊上，按一下 [從資源庫中新增應用程式]。
   ...
   ![從資源庫新增應用程式](./media/active-directory-saas-freshservice-tutorial/IC749322.png "從資源庫新增應用程式")
6. 在**搜尋方塊**中，輸入 **FreshService**。
   
   ![應用程式資源庫](./media/active-directory-saas-freshservice-tutorial/IC790808.png "應用程式資源庫")
...
設定 FreshService 的單一登入需要您從憑證擷取指紋值。 如果您不熟悉這個程序，請參閱 [如何抓取憑證的指紋值](http://youtu.be/YKQF266SAxI)。
...
3. 在 [設定應用程式 URL] 頁面的 [FreshService 單一登入 URL] 文字方塊中，輸入使用者用來登入 Freshdesk 應用程式的 URL (例如：**http://democompany.freshservice.com/**)，然後按 [下一步]。
