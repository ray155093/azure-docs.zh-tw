...
指令碼會列出所有可用的報告，並在 PowerShell 視窗中從 AccountProvisioningEvents 報告傳回 JSON 格式的輸出。它也會建立具有相同輸出的檔案 (JSON、文字和 XML 格式)。您可透過修改指令碼以從其他報告傳回資料來進行實驗，以及取消註解您不需要的輸出格式。
...
- 如需 Azure AD 圖形 API REST 服務的詳細資訊，請參閱 [Azure AD 報告和事件 (預覽)](https://msdn.microsoft.com/library/azure/ad/graph/howto/azure-ad-reports-and-events-preview)。
