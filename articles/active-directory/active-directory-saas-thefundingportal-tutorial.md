<properties
	pageTitle="教學課程：Azure Active Directory 與 The Funding Portal 整合 | Microsoft Azure"
...
4. **[建立 The Funding Portal 測試使用者](#creating-a-the-funding-portal-test-user)** - 使 The Funding Portal 中對應的 Britta Simon 連結到她在 Azure AD 中的代表項目。
...
2. 在傳統入口網站的 [The Funding Portal] 應用程式整合頁面上，按一下 [設定單一登入] 來開啟 [設定單一登入] 對話方塊。

	![設定單一登入][7]

3. 在 [要如何讓使用者登入 The Funding Portal] 頁面上，選取 [Azure AD 單一登入]，然後按 [下一步]。
...
5. 在 [設定在 The Funding Portal 單一登入] 頁面上，按一下 [下載中繼資料]，然後將檔案儲存在您的電腦中。

	![設定單一登入](./media/active-directory-saas-thefundingportal-tutorial/tutorial_thefundingportal_08.png)

6. 若要為您的應用程式設定 SSO，請連絡 The Funding Portal 支援。他們會以適當管道提供協助來讓您設定 SSO。請注意，您必須傳送電子郵件並附加已下載的中繼資料檔案到 info@regenteducation.com。
