...
建立此連結關聯性的方法，就是將 Azure AD 中 [使用者名稱] 的值指派成 Nomadic 中 [Username] (使用者名稱) 的值。
...
在本節中，您會在 Azure 管理入口網站中啟用 Azure AD 單一登入，並在您的 Nomadic 應用程式中設定單一登入。
...
2. 在 [單一登入] 對話方塊頁面上，選取 [SAML 登入] 作為 [模式]，以啟用單一登入。
...
3. 在 [Nomadic 網域與 URL] 區段上，執行下列步驟：
    ...
    a. 在 [登入 URL] 文字方塊中，以下列模式輸入 URL：`https://<company name>.nomadic.fm/signin`
    
    b. 在 [識別碼] 文字方塊中，以下列模式輸入 URL：`https://<company name>.nomadic.fm/auth/saml2/sp`

    > [!NOTE] 
    > 請注意這些不是真正的值。 您必須使用實際的「登入 URL」及「識別碼」來更新這些值。 請連絡 [Nomadic 支援小組](mailto:help@nomadic.fm)以取得這些值。
...
### <a name="creating-a-nomadic-test-user"></a>建立 Nomadic 測試使用者
