<properties
	pageTitle="教學課程：Azure Active Directory 與 Mixpanel 整合 | Microsoft Azure"
...
7. 在結果窗格中，選取 [Mixpanel]，然後按一下 [完成] 加入應用程式。


##  設定並測試 Azure AD 單一登入
本節目標是說明如何以名為 "Britta Simon" 的測試使用者為基礎，設定及測試對 Mixpanel 的 Azure AD 單一登入。

若要讓單一登入運作，Azure AD 必須知道 Mixpanel 與 Azure AD 中互相對應的使用者。換句話說，必須在 Azure AD 使用者和 Mixpanel 中的相關使用者之間建立連結關聯性。

建立此連結關聯性的方法是將 Azure AD 中**使用者名稱**的值指定為 Mixpanel 中 **Username** 的值。
...
本節目標是在 Azure 傳統入口網站啟用 Azure AD 單一登入，並在您的 Mixpanel 應用程式中設定單一登入。
...
3. 在 [設定 App 設定] 對話方塊頁面執行下列步驟：
    ...
    > [AZURE.NOTE] 請在 [https://mixpanel.com/register/](https://mixpanel.com/register/) 註冊，來設定您的登入認證，及連絡 [Mixpanel 支援小組](mailto:support@Mixpanel.com)來為您的租用戶啟用 SSO 設定。如有必要，您也可以向 Mixpanel 支援小組取得您的登入 URL 值。

    b.按 [下一步]。



4. 在 [設定在 Mixpanel 單一登入] 頁面上，執行下列步驟：
...
本節的目標是授與 Britta Simon 對 Mixpanel 的存取權，讓她能夠使用 Azure 單一登入。
