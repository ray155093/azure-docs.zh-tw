<properties
	pageTitle="教學課程：Azure Active Directory 與 Printix 整合 | Microsoft Azure"
...
# 教學課程：Azure Active Directory 與 Printix 整合
...
9. 按一下 [測試] 按鈕，如果測試成功，再按一下 [確定] 按鈕。
	...
	a.按一下 [測試] 按鈕後會顯示 Azure Active Directory 頁面。此處的「測試成功」表示輸入 Azure 測試帳戶的認證後，將會跳出「設定測試正常」訊息。接著按一下 [確定] 按鈕。
...
11. 在傳統入口網站中，選取單一登入設定確認項目，然後按 [下一步]。
...
> [AZURE.NOTE] 如果您需要手動建立使用者，就必須連絡 Printix 支援小組。


### 指派 Azure AD 測試使用者

在本節中，您會把 Printix 的存取權授與 Britta Simon，讓她能夠使用 Azure 單一登入。
