...
若要讓單一登入能夠運作，Azure AD 必須知道 BGS Online 與 Azure AD 中互相對應的使用者。 換句話說，必須建立 Azure AD 使用者和 BGS Online 中相關使用者之間的連結關聯性。

建立此連結關聯性的方法，就是將 Azure AD 中 [使用者名稱] 的值，指派為 BGS Online 中 [使用者名稱] 的值。

若要使用 BGS Online 來設定並測試 Azure AD 單一登入，您需要完成下列建置組塊：
...
在本節中，您會在傳統入口網站中啟用 Azure AD 單一登入，並在您的 BGS Online 應用程式中設定單一登入。

**若要使用 BGS Online 設定 Azure AD 單一登入，請執行下列步驟：**
...
3. 在 [設定 App 設定]  對話方塊頁面執行下列步驟：
    ...
    a. 在 [識別碼] 文字方塊中，輸入您的使用者用來登入 BGS Online 應用程式的 URL。 對於生產環境使用此模式 `https://<company name>.millwardbrown.report`，而對於測試環境使用此模式 `https://millwardbrown.marketingtracker.nl/mt5/`
   
    b. 在 [回覆 URL] 文字方塊中，對於生產環境使用此模式 `https://<company name>.millwardbrown.report/sso/saml/AssertionConsumerService.aspx`，而對於測試環境使用此模式 `https://millwardbrown.marketingtracker.nl/mt5/sso/saml/AssertionConsumerService.aspx`
...
5. 在 [告訴我們這位使用者] 對話方塊頁面上，執行下列步驟： ![建立 Azure AD 測試使用者](./media/active-directory-saas-bgsonline-tutorial/create_aaduser_05.png) 
    ...
    c. 按 [下一步] 。
6. 在 [使用者設定檔] 對話方塊頁面上，執行下列步驟：![建立 Azure AD 測試使用者](./media/active-directory-saas-bgsonline-tutorial/create_aaduser_06.png) 
