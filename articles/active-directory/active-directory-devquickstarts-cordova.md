...
* 針對 iOS 建置和執行
  ...
  * Xcode 5.x 或更新版本。 請從 http://developer.apple.com/downloads 或 [Mac App Store](http://itunes.apple.com/us/app/xcode/id497799835?mt=12) 下載
...
注意：此為**選用步驟**。 本教學課程提供預先佈建的值，您完全不需要在自己的租用戶中佈建，就能看到可運作的範例。 不過，建議您執行這個步驟，並熟悉此程序，因為當您建立自己的應用程式時，就需要這樣做。
...
5. 遵循提示，並建立新的**原生用戶端應用程式** (儘管 Cordova 應用程式是以 HTML 為基礎，我們在此建立原生用戶端應用程式，因此必須選取 `Native Client Application` 選項，否則應用程式無法運作)。 為應用程式輸入易記名稱，然後選取 [原生] 做為 [應用程式類型]。 [重新導向 URI] 是用來傳回應用程式權杖的 URI，例如 `http://MyDirectorySearcherApp`。按一下 [建立] 以建立應用程式。
6. 留在 Azure 入口網站中，選擇您的應用程式，按一下 [設定]，然後選擇 [屬性]。
7. 找到應用程式識別碼值，並將它複製到剪貼簿。
8. 設定應用程式的權限 - 在 [設定] 功能表中，選擇 [必要權限] 區段，依序按一下 [新增] 和 [選取 API]，然後選取 [Windows Azure Active Directory] (這是 AADGraph API)。 然後，按一下 [選取權限]，並選取 [以登入使用者的身分存取目錄]。
...
## <a name="3-add-code-to-authenticate-users-and-obtain-tokens-from-aad"></a>*3.加入程式碼來驗證使用者，並從 AAD 取得權杖*
