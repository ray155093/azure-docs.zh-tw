...
2. 在 [SAML Token 屬性] 對話方塊，針對下表中顯示的每一列執行下列步驟：我們將 Userprincipalname 對應為此處唯一的使用者屬性，但您可以將它對應到可唯一識別組織內某使用者的適當值，並且對應到 Blackboard Learn 使用者名稱欄位。
...
1. 在傳統入口網站的 [Blackboard Learn] 應用程式整合頁面上，按一下 [設定單一登入] 來開啟 [設定單一登入] 對話方塊。
    ...
    ![設定單一登入][6]
2. 在 [要如何讓使用者登入 Blackboard Learn] 頁面上，選取 [Azure AD 單一登入]，然後按一下 [下一步]。
