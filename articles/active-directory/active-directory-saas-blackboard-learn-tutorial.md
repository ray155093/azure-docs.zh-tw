<properties
	pageTitle="教學課程：Azure Active Directory 與 Blackboard Learn 整合 | Microsoft Azure"
...
# 教學課程：Azure Active Directory 與 Blackboard Learn 整合
...
3. **[建立 Blackboard Learn 測試使用者](#creating-a-blackboard-learn-test-user)** - 在 Blackboard Learn 中建立一個 Britta Simon 對應項目，該項目要與 Azure AD 中代表她的項目連結。
...
1. 在 Azure 傳統入口網站中的 [Blackboard Learn] 應用程式整合頁面上，按一下頂端功能表中的 [屬性]。

	![設定單一登入](./media/active-directory-saas-blackboard-learn-tutorial/tutorial_general_80.png)


1. 在 [SAML Token 屬性] 對話方塊，針對下表中顯示的每一列執行下列步驟：我們將 Userprincipalname 對應為此處唯一的使用者屬性，但您可以將它對應到可唯一識別組織內某使用者的適當值，並且對應到 Blackboard Learn 使用者名稱欄位。
...
2. 在傳統入口網站的 [Blackboard Learn] 應用程式整合頁面上，按一下 [設定單一登入] 來開啟 [設定單一登入] 對話方塊。
	 
	![設定單一登入][6]

3. 在 [要如何讓使用者登入 Blackboard Learn] 頁面上，選取 [Azure AD 單一登入]，然後按一下 [下一步]。
...
4. 在 [設定 App 設定] 對話方塊頁面執行下列步驟：
    ...
    a.在 [登入 URL] 文字方塊中，使用下列模式輸入使用者用來登入 Blackboard Learn 應用程式的 URL：**https://\<公司名稱>.blackboard.com/**
	
	b. 按一下 [下一步]
 
5. 在 [設定在 Blackboard Learn 單一登入] 頁面上，執行下列步驟：
...
6. 若要為您的應用程式設定 SSO，請連絡 Blackboard Learn 支援小組，並提供下列資訊：

	• 下載的中繼資料


7. 在傳統入口網站中，選取單一登入設定確認，然後按一下 [下一步]。
...
### 建立 Blackboard Learn 測試使用者

在本節中，您會在 Blackboard Learn 中建立名為 Britta Simon 的使用者。

Blackboard Learn 應用程式支援即時使用者佈建。請確定您已經依**[設定 Azure AD 單一登入](#configuring-azure-ad-single-sign-on)**一節所述，完成設定宣告
...
**若要將 Britta Simon 指派到 Blackboard Learn，請執行下列步驟：**
