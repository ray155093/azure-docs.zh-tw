...
7. 您也應該透過 Metaverse [關注使用者](active-directory-aadconnectsync-service-manager-ui-connectors.md#follow-an-object-and-its-data-through-the-system)一直到 Azure AD 連接器空間。連接器空間物件的輸出規則應該將 [密碼同步] 設為 [True]。在預設組態中，同步規則的名稱是 **Out to AAD - User Join**。
