...
* 您可以讓使用者使用他們的 Azure AD 帳戶自動登入 Moxtra 單一登入 (SSO)
...
本節的目標是要說明如何以名為 "Britta Simon" 的測試使用者為基礎，設定及測試與 Moxtra 搭配運作的 Azure AD SSO。

若要讓 SSO 運作，Azure AD 必須知道 Moxtra 與 Azure AD 中互相對應的使用者。 換句話說，必須在 Azure AD 使用者與 Moxtra 中的相關使用者之間建立連結關聯性。
...
本節的目標是要在 Azure 傳統入口網站中啟用 Azure AD 單一登入，並在您的 Moxtra 應用程式中設定 SSO。 
...
3. 在 [設定應用程式設定]  對話方塊頁面上，執行下列步驟：
  ...
  1. 在 [登入 URL] 文字方塊中，輸入下列 URL：**https://www.moxtra.com/service/#login**。 
  2. 按 [下一步] 。
...
7. 在 [SAML]  頁面上，執行下列步驟：
  ...
  1. 在 [名稱] 文字方塊中，輸入您的設定名稱 (例如：*SAML*)。 
  2. 在 Azure 傳統入口網站的 [設定在 Moxtra 單一登入] 對話方塊頁面上，複製 [實體 ID] 值，然後將它貼至 [IdP 實體 ID] 文字方塊中。 
  3. 在 Azure 傳統入口網站的 [設定在 Moxtra 單一登入] 對話方塊頁面上，複製 [遠端登入 URL] 值，然後將它貼至 [登入 URL] 文字方塊中。 
  4. 在 [AuthnContextClassRef] 文字方塊中，輸入 **urn:oasis:names:tc:SAML:2.0:ac:classes:Password**。 
  5. 在 Azure 傳統入口網站的 [設定在 Moxtra 單一登入] 對話方塊頁面上，複製 [名稱識別碼格式] 值，然後將它貼至 [NameID 格式] 文字方塊中。 
  6. 在記事本中開啟下載的憑證，複製其內容，然後貼到 [憑證] 文字方塊中。    
  7. 在 SAML 電子郵件網域文字方塊中，輸入您的 SAML 電子郵件網域。    
...
9. 在 [單一登入確認] 頁面上，按一下 [完成]。  
  ...
  ![Azure AD 單一登入][11]
19. 若要將自訂屬性對應新增到您的 SAML 權杖屬性組態中，請按一下頂端功能表中的 [屬性] 來開啟 [SAML 權杖屬性] 對話方塊。 
   
  ![設定單一登入](./media/active-directory-saas-moxtra-tutorial/tutorial_general_80.png) 
11. 針對下表中的每個資料列，執行下列步驟：
  ...
  2. 在 [新增使用者屬性] 對話方塊中，輸入屬性名稱以及表格中針對該資料列顯示的屬性值。 
...
### <a name="create-a-moxtra-test-user"></a>建立 Moxtra 測試使用者
...
3. 在 [加入使用者]  對話方塊中，執行下列步驟：
  ...
  4. 在 [事業處] 文字方塊中，輸入 **Dev**。
  5. 在 [部門] 文字方塊中，輸入 **IT**。
  6. 選取 [系統管理員]。
  7. 按一下 [新增] 。

### <a name="assign-the-azure-ad-test-user"></a>指派 Azure AD 測試使用者
本節的目標是授與 Britta Simon 對 Moxtra 的存取權，使她能夠使用 Azure SSO。
