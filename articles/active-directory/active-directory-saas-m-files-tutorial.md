...
本節的目標，是要在 Azure 傳統入口網站中啟用 Azure AD 單一登入，並在您的 M-Files 應用程式中設定單一登入。
...
3. 在 [設定 App 設定]  對話方塊頁面執行下列步驟：
    ...
    a. 在 [登入 URL] 文字方塊中，以下列模式輸入 URL：`https://<tenant-name>.cloudvault.m-files.com/authentication/MFiles.AuthenticationProviders.Core/sso`。

    b. 按 [下一步] 。


1. 在 [設定在 M-Files 單一登入] 頁面上，按一下 [下載中繼資料]，然後將檔案儲存在您的電腦中。
   
    ![設定單一登入](./media/active-directory-saas-m-files-tutorial/tutorial_m_files_09.png)
2. 若要取得為您的應用程式設定的 SSO，請透過 <mailto:support@m-files.com> 連絡您的 M-files 支援小組並提供下載的中繼資料。
   
   > [!NOTE]
   > 如果您想為您的 M-File 傳統型應用程式設定 SSO，請依照下列步驟執行。 如果您只想為 M-Files 的 Web 版本設定 SSO，則不需要任何額外步驟。 
   > 
   > 
3. 請依照下列步驟設定 M-File 傳統型應用程式，以搭配 Azure AD 啟用 SSO。 若要下載 M-Files，請移至 [M-Files 下載](https://www.m-files.com/en/download-latest-version)頁面。
4. 開啟 [M-Files 桌面設定] 視窗。 然後按一下 [新增]。
   
    ![設定單一登入](./media/active-directory-saas-m-files-tutorial/tutorial_m_files_10.png)
5. 在 [文件保存庫連線內容] 視窗中，執行下列步驟︰
    ...
    d. 在 [驗證] 欄位中，選取 [特定的 Windows 使用者]。 然後，系統會提示您簽署頁面。 請插入您的 Azure AD 認證。
