...
* 您可以讓使用者使用他們的 Azure AD 帳戶自動登入 Weekdone 單一登入 (SSO)
...
本節的目標是要說明如何以名為 "Britta Simon" 的測試使用者為基礎，設定及測試與 Weekdone 搭配運作的 Azure AD SSO。

若要讓 SSO 運作，Azure AD 必須知道 Weekdone 中與 Azure AD 互相對應的使用者。 換句話說，必須在 Azure AD 使用者和 Weekdone 中的相關使用者之間建立連結關聯性。

建立此連結關聯性的方法是將 Azure AD 中**使用者名稱**的值指定為 Weekdone 中 **Username** 的值。

若要設定及測試與 Weekdone 搭配運作的 Azure AD SSO，您需要完成下列建置組塊：
...
本節的目標是要在 Azure 傳統入口網站中啟用 Azure AD 單一登入，並在您的 Weekdone 應用程式中設定 SSO。

**若要設定與 Weekdone 搭配運作的 Azure AD SSO，請執行下列步驟：**
...
3. 若要為您的應用程式設定 SSO，請透過 hello@weekdone.com 連絡您的 Weekdone 支援小組。 
4. 將下載的憑證檔案附加至您的郵件，並與 Weekdone 小組分享中繼資料 URL (簽發者 URL、SAML SSO URL 和單一登出服務 URL)，以在其端設定 SSO。
...
### <a name="create-a-weekdone-test-user"></a>建立 Weekdone 測試使用者
...
本節的目標是將對 Weekdone 的存取權授與 Britta Simon，使她能夠使用 Azure SSO。
