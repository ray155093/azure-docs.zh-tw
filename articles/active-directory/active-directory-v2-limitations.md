...
這是 login.contoso.com 的 DNS 子網域。  如果您希望應用程式以 login-east.contoso.com 和 login-west.contoso.com 做為 redirect_uri，您必須依序新增下列 redirect_uri︰

`https://contoso.com`  
`https://login-east.contoso.com`  
`https://login-west.contoso.com`  

可以新增後面兩個，因為它們是第一個 redirect_uri (contoso.com) 的子網域。 即將推出的版本將會移除這項限制。
...
## <a name="restrictions-for-work-&-school-accounts"></a>工作和學校帳戶的限制
v2.0 端點尚未支援 Microsoft 組織/企業使用者特有的一些功能。

##### <a name="device-based-conditional-access,-native-and-mobile-apps,-and-the-microsoft-graph"></a>裝置型條件式存取、原生和行動應用程式以及 Microsoft Graph
