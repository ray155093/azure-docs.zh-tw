...
* 您可以讓使用者使用他們的 Azure AD 帳戶自動登入 Halogen Software 單一登入 (SSO)
...
本節的目標是說明如何以名為 "Britta Simon" 的測試使用者為基礎，使用 Halogen Software 來設定及測試 Azure AD SSO。

若要讓 SSO 運作，Azure AD 必須知道 Halogen Software 與 Azure AD 中互相對應的使用者。 換句話說，Azure AD 使用者和 Halogen Software 中的相關使用者之間必須建立連結關聯性。

建立此連結關聯性的方法，是將 Azure AD 中**使用者名稱**的值，指派為 Halogen Software 中 **Username** 的值。

若要使用 Halogen Software 設定並測試 Azure AD SSO，您需要完成下列建置組塊：
...
本節的目標是要在 Azure 傳統入口網站中啟用 Azure AD SSO，並在您的 Halogen Software 應用程式中設定 SSO。

**若要使用 Halogen Software 設定 Azure AD SSO，請執行下列步驟：**
...
3. 在 [設定應用程式設定]  對話方塊頁面上，執行下列步驟：
    ...
    ![設定 App 設定][10]
   1. 在 [登入 URL] 文字方塊中，使用以下模式輸入使用者登入您的 Halogen Software 應用程式時所使用的 URL：*https://global.hgncloud.com/fabrikam/welcome.jsp*
   2. 按 [下一步] 。
...
8. 在 [SAML 組態]  頁面上，執行下列步驟： 

    ![何謂 Azure AD Connect][14]
  1. 對於 [唯一識別碼]，選取 [NameID]。
  2. 對於 [唯一識別碼對應到]，選取 [Username]。
  3. 若要更新您已下載的中繼資料檔，請按一下 [瀏覽] 來選取檔案，然後按一下 [上傳檔案]。
  4. 若要測試組態，請按一下 [執行測試]。 
    >[!NOTE]
...
### <a name="create-a-halogen-software-test-user"></a>建立 Halogen Software 測試使用者
...
3. 在 [新增使用者]  對話頁面上，執行下列步驟：
  ...
  3. 在 [使用者名稱] 文字方塊中輸入 **Brita Simon 在 Azure 傳統入口網站的使用者名稱**。
  4. 在 [密碼] 文字方塊中輸入 Britta 的密碼。
  5. 按一下 [儲存] 。

### <a name="assign-the-azure-ad-test-user"></a>指派 Azure AD 測試使用者
本節的目標是授與 Britta Simon 對 Halogen Software 的存取權，使她能夠使用 Azure SSO。
