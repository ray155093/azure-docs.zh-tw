...
7. 在結果窗格中，選取 [Halogen Software]，然後按一下 [完成] 加入應用程式。
...
3. 在 [設定應用程式設定] 對話方塊頁面上，執行下列步驟：![設定 App 設定][10]
 
     a. 在 [登入 URL] 文字方塊中，使用下列模式輸入使用者用來登入 Halogen Software 應用程式的 URL：**https://global.hgncloud.com/fabrikam/welcome.jsp*
...
8. 在 [SAML 組態] 頁面上，執行下列步驟：![何謂 Azure AD Connect][14]
    ...
    a.在 [唯一識別碼] 中，選取 [NameID]。

    b.在 [唯一識別碼對應到] 中，選取 [Username]。

    c.若要更新您已下載的中繼資料檔，請按一下 [瀏覽] 來選取檔案，然後按一下 [上傳檔案]。

    d.若要測試組態，請按一下 [執行測試]。

    > [AZURE.NOTE] 您需要等候「SAML 測試已完成。請關閉此視窗。」訊息顯示。然後關閉已開啟的瀏覽器視窗。完成測試之後才會啟用 [啟用 SAML] 核取方塊。
