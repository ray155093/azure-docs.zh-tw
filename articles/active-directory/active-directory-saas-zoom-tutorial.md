...
完成本教學課程之後，您指派給 Zoom 的 Azure AD 使用者就能夠單一登入您 Zoom 公司網站 (服務提供者起始登入) 的應用程式，或是使用[存取面板簡介](active-directory-saas-access-panel-introduction.md)進行單一登入。
...
6. 在**搜尋方塊**中，輸入 **Zoom**。
   ...
   ![應用程式庫](./media/active-directory-saas-zoom-tutorial/IC784694.png "應用程式庫")
7. 在結果窗格中，選取 Zoom，然後按一下 [完成] 以加入應用程式。
   
   ![Zoom](./media/active-directory-saas-zoom-tutorial/IC784695.png "Zoom")

## 設定單一登入
本節的目的是要說明如何依據 SAML 通訊協定來使用同盟，讓使用者能夠用自己的 Azure AD 帳戶在 Zoom 中進行驗證。在此程序中，您必須建立 Base-64 編碼的憑證檔案。如果您不熟悉此程序，請參閱[如何將二進位憑證轉換成文字檔](http://youtu.be/PlgrzUZ-Y1o)。
