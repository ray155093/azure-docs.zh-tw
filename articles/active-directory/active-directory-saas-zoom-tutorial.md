...
完成本教學課程之後，您指派給 Zoom 的 Azure AD 使用者就能夠單一登入您 Zoom 公司網站 (服務提供者起始登入) 的應用程式，或是使用 [存取面板簡介](active-directory-saas-access-panel-introduction.md)
...
## <a name="enabling-the-application-integration-for-zoom"></a>啟用 Zoom 的應用程式整合
本節的目的是要說明如何啟用 Zoom 的應用程式整合。

### <a name="to-enable-the-application-integration-for-zoom-perform-the-following-steps"></a>若要啟用 Zoom 的應用程式整合，請執行下列步驟：
...
5. 在 [欲執行動作] 對話方塊上，按一下 [從資源庫中新增應用程式]。
   ...
   ![從資源庫新增應用程式](./media/active-directory-saas-zoom-tutorial/IC749322.png "從資源庫新增應用程式")
6. 在**搜尋方塊**中，輸入 **Zoom**。
...
1. 在 Azure 傳統入口網站的 [Zoom] 應用程式整合頁面上，按一下 [設定單一登入] 來開啟 [設定單一登入] 對話方塊。
...
8. 在 [單一登入] 區段中，執行下列步驟：
   ...
   1. 在 Azure 傳統入口網站的 [設定在 Zoom 單一登入] 對話方塊頁面上，複製 [單一登入服務 URL] 值，然後貼至 [單一登入 URL] 文字方塊中。
   2. 在 Azure 傳統入口網站的 [設定在 Zoom 單一登入] 對話方塊頁面上，複製 [單一登出服務 URL] 值，然後貼至 [單一登出 URL] 文字方塊中。
   ...
   5. 在 Azure 傳統入口網站的 [設定在 Zoom 單一登入] 對話方塊頁面上，複製 [簽發者 URL] 值，然後貼至 [簽發者] 文字方塊中。
   6. 按一下 [儲存] 。
...
> [!NOTE]
> 您可以使用任何其他的 Zoom 使用者帳戶建立工具或 Zoom 提供的 API，佈建 AAD 使用者帳戶。
> 
> 
...
### <a name="to-assign-users-to-zoom-perform-the-following-steps"></a>若要指派使用者給 Zoom，請執行下列步驟：
