...
6. 如果您想要以 **IDP** 起始模式設定應用程式，請在 Azure 入口網站的 [LinkedIn Lookup 網域和 URL] 區段底下執行下列步驟：
...
7. 如果您想要以 **SP** 起始模式設定應用程式，請勾選 [顯示進階 URL 設定]：
    ...
    > [!NOTE] 
    > 請注意，這不是真正的值。 使用者必須以實際的登入 URL 將這些值更新。 請連絡 [LinkedIn Lookup 用戶端支援小組](https://business.LinkedIn.com/lookup)以取得此值。
...
9. 在 [使用者屬性] 區段中，按一下 [檢視及編輯所有其他使用者屬性]，然後設定屬性。 使用者必須新增名為**部門**的另一個宣告，且值要與 **user.department** 對應。
