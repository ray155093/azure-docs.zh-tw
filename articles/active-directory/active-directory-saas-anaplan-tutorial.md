...
本教學課程旨在說明如何整合 Anaplan 與 Azure Active Directory (Azure AD)。

Anaplan 與 Azure AD 整合提供下列優點：

* 您可以在 Azure AD 中控制可存取 Anaplan 的人員
* 您可以讓使用者使用其 Azure AD 帳戶自動登入 Anaplan 單一登入 (SSO)
...
## <a name="add-anaplan-from-the-gallery"></a>從資源庫新增 Anaplan
若要設定 Anaplan 與 Azure AD 整合，您需要從資源庫將 Anaplan 加入受管理的 SaaS 應用程式清單中。
...
7. 在結果窗格中，選取 [Anaplan]，然後按一下 [完成] 以新增應用程式。
   
    ![選取資源庫中的應用程式](./media/active-directory-saas-anaplan-tutorial/tutorial_anaplan_001.png)

## <a name="configure-and-test-azure-ad-sso"></a>設定並測試 Azure AD SSO
本節的目標是要說明如何以名為 "Britta Simon" 的測試使用者為基礎，設定及測試與 Anaplan 搭配運作的 Azure AD SSO。

若要讓 SSO 運作，Azure AD 必須知道 Anaplan 與 Azure AD 中互相對應的使用者。 換句話說，必須建立 Azure AD 使用者和 Anaplan 中相關使用者之間的連結關聯性。

建立此連結關聯性的方法，是將 Azure AD 中**使用者名稱**的值，指派為 Anaplan 中 **Username** 的值。

若要設定及測試與 Anaplan 搭配運作的 Azure AD SSO，您需要完成下列建置組塊：

1. **[設定 Azure AD 單一登入](#configuring-azure-ad-single-single-sign-on)** - 讓您的使用者能夠使用此功能。
2. **[建立 Azure AD 測試使用者](#creating-an-azure-ad-test-user)** - 使用 Britta Simon 測試 Azure AD 單一登入。
3. **[建立 Anaplan 測試使用者](#creating-a-anaplan-test-user)** - 讓 Britta Simon 在 Anaplan 中有一個對應身分連結到她的 Azure AD 代表身分。
4. **[指派 Azure AD 測試使用者](#assigning-the-azure-ad-test-user)** - 讓 Britta Simon 能夠使用 Azure AD 單一登入。
5. **[測試單一登入](#testing-single-sign-on)** - 驗證組態是否能運作。

### <a name="configure-azure-ad-sso"></a>設定 Azure AD SSO
在本節中，您會在傳統入口網站中啟用 Azure AD SSO，並在您的 Anaplan 應用程式中設定 SSO。

**若要設定與 Anaplan 搭配運作的 Azure AD SSO，請執行下列步驟：**

1. 在傳統入口網站的 **Anaplan** 應用程式整合頁面上，按一下 [設定單一登入] 來開啟 [設定單一登入] 對話方塊。
   
    ![設定單一登入][6] 
2. 在 [您希望使用者如何登入 Anaplan] 頁面上，選取 [Azure AD 單一登入]，然後按 [下一步]。
...
3. 在 [設定應用程式設定] 對話方塊頁面上，執行下列步驟，然後按 [下一步]：
  ...
  1. 在 [登入 URL] 文字方塊中，以下列模式輸入 URL：`https://sdp.anaplan.com/frontdoor/saml/<tenant name>`
  2. 按 [下一步] 。

    > [!NOTE] 
    > 本教學課程中的 [登入 URL] 值只是一個預留位置。 若要取得您環境的實際值，請連絡 Anaplan 支援小組。
    > 
4. 在 [設定在 Anaplan 單一登入] 頁面上，執行下列步驟，然後按 [下一步]：
   
    ![設定單一登入](./media/active-directory-saas-anaplan-tutorial/tutorial_anaplan_05.png)  
  1. 按一下 [下載中繼資料]，然後將檔案儲存在您的電腦上。
  2. 按 [下一步] 。
5. 若要取得為您的應用程式設定的 SSO，請透過 [support@anaplan.com](mailto:support@anaplan.com) 連絡您的 Anaplan 支援小組並提供下列資訊：
...
### <a name="create-a-anaplan-test-user"></a>建立 Anaplan 測試使用者
在本節中，您要在 Anaplan 中建立名為 Britta Simon 的使用者。 請透過 <mailto:support@anaplan.com> 來與 Anaplan 支援小組合作，在 Anaplan 平台中新增使用者。

### <a name="assign-the-azure-ad-test-user"></a>指派 Azure AD 測試使用者
本節的目標是授與 Britta Simon 對 Anaplan 的存取權，使她能夠使用 Azure SSO。

![指派使用者][200]

**若要將 Britta Simon 指派到 Anaplan，請執行下列步驟：**
...
當您在存取面板中按一下 Anaplan 圖格時，應該會自動登入您的 Anaplan 應用程式。
