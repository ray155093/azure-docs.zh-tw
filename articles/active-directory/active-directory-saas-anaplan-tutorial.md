<properties
    pageTitle="教學課程：Azure Active Directory 與 Anaplan 整合 | Microsoft Azure"
...
## <a name="adding-anaplan-from-the-gallery"></a>從資源庫加入 Anaplan
...
本節的目標是要說明如何以名為 "Britta Simon" 的測試使用者為基礎，設定及測試與 Anaplan 搭配運作的 Azure AD 單一登入。

若要讓單一登入運作，Azure AD 必須知道 Anaplan 與 Azure AD 中互相對應的使用者。 換句話說，必須建立 Azure AD 使用者和 Anaplan 中相關使用者之間的連結關聯性。
...
在本節中，您會在傳統入口網站中啟用 Azure AD 單一登入，並在您的 Anaplan 應用程式中設定單一登入。
...
3. 在 [設定應用程式設定] 對話方塊頁面上，執行下列步驟，然後按 [下一步]：
    ...
    a. 在 [登入 URL] 文字方塊中，以下列模式輸入 URL： `https://sdp.anaplan.com/frontdoor/saml/<tenant name>`

    b. 依序按一下 [ **下一步**


    > [AZURE.NOTE] 本教學課程中的 [登入 URL] 值只是一個預留位置。 若要取得您環境的實際值，請連絡 Anaplan 支援小組。
...
本節的目標是要授與 Britta Simon 對 Anaplan 的存取權，讓她能夠使用 Azure 單一登入。
