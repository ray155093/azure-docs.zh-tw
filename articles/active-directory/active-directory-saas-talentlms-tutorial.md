...
## <a name="enabling-the-application-integration-for-talentlms"></a>啟用 TalentLMS 的應用程式整合
本節的目的是要說明如何啟用 TalentLMS 的應用程式整合。

### <a name="to-enable-the-application-integration-for-talentlms-perform-the-following-steps"></a>若要啟用 TalentLMS 的應用程式整合，請執行下列步驟：
...
8. 在 [單一登入] 區段中，執行下列步驟：
    ...
    b.這是另一個 C# 主控台應用程式。 在 Azure 傳統入口網站的 [設定在 TalentLMS 單一登入] 對話方塊頁面上，複製 [識別提供者 ID] 值，然後將它貼到 [識別提供者 (IdP)] 文字方塊中。
   
    c. 從匯出的憑證複製 [指紋] 值，然後將它貼入 [憑證指紋] 文字方塊。
    ...
    d. 在 Azure 傳統入口網站的 [設定在 TalentLMS 單一登入] 對話方塊頁面上，複製 [遠端登入 URL] 值，然後將它貼到 [遠端登入 URL] 文字方塊中。
   
    e. 在 Azure 傳統入口網站的 [設定在 TalentLMS 單一登入] 對話方塊頁面上，複製 [遠端登出 URL] 值，然後將它貼到 [遠端登出 URL] 文字方塊中。
   
    f. 在 [TargetedID] 文字方塊中，輸入 **http://schemas.xmlsoap.org/ws/2005/05/identity/claims/name**。
   
    g. 在 [名字] 文字方塊中，輸入 **http://schemas.xmlsoap.org/ws/2005/05/identity/claims/givenname**。
   
    h. 在 [姓氏] 文字方塊中，輸入 **http://schemas.xmlsoap.org/ws/2005/05/identity/claims/surname**。
   
    i. 在 [電子郵件] 文字方塊中，輸入 **http://schemas.xmlsoap.org/ws/2005/05/identity/claims/emailaddress**。
...
3. 在 [加入使用者]  對話頁面上，執行下列步驟：
    ...
    a. 在下列文字方塊中輸入該 Azure AD 使用者帳戶的相關屬性值：**名字**、**姓氏**、**電子郵件地址**。
...
### <a name="to-assign-users-to-talentlms-perform-the-following-steps"></a>若要指派使用者給 TalentLMS，請執行下列步驟：
