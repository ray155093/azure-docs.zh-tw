...
本教學課程旨在說明如何將 Front 與 Azure Active Directory (Azure AD) 整合。

將 Front 與 Azure AD 整合提供下列優點：

* 您可以在 Azure AD 中控制可存取 Front 的人員
* 您可以讓使用者使用他們的 Azure AD 帳戶自動登入 Front 單一登入 (SSO)
...
7. 在結果窗格中，選取 [Front]，然後按一下 [完成] 來新增應用程式。
   
    ![選取資源庫中的應用程式](./media/active-directory-saas-front-tutorial/tutorial_front_0001.png)

## <a name="configure-and-test-azure-ad-sso"></a>設定並測試 Azure AD SSO
本節的目標是要說明如何以名為 "Britta Simon" 的測試使用者為基礎，設定及測試與 Front 搭配運作的 Azure AD SSO。

若要讓 SSO 運作，Azure AD 必須知道 Front 與 Azure AD 中互相對應的使用者。 換句話說，必須在 Azure AD 使用者與 Front 中的相關使用者之間的連結關聯性。

建立此連結關聯性的方法，就是將 Azure AD 中**使用者名稱**的值指派為 Front 中 **Username** 的值。

若要設定及測試與 Front 搭配運作的 Azure AD SSO，您需要完成下列構成要素：

1. **[設定 Azure AD 單一登入](#configuring-azure-ad-single-single-sign-on)** - 讓您的使用者能夠使用此功能。
2. **[建立 Azure AD 測試使用者](#creating-an-azure-ad-test-user)** - 使用 Britta Simon 測試 Azure AD 單一登入。
3. **[建立 Front 測試使用者](#creating-a-front-test-user)** - 在 Front 中建立 Britta Simon 的對應項目，且該項目與 Azure AD 中代表 Britta Simon 的項目連結。
4. **[指派 Azure AD 測試使用者](#assigning-the-azure-ad-test-user)** - 讓 Britta Simon 能夠使用 Azure AD 單一登入。
5. **[測試單一登入](#testing-single-sign-on)** - 驗證組態是否能運作。

### <a name="configuring-azure-ad-sso"></a>設定 Azure AD SSO
在本節中，您會在傳統入口網站中啟用 Azure AD SSO，然後在您的 Front 中設定單一登入。

**若要設定與 Front 搭配運作的 Azure AD SSO，請執行下列步驟：**

1. 在傳統入口網站的 [Front] 應用程式整合頁面上，按一下 [設定單一登入] 來開啟 [設定單一登入] 對話方塊。
   
    ![設定單一登入][6] 
2. 在 [要如何讓使用者登入 Front] 頁面上，選取 [Azure AD 單一登入]，然後按 [下一步]。
...
3. 在 [設定應用程式設定] 對話方塊頁面上，如果您想要以 **IDP 起始模式**設定應用程式，請執行下列步驟，然後按 [下一步]：
  ...
  1. 在 [識別碼] 文字方塊中，以下列模式輸入 URL：`https://<company name>.frontapp.com`。
  2. 在 [回覆 URL] 文字方塊中，以下列模式輸入 URL：`https://<company name>.frontapp.com/sso/saml/callback`
  3. 按 [下一步] 。
4. 如果您想要在 [設定應用程式設定] 對話方塊頁面上以 **SP 起始模式**設定應用程式，則請按一下 [顯示進階設定 (選擇性)]，然後輸入**登入 URL** 並按 [下一步]。
  ...
  1. 在 [登入 URL] 文字方塊中，以下列模式輸入 URL：`https://<company name>.frontapp.com`
  2. 按 [下一步] 。
   
   >[!NOTE]
   >請注意這些不是真正的值。 您必須使用實際的「單一登入 URL」、「識別碼」及「回覆 URL」來更新這些值。 若要取得這些值，您可以參考**步驟 12** 以取得詳細資訊，或透過 [support@frontapp.com](emailTo:support@frontapp.com) 連絡 Front。
   >  
5. 在 [設定在 Front 單一登入] 頁面上，執行下列步驟，然後按 [下一步]：
   
 ![設定單一登入](./media/active-directory-saas-front-tutorial/tutorial_front_06.png) 
 1. 按一下 [下載憑證]，然後將檔案儲存在您的電腦上。
 2. 按 [下一步] 。
6. 以系統管理員身分登入 Front 租用戶。
7. 移至 [設定] \(左側資訊看板底部的齒輪圖示) > [喜好設定]。
...
本節的目標是要在 Front 中建立一個名為 Britta Simon 的使用者。請與 Front 支援小組合作以在 Front 帳戶中新增使用者。

### <a name="assign-the-azure-ad-test-user"></a>指派 Azure AD 測試使用者
本節的目標是授與 Britta Simon 對 Front 的存取權，使她能夠使用 Azure SSO。
