...
* 您可以讓使用者透過他們的 Azure AD 帳戶，使用單一登入 (SSO) 自動登入 Allocadia
...
## <a name="add-allocadia-from-the-gallery"></a>從資源庫新增 Allocadia
...
在本節中，您會以名為 "Britta Simon" 的測試使用者身分，設定及測試與 Allocadia 搭配運作的 Azure AD SSO。

若要讓 SSO 運作，Azure AD 必須知道 Allocadia 與 Azure AD 中互相對應的使用者。 換句話說，必須在 Azure AD 使用者與 Allocadia 中的相關使用者之間建立連結關聯性。

建立此連結關聯性的方法，就是指派 Azure AD 中**使用者名稱**的值做為 Allocadia 中 **Username** 的值。

若要設定及測試與 Allocadia 搭配運作的 Azure AD SSO，您需要完成下列建置組塊：

1. **[設定 Azure AD 單一登入](#configuring-azure-ad-single-single-sign-on)** - 讓您的使用者能夠使用此功能。
2. **[建立 Azure AD 測試使用者](#creating-an-azure-ad-test-user)** - 使用 Britta Simon 測試 Azure AD 單一登入。
3. **[建立 Allocadia 測試使用者](#creating-an-allocadia-test-user)** - 在 Allocadia 中建立一個與 Azure AD 中代表 Britta Simon 的項目連結的 Britta Simon 對應項目。
4. **[指派 Azure AD 測試使用者](#assigning-the-azure-ad-test-user)** - 讓 Britta Simon 能夠使用 Azure AD 單一登入。
5. **[測試單一登入](#testing-single-sign-on)** - 驗證組態是否能運作。

### <a name="configure-azure-ad-single-sign-on"></a>設定 Azure AD 單一登入
在本節中，您會在傳統入口網站中啟用 Azure AD SSO，並在您的 Allocadia 應用程式中設定 SSO。
...
5. 在 [設定應用程式設定]  對話方塊頁面上，執行下列步驟：
  ...
  1. 在 [識別碼] 方塊中，以下列模式輸入 URL：如果是測試環境，使用像是 **"https://na2standby.allocadia.com"** 的 URL，如果是生產環境，則使用 **"https://na2.allocadia.com"**。
  2. 在 [回覆 URL] 中，以下列模式輸入 URL：如果是測試環境，請使用像 **"https://na2standby.allocadia.com/allocadia/saml/SSO"** 的 URL 模式，如果是生產環境，則請使用 **"https://na2.allocadia.com/allocadia/saml/SSO"**
...
7. 若要為您的應用程式設定 SSO，請連絡 [Allocadia 支援](mailTo:support@allocadia.com) 小組，他們將會協助您設定 SSO。 請注意，您必須傳送電子郵件並附加已下載的中繼資料檔案，才能在 Allocadia 端設定 SSO。
   ...
   >[!NOTE]
   >請確定 Allocadia 小組將測試環境中的「識別碼」值設定為 **"https://na2standby.allocadia.com"**，如果是生產環境，則應該是：**"https://na2.allocadia.com"**
   >  
...
### <a name="create-an-allocadia-test-user"></a>建立 Allocadia 測試使用者
...
>[!NOTE]
>如果您需要手動建立使用者或需要建立使用者批次，則必須連絡 Allocadia 支援小組。 
> 

### <a name="assign-the-azure-ad-test-user"></a>指派 Azure AD 測試使用者
在本節中，您會將 Allocadia 的存取權授與 Britta Simon，讓她能夠使用 Azure SSO。
