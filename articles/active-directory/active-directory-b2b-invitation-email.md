...
來自您租用戶的邀請電子郵件會使用您可能已為租用戶設定的公司商標。 如果您想要利用這項功能，以下是有關如何設定的詳細資料。 橫幅標誌將會顯示在電子郵件中。 請依照這裡的影像大小和品質指示來獲得最佳效果。 此外，公司名稱也會顯示在對動作的呼叫中。
...
### <a name="body"></a>內文
這會包含邀請者在 UX 中輸入或透過邀請 API 傳遞的訊息。 這只是一個簡單的文字方塊，基於安全考量，將不會處理 html 標記。
