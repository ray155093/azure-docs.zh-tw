...
如果這是您第一次使用 Azure AD 應用程式 Proxy，建議您在發佈應用程式前先從私人網路發佈網站，進而測試連接器。
...
5. 提供您的應用程式的下列資訊：
	...
	- **預先驗證方法**︰應用程式 Proxy 在給予您的應用程式存取權前，將用來驗證使用者的方法。從下拉式功能表選擇其中一個選項。
...
為了讓使用者存取您發佈的應用程式，您必須以個別或群組方式指派他們。對於需要預先驗證的應用程式，這會授與使用該應用程式的權限。對於不需要預先驗證的應用程式，使用者不需要權限，但仍需要被指派至該應用程式，該應用程式才會出現在其應用程式清單中。
