<properties
	pageTitle="教學課程：Azure Active Directory 與 Predictix Assortment Planning 整合 | Microsoft Azure"
...
# 教學課程：Azure Active Directory 與 Predictix Assortment Planning 整合
...
7. 在結果窗格中，選取 [Predictix Assortment Planning]，然後按一下 [完成] 來加入應用程式。
...
3. 在 [設定應用程式設定] 對話方塊頁面上，執行下列步驟：
    ...
    a.在 [登入 URL] 文字方塊中，使用以下模式輸入您使用者用以登入 Predictix Assortment Planning 應用程式的 URL：**https://\<公司名稱定價>.ap.predictix.com/sso/request**。
...
### 建立 Predictix Assortment Planning 測試使用者
