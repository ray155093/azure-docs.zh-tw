...
若要設定 Panopto 與 Azure AD 整合，您需要從資源庫將 Panopto 新增到受管理的 SaaS 應用程式清單。
...
6. 在 [Panopto 組態] 區段上，按一下 [設定 Panopto] 以開啟 [設定登入] 視窗。 從 [快速參考] 區段中複製 [登出 URL、SAML 實體識別碼和 SAML 單一登入服務 URL]。
...
10. 在 [SAML 提供者] 區段中，執行下列步驟：
    ...
    b.這是另一個 C# 主控台應用程式。 在 [執行個體名稱]  文字方塊中，輸入執行個體的名稱。
