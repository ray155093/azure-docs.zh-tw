...
活動記錄警示是在新的活動記錄事件符合警示中指定的條件時啟動的警示。 它們是 Azure 資源，因此可使用 Azure 入口網站或 Azure Resource Manager (ARM) 加以管理。 本文將說明如何使用 Azure 入口網站來設定活動記錄事件的警示。
...
您可以在[這裡](monitoring-activity-log-alerts-on-service-notifications.md)了解服務健康狀態通知的活動記錄警示
...
10. 然後，透過提供動作的下列各項來定義動作的清單
    ...
    b.這是另一個 C# 主控台應用程式。 **動作類型：**選擇動作類型：簡訊、電子郵件或 Webhook
