# 概觀
## [什麼是 Azure Stack？](azure-stack-poc.md)
## [有哪些新功能？](azure-stack-whats-new.md)
## [重要功能與概念](azure-stack-key-features.md)
## [POC 架構](azure-stack-architecture.md)

# 開始使用
## [前期工作](azure-stack-first-scenarios.md)
## [佈建虛擬機器](azure-stack-provision-vm.md)
## [建立儲存體帳戶](azure-stack-provision-storage-account.md)
## [使用 CLI 連線](azure-stack-connect-cli.md)
## [使用 PowerShell 連線](azure-stack-connect-powershell.md)
## 部署 Azure Stack POC
### [部署必要條件](azure-stack-deploy.md)
### [部署](azure-stack-run-powershell-script.md)
### [連接至 Azure Stack POC](azure-stack-connect-azure-stack.md)
### [重新部署 Azure Stack](azure-stack-redeploy.md)
## [常見問題集](azure-stack-faq.md)

# 作法
## [管理使用者權限](azure-stack-manage-permissions.md)
## [新增 Azure Stack 租用戶](azure-stack-add-new-user-aad.md)
## 為租用戶提供服務
### [設定配額](azure-stack-setting-quotas.md)
### [建立方案](azure-stack-create-plan.md)
### [建立優惠](azure-stack-create-offer.md)
### [訂閱提供項目](azure-stack-subscribe-plan-provision-vm.md)
### [在 Azure Stack 中委派提供項目](azure-stack-delegated-provider.md)
## 部署範本
### [範本概觀](azure-stack-arm-templates.md)
### [使用入口網站部署範本](azure-stack-deploy-template-portal.md)
### [使用 PowerShell 部署範本](azure-stack-deploy-template-powershell.md)
### [使用 Visual Studio 部署範本](azure-stack-deploy-template-visual-studio.md)
### [使用命令列部署範本](azure-stack-deploy-template-command-line.md)
## 規劃和管理儲存體
### [概觀](azure-stack-storage-overview.md)
### [管理儲存體帳戶](azure-stack-manage-storage-accounts.md)
### [差異與注意事項](azure-stack-acs-differences-tp2.md)
## 規劃和管理網路
### [適用於 Azure Stack 的 iDNS](azure-stack-understanding-dns-in-tp2.md)
### [了解站對站 VPN 連線](azure-stack-create-vpn-connection-one-node-tp2.md)
### [檢視公用 IP 位址使用狀況](azure-stack-viewing-public-ip-address-consumption-in-tp2.md)
## 將內容新增到 Azure Stack
### [Marketplace 概觀](azure-stack-marketplace.md)
### [建立及發佈 Marketplace 項目](azure-stack-create-and-publish-marketplace-item.md)
### [新增自訂虛擬機器映像](azure-stack-add-vm-image.md)
## [管理更新](azure-stack-updates.md)
## 使用金鑰保存庫管理安全值
### [簡介](azure-stack-kv-intro.md)
### [開始使用](azure-stack-kv-getting-started.md)
### [使用金鑰保存庫密碼部署虛擬機器](azure-stack-kv-deploy-vm-with-secret.md)
### [使用金鑰保存庫憑證建立虛擬機器](azure-stack-kv-push-secret-into-vm.md)
### [金鑰保存庫範例應用程式](azure-stack-kv-sample-app.md)
## 追蹤計費與退款的使用情況
### [計費與退款概觀](azure-stack-billing-and-chargeback.md)
### [提供者資源使用狀況 API](azure-stack-provider-resource-api.md)
### [租用戶資源使用狀況 API](azure-stack-tenant-resource-usage-api.md)
### [使用狀況常見問題集](azure-stack-usage-related-faq.md)
## [部署 Linux 虛擬機器](azure-stack-linux.md)
## 部署 SQL 或 MySQL 資源提供者
### [在 Azure Stack 上使用 MySQL 資料庫做為 PaaS](azure-stack-mysql-rp-deploy-short.md)
### [在 Azure Stack 上部署 SQL Server 資源提供者配接器](azure-stack-sql-rp-deploy-long.md)
### [在 Azure Stack 上使用 SQL Database](azure-stack-sql-rp-deploy-short.md)

## 規劃和部署 Web Apps
### [Azure Stack Web Apps 概觀](azure-stack-webapps-overview.md)
### [開始使用 Azure Stack Web Apps 之前](azure-stack-webapps-before-you-get-started.md)
### [在 Azure Stack 上部署 MySQL 資源提供者以搭配 Web Apps 使用](azure-stack-mysql-rp-deploy-long.md)
### [Web Apps：新增更多 Web 背景工作角色](azure-stack-webapps-add-worker-roles.md)
### [將 Web Apps 資源提供者新增到 Azure Stack](azure-stack-webapps-deploy.md)

## [疑難排解](azure-stack-troubleshooting.md)
### [從失敗步驟重新執行部署](azure-stack-rerun-deploy.md)
### [Web Apps 資源提供者](azure-stack-webapps-troubleshoot-known-issues.md)

# 參考
## [工具](azure-stack-tools-paas-services.md)

# 資源
## [MSDN 論壇](https://social.msdn.microsoft.com/Forums/azure/en-US/home?forum=AzureStack)  
## [Stack Overflow](http://stackoverflow.com/questions/tagged/azure-stack)





<!--HONumber=Nov16_HO2-->


