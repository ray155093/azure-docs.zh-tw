...
> [!IMPORTANT]
> 這項功能目前為預覽狀態。僅適用於使用 **2015-02-28-Preview** 版本的 REST API。請記住，預覽 API 是針對測試與評估，不應該用於生產環境。
> 
> 
...
> [!IMPORTANT]
> 如果在索引中沒有索引鍵欄位的明確對應，Azure 搜尋服務會自動使用 `metadata_storage_path` (上述的第二個選項) 做為索引鍵並且啟用 base-64 的索引鍵編碼。
> 
> 
...
> [!WARNING]
> 如果您只刪除 blob，而非使用刪除偵測原則，對應的文件將不會從搜尋索引中移除。
> 
> 
