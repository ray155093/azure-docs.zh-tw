...
記錄檔 Blob 包含您的搜尋服務流量記錄檔。每個 Blob 會一個名為**記錄**的根物件，其中包含記錄檔物件的陣列。在每個 blob 中，同一小時內發生的所有作業都有記錄。
...
[Power BI Desktop](https://powerbi.microsoft.com/zh-TW/desktop)：探索您的資料，並將您的資料視覺化。請參閱下一節的入門查詢︰
