...
## <a name="iii-define-your-azure-search-index-using-the-index-class"></a>III. 使用 `Index` 類別定義 Azure 搜尋服務索引
...
> [!NOTE]
> 請注意，在建構函式中傳遞 `AnalyzerName.FrLucene`，`Field` 的類型將會自動成為 `DataType.String`，而且必須將 `IsSearchable` 設為 `true`。
> 
> 
