...
基本和標準搜尋會因為您所登入訂用帳戶專屬的特定資源和架構，而收取費用。基本和標準搜尋會在使用者定義的大量資料分割 (儲存體) 及複本 (服務工作負載) 中進行配置，並以搜尋單元計費。您可以藉由個別擴充資料分割或複本規模，來新增更多需要的資源。
...
角色|工作
---|---
擁有者|啟動、停止或刪除服務。<p>產生並檢視管理金鑰和查詢金鑰。<p>檢視服務狀態，包括索引計數、索引名稱、文件計數和儲存體大小。<p>新增或刪除角色成員資格 (僅限擁有者可以管理角色成員資格)。<p>訂用帳戶和服務系統管理員在擁有者角色中具有自動成員資格。
