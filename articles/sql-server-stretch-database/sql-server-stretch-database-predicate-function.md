...
---
# <a name="select-rows-to-migrate-by-using-a-filter-function-stretch-database"></a>使用篩選函數來選取要移轉的資料列 (Stretch Database)
...
## <a name="a-nameaddafterwizaadd-a-filter-function-after-running-the-wizard"></a><a name="addafterwiz"></a>在執行精靈後新增篩選函數
...
## <a name="how-stretch-database-applies-the-filter-function"></a>Stretch Database 套用篩選函數的方法
...
## <a name="a-namereplacepredicateareplace-an-existing-filter-function"></a><a name="replacePredicate"></a>取代現有的篩選函數
您可以透過再次執行 **ALTER TABLE** 陳述式並為 **FILTER\_PREDICATE** 參數指定新值，來取代先前指定的篩選函式。 例如：
...
若要移轉整個資料表，而非選取的資料列，請透過將 **FILTER\_PREDICATE** 設定為 Null 來移除現有函式。 例如：
...
若要檢查套用至資料表的篩選函式，請開啟 **sys.remote\_data\_archive\_tables** 目錄檢視，查看 **filter\_predicate** 資料行的值。 如果值為 Null，便代表整個資料表皆符合封存資格。 如需詳細資訊，請參閱 [sys.remote_data_archive_tables (Transact-SQL)](https://msdn.microsoft.com/library/dn935003.aspx)。

## <a name="security-notes-for-filter-functions"></a>篩選函數的安全性注意事項
遭入侵的帳戶若具備 db_owner 權限，便能夠執行下列操作。  
