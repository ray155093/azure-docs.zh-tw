---
title: "還原已啟用延展功能的資料庫 |Microsoft Docs"
description: "了解如何還原已啟用 Stretch 的資料庫。\\-"
...
---
# <a name="restore-stretch-enabled-databases"></a>還原已啟用延展功能的資料庫
...
還原 SQL Server 資料庫之後，您必須執行 **sys.sp_rda_reauthorize_db** 預存程序來重新建立已啟用延展功能之 SQL Server 資料庫與遠端 Azure 資料庫之間的連線。 如需詳細資訊，請參閱 [還原 SQL Server 資料庫與遠端 Azure 資料庫之間的連線](#restore-the-connection-between-the-sql-server-database-and-the-remote-azure-database)。

## <a name="restore-your-remote-azure-data"></a>還原您的遠端 Azure 資料
...
5. 按一下您想要還原的已刪除資料庫。
6. 指定新的 [資料庫名稱]，然後按一下 [建立]。
7. 資料庫還原程序將會開始，您可以使用 [通知] 來監視此程序。
...
1. 如果您即將連接到已還原但名稱不同或在不同區域中的 Azure 資料庫，請執行 [sys.sp_rda_deauthorize_db](https://msdn.microsoft.com/library/mt703716.aspx) 預存程序與先前的 Azure 資料庫中斷連線。  
2. 執行 [sys.sp_rda_reauthorize_db](https://msdn.microsoft.com/library/mt131016.aspx) 預存程序，以將已啟用延展功能的本機資料庫重新連接到 Azure 資料庫。  
