...
2. 瀏覽至[線上 Swagger 編輯器]。抵達之後，依序按一下 [File] \(檔案) -> [Paste JSON] \(貼上 JSON) 功能表項目 。
...
5. 依序選取 [Generate Server] \(產生伺服器) -> \[JAX RS] 功能表選項，來建立伺服器端的程式碼結構，讓您稍後能拿來新增模擬實作。
...
3. 使用 [Visual Studio Code] 或您偏好的文字編輯器，來開啟「ContactsApiServiceImpl.java」服務實作檔案 (位於「src/main/java/io/swagger/api/impl」資料夾)。
