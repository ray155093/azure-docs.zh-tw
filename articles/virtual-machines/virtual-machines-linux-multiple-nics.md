---
title: "建立連接多個 NIC 的 Linux VM | Microsoft Docs"
...
---
# <a name="creating-a-linux-vm-with-multiple-nics"></a>建立連接多個 NIC 的 Linux VM
...
建立兩個 NIC，將一個 NIC 連接到前端子網路，並將另一個 NIC 連接到後端子網路。 以下範例將建立兩個子網路，名為 `myNic1` 和 `myNic2`，並將它們附加到您的子網路：
...
最後，建立您的 VM，並連接您先前建立的兩個 NIC。 下列範例會建立名為 `myVM` 的 VM：
...
## <a name="creating-multiple-nics-using-azure-cli"></a>使用 Azure CLI 建立多個 NIC
...
通常您也可以建立[網路安全性群組](../virtual-network/virtual-networks-nsg.md)或[負載平衡器](../load-balancer/load-balancer-overview.md)來協助管理，以及將流量分散到您的 VM。 同樣地，命令與使用多個 NIC 時所使用的相同。 下列範例會建立名為 `myNetworkSecurityGroup` 的網路安全性群組：
