...
以下有兩個快速的範例，有關使用 [Fleet](https://github.com/coreos/fleet) 和 [Docker](https://www.docker.com/) 在 Azure 的 [CoreOS] 虛擬機器的叢集上執行應用程式。
...
## 範例 1：使用 Docker 的 Hello World</a>
...
>[AZURE.IMPORTANT] 若要執行高可用性的 Web 伺服器，您將需要在虛擬機器上設定負載平衡的 HTTP 端點 (公用連接埠 80、私人連接埠 80)。您可以在建立 CoreOS 叢集後透過 Azure 傳統入口網站或 **Azure VM 端點** 命令執行這項動作。如需步驟，請參閱[設定負載平衡集]。
...
* 您可以在 Azure 上三個節點的 CoreOS 叢集嘗試更多動作。透過閱讀 [Tim Park 的 CoreOS 教學課程]、[Patrick Chanezon 的 CoreOS 教學課程]、[Docker] 文件和 [CoreOS Overview]，瀏覽如何建立更複雜的叢集、如何使用 Docker 建立更有趣的應用程式。

* 若要開始在 Azure 資源管理員中使用 Fleet 和 CoreOS，請嘗試使用[快速入門範本](https://github.com/Azure/azure-quickstart-templates/tree/master/coreos-with-fleet-multivm)。
...
[設定負載平衡集]: ../load-balancer/load-balancer-get-started-internet-classic-cli.md
