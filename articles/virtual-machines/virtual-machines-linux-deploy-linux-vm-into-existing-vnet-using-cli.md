---
title: "使用 CLI 將 Linux VM 部署至現有 Azure 虛擬網路 | Microsoft Docs"
...
本文說明如何使用 CLI 旗標將 VM 部署至現有的虛擬網路 (VNet)。  這些需求包括：
...
如果您需要快速完成工作，下列章節詳細說明需要的命令。 每個步驟的詳細資訊和內容可在文件其他地方找到，[從這裡開始](virtual-machines-linux-deploy-linux-vm-into-existing-vnet-using-cli?toc=%2fazure%2fvirtual-machines%2flinux%2ftoc.json#detailed-walkthrough)。
...
像 VNet 和 NSG 之類的 Azure 資產，最好當做很少部署的靜態且長久的資源。  VNet 部署之後可供新的部署重複使用，完全不會對基礎結構造成負面影響。  將 VNet 想像成傳統的硬體網路交換器，您就不需要在每次部署時設定全新的硬體交換器。  有了正確設定的 VNet，我們就可以一次又一次地將新的伺服器部署至該 VNet，整個 VNet 生命週期內所需的變動極少 (如果有的話)。
