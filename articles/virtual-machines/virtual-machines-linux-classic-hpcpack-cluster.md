...
* **Linux 散發** - 目前 HPC Pack 針對計算節點支援下列 Linux 發佈。 您可以使用這些發佈的 Marketplace 版本，或提供您自己的版本。
    ...
    * **Ubuntu Server**：14.04 LTS、16.04 LTS
    ...
    >[AZURE.TIP]若要使用 Azure RDMA 網路搭配其中一個支援 RDMA 的 VM 大小，請從 Azure Marketplace 指定 SUSE Linux Enterprise Server 12 HPC 或 CentOS 型 HPC 映像。 如需詳細資訊，請參閱 [關於 H 系列和計算密集型 A 系列 VM](virtual-machines-linux-a8-a9-a10-a11-specs.md)。
...
3. 執行下列命令來部署 HPC Pack 叢集。 這個範例假設組態檔位於 E:\HPCDemoConfig.xml
    ...
    >[AZURE.NOTE]在此範例中，因為未指定 **-LogFile** 參數，所以指令碼會自動產生記錄檔。 記錄檔並非即時寫入，但在驗證和部署結束時會加以收集。 如果 PowerShell 程序在指令碼執行中時停止，將會遺失某些記錄檔。
...
>[AZURE.NOTE]第二個命令中的 “\`” 符號是 PowerShell 的逸出符號。 “\`,” 表示 “,” (逗號) 是命令的一部分。
