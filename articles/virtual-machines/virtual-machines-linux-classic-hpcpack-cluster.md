...
在 Azure 中設定 Microsoft HPC Pack 叢集，其中包含一個執行 Windows Server 的前端節點，以及數個執行支援之 Linux 散發套件的計算節點。探索叢集的 Linux 節點與 Windows 前端節點之間移動資料的一些選項。了解如何將 Linux HPC 作業提交至叢集。
...
HPC Pack IaaS 部署指令碼會使用 XML 組態檔作為輸入，以描述 HPC 叢集的基礎結構。下列範例組態檔會部署一個小型的叢集，由 HPC Pack 前端節點和兩個大小為 A7 的 CentOS 7.0 Linux 計算節點所組成。請根據環境和所要的叢集組態視需要修改檔案，並將它以 HPCDemoConfig.xml 之類的名稱儲存。例如，您需要提供您的訂用帳戶名稱、唯一儲存體帳戶名稱，以及雲端服務名稱。此外，您可能會想要為計算節點選擇支援的不同 Linux 映像。如需組態檔中元素的詳細資訊，請參閱指令碼資料夾中的 Manual.rtf 檔案與[使用 HPC Pack IaaS 部署指令碼建立 HPC 叢集](virtual-machines-windows-classic-hpcpack-cluster-powershell-script.md)。
...
3. 執行下列命令來部署 HPC Pack 叢集。這個範例假設組態檔位於 E:\\HPCDemoConfig.xml
    ...
    因為未指定 **-LogFile** 參數，所以指令碼會自動產生記錄檔。記錄檔並非即時寫入，但在驗證和部署結束時會加以收集。如果 PowerShell 程序在指令碼執行中時停止，將會遺失某些記錄檔。
...
在此範例中，allvhdsje 是您的儲存體帳戶名稱、storageaccountkey 是您的儲存體帳戶金鑰，而 rdma 則是 Azure 檔案共用名稱。Azure 檔案共用會掛接到前端節點的 Z: 上。
