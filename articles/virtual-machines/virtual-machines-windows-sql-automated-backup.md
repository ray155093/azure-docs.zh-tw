...
- 如果您打算使用 PowerShell 設定自動備份，請[安裝最新的 Azure PowerShell 命令](../powershell-install-configure.md)。

>[AZURE.NOTE] 自動備份相依於 SQL Server IaaS 代理程式擴充。目前的 SQL 虛擬機器資源庫映像預設會新增這項擴充。如需詳細資訊，請參閱 [SQL Server IaaS 代理程式擴充](virtual-machines-windows-sql-server-agent-extension.md)。
...
在 Resource Manager 部署模型中建立新的 SQL Server 2014 虛擬機器時，您可以使用 Azure 入口網站設定自動備份。

在 [SQL Server 設定] 刀鋒視窗中選取 [自動備份]。下列的 Azure 入口網站螢幕擷取畫面顯示 [SQL 自動備份] 刀鋒視窗。
...
針對現有的 SQL Server 虛擬機器，您必須使用 PowerShell 設定自動備份設定。
...
佈建 SQL VM 之後，請使用 PowerShell 設定自動批次。

在下列 PowerShell 範例中，是針對現有的 SQL Server 2014 VM 設定自動備份。**AzureRM.Compute\\New-AzureVMSqlServerAutoBackupConfig** 命令會設定自動備份設定，將備份儲存在與虛擬機器相關聯的 Azure 儲存體帳戶中。這些備份將會保留 10 天。**Set-AzureRmVMSqlServerExtension** 命令會使用這些設定更新指定的 Azure VM。
...
若要停用自動備份，請執行相同的指令碼，但不要對 **AzureRM.Compute\\New-AzureVMSqlServerAutoBackupConfig** 命令使用 **-Enable** 參數。沒有 **-Enable** 參數即通知命令停用此功能。和安裝一樣，可能需要幾分鐘的時間來停用自動備份。
