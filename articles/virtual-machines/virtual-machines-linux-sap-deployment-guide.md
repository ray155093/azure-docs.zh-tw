<properties
   pageTitle="Linux 虛擬機器 (VM) 上的 SAP NetWeaver - 部署指南 | Microsoft Azure"
...
# Azure 虛擬機器 (VM) 上的 SAP NetWeaver - 部署指南
...
這份白皮書逐步說明如何準備 Azure 虛擬機器來部署以 SAP NetWeaver 為基礎的應用程式。它假設您已知[規劃和實作指南][planning-guide]所含的資訊。否則，請先閱讀這份個別文件。
...
## 簡介
全球已有大量公司使用以 SAP NetWeaver 為基礎的應用程式 (最主要是 SAP Business Suite) 來執行其任務關鍵性商務程序。因此，系統健康狀態是十分重要的資產，而且在故障 (包括效能事件) 時提供企業支援的能力也成為重要需求。Microsoft Azure 提供優越的平台工具，可容納所有商務關鍵應用程式的支援需求。本指南確定已設定要部署 SAP Software 的 Microsoft Azure 虛擬機器，以提供企業支援，不論虛擬機器的建立方式為何 (取自 Azure Marketplace 還是使用客戶特定映像)。以下詳細說明所有必要的安裝步驟。
...
#### 網際網路連線
若要下載和執行必要的工具和指令碼，需要網際網路連線。此外，執行 Azure Enhanced Monitoring Extension 的 Microsoft Azure 虛擬機器需要存取網際網路。如果此 Azure VM 是 Azure 虛擬網路或內部部署網域的一部分，請務必設定相關 Proxy 設定 (如這份文件的[設定 Proxy][deployment-guide-configure-proxy] 一章所述)。
...
這類 Azure 儲存體帳戶或 Azure 虛擬網路應該已經建立和設定。[規劃和實作指南][planning-guide]涵蓋其建立和設定方式。
...
#### 資源群組
資源群組是新的概念，內含具有相同生命週期的所有資源 (例如，同時予以建立和刪除)。如需資源群組的詳細資訊，請閱讀[本文][resource-group-overview]。

### <a name="42ee2bdb-1efc-4ec7-ab31-fe4c22769b94"></a>SAP 資源
在進行設定工作期間，需要下列資源︰

* SAP 附註 [1928533]
	* 支援部署 SAP Software 之 Azure 虛擬機器大小的清單 
	* 每個 Azure 虛擬機器大小的重要容量資訊
	* 支援的 SAP 軟體和 OS 與 DB 組合
* SAP 附註 [2015553]，列出將 SAP 軟體部署到 Microsoft Azure 時 SAP 所支援的必要條件。
* SAP 附註 [1999351]，包含 Enhanced Azure Monitoring for SAP 的其他疑難排解資訊。
* SAP 附註 [2178632]，包含 Microsoft Azure 上 SAP 之所有可用監視度量的詳細資訊。 
* SAP 附註 [1409604]，包含部署於新的 Azure Resource Manager 時，Microsoft Azure 上所需的 SAP Host Agent for Windows 版本。
* SAP 附註 [2191498]，包含部署於新的 Azure Resource Manager 時，Microsoft Azure 上所需的 SAP Host Agent for Linux 版本。
* SAP 附註 [2243692]，包含位於 Azure 上 Linux 的 SAP 授權資訊
* [SCN](https://wiki.scn.sap.com/wiki/display/HOME/SAPonLinuxNotes)，包含適用於 Linux 的所有必要 SAP 附註
* 屬於 [Azure PowerShell][azure-ps] 一部分的 SAP 特定 PowerShell Cmdlet
* 屬於 [Azure CLI][azure-cli] 一部分的 SAP 特定 Azure CLI
...
您想要從 Azure Marketplace 使用 Microsoft 或協力廠商提供的映像來部署 VM。在 Microsoft Azure 上部署您的 VM 之後，請遵循相同的方針和工具在 VM 內安裝 SAP 軟體，就像是在內部部署環境中一樣。為了在 Azure VM 內安裝 SAP 軟體，SAP 和 Microsoft 建議將 SAP 安裝媒體上傳並儲存到 Azure VHD，或建立作為「檔案伺服器」並包含所有必要 SAP 安裝媒體的 Azure VM。

[comment]: <> (MSSedusch TODO 為什麼需要建議使用檔案管理 (例如檔案伺服器或 VHD)？ 這是否與內部部署不同？)

如需詳細資訊，請參閱[案例 1：從 Azure Marketplace 為 SAP 部署 VM][deployment-guide-3.2] 一章。

#### <a name="3688666f-281f-425b-a312-a77e7db2dfab"></a>使用自訂映像部署 VM
因為您 OS 或 DBMS 版本的相關特定修補程式需求，所以 Azure Marketplace 所提供的映像可能不符合您的需求。因此，您可能必須使用您自己「私人」的 OS/DB VM 映像建立 VM，之後可多次部署此映像。Windows 與 Linux 映像之間的私人映像建立步驟不同。

___

> ![Windows][Logo_Windows] Windows
>
> 若要準備可用來部署多部虛擬機器的 Windows 映像，必須在內部部署 VM 上抽象化/一般化 Windows 設定 (如 Windows SID 和主機名稱)。作法是使用 <https://technet.microsoft.com/library/cc721940.aspx> 上所述的 sysprep。
>
> ![Linux][Logo_Linux] Linux
>
> 若要準備可用來部署多部虛擬機器的 Linux 映像，必須在內部部署 VM 上抽象化/一般化一些 Linux 設定。作法是使用[本文][virtual-machines-linux-capture-image]或[本文][virtual-machines-linux-agent-user-guide-command-line-options]中所述的 waagent -deprovision。

___

設定資料庫內容的方式是使用 SAP Software Provision Manager 安裝新的 SAP 系統、從連接到虛擬機器的 VHD 還原資料庫備份，或直接從 Azure 儲存體 (若為 DBMS 支援) 還原資料庫備份 (請參閱 [DBMS 部署指南][dbms-guide])。如果您已在內部部署 VM (特別是 2 層系統) 中安裝 SAP 系統，則可以在部署 Azure VM 之後，透過 SAP Software Provisioning Manager 支援的「系統重新命名」程序來調整 SAP 系統設定 (SAP 附註 [1619720])。否則，您稍後可以在部署 Azure VM 之後再安裝 SAP 軟體。

如需詳細資訊，請參閱[案例 2：使用自訂映像為 SAP 部署 VM][deployment-guide-3.3] 一章。

#### 使用非一般化磁碟將 VM 從內部部署移至 Microsoft Azure
您想要將特定 SAP 系統從內部部署移至 Microsoft Azure。作法是將包含 OS、SAP 二進位檔和最終 DBMS 二進位檔的 VHD，以及包含 DBMS 資料和記錄檔的 VHD 上傳至 Microsoft Azure。相對於上面[使用自訂映像部署 VM][deployment-guide-3.1.2] 一章所述的案例，您可以在 Azure VM 中保留主機名稱、SAP SID 和 SAP 使用者帳戶，因為這些項目已在內部部署環境中設定。因此，並不需要一般化作業系統。此情況最適用於跨單位案例，其中 SAP 地標的一部分是在內部部署環境中執行，一部分是在 Microsoft Azure 上執行。

如需詳細資訊，請參閱[案例 3：使用非一般化 Azure VHD 和 SAP 從內部部署移動 VM][deployment-guide-3.4] 一章。

### <a name="db477013-9060-4602-9ad4-b0316f8bb281"></a>案例 1：從 Azure Marketplace 為 SAP 部署 VM
Microsoft Azure 可讓您從 Azure Marketplace 部署 VM 執行個體，而 Azure Marketplace 提供 Windows Server 的一些標準 OS 映像以及不同的 Linux 散發套件。它也可以部署包含 DBMS SKU 的映像 (例如 SQL Server)。如需搭配使用那些映像與 DBMS SKU 的詳細資訊，請參閱 [DBMS 部署指南][dbms-guide]。
...
#### 使用 Azure 入口網站建立虛擬機器
使用 Azure Marketplace 中的映像建立新虛擬機器的最簡單方式，是透過 Azure 入口網站。瀏覽至 <https://portal.azure.com/#create>。輸入您想要部署的作業系統類型 (例如 Windows 或 SUSE)，然後選取版本。請務必選取 "Azure Resource Manager" 部署模型，然後按一下 [建立]。
...
1. 基本概念
    ...
    1. 位置︰選取應該部署新虛擬機器的位置。如果您想要將虛擬機器連接到內部部署網路，請務必選取將 Azure 連接到內部部署網路的虛擬網路位置。如需詳細資訊，請參閱[規劃指南][planning-guide]中的 [Microsoft Azure 網路][planning-guide-microsoft-azure-networking]。
1. 大小︰如需所支援 VM 類型的清單，請閱讀 SAP 附註 [1928533]。如果您想要使用進階儲存體，也請務必選取正確的類型。並非所有 VM 類型都支援進階儲存體。如需詳細資訊，請參閱[規劃指南][planning-guide]中的[儲存體：Microsoft Azure 儲存體和資料磁碟][planning-guide-storage-microsoft-azure-storage-and-data-disks]和 [Azure 進階儲存體][planning-guide-azure-premium-storage]一章。
1. Settings
    1. 儲存體帳戶：您可以選取現有儲存體帳戶，或建立新的儲存體帳戶。如需不同儲存體類型的詳細資訊，請閱讀 [DBMS 指南][dbms-guide]的 [Microsoft Azure 儲存體][dbms-guide-2.3]一章。請注意，執行 SAP 應用程式時並不支援所有儲存體類型。
    ...
    1. 網路安全性群組：如需詳細資訊，請參閱[什麼是網路安全性群組 (NSG)][virtual-networks-nsg]。
    1. 監視︰您可以停用診斷設定。執行命令來啟用 Azure Enhanced Monitoring 時，會自動予以啟用 (如[設定監視][deployment-guide-configure-monitoring-scenario-1]一章所述)。
...
開啟上述其中一個範本之後，Azure 入口網站會瀏覽至 [編輯參數] 面板。輸入以下資訊：

* **sapSystemId**：SAP 系統識別碼
* **osType**：您想要部署的作業系統 (例如 Windows Server 2012 R2 或 SLES 12)
    * 這份清單僅包含 Microsoft Azure 上 SAP 所支援的版本。
* **sapSystemSize**：SAP 系統的大小
    * 新系統將提供的 SAP 數量。如果您不確定系統將需要多少 SAP，請詢問您的 SAP 技術合作夥伴或系統整合者。
* **systemAvailability**：(僅 3 層範本) 系統可用性 
    * 選取適用於 HA 安裝之組態的 HA。將為 ASCS 建立兩部資料庫伺服器和兩部伺服器。
* storageType：(僅 2 層範本) 應該使用的儲存體類型 
...
* **adminUsername** 和 **adminPassword**：使用者名稱和密碼
    * 建立可用來登入電腦的新使用者。
* **newOrExistingSubnet**︰決定應該建立新的虛擬網路和子網路，還是應該使用現有子網路。如果您已經有連接到內部部署網路的虛擬網路，請選取現有虛擬網路。
* **subnetId**：虛擬機器應該連接的子網路識別碼。選取將虛擬機器連接到內部部署網路之 VPN 或快速路由虛擬網路的子網路。識別碼通常看起來像 /subscriptions/`<subscription id`>/resourceGroups/`<resource group name`>/providers/Microsoft.Network/virtualNetworks/`<virtual network name`>/subnets/`<subnet name`>

輸入所有參數之後，請選取您想要使用的訂用帳戶和資源群組。您可以選取現有資源群組，或選取下拉式功能表中的 [+ 新增] 來建立新的資源群組。如果您建立新的資源群組，則也必須選取將在其中建立資源群組和虛擬機器的區域。

請檢閱法律條款，並接受法律條款，然後按一下 [建立]。
...
#### 加入網域 (僅限 Windows)
如果透過 Azure 站對站或快速路由 (在[規劃和實作指南][planning-guide]中，也稱為跨單位) 將 Azure 中的部署連接到內部部署 AD/DNS，則 VM 必須加入內部部署網域。這份文件的[將 VM 加入內部部署網域 (僅限 Windows)][deployment-guide-4.3] 一章會說明此步驟的考量。

#### <a name="ec323ac3-1de9-4c3a-b770-4ff701def65b"></a>設定監視
如這份文件的[設定 Azure Enhanced Monitoring Extension for SAP][deployment-guide-4.5] 一章所述，設定 Azure Enhanced Monitoring Extension for SAP。

請針對這份文件的 [SAP 資源][deployment-guide-2.2]一章所列資源中的必要最小 SAP Kernel 和 SAP Host Agent 版本，檢查 SAP 監視的必要條件。

#### 監視檢查
請檢查監視是否運作 (如[在 Azure 上檢查是否有 SAP 的端對端監視安裝程式並進行疑難排解][deployment-guide-troubleshooting-chapter]一章所述)。

#### 後置部署步驟
VM 在建立之後將會進行部署，然後由您將所有必要軟體元件安裝到 VM。因此，這類型的 VM 部署需要安裝已在某個其他 VM 的 Microsoft Azure 中的軟體，或作為可連接的磁碟。或者，我們正在查看指定內部部署資產 (安裝共用) 連線的跨單位案例。

### <a name="54a1fc6d-24fd-4feb-9c57-ac588a55dff2"></a>案例 2：使用自訂映像為 SAP 部署 VM
如[規劃和實作指南][planning-guide]中的詳細步驟所述，有方法可以準備和建立自訂映像，以及使用它來建立多個新 VM。流程圖中的一系列步驟如下︰
...
若要透過 Azure 入口網站使用私人 OS 映像來建立部署，請使用 [azure-quickstart-templates github 儲存機制][azure-quickstart-templates-github]上所發佈的其中一個 SAP 範本。您也可以使用 [PowerShell][virtual-machines-upload-image-windows-resource-manager] 手動建立虛擬機器。

* [2 層組態 (僅一部虛擬機器) 範本][sap-templates-2-tier-user-image]：如果您想要建立僅使用一部虛擬機器和您專屬 OS 映像的 2 層系統，請使用此範本。
* [3 層組態 (多部虛擬機器) 範本][sap-templates-3-tier-user-image]：如果您想要建立使用多部虛擬機器和您專屬 OS 映像的 3 層系統，請使用此範本。

開啟上述其中一個範本之後，Azure 入口網站會瀏覽至 [編輯參數] 面板。輸入以下資訊：

* **sapSystemId**：SAP 系統識別碼
* **osType**：您想要部署的作業系統類型 (Windows 或 Linux)
* **sapSystemSize**：SAP 系統的大小
    * 新系統將提供的 SAP 數量。如果您不確定系統將需要多少 SAP，請詢問您的 SAP 技術合作夥伴或系統整合者。
* **systemAvailability**：(僅 3 層範本) 系統可用性 
    * 選取適用於 HA 安裝之組態的 HA。將為 ASCS 建立兩部資料庫伺服器和兩部伺服器。
...
* **adminUsername** 和 **adminPassword**：使用者名稱和密碼
    * 建立可用來登入電腦的新使用者。
* **userImageVhdUri**：私人 OS 映像 VHD 的 URI (例如 https://`<accountname`>.blob.core.windows.net/vhds/userimage.vhd)
* **userImageStorageAccount**：儲存私人 OS 映像的儲存體帳戶名稱 (例如上述範例 URI 中的 `<accountname`>)
* **newOrExistingSubnet**︰決定應該建立新的虛擬網路和子網路，還是應該使用現有子網路。如果您已經有連接到內部部署網路的虛擬網路，請選取現有虛擬網路。
* **subnetId**：虛擬機器應該連接的子網路識別碼。選取將虛擬機器連接到內部部署網路之 VPN 或快速路由虛擬網路的子網路。識別碼通常看起來像 /subscriptions/`<subscription id`>/resourceGroups/`<resource group name`>/providers/Microsoft.Network/virtualNetworks/`<virtual network name`>/subnets/`<subnet name`>
...
#### 安裝 VM 代理程式 (僅限 Linux)
如果您想要使用上述範本，必須已在使用者映像中安裝 Linux 代理程式。否則，部署將會失敗。在使用者映像中下載並安裝 VM 代理程式 (如這份文件的[下載、安裝和啟用 Azure VM 代理程式][deployment-guide-4.4]一章所述)。如果您未使用上述範本，則之後也可以安裝 VM 代理程式。
...
#### 設定監視
如這份文件的[設定 Azure Enhanced Monitoring Extension for SAP][deployment-guide-4.5] 一章所述，設定 Azure Monitoring Extension for SAP。請針對這份文件的 [SAP 資源][deployment-guide-2.2]一章所列資源中的必要最小 SAP 核心和 SAP Host Agent 版本，檢查 SAP 監視的必要條件。
...
假設磁碟已上傳並定義於 Azure (請參閱[規劃和實作指南][planning-guide])，請遵循下列步驟。

#### Create virtual machine
若要透過 Azure 入口網站使用私人 OS 磁碟來建立部署，請使用 [azure-quickstart-templates github 儲存機制][azure-quickstart-templates-github]上所發佈的 SAP 範本。您也可以使用 PowerShell 或 Azure CLI 手動建立虛擬機器。

* [2 層組態 (僅一部虛擬機器) 範本][sap-templates-2-tier-os-disk]
    * 如果您想要建立僅使用一部虛擬機器的 2 層系統，請使用此範本。

開啟上述範本之後，Azure 入口網站會瀏覽至 [編輯參數] 面板。輸入以下資訊：
...
* **storageType**：(僅 2 層範本) 應該使用的儲存體類型 
    * 對於更大的系統，強烈建議使用進階儲存體。如需不同儲存體類型的詳細資訊，請閱讀 
        ...
        * [進階儲存體：Azure 虛擬機器工作負載適用的高效能儲存體][storage-premium-storage-preview-portal]
        * [Microsoft Azure 儲存體簡介][storage-introduction]
* **osDiskVhdUri**：私人 OS 磁碟的 URI (例如 https://`<accountname`>.blob.core.windows.net/vhds/osdisk.vhd)
* **newOrExistingSubnet**︰決定應該建立新的虛擬網路和子網路，還是應該使用現有子網路。如果您已經有連接到內部部署網路的虛擬網路，請選取現有虛擬網路。
* **subnetId**：虛擬機器應該連接的子網路識別碼。選取將虛擬機器連接到內部部署網路之 VPN 或快速路由虛擬網路的子網路。識別碼通常看起來像 /subscriptions/`<subscription id`>/resourceGroups/`<resource group name`>/providers/Microsoft.Network/virtualNetworks/`<virtual network name`>/subnets/`<subnet name`>
...
#### 安裝 VM 代理程式
如果您想要使用上述範本，必須已在 OS 磁碟中安裝 Linux 代理程式。否則，部署將會失敗。在 VM 中下載並安裝 VM 代理程式 (如這份文件的[下載、安裝和啟用 Azure VM 代理程式][deployment-guide-4.4]一章所述)。
...
請針對這份文件的 [SAP 資源][deployment-guide-2.2]一章所列資源中的必要最小 SAP 核心和 SAP Host Agent 版本，檢查 SAP 監視的必要條件。
...
* 聯合 MS/SAP 小組已擴充監視功能，並決定新增更多計數器或刪除一些計數器。 
* Microsoft 推出新版本的基礎 Azure 基礎結構來提供監視資料，並針對這些變更來調整 Azure Enhanced Monitoring Extension for SAP。
* 您可以新增掛接到 Azure VM 的額外 VHD，或移除 VHD。在此情況下，您需要更新儲存體相關資料的集合。如果您透過新增端點、刪除端點或將 IP 位址指派給 VM 來變更組態，則這不會影響監視組態。
* 您可以變更 Azure VM 大小 (例如從 A5 到任何其他大小的 VM)。
* 您可以在 Azure VM 中新增網路介面

若要更新監視組態，請如下繼續進行︰

* 遵循這份文件的[設定 Azure Enhanced Monitoring Extension for SAP][deployment-guide-4.5] 一章所述的步驟，更新監視基礎結構。重新執行本章所述的指令碼將會偵測到已部署監視組態，並且將執行對監視組態的必要變更。 

___

> ![Windows][Logo_Windows] Windows
>
> 對於更新 Azure VM 代理程式，並不需要使用者介入。VM 代理程式會自動自行更新，而且不需要 VM 重新開機。
>
> ![Linux][Logo_Linux] Linux
>
> 請遵循[本文][virtual-machines-linux-update-agent]中的步驟，更新 Azure Linux 代理程式。
...
* 在＜命令列工具＞小節下方，有一個小節稱為＜Windows PowerShell＞。遵循 [安裝] 連結。
* 所跳出的 Microsoft 下載管理員會包含結尾為 .exe 的行項目。選取 [執行] 選項。
* 會出現快顯視窗，詢問是否要執行 Microsoft Web Platform Installer。按 [是]
* 即會出現與下面類似的畫面：
 
![適用於 Azure PowerShell Cmdlet 的安裝畫面][deployment-guide-figure-500] <a name="figure-5"></a>

* 按 [安裝]，並接受使用者授權合約。

經常檢查是否已更新 PowerShell Cmdlet。通常會每個月進行更新。最簡單的作法是遵循如上所述的安裝步驟，直到[本圖][deployment-guide-figure-5]所顯示的安裝畫面。在此畫面中，會顯示 Cmdlet 的發行日期以及實際版本號碼。除非 SAP 附註 [1928533] 或 [2015553] 特別指出，否則建議使用最新版本的 Azure PowerShell Cmdlet。
...
在[本圖][deployment-guide-figure-6]中，結果應該顯示如下。

![Azure PS Cmdlet 版本檢查的結果][deployment-guide-figure-600] <a name="figure-6"></a>

如果桌上型/膝上型電腦上安裝的 Azure Cmdlet 版本是最新版本，則啟動 Microsoft Web Platform Installer 後的第一個畫面會與[本圖][deployment-guide-figure-5]顯示的畫面有些不同。

請注意下[圖][deployment-guide-figure-7]中下面的紅色圓圈。
 
![Azure PowerShell Cmdlet 的安裝畫面，表示已安裝最新版本的 Azure PS Cmdlet][deployment-guide-figure-700] <a name="figure-7"></a>

如果畫面看起來與[上面][deployment-guide-figure-7]相同 (表示已安裝最新 Azure Cmdlet 版本)，則不需要繼續安裝。在此情況下，您可以在這個階段 [結束] 安裝。
...
在[本圖][deployment-guide-figure-azure-cli-version]中，結果應該顯示如下。

![Azure CLI 版本檢查的結果][deployment-guide-figure-760] <a name="0ad010e6-f9b5-4c21-9c09-bb2e5efb3fda"></a>

### <a name="31d9ecd6-b136-4c73-b61e-da4a29bbc9cc"></a>將 VM 加入內部部署網域 (僅限 Windows)
如果將 SAP VM 部署到將內部部署 AD 和 DNS 擴充到 Azure 的跨單位案例，則必須將 VM 加入內部部署網域。將 VM 加入內部部署網域的詳細步驟以及成為內部部署網域成員所需的額外軟體是依客戶而定。通常將 VM 加入內部部署網域，即表示安裝其他軟體 (例如惡意程式碼防護軟體或者各種備份或監視軟體代理程式)。
...
從非一般化 (例如未針對 Windows 進行 sysprep 處理) 的 OS 映像部署適用於 SAP 的 VM 時，需要下列步驟。針對從 Azure Marketplace 部署的虛擬機器，不需要安裝代理程式。這些映像已經包含 Azure 代理程式。

#### <a name="b2db5c9a-a076-42c6-9835-16945868e866"></a>Windows

* 下載 Azure VM 代理程式︰
	* 從下列位置下載 Azure VM 代理程式安裝程式封裝：<https://go.microsoft.com/fwlink/?LinkId=394789>
	* 在膝上型電腦或伺服器上本機儲存 VM 代理程式 MSI 封裝
* 安裝 Azure VM 代理程式︰
	* 使用終端機服務 (RDP) 連接到已部署的 Azure VM
	* 在 VM 上開啟 Windows 檔案總管視窗，然後開啟 VM 代理程式之 MSI 檔案的目標目錄
	* 將 Azure VM 代理程式安裝程式 MSI 檔案從本機膝上型電腦/伺服器拖放到 VM 中 VM 代理程式的目標目錄
	* 按兩下 VM 中的 MSI 檔案
	* 對於加入內部部署網域的 VM，請確定最終網際網路 Proxy 設定適用於 VM 中的 Windows 本機系統帳戶 (S-1-5-18)，而且如[設定 Proxy][deployment-guide-configure-proxy] 一章所述。VM 代理程式將在此內容中執行，而且必須可以連接到 Azure。
...
#### Windows
這些設定也必須適用於 LocalSystem 帳戶，才能存取網際網路。如果您的 Proxy 設定不是透過群組原則所設定，則可以遵循這些步驟為 LocalSystem 帳戶進行設定。

1.	開啟 gpedit.msc
1.	瀏覽至 [電腦設定] –> [系統管理範本] -> [Windows 元件] -> [Internet Explorer]，然後啟用 [為每台電腦建立 Proxy 設定 - 而不是每個使用者]
1.	開啟 [控制台]，然後瀏覽至 [網路和網際網路] -> [網際網路選項]
1.	開啟 [連線] 索引標籤，然後按一下 [區域網路 (LAN) 設定]
1.	停用 [自動偵測設定]
1.	啟用 [在您的區域網路使用 Proxy 伺服器]，然後輸入 Proxy 主機和連接埠
...
/etc/waagent.conf 中的 Proxy 設定也適用於所需的 VM 擴充功能。如果您想要使用 Azure 儲存機制，請確定這些儲存機制的流量不會經過內部部署內部網路。如果您已建立使用者定義的路由來啟用強制通道，請務必新增路由，以將路由傳送到 104.45.17.148 的流量直接路由傳送到網際網路，而不是透過站對站連線。您也需要新增 /etc/regionserverclnt.cfg 中所列 IP 位址的路由。下面的螢幕擷取畫面顯示範例。如需使用者定義的路由的詳細資訊，請參閱[本文][virtual-networks-udr-overview]。
...
如[Microsoft Azure 上適用於 SAP 的 VM 部署案例][deployment-guide-3]一章所述準備好 VM 之後，即在電腦上安裝 Azure VM 代理程式。下一個重要步驟是部署 Azure Enhanced Monitoring Extension for SAP (位於 Microsoft Azure 全球資料中心內的 Azure 擴充功能儲存機制)。如需詳細資訊，請參閱[規劃和實作指南][planning-guide-9.1]。

您可以使用 Azure PowerShell 或 Azure CLI 安裝和設定 Azure Enhanced Monitoring Extension for SAP。如果您想要在使用 Windows 電腦的 Windows 或 Linux VM 上安裝擴充功能，請閱讀 [Azure PowerShell][deployment-guide-4.5.1] 一章。若要在使用 Linux 桌上型電腦的 Linux VM 上安裝擴充功能，請閱讀 [Azure CLI][deployment-guide-4.5.2] 一章。

#### <a name="987cf279-d713-4b4c-8143-6b11589bb9d4"></a>Azure PowerShell for Linux 和 Windows VM
...
提供您的帳戶資料和 Azure 虛擬機器之後，指令碼會部署所需的擴充功能，並啟用所需的功能。這可能需要數分鐘的時間。如需 Set-AzureRmVMAEMExtension 的詳細資訊，請閱讀[此 MSDN 文章][msdn-set-azurermvmaemextension]。
...
* 若要確認已成功執行 Azure Enhanced Monitoring 的所有步驟，而且 Azure 基礎結構提供必要的資料，請繼續進行 Azure Enhanced Monitoring Extension for SAP 的整備檢查 (如這份文件的 [Azure Enhanced Monitoring for SAP 整備檢查][deployment-guide-5.1]一章所述)。 
* 若要繼續執行此動作，請等候 15-30 分鐘，直到 Azure Diagnostics 收集到相關資料。

#### <a name="408f3779-f422-4413-82f8-c57a23b4fc2f"></a>Azure CLI for Linux VM
請遵循[本文][install-extension-cli]中的步驟，從 Linux 膝上型電腦/桌上型電腦在 Linux VM 上安裝 Azure Enhanced Monitoring Extension for SAP。

 [comment]: <> (MSSedusch TODO 檢查連結是否仍然有效)

## <a name="564adb4f-5c95-4041-9616-6635e83a810b"></a>在 Azure 上檢查是否有 SAP 的端對端監視安裝程式並進行疑難排解
部署 Azure VM 並設定相關 Azure 監視基礎結構之後，請檢查 Azure Enhanced Monitoring 的所有元件是否正確地運作。

因此，請執行 Azure Enhanced Monitoring Extension for SAP 整備檢查 (如 [Azure Enhanced Monitoring for SAP 整備檢查][deployment-guide-5.1]一章所述)。如果這項檢查的結果是正向的，並且取得所有相關效能計數器，則已成功設定 Azure 監視。在此情況下，請繼續安裝 SAP Host Agent (如這份文件的 [SAP 資源][deployment-guide-2.2]一章所列的「SAP 附註」所述)。如果整備檢查結果表示遺失計數器，請繼續執行 Azure 監視基礎結構組態的健康狀態檢查 (如[Azure 監視基礎結構組態的健康狀態檢查][deployment-guide-5.2]一章中所述)。如果發生任何 Azure Monitoring Configuration 問題，請參閱[適用於 SAP 之 Azure 監視基礎結構的進一步疑難排解][deployment-guide-5.3]一章以取得疑難排解的詳細說明。

### <a name="bb61ce92-8c5c-461f-8c53-39f5e5ed91f2"></a>Azure Enhanced Monitoring for SAP 整備檢查
運用這項檢查，您可以確定基礎 Azure 監視基礎結構完整提供 SAP 應用程式內所顯示的度量。

#### 在 Windows VM 上執行整備檢查
若要執行整備檢查，請登入 Azure 虛擬機器 (不需要系統管理員帳戶)，然後執行下列步驟︰

* 請開啟 Windows 命令提示字元，並切換至 Azure Monitoring Extension for SAP 的安裝資料夾：C:\\Packages\\Plugins\\Microsoft.AzureCAT.AzureEnhancedMonitoring.AzureCATExtensionHandler\`<version`>\\drop
...
> [AZURE.NOTE] azperflib.exe 會以迴圈形式執行，並每隔 60 秒更新收集到的計數器。若要完成迴圈，請關閉命令視窗。

如果未安裝 Azure Enhanced Monitoring Extension 或 ‘AzureEnhancedMonitoring’ 服務未執行，則尚未正確設定擴充功能。在此情況下，請遵循[適用於 SAP 之 Azure 監視基礎結構的進一步疑難排解][deployment-guide-5.3]一章，以取得如何重新部署擴充功能的詳細指示。

##### 檢查 azperflib.exe 的輸出
azperflib.exe 輸出會顯示適用於 SAP 的所有已填入 Azure 效能計數器。在所收集計數器清單底部，您會看到摘要和健康狀態指標，表示 Azure 監視的狀態。
 
![執行 azperflib.exe 的健康狀態檢查輸出表示沒有任何問題][deployment-guide-figure-1100] <a name="figure-11"></a>

在[上圖][deployment-guide-figure-11]中，檢查針對 [計數器總計] 數量輸出所傳回的結果 (報告為空白而且是針對顯示在上方的「健康狀態檢查」)。

您可以解譯結果值，如下所示︰

| Azperflib.exe 結果值 | Azure 監視整備狀態 |
| ------------------------------|----------------------------------- |
| **計數器總計：空白** | 下列 2 個 Azure 儲存體計數器可為空白：<ul><li>Storage Read Op Latency Server msec</li><li>Storage Read Op Latency E2E msec</li></ul>所有其他計數器都必須包含值。 |
| **健康狀態檢查** | 只有在傳回狀態顯示正常時，才是正常 |

如果 azperflib.exe 的兩個傳回值都未顯示正確地傳回所有填入的計數器，請遵循 Azure 監視基礎結構組態的健康狀態檢查指示 (如下面的 [Azure 監視基礎結構組態的健康狀態檢查][deployment-guide-5.2]一章所述)。
...
* 如 SAP 附註 1031096 所述來安裝 SAP Host Agent，並檢查 saposcol 的輸出
    ...
    * 執行 dump ccm
    * 檢查 "Virtualization\_Configuration\\Enhanced Monitoring Access" 度量是否為 true
* 如果您已安裝 SAP NetWeaver ABAP 應用程式伺服器，請開啟交易 ST06，並檢查是否已啟用增強監視。

如果有上述任何檢查失敗，請遵循[適用於 SAP 之 Azure 監視基礎結構的進一步疑難排解][deployment-guide-5.3]一章，以取得如何重新部署擴充功能的詳細指示。

### <a name="e2d592ff-b4ea-4a53-a91a-e5521edb6cd1"></a>Azure 監視基礎結構組態的健康狀態檢查
如果未正確地提供部分監視資料 (如上方 [Azure Enhanced Monitoring for SAP 整備檢查][deployment-guide-5.1]一章所述之測試所指出)，請執行 Test-AzureRmVMAEMExtension Cmdlet 來測試適用於 SAP 之 Azure 監視基礎結構和 Azure Monitoring Extension for SAP 的目前組態是否正確。

若要測試監視組態，請執行下列序列︰

* 確定您已安裝最新版本的 Microsoft Azure PowerShell Cmdlet (如這份文件的[部署 Azure PowerShell Cmdlet][deployment-guide-4.1] 一章中所述)。
* 執行下列 PowerShell Cmdlet。如需可用環境的清單，請執行 Commandlet Get-AzureRmEnvironment。如果您想要使用公用 Azure，則您的環境是 AzureCloud。若為中國的 Azure，請選取 AzureChinaCloud。
...
![SAP 特定 Azure Cmdlet Test-VMConfigForSAP\_GUI 的輸入畫面][deployment-guide-figure-1200]

輸入您帳戶和 Azure 虛擬機器的相關資訊之後，指令碼將測試所選虛擬機器的組態。
 
![適用於 SAP 之 Azure 監視基礎結構的成功測試輸出][deployment-guide-figure-1300]

請確定每項檢查都標示為正常。如果某些檢查未成功，請執行更新 Cmdlet (如這份文件的[設定 Azure Enhanced Monitoring Extension for SAP][deployment-guide-4.5] 一章所述)。請再多等待 15 分鐘，然後重新執行 [Azure Enhanced Monitoring for SAP 整備檢查][deployment-guide-5.1]和 [Azure 監視基礎結構組態的健康狀態檢查][deployment-guide-5.2]章節中所述的檢查。如果檢查仍然表示某些或所有計數器都有問題，請繼續[適用於 SAP 之 Azure 監視基礎結構的進一步疑難排解][deployment-guide-5.3]一章。

### <a name="fe25a7da-4e4e-4388-8907-8abc2d33cfd8"></a>適用於 SAP 之 Azure 監視基礎結構的進一步疑難排解

#### ![Windows][Logo_Windows] 根本未顯示 Azure 效能計數器
Windows 服務 ‘AzureEnhancedMonitoring’ 會收集 Azure 上的效能度量。如果尚未正確安裝服務，或未在 VM 中執行服務，則根本不會收集任何效能度量。

##### Azure Enhanced Monitoring Extension 的安裝目錄是空白 
...
##### Azure Enhanced Monitoring 的服務不存在 

###### 問題
Windows 服務 ‘AzureEnhancedMonitoring’ 不存在。Azperflib.exe：azperlib.exe 輸出擲回錯誤 (如[下圖][deployment-guide-figure-14]所示)。
 
![執行 azperflib.exe 即表示 Azure Enhanced Monitoring Extension for SAP 的服務未執行][deployment-guide-figure-1400] <a name="figure-14"></a>

###### 方案
如果服務不存在 (如[上圖][deployment-guide-figure-14]所示)，表示尚未正確安裝 Azure Monitoring Extension for SAP。根據 [Microsoft Azure 上適用於 SAP 的 VM 部署案例][deployment-guide-3]一章中針對您部署案例所述的步驟，重新部署此擴充功能。

部署此擴充功能之後，請在 1 個小時之後重新檢查 Azure VM 內是否提供 Azure 效能計數器。

##### Azure Enhanced Monitoring 的服務存在，但無法啟動 
...
#### ![Windows][Logo_Windows] 遺失一些 Azure 效能計數器
...
#### ![Linux][Logo_Linux] 根本未顯示 Azure 效能計數器
...
#### ![Linux][Logo_Linux] 遺失一些 Azure 效能計數器

從數個來源取得資料的 Deamon 會收集 Azure 上的效能度量。某些組態資料是在本機進行收集、從 Azure Diagnostics 讀取效能度量，並從您登入儲存體訂用帳戶層級開始使用儲存體計數器。

如需完整和最新的已知問題清單，請參閱 SAP 附註 [1999351]，包含 Enhanced Azure Monitoring for SAP 的其他疑難排解資訊。
