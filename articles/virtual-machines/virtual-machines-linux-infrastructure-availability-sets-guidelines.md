...
Azure 中的基礎結構被分為「更新網域」和「容錯網域」。這些網域是會根據主機是共用一般更新週期，或共用類似的實體基礎結構 (例如電源和網路功能) 來定義。Azure 會自動將您位於可用性設定組內的 VM 散佈於網域上，以維護可用性和容錯。根據應用程式的大小，以及可用性設定組內的 VM 數目，您可以調整想要使用的網域數目。您可以深入了解[管理可用性及更新和容錯網域的使用](virtual-machines-linux-manage-availability.md)。
