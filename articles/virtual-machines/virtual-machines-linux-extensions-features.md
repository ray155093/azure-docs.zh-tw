...
- 使用 Microsoft 監視代理程式 VM 擴充功能設定虛擬機器的監視。 如需詳細資訊，請參閱[啟用或停用 VM 監視](virtual-machines-linux-vm-monitoring.md)。
...
Azure 虛擬機器擴充功能可以使用 `azure vm extension set` 命令對現有的虛擬機器執行。 這個範例會針對虛擬機器執行自訂指令碼延伸模組。
...
如需詳細資訊，請參閱[使用 Linux VM 擴充功能編寫 Azure Resource Manager 範本](virtual-machines-linux-extensions-authoring-templates.md)。
...
| 擴充功能名稱 | 說明 | 詳細資訊 |
| --- | --- | --- |
| Linux 的自訂指令碼擴充功能 |對「Azure 虛擬機器」執行指令碼 |[Linux 的自訂指令碼擴充功能](virtual-machines-linux-extensions-customscript.md?toc=%2fazure%2fvirtual-machines%2flinux%2ftoc.json) |
| Docker 擴充功能 |安裝 Docker 背景程式以支援遠端 Docker 命令。 |[Docker VM 擴充功能](virtual-machines-linux-dockerextension.md?toc=%2fazure%2fvirtual-machines%2flinux%2ftoc.json) |
