...
4. 填寫虛擬機器 [基本資訊] 表單。 對於 [資源群組]，建立新的資源群組。 資源群組是在其中建立和共同管理 Azure 資源的邏輯容器。 完成時，按一下 [確定]。
...
1. 按一下虛擬機器刀鋒視窗上的 [連線] 按鈕。 這會建立並下載遠端桌面通訊協定檔案 (.rdp 檔案)，該檔案就像是用來連接到您的電腦的捷徑。 開啟此檔案以連線至您的 VM。

    ![入口網站 9](./media/virtual-machine-quick-start/portal-quick-start-9.png) 

3. 按一下 [遠端桌面連線] 視窗上的 [連線]。

    ![入口網站 10](./media/virtual-machine-quick-start/portal-quick-start-10.png) 

4. 輸入在建立虛擬機器時指定的使用者名稱和密碼，然後按一下 [確定]。

5. 您可能會收到憑證警告，按一下 [是] 進行連線。
