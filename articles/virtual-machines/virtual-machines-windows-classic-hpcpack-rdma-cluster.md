...
5. **將節點新增至叢集**
   ...
   > [!NOTE]
   > 在每個使用計算密集型執行個體來進行的「將暴增的工作負載移至 Azure」部署中，除了您指定的 Azure 背景工作角色執行個體以外，HPC Pack 還會自動部署至少 2 個支援 RDMA 的執行個體 (例如 A8) 作為 Proxy 節點。 Proxy 節點會使用配置給訂用帳戶的核心，因此會隨著 Azure 背景工作角色執行個體產生費用。
   > 
   > 
6. **啟動 (佈建) 節點並使其上線執行工作**
