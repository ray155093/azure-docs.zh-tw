...
作業系統磁碟和資料磁碟的大小上限為 1023 GB (一個 GB 是 1024<sup>3</sup> 位元組)，因為 blob 的大小上限為 1024 GB 且必須包含 VHD 檔案的中繼資料 (頁尾)。您可以使用邏輯磁碟區管理員 (LVM) 來超越此限制，方法是共用資料磁碟以向 VM 展現大於 1023GB 的邏輯磁碟區。
...
根據 VM 的大小而定，Azure 會強制限制資料磁碟數量和可用頻寬。如需詳細資訊，請參閱[虛擬機器的大小](virtual-machines-linux-sizes.md)。
