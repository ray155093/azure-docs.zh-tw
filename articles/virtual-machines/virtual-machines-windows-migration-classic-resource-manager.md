...
本文說明如何將基礎結構即服務 (IaaS) 資源從「傳統」部署模型移轉至 Resource Manager 部署模型。 您可以進一步了解 [Azure Resource Manager 功能和優點](../resource-group-overview.md)。 我們會詳細說明如何使用虛擬網路站對站閘道，將您訂用帳戶中並存之兩個部署模型的資源連接在一起。 
...
>[AZURE.NOTE] 在一些移轉案例中，Azure 平台會將您的虛擬機器停止、解除配置及重新啟動。 這會造成短暫的資料平面停機時間。
...
>[AZURE.NOTE] 在此移轉範圍內，移轉期間可能會有一段時間不允許進行管理平面作業。 針對先前所述的某些組態，將會發生資料平面停機時間。
...
>[AZURE.NOTE] Resource Manager 部署模型並沒有「傳統」映像和磁碟的概念。 移轉儲存體帳戶時，「傳統」映像和磁碟不會顯示在 Resource Manager 堆疊中，但是作為基礎的 VHD 會繼續留在儲存體帳戶中。 
...
網路 | 使用 VNet 對等互連的虛擬網路。 (將 VNet 移轉至 ARM，仍後移轉至對等) 深入了解 [VNet 對等互連] (../virtual-network/virtual-network-peering-overview.md)。
網路 | 流量管理員設定檔。
...
>[AZURE.NOTE] 下列各節描述的所有作業都是等冪的。 如果您有除了不支援的功能或組態錯誤以外的任何問題，建議您重新嘗試準備、中止或認可作業。 平台將會重新嘗試該動作。
...
>[AZURE.NOTE] 在這個移轉階段中，會將不在傳統「虛擬網路」中的「虛擬機器」停止並解除配置。
...
>[AZURE.NOTE] 這是一種等冪作業。 如果發生失敗，建議您重新嘗試執行該作業。 如果持續發生失敗，請建立支援票證，或是在我們的 [VM 論壇](https://social.msdn.microsoft.com/Forums/azure/en-US/home?forum=WAVirtualMachinesforWindows)上建立一篇標籤為 ClassicIaaSMigration 的論壇文章。
