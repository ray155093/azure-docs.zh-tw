...
若要利用 Azure N 系列 VM (執行支援的 Linux 散發套件) 的 GPU 功能，您必須在部署之後於每個 VM 上安裝 NVIDIA 圖形驅動程式。 驅動程式設定資訊也適用於 [Windows VM](virtual-machines-windows-n-series-driver-setup.md?toc=%2fazure%2fvirtual-machines%2fwindows%2ftoc.json)。
...
如需 N 系列 VM 規格、儲存體容量與磁碟的詳細資料，請參閱[虛擬機器的大小](virtual-machines-linux-sizes.md?toc=%2fazure%2fvirtual-machines%2flinux%2ftoc.json)。 另請參閱 [N 系列 VM 的一般考量](#general-considerations-for-n-series-vms)。
...
* 如果您想要擷取已安裝 NVIDIA 驅動程式之 Linux VM 的映像，請參閱[如何一般化和擷取 Linux 虛擬機器](virtual-machines-linux-capture-image.md?toc=%2fazure%2fvirtual-machines%2flinux%2ftoc.json)。
