 ...
 * 用於 Azure 資源群組的工作或學校識別碼。如果您有個人帳戶且使用 Microsoft ID 登入，則您需要[以您的個人識別碼建立一個工作識別碼](virtual-machines-windows-create-aad-work-id.md)。
...
11. 資源群組佈建之後，您可以在 Azure 傳統入口網站上看到群組中的所有資源。如以下螢幕擷取畫面所示，資源群組內有包含三個 VM 的虛擬網路，這些 VM 加入同一個可用性設定組。該群組也包含負載平衡器，其具有關聯的公用 IP。
  ...
  ![在 Azure 傳統入口網站佈建的資源群組](media/virtual-machines-linux-deis-cluster/resource-group.png)
...
![負載平衡器上的 NAT 規則](media/virtual-machines-linux-deis-cluster/nat-rules.png)
...
4. 在 GitHub 加入 id\_rsa.pub，或您自行選擇的公用金鑰。您可以使用 SSH 金鑰設定畫面上的 [Add SSH key] 按鈕來加入。
  ...
  ![Github 金鑰](media/virtual-machines-linux-deis-cluster/github-key.png)
<p />
5. 註冊新的使用者：
