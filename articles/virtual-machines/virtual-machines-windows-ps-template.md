---
title: "使用 Resource Manager 範本建立 VM | Microsoft Docs"
...
> [!IMPORTANT]
> 如果您希望 VM 屬於可用性設定組的一部分，可在建立 VM 時將其加入設定組。 目前不支援在建立 VM 之後，再將其加入可用性設定組。
>
>

## <a name="step-1-create-the-template-file"></a>步驟 1：建立範本檔案
您可以使用 [編寫 Azure Resource Manager 範本](../azure-resource-manager/resource-group-authoring-templates.md)中的資訊來建立專屬範本。 您也可以從 [Azure 快速入門範本](https://azure.microsoft.com/documentation/templates/)部署已為您建立的範本。
...
2. [參數](../azure-resource-manager/resource-group-authoring-templates.md#parameters)並非總是必要的項目，但它們能提供在部署範本時輸入值的方式。 在 ContentVersion 元素之後加入參數元素及其子元素：
...
3. [變數](../azure-resource-manager/resource-group-authoring-templates.md#variables) 可以在範本中用來指定會經常變更的值或需要透過參數值組合建立的值。 在參數區段之後加入變數元素：
...
4. [資源](../azure-resource-manager/resource-group-authoring-templates.md#resources) ，例如虛擬機器、虛擬網路，以及儲存體帳戶接下來會在範本中定義。 在變數區段之後加入資源區段：
  ...
  > [!NOTE]
  > 本文會建立執行 Windows Server 作業系統版本的虛擬機器。 若要深入了解如何選取其他映像，請參閱 [使用 Windows PowerShell 和 Azure CLI 來瀏覽和選取 Azure 虛擬機器映像](virtual-machines-linux-cli-ps-findimage.md?toc=%2fazure%2fvirtual-machines%2flinux%2ftoc.json)。

5. 將範本檔案另存為 *VirtualMachineTemplate.json*。
...
1. 在文字編輯器中，將此 JSON 內容複製到名為 *Parameters.json*的新檔案中：
  ...
  ```
   ...
   > [!NOTE]
   > 進一步了解 [使用者名稱和密碼需求](virtual-machines-windows-faq.md#what-are-the-username-requirements-when-creating-a-vm)。
   >
   >
2. 儲存參數檔案。
...
> [!NOTE]
> 您也可以從 Azure 儲存體帳戶部署範本和參數。 若要深入了解，請參閱[搭配使用 Azure PowerShell 與 Azure 儲存體](../storage/storage-powershell-guide-full.md)。
>
>
