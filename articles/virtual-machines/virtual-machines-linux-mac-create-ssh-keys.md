---
title: "建立 Linux VM 的 SSH 金鑰組 | Microsoft Docs"
description: "安全地建立 Linux VM 的 SSH 公用和私用金鑰組。"
...
依預設，SSH 金鑰會保留在 `~/.ssh` 目錄中。  如果您沒有 `~/.ssh` 目錄，`ssh-keygen` 命令會使用正確的權限為您建立。  `-N` cli 旗標是用來加密私密 SSH 金鑰的密碼，而「不是」您的使用者密碼。
