---
title: 標題︰Windows 虛擬機器 (VM) 上的 SAP NetWeaver - 部署指南 | Microsoft Docs
...
---
# Windows 虛擬機器 (VM) 上的 SAP NetWeaver - 部署指南
...
* [Windows 虛擬機器 (VM) 上的 SAP NetWeaver - 部署指南 (本文件)][deployment-guide]
...
#### 安裝 VM 代理程式
如果您想要使用上述的範本，則「VM 代理程式」必須已安裝在 OS 磁碟中。否則，部署將會失敗。請如這份文件的[下載、安裝和啟用 Azure VM 代理程式][deployment-guide-4.4]一章所述，下載「VM 代理程式」並安裝在 VM 中。
