---
title: "Azure 網路基礎結構指導方針 - Windows | Microsoft Docs"
...
透過使用資源群組，您便能彈性地設計您的虛擬網路元件。 VM 可以連接到其資源群組之外的虛擬網路。 一個常見的設計方式，便是建立可由一般小組管理且包含核心網路基礎結構的集中式資源群組，並將 VM 和其應用程式部署到其他的資源群組。 這種方式可讓應用程式擁有者存取包含其 VM 的資源群組，而不需開放存取更廣泛虛擬網路資源的設定。
...
如果內部部署的使用者和電腦需要持續連線到 Azure 虛擬網路中的 VM，則可建立跨單位的虛擬網路。  使用 ExpressRoute 或站對站 VPN 連線，將它連接到您的內部部署網路。
