...
本文描述套用至虛擬機器的 Azure Resource Manager 範本的各個部分。 本文不會說明建立虛擬機器的完整範本；您需要儲存體帳戶、網路介面、公用 IP 位址與虛擬網路的資源定義。 如需有關如何將這些資源一起定義的詳細資訊，請參閱 [Resource Manager 範本逐步解說](../resource-manager-template-walkthrough.md)。
...
- [名稱](virtual-machines-linux-infrastructure-naming-guidelines.md)和認證
- 磁碟和[作業系統設定](virtual-machines-windows-cli-ps-findimage.md)
...
雖然[擴充功能](virtual-machines-windows-extensions-features.md)是不同的資源，它們會緊密繫結至 VM。 擴充功能可新增為 VM 的子資源或不同的資源。 範例會示範要新增至 VM 的[診斷擴充功能](virtual-machines-windows-extensions-diagnostics-template.md)：
...
有許多您可以在 VM 上安裝的擴充功能，而最有用的可能是[自訂指令碼擴充功能](virtual-machines-windows-extensions-customscript.md)。 在範例中，名為 start.ps1 的 PowerShell 指令碼第一次啟動時會在每個 VM 上執行︰
...
![取得部署資訊](./media/virtual-machines-windows-template-description/virtual-machines-deployment-info.png)
