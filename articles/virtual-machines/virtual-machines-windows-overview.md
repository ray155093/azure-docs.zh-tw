...
當您在 Azure 中建置應用程式基礎結構時，總是會有許多[設計考量](virtual-machines-windows-infrastructure-virtual-machine-guidelines.md)。 在您開始之前，仔細考量 VM 的這些層面很重要︰
...
### <a name="naming"></a>命名
虛擬機器指派一個[名稱](virtual-machines-windows-infrastructure-naming-guidelines.md)給它，而它設定一個電腦名稱作為作業系統的一部分。 VM 的名稱最多可為 15 個字元。

如果您使用 Azure 來建立作業系統磁碟，則電腦名稱和虛擬機器名稱為相同。 如果您[上傳並使用您自己的映像](virtual-machines-windows-upload-image.md) (該映像包含先前所設定的作業系統)，並用它來建立虛擬機器，則名稱可能會不同。 我們建議您，當您上傳自己的映像檔時，將作業系統中的電腦名稱設為與虛擬機器的名稱相同。
...
您使用的 VM [大小](virtual-machines-windows-sizes.md)是由所您要執行的工作負載所決定。 您所選的大小會決定例如處理電源、記憶體和儲存體容量等因素。 Azure 提供了各種不同的大小，以支援許多類型的用法。
...
虛擬機器是使用[虛擬硬碟 (VHD)](virtual-machines-windows-about-disks-vhds.md) 來儲存其作業系統 (OS) 和資料。 VHD 也能夠使用於您可以選擇用來安裝 OS 的映像。 
...
您可以選擇[上傳並使用您自己的映像](virtual-machines-windows-upload-image.md)，當您這麼做時，不會使用發行者名稱、優惠和 SKU。

### <a name="extensions"></a>擴充功能
VM [擴充](virtual-machines-windows-extensions-features.md)會透過 post 部署設定及自動化工作為您的 VM 提供其他功能。

可以使用擴充功能來完成這些常見工作︰

* **執行自訂指令碼** – [自訂指令碼延伸模組](virtual-machines-windows-extensions-customscript.md)可藉由當佈建 VM 時執行您的指令碼來協助您在 VM 上設定工作負載。
* **部署和管理組態** – [PowerShell 期望狀態組態 (DSC) 延伸模組](virtual-machines-windows-extensions-dsc-overview.md)可協助您設定 VM 上的 DSC 來管理組態和環境。
...
| [資料磁碟](virtual-machines-windows-attach-disk-portal.md) |否 |VM 可以包含資料磁碟來擴充儲存體功能。 |
...
| 方法 | 文章 |
| --- | --- |
| Azure 入口網站 |[使用入口網站建立執行 Windows 的虛擬機器](virtual-machines-windows-hero-tutorial.md) |
| 範本 |[利用 Resource Manager 範本建立 Windows 虛擬機器](virtual-machines-windows-ps-template.md) |
| Azure PowerShell |[使用 PowerShell 建立 Windows VM](virtual-machines-windows-ps-create.md) |
| 用戶端 SDK |[使用 C# 部署 Azure 資源](virtual-machines-windows-csharp.md) |
| REST API |[建立或更新 VM](https://msdn.microsoft.com/library/mt163591.aspx) |

您希望它絕對不會發生，但偶爾會發生錯誤。 如果您發生這種情況，請查看[疑難排解在 Azure 中建立 Windows 虛擬機器時發生的 Resource Manager 部署問題](virtual-machines-windows-troubleshoot-deployment-new-vm.md)中的資訊。
...
| Azure PowerShell |如需使用 PowerShell 來管理 VM 的相關資訊，請參閱[使用 Resource Manager 和 PowerShell 來管理 Azure 虛擬機器](virtual-machines-windows-ps-manage.md)。 |
| REST API |使用[取得 VM 資訊](https://msdn.microsoft.com/library/mt163682.aspx)作業來取得 VM 的相關資訊。 |
| 用戶端 SDK |如需使用 C# 來管理 VM 的相關資訊，請參閱[使用 Azure Resource Manager 和 C# 來管理 Azure 虛擬機器](virtual-machines-windows-csharp-manage.md)。 |

### <a name="log-on-to-the-vm"></a>登入 VM
您使用 Azure 入口網站中的 [連線] 按鈕[啟動遠端桌面 (RDP) 工作階段](virtual-machines-windows-connect-logon.md)。 嘗試使用遠端連線時，有時可能會出錯。 如果您發生這種情況，請參閱[疑難排解遠端桌面連線至執行 Windows 的 Azure 虛擬機器](virtual-machines-windows-troubleshoot-rdp-connection.md)中的說明資訊。

### <a name="manage-availability"></a>管理可用性
請務必了解如何[確保應用程式的高可用性](virtual-machines-windows-manage-availability.md)。 此組態需要建立多個 VM 以確保至少有一個 VM 正在執行。

為了讓您的部署符合 99.95 的 VM 服務等級協定資格，您必須部署兩個或更多在[可用性設定組](virtual-machines-windows-infrastructure-availability-sets-guidelines.md)內執行工作負載的 VM。 此組態可確保您的 VM 會分散多個容錯網域，且部署至具有不同維護期間的主機。 完整 [Azure SLA](https://azure.microsoft.com/support/legal/sla/virtual-machines/v1_0/) 說明保證的 Azure 整體可用性。
...
* 如果您的目的是要使用 Linux VM，請查看 [Azure 與 Linux](virtual-machines-linux-azure-overview.md)。
* 在[範例 Azure 基礎結構逐步解說](virtual-machines-windows-infrastructure-example.md)中深入了解設定基礎結構的指導方針。
* 請確定您遵循[在 Azure 上執行 Windows VM 的最佳作法](virtual-machines-windows-guidance-compute-single-vm.md)。
