...
MHANA One | Red Hat Enterprise Linux、SUSE Linux Enterprise | DS14\_v2 (正式運作後)
...
摘要︰本文件說明如何在 Azure 中保護 SAP 單點失敗元件，例如 SAP ASCS/SCS 和 DBMS。SAP ASCS/SCS、 DBMS 及應用程式伺服器的元件是 SAP NetWeaver 系統 (例如 SAP NetWeaver ABAP、SAP NetWeaver Java、SAP NetWeaver ABAP+Java 系統) 功能不可或缺的要素。因此，高可用性功能
