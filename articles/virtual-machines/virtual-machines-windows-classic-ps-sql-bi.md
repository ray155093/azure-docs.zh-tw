...
* 使用 SQL Server Enterprise Edition 時，虛擬機器的最小建議大小為 **A3** 。 Analysis Services 和 Reporting Services 的 SQL Server BI 部署建議的虛擬機器大小為 **A4** 。
    ...
    如需關於目前 VM 大小的詳細資訊，請參閱 [Azure 的虛擬機器大小](virtual-machines-linux-sizes.md)。
...
1. 在左窗格中，按一下 [Web Portal URL](入口網站 URL.md) 或 2012 和 2014 版的 [報表管理員 URL]。
2. 按一下 [Apply (套用)] 。
...
* 建立包含報表的 .VHD 硬碟機，然後上傳並連接磁碟機。
  ...
  2. 建立及安裝管理憑證。
  3. 使用 Add-AzureVHD Cmdlet 將 VHD 檔案上傳至 Azure [建立及上傳 Windows Server VHD 至 Azure](virtual-machines-windows-classic-createupload-vhd.md)。
  4. 將磁碟連接至虛擬機器。
