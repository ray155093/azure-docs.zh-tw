...
>[AZURE.NOTE]如果發生中斷前尚未設定 Azure Site Recovery、虛擬機器備份、讀取權限異地備援儲存體或異地備援儲存體，這便是您的最佳選項。如果儲存 VM 虛擬硬碟 (VHD) 的儲存體帳戶已設定異地備援儲存體或讀取權限異地備援儲存體，您可以指望復原基本映像 VHD，並嘗試用它佈建新的 VM。因為除非使用 Azure VM 備份或 Azure Site Recovery，否則不保證能夠同步處理資料，所以這不是慣用的選項。因此，不保證此選項可以運作。

想要立即存取虛擬機器的客戶有下列兩個選項可用。

>[AZURE.NOTE]請注意，下列兩個選項都可能會遺失部分資料。
