...
本文假設您已安裝 Azure PowerShell 1.0.x 版。我們建議您使用這個版本，因為較舊版本的 PowerShell 將不會加入新的資源管理員功能。請參閱 [Azure PowerShell 1.0](https://azure.microsoft.com/blog/azps-1-0/) 來深入了解版本之間的差異。
...
[Azure 資源總管 (Preview)](https://azure.microsoft.com/blog/azure-resource-explorer-a-new-tool-to-discover-the-azure-api/) 是一款新工具，可用以管理在資源管理員部署模型中建立的 Azure 資源。您可以利用這個工具來輕鬆地

- 探索 Azure 資源管理 API。
- 取得 API 文件。
- 直接在您的 Azure 訂用帳戶中呼叫 API。

若要深入了解運用這強大工具可進行的所有工作，請觀賞以下影片：[Azure Resource Manager Explorer with David Ebbo (與 David Ebbo 一起探討 Azure Resource Manager)](https://channel9.msdn.com/Shows/Azure-Friday/Azure-Resource-Manager-Explorer-with-David-Ebbo)。
...
7. 您可以在 [擷取] 動作按鈕下，設定擷取映像所用的值。您填入的值看起來可能會類似下圖。
	...
	按一下 [擷取] 動作按鈕，可擷取您虛擬機器的映像。如此會為該映像建立新的 VHD，以及 JSON 範本檔案。截至目前為止還無法透過資源總管或 [Azure 入口網站](https://portal.azure.com)存取這些項目。

8. 若要存取新的映像 VHD 及範本，請下載並安裝用來管理儲存體資源的 Azure 工具：[Azure 儲存體總管](http://storageexplorer.com/)。此安裝程式將會在您的本機電腦上安裝 Azure 儲存體總管。
	...
	- 請按兩下 [Microsoft.Compute] > [映像]，此時畫面會顯示您所有的映像資料夾。按兩下您剛才從資源總管擷取影像時，為 **destinationContainerName** 變數所輸入的資料夾名稱。如此會顯示 VHD 以及 JSON 範本檔案。
...
請使用下列範例 PowerShell 指令碼，來為新的 VM 設定虛擬網路和 NIC。為變數 (以 **$** 符號表示) 使用適合您應用程式的值。
...
下列 PowerShell 指令碼示範如何設定虛擬機器組態，以及如何將擷取的 VM 映像當做新安裝的來源。</br>
