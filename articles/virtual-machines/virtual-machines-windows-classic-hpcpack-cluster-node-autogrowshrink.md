...
* **針對部署在 Azure VM (傳統部署模型) 中的叢集** - 在前端節點 VM 執行指令碼，因為它相依於所安裝的 **Start-HpcIaaSNode.ps1** 和 **Stop-HpcIaaSNode.ps1** 指令碼。 這些指令碼額外需要 Azure 管理憑證或發佈設定檔 (請參閱 [在 Azure 中的 HPC Pack 叢集管理計算節點](virtual-machines-windows-classic-hpcpack-cluster-node-manage.md))。 請確定您所需的所有計算節點 VM 已加入叢集。 它們可能處於「已停止」狀態。
