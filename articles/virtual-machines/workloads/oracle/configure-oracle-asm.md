...
10. 變更 Root、Oracle 和 Grid 使用者的密碼。 (您稍後在安裝期間會使用密碼。)
...
7. 檢查可用的交換空間。 您必須至少有 6.8 GB 的交換空間以便安裝格線。 Linux Azure VM 依預設並未啟用並設定交換。

強烈建議您，使用 waagent 來設定交換空間，讓它一律會建立在暫時的磁碟 (暫存磁碟) 上。 如需詳細資訊，請參閱[如何在 Linux Azure 虛擬機器中新增分頁檔](https://support.microsoft.com/en-us/help/4010058/how-to-add-a-swap-file-in-linux-azure-virtual-machines)。
...
1. 將 [PuTTY](http://www.putty.org/) 和 [Xming](https://xming.en.softonic.com/) 下載到 Windows 電腦。
...
6. 啟動 PuTTY。 在 [類別] 窗格中，移至 [連線] > [SSH] > [驗證]。 在 [用於驗證的私密金鑰檔] 方塊中，瀏覽至您稍早產生的金鑰。
...
6. (選擇性：) 在 [指定管理選項] 頁面上，選取 [向 Enterprise Manager (EM) Cloud Control 進行註冊] 核取方塊。
...
12. 在 [修復指令碼] 對話方塊中，選取 [確定]。

    ![[修復指令碼] 對話方塊的螢幕擷取畫面](./media/oracle-asm/install11.png)
...
14. 此時會出現警告對話方塊。 選取 [是] 以繼續進行。
    ...
    ![[警告] 對話方塊的螢幕擷取畫面](./media/oracle-asm/install14.png)
...
2. 在 [設定 ASM: 磁碟群組] 對話方塊中，選取 [建立] 按鈕，然後再選取 [顯示進階選項]。

   ![[設定 ASM: 磁碟群組] 對話方塊的螢幕擷取畫面](./media/oracle-asm/asm01.png)
...
4. 在 [設定 ASM: 磁碟群組] 對話方塊中，選取 [建立] 按鈕，然後再選取 [顯示進階選項]。

   ![[設定 ASM: 磁碟群組] 對話方塊的螢幕擷取畫面](./media/oracle-asm/asm03.png)
