...
1. 將 kickstart 檔案放在可從安裝系統存取的位置。
2. 在 Hyper-V 管理員中建立新的 VM。在 [連線虛擬硬碟] 頁面上，選取 [稍後連接虛擬硬碟]，並完成 [新增虛擬機器精靈]。
3. 開啟 VM 設定：
   ...
   c.將 BIOS 設定成從 CD 開機。
4. 啟動 VM。當安裝指南出現時，請按 **Tab** 鍵以設定開機選項。
5. 在開機選項結尾輸入 `inst.ks=<the location of the kickstart file>`，然後按 **Enter** 鍵。
6. 等待安裝完成。完成後，VM 將會自動關閉。您現在可以將 Linux VHD 上傳至 Azure。
