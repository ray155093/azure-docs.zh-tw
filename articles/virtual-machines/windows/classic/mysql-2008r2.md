...
如果您還沒有執行 Windows Server 2012 R2 的 VM，則可以使用這個[教學課程](tutorial.md)來建立虛擬機器。 

## <a name="attach-a-data-disk"></a>連接資料磁碟
建立虛擬機器之後，您可以選擇連接其他資料磁碟。 對於生產工作負載建議執行此操作，這可避免包括作業系統的 OS 磁碟機 (C:) 空間不足。

請參閱[如何將資料磁碟附加至 Windows 虛擬機器](attach-disk.md)，並依照指示連接空的磁碟。 將主機快取設定設為 [無] 或 [唯讀]。
...
> [!NOTE]
> 您應該仔細考慮這麼做對安全性的影響，因為這會將 MySQL 伺服器服務提供給網際網路上的所有電腦。 您可以定義來源 IP 位址的集合，以便使用具備存取控制清單 (ACL) 的端點。 如需詳細資訊，請參閱[如何設定虛擬機器的端點](setup-endpoints.md)。
> 
> 
