...
在您要上傳至 Azure 的虛擬機器上，使用[系統管理權限](https://technet.microsoft.com/library/cc947813.aspx)從命令提示字元視窗執行下列所有的命令。
...
## <a name="run-sysprep--a-idstep23a"></a>執行 Sysprep  <a id="step23"></a>    
如果您想要建立映像以部署至多部電腦，您需要先[執行 Sysprep 來將該映像一般化](virtual-machines-windows-generalize-vhd.md?toc=%2fazure%2fvirtual-machines%2fwindows%2ftoc.json)，然後將 VHD 上傳至 Azure。 您不需要執行 Sysprep 即可使用特定的 VHD。 如需詳細資訊，請參閱下列文章。
   
   * [使用 Sysprep 將 Windows 虛擬機器一般化](virtual-machines-windows-generalize-vhd.md?toc=%2fazure%2fvirtual-machines%2fwindows%2ftoc.json)
