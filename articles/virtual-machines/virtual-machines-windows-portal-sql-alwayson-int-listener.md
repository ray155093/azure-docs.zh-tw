...
如有需要，您可以[手動設定 AlwaysOn 可用性群組](virtual-machines-windows-portal-sql-alwayson-availability-groups-manual.md)。
...
- 在負載平衡器的 [設定] 刀鋒視窗上，按一下 [IP 位址]。[IP 位址] 刀鋒視窗會顯示這是與您的 SQL Server 位於相同虛擬網路上的私用人負載平衡器。 
...
負載平衡器現在便有 IP 位址。請記下此 IP 位址。當您在叢集上建立接聽程式時，您將使用此 IP 位址。在本文稍後的 PowerShell 指令碼，將此位址用於 `$ILBIP` 變數。
...
- 按一下 [選擇虛擬機器] 之下的 [選擇可用性設定組]，並指定 SQL Server 虛擬機器所屬的可用性設定組。

- 選擇可用性設定組之後，請按一下 [選擇虛擬機器]。按一下兩個裝載可用性群組中的 SQL Server 執行個體的虛擬機器。按一下 [選取]。

- 按一下 [確定]，關閉 [選擇虛擬機器] 和 [新增後端集區] 的刀鋒視窗。
...
| **通訊協定** | **TCP** |
| **連接埠** | *1433* |
| **後端連接埠** | *1433*。請注意，這將停用，因為此規則使用 [浮動 IP (伺服器直接回傳)。 |
...
- 選取 [網路] 節點，然後記下叢集網路名稱。此名稱將使用於 PowerShell 指令碼的 `$ClusterNetworkName` 變數中。
...
- 按一下 [IP 位址] 之下的 [靜態 IP 位址]，並將靜態 IP 位址設定為您在 Azure 入口網站上設定負載平衡器 IP 位址時所用的相同位址。針對此位址啟用 NetBIOS，然後按一下 [確定]。如果您的方案跨越多個 Azure VNet，請針對每個 IP 資源重複此步驟。
...
- 在 [連接埠] 方塊中，使用您稍早所用的 $EndpointPort，指定可用性群組接聽程式的連接埠號碼 (預設值是 1433)，然後按一下 [確定]。
