...
VM 完成佈建之後，使用 VM 的外部 IP 位址 (或 DNS 名稱) 以及您設定和自訂的外部連接埠編號來設定 VM 的 SSH。如需連接詳細資料，請參閱[如何登入執行 Linux 的虛擬機器](virtual-machines-linux-classic-log-on.md)。您應該以您在 VM 上設定的使用者身分來執行命令，除非需要根目錄存取才能完成步驟。
