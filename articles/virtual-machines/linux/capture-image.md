...
若要重複使用在 Azure 中部署和設定的虛擬機器 (VM)，您可擷取 VM 的映像。 此程序也牽涉到將 VM 一般化，以便在從映像部署新 VM 之前，移除個人帳戶資訊。 本文詳細說明如何針對使用 Azure 受控磁碟的 VM，透過 Azure CLI 2.0 擷取 VM 映像。 這些磁碟是由 Azure 平台處理，不需要任何準備或位置來儲存它們。 如需詳細資訊，請參閱 [Azure 受控磁碟概觀](../../storage/storage-managed-disks-overview.md)。 本文詳述如何使用 Azure CLI 2.0 來擷取 Linux VM。 您也可以使用 [Azure CLI 1.0](capture-image-nodejs.md) 來執行這些步驟。

> [!TIP]
> 如果您想要建立一份您現有 Linux VM 的複本，當中包含其特殊的備份或偵錯狀態，請參閱[建立在 Azure 上執行的 Linux 虛擬機器複本](copy-vm.md)。 如果您想要從內部部署 VM 上傳 Linux VHD，請參閱[上傳自訂磁碟映像並從這個映像建立 Linux VM](upload-vhd.md)。  


## <a name="before-you-begin"></a>開始之前
請確保符合下列必要條件︰

* **在 Resource Manager 部署模型中建立的 Azure VM** - 如果您尚未建立 Linux VM，可以使用[入口網站](quick-create-portal.md)、[Azure CLI](quick-create-cli.md) 或 [Resource Manager 範本](cli-deploy-templates.md)。 視需要設定 VM。 例如，[新增資料磁碟](add-disk.md)、套用更新，並安裝應用程式。 
...
1. 透過 SSH 連線至您的 VM 並使用 `waagent -deprovision` 加以佈建。 +user 參數也會移除最後一個佈建的使用者帳戶。 如果您將帳戶認證備份至 VM 中，則省略此 +user 參數。 下列範例會移除最後一個佈建的使用者帳戶：
...
3. 使用 [az vm generalize](/cli/azure/vm#generalize) 將 VM 一般化。 如果您已使用一項工具 (例如 [Packer](http://www.packer.io)) 來建置來源 VM，請略過此步驟，因為您的映像已經一般化。
...
在下列步驟中，您將取消佈建現有的 VM、解除配置及一般化 VM，然後建立映像。 您可以使用此映像，在您訂用帳戶的任何資源群組中建立 VM。 此程序讓 [Azure 受控磁碟](../../storage/storage-managed-disks-overview.md)的優勢超越非受控磁碟。 使用非受控磁碟，您受限於在與所複製 VHD blob 相同的儲存體帳戶中建立 VM。 使用受控磁碟，您可建立可跨整個訂用帳戶部署的映像資源。

## <a name="step-1-remove-the-azure-linux-agent"></a>步驟 1：移除 Azure Linux 代理程式
若要讓 VM 可供一般化，請使用 Azure VM 代理程式來取消佈建 VM，以刪除相關檔案和資料。 在目標 Linux VM 上使用 `waagent` 命令搭配 -deprovision 參數。 +user 參數也會移除最後一個佈建的使用者帳戶。 如果您將帳戶認證備份至 VM 中，則省略此 +user 參數。 下列範例會移除最後一個佈建的使用者帳戶。 如需詳細資訊，請參閱 [Azure Linux 代理程式使用者指南](../windows/agent-user-guide.md)。
...
2. 使用 [az vm generalize](/cli//azure/vm#generalize) 將 VM 一般化。 如果您已使用一項工具 (例如 [Packer](http://www.packer.io)) 來建置來源 VM，請略過此步驟，因為您的映像已經一般化。 下列範例會將名為 myResourceGroup 的資源群組中名為 myVM 的 VM 一般化：
