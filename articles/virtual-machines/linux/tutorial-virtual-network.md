...
在本教學課程中，您將了解如何在虛擬網路中建立多部虛擬機器 (VM)，並設定這些機器間的網路連線。 完成時，將可從網際網路透過連接埠 22 (適用於 SSH 連線) 或連接埠 80 (適用於 HTTP 連線) 存取「前端」VM。 具有 MySQL 資料庫的「後端」VM 將被隔離，而只有從前端 VM 透過連接埠 3306 才能存取它。
...
- **myVMPublicIP** – 可啟用對 `myFrontendVM` 之網際網路存取的公用 IP 位址。
...
您也可以使用 NSG 來設定 VM 的內部通訊。 在本節中，您將了解如何在網路中建立額外的子網路並將 NSG 指派給它，以允許透過連接埠 3306 從 `myFrontendVM` 連線到 `myBackendVM`。 此子網路會接著在 VM 建立後會指派給 VM。

使用 [az network nsg create](https://docs.microsoft.com/cli/azure/network/nsg#create) 來新增一個名為 `myBackendNSG` 的新網路安全性群組。 
...
設定連接埠來讓 `myFrontendVM` 和 `myBackendVM` 在 VNet 中彼此互相通訊。 使用 `az network rule create` 來新增一個僅允許從 `myVMSubnet` 傳送流量到 `myBackendSubnet` 的 NSG 規則：
...
使用 [az network vnet subnet create](https://docs.microsoft.com/cli/azure/network/vnet/subnet#create) 將 `myBackEndSubnet` 新增到 `myFrontendVMVNet`：
...
使用 `az vm create` 來建立使用 `myBackendSubnet` 的 `myBackendVM`：
...
後端 VM 不應該可供公開存取。 在本節中，您將了解如何使用 SSH 來登入 `myFrontendVM`，然後使用 SSH 從 `myFrontendVM` 登入 `myBackendVM`。
...
從 `myFrontendVM` 連線到 `myBackendVM`：
...
安裝 MySQL 是為了示範如何將應用程式安裝在 `myBackendVM` 上，實際上在本教學課程中並未使用它。
