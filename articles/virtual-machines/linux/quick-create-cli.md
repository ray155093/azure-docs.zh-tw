...
Azure CLI 可用來從命令列或在指令碼中建立和管理 Azure 資源。 本指南詳細說明如何使用 Azure CLI 來部署執行 Ubuntu 16.04 LTS 的虛擬機器。 一旦部署伺服器，我們會透過 SSH 連線至 VM，以便安裝 NGINX。 
...
依預設只能透過 SSH 連線至 Azure 中部署的 Linux 虛擬機器。 如果此 VM 即將成為 Web 伺服器，您需要從網際網路開啟連接埠 80。  需使用單一命令才能開啟所需的連接埠。  
...
[建立高可用性的虛擬機器教學課程](create-cli-complete.md)

[瀏覽 VM 部署 CLI 範例](cli-samples.md)
