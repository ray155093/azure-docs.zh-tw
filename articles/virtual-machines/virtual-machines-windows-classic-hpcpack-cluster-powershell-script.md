...
對於網域樹系中的所有 VM，都會停用 Windows 更新的自動安裝。 所有雲端服務都直接建立在「東亞」位置中。 計算節點會建立在三個雲端服務和三個儲存體帳戶中：_MyHPCCNService01_ 和 _mycnstorage01_ 中的 _MyHPCCN-0001_ 至_MyHPCCN-0005_；_MyHPCCNService02_ 和 _mycnstorage02_ 中的 _MyHPCCN-0006_ 至 _MyHPCCN0010_；以及 _MyHPCCNService03_ 和 _mycnstorage03_ 中的 _MyHPCCN-0011_ 至 _MyHPCCN-0012_)。 計算節點會從擷取自雲端節點的現有私人映像建立。 自動增加和縮減服務會根據預設的增加和縮減間隔來啟用。
