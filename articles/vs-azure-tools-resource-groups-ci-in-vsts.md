...
要部署 Azure 範本，您需要執行各種階段的工作，包括組建、測試、複製到 Azure (也稱為「暫存」) 及部署範本。  有兩種不同的方式可部署範本 Visual Studio Team Services (VS Team Services)。 兩種方法所產生的結果都相同，因此請選擇最符合您工作流程的方法。
...
本文將示範兩個選項。  第一個選項的優點是能夠使用開發人員在 Visual Studio 中所使用相同的指令碼，在整個生命週期中提供一致性。  第二個選項可提供內建指令碼的方便替代選項。  這兩個程序假設您已經在 VS Team Services 中簽入 Visual Studio 部署專案。

## <a name="copy-artifacts-to-azure"></a>將構件複製到 Azure
無論何種情況，如果有任何需要進行範本部署的構件，您需要提供 Azure Resource Manager 存取權。 這些構件包括下列檔案：
...
當您使用 Visual Studio (或以 Visual Studio 程式碼片段建置的) 提供的範本時，PowerShell 指令碼不但會暫存構件，也會參數化資源 URI 以進行不同的部署。 接著，指令碼會將構件複製到 Azure 中的安全容器，並為該容器建立 SaS 權杖，再將該資訊傳遞至範本部署。 請參閱 [建立範本部署](https://msdn.microsoft.com/library/azure/dn790564.aspx) 以深入了解巢狀範本。  當在 VS Team Services 中使用工作時，您必須針對範本部署選取適當的工作，且如果必要，須從預備步驟將參數值傳遞至範本部署。
...
下列步驟會使用在您的專案中執行 PowerShell 指令碼的單一工作，逐步引導您進行在 VS Team Services 設定連續部署所需的步驟。 
...
4. 選擇 [Azure PowerShell]  組建步驟，再填上其值。
   ...
   5. 在 [新增 Azure 訂用帳戶]  對話方塊中新增 Azure 訂用帳戶資訊。 您必須先提供下列項目：
      ...
      * 服務主體金鑰
      * 租用戶識別碼
   6. 在 [訂用帳戶]  名稱方塊中新增您選擇的名稱。 此值稍後會出現在 VS Team Services 中的 [Azure 訂用帳戶] 下拉式清單中。 
...
5. 編輯組建定義並選擇 **Azure PowerShell** 建置步驟。 選取 [Azure 訂用帳戶] 下拉式清單方塊中的訂用帳戶。 (如果訂用帳戶未出現，請選擇 [管理] 連結旁的 [重新整理]按鈕。) 
...
下列步驟會使用內建工作逐步引導您進行在 VS Team Services 設定連續部署所需的步驟。
...
5. 選擇 **Azure 檔案複製**工作並填入其值。
   ...
   如果您已經將 Azure 服務端點加入 VS Team Services，請在 [Azure 訂用帳戶]  下拉式清單方塊中選擇訂用帳戶。  如果您沒有訂用帳戶，請參閱[選項 1](#detailed-walkthrough-for-option-1)查看在 VS Team Services 中設定訂用帳戶的指示。
   
   * 來源 - 輸入 **$(Build.StagingDirectory)**
   * Azure 連線類型 - 選取 **Azure Resource Manager**
   * Azure RM 訂用帳戶 - 在 **Azure 訂用帳戶** 下拉式清單方塊中選取您想要使用的儲存體帳戶訂用帳戶。 如果訂用帳戶未出現，請選擇 [管理] 連結旁的 [重新整理]按鈕。
   * 目的地類型 - 選取 **Azure Blob**
   * RM 儲存體帳戶 - 選取您想要針對預備構件使用的儲存體帳戶
   * 容器名稱 - 輸入您想要針對預備使用的容器名稱，它可以是任何有效的容器名稱，但請使用一個專門用於此組建定義的名稱
...
6. 選擇 [Azure 資源群組部署]  組建步驟，再填上其值。
   ...
   * Azure 連線類型 - 選取 **Azure Resource Manager**
   * Azure RM 訂用帳戶 - 在 **Azure 訂用帳戶** 下拉式清單方塊中選取部署的訂用帳戶。 這通常會與上一個步驟中使用的訂用帳戶相同。
   * 動作 - 選取 **建立或更新資源群組**
