...
4. 選擇 [Azure PowerShell] 組建步驟，再填上其值。
   ...
   7. 如果您不知道 Azure 訂用帳戶識別碼，可以使用以下其中一個命令取得。
      ...
      `azure account show`
   8. 要取得服務主體識別碼、服務主體金鑰及租用戶識別碼，請依照[使用入口網站建立 Active Directory 應用程式和服務主體](resource-group-create-service-principal-portal.md)或[以 Azure 資源管理員驗證服務主體](resource-group-authenticate-service-principal.md)中的程序。
   9. 在 [新增 Azure 訂用帳戶] 對話方塊中新增服務主體識別碼、服務主體金鑰及租用戶識別碼值，再選擇 [確定] 按鈕。
      
      現在，您擁有可執行 Azure PowerShell 指令碼的有效服務主體。
5. 編輯組建定義並選擇 **Azure PowerShell** 建置步驟。在 [Azue 訂用帳戶] 下拉式清單方塊中選取訂用帳戶。(如果訂用帳戶未出現，請選擇 [管理] 連結旁的 [重新整理] 按鈕。)
   
   ![][8]
6. 提供 Deploy-AzureResourceGroup.ps1 PowerShell 指令碼的路徑。要這樣做，請選擇 [指令碼路徑] 方塊旁的省略符號 (…) 按鈕，瀏覽到您專案的 [指令碼] 資料夾中的 Deploy-AzureResourceGroup.ps1 PowerShell 指令碼，選取並選擇 [確定] 按鈕。
