...
> [!IMPORTANT]
> 如果 [Redis 度量] 刀鋒視窗上顯示了下列訊息，請遵循[啟用快取診斷](#enable-cache-diagnostics)一節中的步驟來啟用快取診斷。
> 
...
## <a name="enable-cache-diagnostics"></a>啟用快取診斷
...
> [!IMPORTANT]
> 相同區域和訂用帳戶中的快取會共用相同的診斷儲存體設定，在組態變更時 (啟用/停用診斷或變更儲存體帳戶)，會套用至訂用帳戶中所有位於該區域的快取。
> 
> 
...
> [!NOTE]
> 只有儲存在所選取儲存體帳戶中的度量才會顯示在 Azure 入口網站中。 如果您變更儲存體帳戶，則先前設定之儲存體帳戶中的資料仍然可供下載，但不會顯示在 Azure 入口網站中。  
> 
> 
...
## <a name="how-to-view-metrics-and-customize-charts"></a>如何檢視度量以及自訂圖表
...
如果您按一下 [儲存]，您的變更將會保留到您退出 [度量] 刀鋒視窗為止。 當您後續返回頁面時，您將會看見原有的度量和時間範圍。 如需自訂圖表的詳細資訊，請參閱[監視服務度量](../monitoring-and-diagnostics/insights-how-to-customize-monitoring.md)。

若要檢視圖表上一段特定時間的度量，請將滑鼠放在圖表上對應至想要時間的其中一個特定橫條或點，並顯示該間隔的度量。

![檢視圖表詳細資料][redis-cache-view-chart-details]

## <a name="how-to-monitor-a-premium-cache-with-clustering"></a>如何監視含叢集的進階快取
...
## <a name="operations-and-alerts"></a>作業和警示
[Redis 快取] 刀鋒視窗上的 [作業] 區段具有 [事件] 和 [警示規則] 區段。

![作業][redis-cache-operations-events]

若要查看最新快取作業的清單，請按一下 [事件] 圖表顯示 [事件] 刀鋒視窗。 作業的範例包括擷取和重新產生存取金鑰，以及警示規則的啟動和解決。 如需每個事件的詳細資訊，請按一下 [事件]  分頁中的事件。

如需事件的詳細資訊，請參閱[檢視事件和稽核記錄檔](../monitoring-and-diagnostics/insights-debugging-with-events.md)。
...
您可以從特定監視圖的 [度量] 刀鋒視窗或是 [警示規則] 刀鋒視窗檢視和設定警示規則。
...
若要檢視快取執行個體的所有警示規則，請按一下 [Redis 快取] 刀鋒視窗中的 [警示規則] 部分。 若只要檢視使用特定度量的警示規則，請瀏覽含有該度量之圖表的 [度量]  分頁。

![警示規則][redis-cache-alert-rules]

若要新增警示規則，請按一下 [度量] 刀鋒視窗或 [警示規則] 刀鋒視窗中的 [新增警示]。 

將想要的規則條件輸入至 [新增警示規則] 刀鋒視窗，然後按一下 [確定]。 

![新增警示規則][redis-cache-add-alert]

> [!NOTE]
> 按一下 [度量] 刀鋒視窗中的 [新增警示] 以建立警示規則時，只有該分頁之圖表上顯示的度量才會出現在 [度量] 下拉式清單中。 按一下 [警示規則] 刀鋒視窗中的 [新增警示] 以建立警示規則時，所有快取度量都會出現在 [度量] 下拉式清單中。
> 
> 

警示規則在儲存之後，會出現在 [警示規則] 刀鋒視窗上，以及顯示警示規則中所使用度量之任何圖表的 [度量] 刀鋒視窗上。 若要編輯警示規則，請按一下要顯示 [編輯規則]  分頁之警示規則的名稱。 從 [編輯規則]  分頁中，您可以編輯規則的屬性、刪除或停用警示規則，或重新啟用先前已停用的規則。

> [!NOTE]
> 您對規則屬性進行的任何變更可能需要一些時間，才會反映在 [警示規則] 刀鋒視窗或 [度量] 刀鋒視窗上。
> 
> 

啟動警示規則時，會根據警示規則的組態傳送電子郵件，並且會在 [Redis 快取] 刀鋒視窗的 [警示規則] 部分中顯示警示圖示。

警示條件不再評估為 true 時，會將警示規則視為已解決。 解決警示規則條件之後，會將警示圖示取代為核取記號。 如需警示啟動和解決方法的詳細資料，請按一下 [Redis 快取] 刀鋒視窗的 [事件] 部分，以檢視 [事件] 刀鋒視窗上的事件。

如需 Azure 中警示的詳細資訊，請參閱[接收警示通知](../monitoring-and-diagnostics/insights-receive-alert-notifications.md)。

## <a name="metrics-on-the-redis-cache-blade"></a>[Redis 快取] 刀鋒視窗上的度量
