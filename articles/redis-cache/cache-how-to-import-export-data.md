...
2. 按一下 [選擇儲存體容器]  ，然後選取所需的儲存體帳戶。 儲存體帳戶必須與您的快取位於相同的訂用帳戶和區域中。
   ...
   > [!IMPORTANT]
   > 匯入/匯出使用分頁 blob，此時傳統和 ARM 儲存體帳戶支援這些 blob，但是 [Blob 儲存體帳戶](../storage/storage-blob-storage-tiers.md#blob-storage-accounts) 不支援。
   >
   >
...
### <a name="will-my-cache-be-available-during-an-importexport-operation"></a>我的快取在匯入/匯出作業期間可供使用嗎？
