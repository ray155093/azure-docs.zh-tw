...
* 如果您在 [azureiotsuite.com][lnk-azureiotsuite] 中刪除預先設定的解決方案，則會刪除在建立預先設定的解決方案時所佈建的所有資源；如果您新增了其他資源到資源群組，這些資源也會一併刪除。 
* 如果您在 [Azure 入口網站][lnk-azure-portal]刪除資源群組，您只刪除了該資源群組中的資源；您也必須在 [Azure 傳統入口網站][lnk-classic-portal]中刪除與預先設定解決方案相關聯的 Azure Active Directory 應用程式。
...
1. 從 [Azure 入口網站][lnk-azure-portal]取得 Bing Maps API for Enterprise QueryKey： 
   ...
   3. 您會注意到兩個金鑰 ︰MasterKey 和 QueryKey。 複製 QueryKey 的值。
      ...
      > [!NOTE]
      > 沒有 Bing Maps API for Enterprise 帳戶嗎？ 按一下 [+ 新增]，搜尋 Bing Maps API for Enterprise 並依照提示執行，在 [Azure 入口網站][lnk-azure-portal]中建立一個帳戶。
      > 
      > 
