...
**竄改**︰可將裝置重新設定在未知狀態中運作，以控制系統 (在已知校正參數的範圍以外)，並提供可誤導的資料。權限提高︰具有特定功能的裝置可能會被強制執行其他動作。例如，程式設計為半開的閥門可能會受騙而完全打開。
...
| **元件** | **Threat** | **緩和** | **風險** | **實作** |
|-----------------------------|------------|-----------------------------------------------------|---------------------------------------------------------------------------------------------------------------|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| 裝置 IoT 中樞 | TID | (D)使用 TLS (PSK/RSA) 以加密流量 | 竊聽或干擾裝置與閘道器之間的通訊 | 通訊協定層級的安全性。使用自訂通訊協定時，我們需要了解如何對其進行保護。在大部分情況下，通訊主要是發生在裝置到 IoT 中樞這一段位置 (由裝置起始連線)。 |
| 裝置裝置 | TID | (D)使用 TLS (PSK/RSA) 以加密流量。 | 讀取裝置間傳輸中的資料。竄改資料。多載具有新連線的裝置 | 通訊協定層級的安全性 (HTTP (S)/AMQP/MQTT/CoAP)。使用自訂通訊協定時，我們需要了解如何對其進行保護。DoS 威脅的安全防護功能是透過雲端閘道器或現場閘道器，將裝置設為對等，使其僅可以網路用戶端形式運作。在閘道器代理對等之後，對等作業可能會導致對等之間直接連線。 |
| 外部實體裝置 | TID | 外部實體與裝置的強式配對 | 竊聽裝置的連線。干擾裝置的通訊 | 使用 NFC/Bluetooth LE 安全地配對外部實體與裝置。控制裝置 (實體) 的操作面板 |
| 現場閘道器雲端閘道器 | TID | 使用 TLS (PSK/RSA) 以加密流量。 | 竊聽或干擾裝置與閘道器之間的通訊 | 通訊協定層級的安全性 (HTTP (S)/AMQP/MQTT/CoAP)。使用自訂通訊協定時，我們需要了解如何對其進行保護。 |
| 裝置雲端閘道器 | TID | 使用 TLS (PSK/RSA) 以加密流量。 | 竊聽或干擾裝置與閘道器之間的通訊 | 通訊協定層級的安全性 (HTTP (S)/AMQP/MQTT/CoAP)。使用自訂通訊協定時，我們需要了解如何對其進行保護。 |
...
**竄改**︰攻擊者可能會攔截或部分覆寫廣播，並傳送錯誤的資訊資訊洩漏︰攻擊者可能竊聽廣播並未經授權取得資訊拒絕服務︰攻擊者可能會中斷廣播的訊號，並拒絕資訊散發
