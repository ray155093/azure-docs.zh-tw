...
> [AZURE.NOTE] 您不需要 GitHub 帳戶，即可存取 [GitHub MyDriving 儲存機制](https://github.com/Azure-Samples/MyDriving)中的 MyDriving 程式碼。
...
-   VINLookupApplication 專案未載入：請確定已安裝 [Azure SDK for Visual Studio 2015](https://go.microsoft.com/fwlink/?linkid=518003&clcid=0x409)。
...
## 在 Xamarin Test Cloud 上測試行動應用程式
...
> [AZURE.NOTE] 大多數 Azure 服務都有取決於定價層的相關聯成本。如果您不熟悉 Azure，您可以[免費試用](https://azure.microsoft.com/free/)。不過，如果您不打算使用 MyDriving 系統中的特定元件，請務必移除以避免產生成本。本文後面的「估計作業成本」一節提供一般服務費用的摘要。
...
## 在 Visual Studio Team Services 中設定組建和測試定義
...
>[AZURE.NOTE] 如果您不熟悉 Azure，這裡有[免費帳戶](https://azure.microsoft.com/free/)。
...
| [Machine Learning](https://azure.microsoft.com/documentation/services/machine-learning/)<br/> 自適性回應 | 每月每個基座美金 $10 元。<br/> + 3 小時實驗 * 每個實驗小時美金 $1 元。<br/> + 3.5 小時的 API CPU * 每個生產環境 CPU 小時美金 $2 元。<br/> API CPU 時間假設每天重新訓練 5 分鐘，不過這會隨輸入資料愈多而增加。<br/> + 每天計分 2 分鐘以處理每天 400 趟行程。 | 美金 $20 元 |
...
| Azure App Service 的 [SQL Database](https://azure.microsoft.com/pricing/details/sql-database/)| 基本層；單一資料庫模型。 | 美金 $5 元 |
