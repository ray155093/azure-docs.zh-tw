...
- **最小化所需的連線數目**。服務連線會吸收資源。限制所需數目，並確保能儘量重複使用現有連線。例如，在執行驗證之後，適當使用模擬來執行做為特定身分識別的程式碼。這樣有助於重複使用連線來充分運用連線集區。
	...
	> [AZURE.NOTE]︰某些服務的 API 會在遵循服務特定方針時自動重複使用連線。請務必了解，針對應用程式所使用之每個服務重複使用連線的條件。
...
- **使用工作平行程式庫 (TPL) 來執行非同步作業**。TPL 可讓您輕鬆地寫入執行 I/O 繫結作業的非同步程式碼。儘可能使用 _ConfigureAwait(false)_，以排除特定同步處理內容上接續的相依性。這可減少發生執行緒死結的機會。
- **在部署期間或在應用程式啟動時建立資源相依性**。避免重複呼叫測試資源存在與否的方法，若資源不存在，請建立資源(Azure 儲存體用戶端程式庫中像是 _CloudTable.CreateIfNotExists_ 和 _CloudQueue.CreateIfNotExists_ 的方法會遵循這個模式)。若在每次存取儲存體資料表或儲存體佇列前叫用這些方法，會造成大量的額外負荷 。因此：
 - 請於應用程式進行部署或第一次啟動時建立所需資源 (可在 Web 或背景工作角色的啟動程式碼中，對每個資源 _CreateIfNotExists_ 發出單一呼叫)。 不過，若您的程式碼嘗試存取不存在的資源，請務必處理可能發生的例外狀況。如果出現這些情況，您應記錄例外狀況，並儘可能警示操作員提醒資源遺漏。
