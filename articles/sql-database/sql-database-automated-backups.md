...
Azure SQL Database 服務會利用自動備份保護所有資料庫，此備份會保留 7 天 (適用於基本資料庫)、14 天 (適用於標準資料庫) 和 35 天 (適用於高階資料庫。您可以使用這些自動備份來執行時間點還原，以及在意外資料損毀或刪除後，還原已刪除的資料庫。
