...
您可以使用 Azure [SQL Database 自動備份](sql-database-automated-backups.md)來建立 SQL Database 的複本。複製作業會複製交易記錄的結尾，然後使用完整、差異及交易記錄備份 (其為要建立的自動備份一部分)，以便與截至最後一個交易記錄備份為止的來源資料庫維持交易一致性。
