---
title: "Azure 入口網站︰建立和查詢單一 SQL Database | Microsoft Docs"
...
Azure SQL Database 會使用一組定義的計算和儲存體資源建立於邏輯伺服器的 Azure 資源群組內。 請遵循下列步驟來建立包含 Adventure Works LT 範例資料的 SQL Database。 

1. 按一下 Azure 入口網站左上角的 [新增] 按鈕。

2. 從 [新增] 刀鋒視窗中選取 [資料庫]，然後從 [資料庫] 刀鋒視窗中選取 [SQL Database]。
...
SQL Database 服務會建立防火牆來防止外部應用程式和工具連線至新的伺服器和資料庫。 請遵循下列步驟來為您的 IP 位址建立 SQL Database 伺服器層級防火牆規則，以便讓外部連線能夠穿過 SQL Database 防火牆。 
...
1. 在資料庫的 [SQL Databse] 頁面上，按一下工具列上的 [工具]。 [查詢編輯器] 預覽頁面隨即開啟。
...
您可以選擇儲存本快速入門中建立的資源，以供後續的教學課程使用。 或者，您可以使用下列步驟，在 Azure 入口網站中刪除本快速入門所建立的所有資源。
