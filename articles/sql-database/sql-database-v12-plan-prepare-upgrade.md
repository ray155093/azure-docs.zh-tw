...
---
# <a name="plan-and-prepare-to-upgrade-to-sql-database-v12"></a>規劃和準備升級至 SQL Database V12
...
| 標題和連結 | 內容的描述 |
|:--- |:--- |
| [Azure SQL Database 功能](sql-database-features.md) |提供 Azure SQL Database 與 SQL Server 的功能矩陣。 |
| [在 SQL Database V12 中建立資料庫](sql-database-get-started.md) |描述如何建立全新 V12 版本的 Azure SQL Database。 內容描述各種選項，而非只是空的資料庫。 |

## <a name="plan-ahead"></a>事先規劃
下列小節描述在您採取動作將 Azure SQL Database 升級至 V12 之前，必須學習的知識和決策制訂。

### <a name="version-clarification"></a>版本釐清
本文件是關於將 Microsoft Azure SQL Database 從 V11 升級至 V12 版。 更正式的版本編號會接近 Transact-SQL 陳述式 **SELECT @@version;** 所回報的下列兩個值：

* 12.0.2000.8 (或稍微新一點的版本，V12)
* 11.0.9228.18 *(V11)*
  * 有時 V11 也稱為 SAWA v2。
...
### <a name="review-the-geo-replication-configuration"></a>檢閱異地複寫設定
...
### <a name="suspend-geo-replication-during-upgrade"></a>在升級期間暫停異地複寫
如果「異地複寫」在您資料庫中是作用中狀態，將無法升級至 V12。 您必須先重新設定資料庫以停止使用「異地複寫」。

在升級完成後，您可以設定資料庫來再次使用「異地複寫」。

### <a name="open-ports-on-an-azure-vm-for-client-connectivity"></a>開啟 Azure VM 上的連接埠以供用戶端連線使用
...
## <a name="a-idlimitationsalimitations-during-and-after-upgrade-to-v12"></a><a id="limitations"></a>升級至 V12 期間和之後的限制
### <a name="portals-for-v12"></a>V12 的入口網站
...
我們鼓勵您使用 Visual Studio 2015 (VS2015) 來連線到您的 Azure SQL Database。 VS2015 可用於類似於以下的工作：

* 執行 Transact-SQL 陳述式。
* 設計結構描述。
* 開發線上或離線資料庫。

或者，您可以改為下載免費的 [Visual Studio Community 2015](https://www.visualstudio.com/vs/community/)，這是功能完整的 VS2015 版本。

在較舊的「Azure 傳統入口網站」中，您可以在 [資料庫] 頁面上按一下 [在 Visual Studio 中開啟]，以在電腦上啟動 VS2015 來連線到您的 Azure SQL Database。

如需另一個替代方式，您可以使用 SQL Server Management Studio (SSMS) 2014 與 [CU6](http://support.microsoft.com/kb/3031047/) 來連線至 Azure SQL Database。 此部落格文章包含更多詳細資料：<br/>[Azure SQL Database 的用戶端工具更新](https://azure.microsoft.com/blog/2014/12/22/client-tooling-updates-for-azure-sql-database/)來執行這項工作。
...
### <a name="limitation-during-upgrade-to-v12"></a>升級至 V12 *期間* 的限制
升級至 V12 期間，V11 資料庫仍然可供存取資料。 還有一些限制需要注意。

| 升級至 V12  | 說明 |
|:--- |:--- |
| 升級期間 |升級期間取決於大小、版本和伺服器中的資料庫數目而定。 伺服器的升級程序可能會執行數小時至數天，尤其對於具有下列資料庫的伺服器：<br/><br/>* 大於 50 GB，或<br/>* 在非進階服務層<br/><br/>升級期間在伺服器上建立新資料庫，也會增加升級的時間。 |
...
### <a name="limitation-after-upgrade-to-v12"></a>升級至 V12 *之後* 的限制
| 升級至 V12  | 說明 |
|:--- |:--- |
| 無法還原成 V11 |在升級完成之後，就無法還原或復原結果。 |
| Web 或商務層 |在升級開始之後，新的 V12 資料庫的伺服器就無法再辨識或接受 Web 或商務服務層。 |

### <a name="export-and-import-after-upgrade-to-v12"></a>在升級至 V12 *之後* 匯出和匯入
...
不過，若要使用這些工具，您必須先擁有或安裝其最新的更新，以確保它們支援新的 V12 功能：
...
### <a name="restore-to-v12-of-a-deleted-v11-database"></a>將刪除的 V11 資料庫還原至 V12
...
## <a name="failure-resolution"></a>失敗解決方式
