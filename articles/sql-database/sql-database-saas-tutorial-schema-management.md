---
title: "針對多個租用戶管理結構描述 (使用 Azure SQL Database 的範例 SaaS 應用程式) | Microsoft Docs"
description: "針對使用 Azure SQL Database 之 SaaS 應用程式中的多個租用戶管理結構描述"
...
---
# <a name="manage-schema-for-multiple-tenants-in-the-wtp-saas-application"></a>針對 WTP SaaS 應用程式中的多個租用戶管理結構描述

WTP 應用程式教學課程的簡介說明 WTP 應用程式如何使用其初始結構描述佈建租用戶資料庫，並在類別目錄中註冊它。 就像任何應用程式一樣，WTP 應用程式會隨著時間演進，而且有時候會需要變更資料庫。 變更可能包含新增或變更的結構描述、新增或變更的參考資料，以及例行性資料庫維護工作以確保最佳的應用程式效能。 有了 SaaS 應用程式，這些變更需要以經過協調的方式部署到可能非常大規模的租用戶資料庫群。 變更也需要整合到佈建程序中，以供未來的租用戶資料庫使用。

本教學課程會探討兩種案例：為所有租用戶部署參考資料更新，以及針對包含參考資料的資料表重新調整索引。 [彈性作業](sql-database-elastic-jobs-overview.md)功能是用來跨所有租用戶執行這些作業，而「範本」租用戶資料庫則是用來做為新資料庫的範本。
...
1. 使用您部署 WTP 應用程式時使用的使用者名稱，修改指令碼中所有 3 個位置的 \<user\>
1. 確定您已連線到 jobaccount 資料庫，然後按 **F5** 以執行指令碼

* **sp\_add\_target\_group** 會建立目標群組名稱 DemoServerGroup，我們現在需要新增目標成員。
* **sp\_add\_target\_group\_member** 會新增「伺服器」目標成員類型，這會將該伺服器 (請注意，這是包含租用戶資料庫的 customer1-&lt;WtpUser&gt; 伺服器) 中的所有資料庫視為作業執行時應包含在作業中的項目，其次是新增「資料庫」目標成員類型，具體而言是「範本」資料庫 baseTenantDB，它位於 catalog-&lt;WtpUser&gt; 伺服器上，最後，另一個「資料庫」目標群組成員類型會包括稍後教學課程要使用的 adhocanalytics 資料庫。
...
1. 開啟 SSMS 並連線到 catalog-&lt;WtpUser&gt;.database.windows.net 伺服器
1. 開啟檔案 …\\Learning Modules\\Schema Management\\OnlineReindex.sql
1. 按一下滑鼠右鍵並選取 [連線]，然後連線到 catalog-&lt;WtpUser&gt;.database.windows.net 伺服器 (如果尚未連線)
1. 確定您已連線到 jobaccount 資料庫，然後按 F5 以執行指令碼
