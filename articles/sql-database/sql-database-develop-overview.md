...
# SQL Database 開發概觀
這篇文章會引導開發人員在撰寫程式碼以連接到 Azure SQL Database 時應考慮的的基本考量。
...
## 復原功能
當連接到 SQL Database 發生暫時性錯誤時，您的程式碼應該重試呼叫。我們建議重試邏輯使用輪詢邏輯，因此它不需要同時重試多個用戶端而讓 SQL Database 超過負荷。
...
* 如果您的用戶端在 Azure 虛擬機器 (VM) 上執行時，用戶端程式連接至 SQL Database V12，您就必須開啟此 VM 上特定範圍的連接埠。詳細資訊：[針對 ADO.NET 4.5 和 SQL Database V12 的 1433 以外的連接埠](sql-database-develop-direct-route-ports-adonet-v12.md)
