...
異地還原和還原時間點使用相同的技術，但有一個重要的差異。它會從異地複寫 blob 儲存體 (RA-GRS) 中最新的每日備份複本還原資料庫。對於每個作用中的資料庫，此服務會維護備份鏈結，其包含每週完整備份、多個每日差異備份以及每隔 5 分鐘儲存一次的交易記錄。這些 blob 都會進行異地複寫，這樣可保證即使在主要區域中發生大規模失敗之後，仍可使用每日備份。以下顯示複製到儲存體容器之每週和每日備份的異地複寫。
...
異地還原可供所有服務層使用，而且它是 SQL Database 中最基本的災害復原解決方案，具備最長的 RPO 和預估復原時間 (ERT)。對於大小上限為 2 GB 的 Basic 資料庫，異地還原提供 ERT 為 12 小時的合理 DR 解決方案。對於較大的標準或高階資料庫，如果需要大幅縮短的復原時間，或需要減少資料遺失的可能性，您應該考慮使用作用中的異地複寫。作用中的異地複寫提供較低的 RPO 和 ERT，因為它只需要您起始連續複寫次要資料庫的容錯移轉。如需詳細資料，請參閱[作用中的異地複寫](sql-database-geo-replication-overview.md)。
