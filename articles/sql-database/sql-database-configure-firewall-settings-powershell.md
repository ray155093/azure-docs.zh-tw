...
---
# <a name="configure-azure-sql-database-server-level-firewall-rules-by-using-powershell"></a>使用 PowerShell 設定 Azure SQL Database 伺服器層級防火牆規則
...
若要建立新的伺服器層級防火牆規則，請執行 [New-AzureRmSqlServerFirewallRule](https://msdn.microsoft.com/library/azure/mt603860\(v=azure.300\).aspx) Cmdlet。 下列範例會在伺服器 Contoso 上啟用某個範圍的 IP 位址。
