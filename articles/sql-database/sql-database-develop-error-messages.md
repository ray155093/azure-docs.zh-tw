...
|40501|20|服務目前忙碌中。在 10 秒後重試要求。事件識別碼：%ls。代碼：%d。<br/><br/>*注意：*如需詳細資訊，請參閱：<br/>• [Azure SQL Database 資源限制](sql-database-resource-limits.md)。
...
|40553|16|已終止工作階段，因為過度使用記憶體。請嘗試修改查詢以處理較少的資料列。<br/><br/>*注意：*減少您 Transact-SQL 程式碼中的 `ORDER BY` 和 `GROUP BY` 作業數目，有助於降低查詢的記憶體需求。|
