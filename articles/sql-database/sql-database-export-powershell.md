...
本文提供將 Azure SQL Database 封存到儲存於 Azure Blob 儲存體中 [BACPAC](https://msdn.microsoft.com/library/ee210546.aspx#Anchor_4) 檔案的指示。本文說明如何使用 PowerShell 來達成此目的。
...
- 為了讓封存檔處於交易一致狀態，您必須確定在匯出期間未發生任何寫入活動，或您是從 Azure SQL Database 的[交易一致性複本](sql-database-copy.md)匯出。
...
> [AZURE.IMPORTANT] 若要保證在交易上一致的 BACPAC 檔案，您應該先[建立資料庫的複本](sql-database-copy-powershell.md)，然後匯出資料庫複本。
...
將下列 `VARIABLE-VALUES` 以您特定 Azure 資源的值取代。資料庫名稱便是您想要匯出的現有資料庫。
