...
如果您需要從目標伺服器排除其他資料庫，請變更指令碼以符合您的準則。 依照預設。

指令碼需要輸出資料庫來儲存中繼資料以供分析。 您可以使用新的或現有的資料庫。 雖然執行工具不需要太多技術，輸出資料庫應該在不同的伺服器，以避免影響分析結果。 輸出資料庫的效能層級應該至少為 S0 或更高。 長時間收集許多資料庫的資料時 ，您可以考慮將輸出資料庫升級至較高的效能層級。
