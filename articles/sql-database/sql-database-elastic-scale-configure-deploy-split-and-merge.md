...
5. 執行 ExecuteSampleSplitMerge.ps1 指令碼，以執行分割作業 (將第一個分區的一半資料移至第二個分區)，然後執行合併作業 (將資料移回至第一個分區)。 如果您設定 SSL，而且保持停用 http 端點，請確定您改為使用 https:// 端點。
   ...
   如果您收到下列錯誤，很可能是 Web 端點的憑證有問題。 使用您最愛的網頁瀏覽器嘗試連線到的 Web 端點，並檢查是否有憑證錯誤。
     ...
     Invoke-webrequest︰基礎連接已關閉︰無法建立 SSL/TLSsecure 通道的信任關係。
   
   如果成功，輸出應該看起來如下：
   
   > .\ExecuteSampleSplitMerge.ps1 -UserName 'mysqluser' -Password 'MySqlPassw0rd' -ShardMapManagerServerName 'abcdefghij.database.windows.net' -SplitMergeServiceEndpoint 'http://mysplitmergeservice.cloudapp.net' –CertificateThumbprint 0123456789abcdef0123456789abcdef01234567 Sending split request Began split operation with id dc68dfa0-e22b-4823-886a-9bdc903c80f3 Polling split-merge request status. 按 Ctrl + C 來結束進度︰0 %|狀態: 排入佇列 |詳細資料：[資訊] 已佇列要求進度：%5 |狀態︰啟動 |詳細資料：[資訊] 針對要求啟動分割合併狀態機器。
   > 進度︰5 %|狀態︰啟動 |詳細資料︰[資訊] 在目標分區上執行資料一致性檢查。
   > 進度︰20 %|狀態︰CopyingReferenceTables |詳細資料︰[資訊] 從來源將參考資料表移動到目標分區。
   > 進度︰20 %|狀態︰CopyingReferenceTables |詳細資料︰[資訊] 等待參考資料表複製完成。
   > 進度︰20 %|狀態︰CopyingReferenceTables |詳細資料︰[資訊] 從來源將參考資料表移動到目標分區。
   > 進度︰44 %|狀態︰CopyingShardedTables |詳細資料：[資訊] 移動分區資料表的索引鍵範圍 [100:110) 進度︰44 %|狀態︰CopyingShardedTables |詳細資料：[資訊] 成功複製資料表 [dbo].[MyShardedTable] 的索引鍵範圍 [100:110) ......進度︰90 %|狀態︰完成 |詳細資料：[資訊] 已成功刪除資料表 [dbo] 中的 shardlet。
   > 進度︰90 %|狀態︰完成 |詳細資料：[資訊] 處理要求時刪除任何建立的暫存資料表。
   > 進度︰100 %|狀態︰成功 |詳細資料：[資訊] 已成功處理要求。
   > 傳送合併要求開始具有識別碼 6ffc308f-d006-466b-b24e-857242ec5f66 輪詢要求狀態的合併作業。 按 Ctrl + C 來結束進度︰0 %|狀態: 排入佇列 |詳細資料：[資訊] 已佇列要求進度：%5 |狀態︰啟動 |詳細資料：[資訊] 針對要求啟動分割合併狀態機器。
   > 進度︰5 %|狀態︰啟動 |詳細資料︰[資訊] 在目標分區上執行資料一致性檢查。
   > 進度︰20 %|狀態︰CopyingReferenceTables |詳細資料︰[資訊] 從來源將參考資料表移動到目標分區。
   > 進度︰44 %|狀態︰CopyingShardedTables |詳細資料：[資訊] 移動分區資料表的索引鍵範圍 [100:110) 進度︰44 %|狀態︰CopyingShardedTables |詳細資料：[資訊] 成功複製資料表 [dbo].[MyShardedTable] 的索引鍵範圍 [100:110) ......進度︰90 %|狀態︰完成 |詳細資料：[資訊] 已成功刪除資料表 [dbo] 中的 shardlet。
   > 進度︰90 %|狀態︰完成 |詳細資料：[資訊] 處理要求時刪除任何建立的暫存資料表。
