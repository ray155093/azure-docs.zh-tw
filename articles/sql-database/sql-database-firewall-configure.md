...
> [AZURE.IMPORTANT] 這個選項會設定防火牆，以允許所有來自 Azure 的連線，包括來自其他客戶訂用帳戶的連線。選取這個選項時，請確定您的登入和使用者權限會限制為只有授權的使用者才能存取。

您有兩種方式可以啟用來自 Azure 的連線：

- 在 [Microsoft Azure 入口網站](https://portal.azure.com/)中建立新的伺服器時，選取核取方塊 [允許 Azure 服務存取伺服器]。

- 在[傳統入口網站](http://go.microsoft.com/fwlink/p/?LinkID=161793)中，從伺服器上 [設定] 索引標籤的 [允許的服務] 區段下方，針對 [Microsoft Azure 服務] 按一下 [是]。
