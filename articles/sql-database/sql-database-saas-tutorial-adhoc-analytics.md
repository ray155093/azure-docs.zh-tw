...
在本教學課程中，您可以建立臨機操作分析資料庫並對所有租用戶執行數個查詢。 這些查詢可以擷取藏在 Wingtip SaaS 應用程式日常操作資料中的深入解析。
...
在資料庫範例中，我們會透過 Venue 名稱的雜湊來計算整數識別碼，但您可以使用任何方法來導入唯一值。 這和我們在目錄中用來建立租用戶金鑰的方法類似，但 adhocanalytics 資料庫中的目錄金鑰和租用戶識別碼不需要相同。
...
1. 開啟 ...\\Learning Modules\\Operational Analytics\\Adhoc Analytics\\*Deploy-AdhocAnalyticsDB.ps1*
1. 向下捲動到「將 `$commandText` 指派給 SQL 指令碼」的章節。 檢閱指令碼並注意下列事項︰
...
1. 現在，在 *PowerShell ISE* 中開啟 ...\\Learning Modules\\Operational Analytics\\Adhoc Analytics\\*Demo-AdhocAnalytics.ps1*，然後設定下列值：
...
![adhocanalytics 資料庫](media/sql-database-saas-tutorial-adhoc-analytics/adhocanalytics.png)

## <a name="run-ad-hoc-analytics-queries"></a>執行臨機操作分析查詢

adhocanalytics 資料庫已完成設定，接下來請執行一些臨機操作查詢：
