---
title: "使用 Azure Active Directory 驗證連線到 SQL Database 或 SQL 資料倉儲 | Microsoft Docs"
...
---
# <a name="connecting-to-sql-database-or-sql-data-warehouse-by-using-azure-active-directory-authentication"></a>使用 Azure Active Directory 驗證連線到 SQL Database 或 SQL 資料倉儲
...
下列 Azure AD 的成員可在 Azure SQL Server 或 SQL 資料倉儲中佈建：
...
* 只有 SQL Server 的 Azure AD 系統管理員可以一開始就使用 Azure Active Directory 帳戶連接到 Azure SQL Server 或 Azure SQL 資料倉儲。 Active Directory 系統管理員可以設定後續的 Azure AD 資料庫使用者。   
* 建議將連接逾時設定為 30 秒。   
...
## <a name="1-create-and-populate-an-azure-ad"></a>1.建立和填入 Azure AD
...
## <a name="2-ensure-your-sql-database-is-version-12"></a>2.確定您的 SQL Database 是第 12 版   
最新的 SQL Database V12 支援 Azure AD 驗證。 如需 SQL Database V12 的相關資訊並了解是否供您所在區域使用，請參閱 ([SQL Database 功能](sql-database-features.md)。 Azure SQL 資料倉儲不需要執行這個步驟，因為 SQL 資料倉儲只適用於 V12。
...
或者，遵循 [建立您的第一個 Azure SQL Database](sql-database-get-started.md)中所列的步驟，在 SQL Database V12 中建立新的資料庫。 

>  [!TIP]   
>  選取新資料庫的訂用帳戶之前參閱下一個步驟。   

## <a name="3-optional-associate-or-change-the-active-directory-that-is-currently-associated-with-your-azure-subscription"></a>3.選用：和目前與您的 Azure 訂用帳戶相關聯的 active directory 產生關聯並加以變更
...
## <a name="4-create-an-azure-ad-administrator-for-azure-sql-server"></a>4.建立 Azure SQL Server 的 Azure AD 系統管理員
每個 Azure SQL Server (裝載 SQL Database 或「SQL 資料倉儲」) 一開始都只有一個伺服器系統管理員帳戶，也就是整個 Azure SQL Server 的系統管理員。 第二個 SQL Server 系統管理員必須建立，也就是 Azure AD 帳戶。 這個主體會在 master 資料庫中建立為自主資料庫使用者。 身為系統管理員，伺服器系統管理員帳戶是每個使用者資料庫中的 **db_owner** 角色成員，並且會進入每個使用者資料庫做為 **dbo** 使用者。 如需伺服器系統管理員帳戶的詳細資訊，請參閱 [Azure SQL Database 安全性方針和限制](sql-database-manage-logins.md)中的**管理資料庫和登入 Azure SQL Database** 和[登入和使用者](sql-database-security-guidelines.md)等節。
...
> [!NOTE]
> 使用者如果不是以 Azure AD 帳戶 (包括 Azure SQL Server 系統管理員帳戶) 為基礎，就無法建立以 Azure AD 為基礎的使用者，因為他們不具備向 Azure AD 驗證建議之資料庫使用者的權限。
> 
> 

### <a name="provision-an-azure-active-directory-administrator-for-your-azure-sql-server-by-using-the-azure-portal"></a>使用 Azure 入口網站佈建 Azure SQL Server 的 Azure Active Directory 系統管理員
...
### <a name="provision-an-azure-ad-administrator-for-azure-sql-server-by-using-powershell"></a>使用 PowerShell 佈建 Azure SQL Server 的 Azure AD 系統管理員
...
> [!NOTE]
> Azure PowerShell 命令 ```Set-AzureRmSqlServerActiveDirectoryAdministrator``` 不會阻止您為不支援的使用者佈建 Azure AD 系統管理員。 您可以佈建不支援的使用者，但是該使用者無法連線到資料庫。 (請參閱以上 **Azure AD 功能和限制** 中支援的系統管理員清單)。
> 
> 
...
## <a name="5-configure-your-client-computers"></a>5.設定用戶端電腦
...
## <a name="6-create-contained-database-users-in-your-database-mapped-to-azure-ad-identities"></a>6.在對應至 Azure AD 身分識別的資料庫中建立自主資料庫使用者
### <a name="about-contained-database-users"></a>關於自主資料庫使用者
...
#### <a name="connect-using-active-directory-integrated-authentication"></a>使用 Active Directory 整合式驗證進行連接
...
#### <a name="connect-using-active-directory-password-authentication"></a>使用 Active Directory 密碼驗證進行連接
...
### <a name="create-an-azure-ad-contained-database-user-in-a-user-database"></a>在使用者資料庫中建立 Azure AD 自主資料庫使用者
...
> [!NOTE]
> 移除 Azure SQL Server 的 Azure Active Directory 系統管理員可防止任何 Azure AD 驗證使用者連線到伺服器。 必要時，SQL Database 系統管理員可以手動刪除無法使用的 Azure AD 使用者。   
...
## <a name="7-connect-by-using-azure-ad-identities"></a>7.使用 Azure AD 身分識別連接
...
### <a name="71-connecting-using-integrated-windows-authentication"></a>7.1. 使用整合式 (Windows) 驗證進行連接
...
### <a name="72-connecting-with-an-azure-ad-principal-name-and-a-password"></a>7.2. 使用 Azure AD 主體名稱與密碼進行連接
...
### <a name="73-connecting-with-an-azure-ad-token"></a>使用 Azure AD 權杖的 7.3 連接
...
### <a name="connecting-with-sqlcmd"></a>使用 sqlcmd 進行連線
