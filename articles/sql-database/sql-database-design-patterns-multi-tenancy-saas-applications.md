...
||[彈性交易︰](sql-database-elastic-transactions-overview.md)能夠以不可部分完成和隔離的方式，處理多個資料庫的變更。當應用程式在數個資料庫作業之間需要保證「全有或全無」時，就需要此功能。 |
...
如需可為「軟體即解決方案」(SaaS) 案例提供解決方案的範例應用程式 (其會利用彈性集區來提供符合成本效益、可調整的 SaaS 應用程式資料庫後端)，請參閱[適用於 Saas 的彈性集區自訂儀表板](https://github.com/Microsoft/sql-server-samples/tree/master/samples/manage/azure-sql-db-elastic-pools-custom-dashboard)。

若要將現有的資料庫轉換為使用該工具，請參閱[將現有的資料庫移轉到相應放大的資料庫](sql-database-elastic-convert-to-use-elastic-tools.md)。
