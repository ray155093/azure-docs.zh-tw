...
若要了解如何為嚴重損壞情況做準備，以及何時可復原資料庫，請瀏覽我們的[商務持續性的設計](sql-database-business-continuity-design.md)頁面。
...
>[AZURE.NOTE] 您應該在災害復原演練期間設定和測試伺服器防火牆規則與登入 (及其權限)。這些伺服器層級物件及其設定可能無法在中斷期間使用。如需詳細資訊，請參閱[執行嚴重損壞修復演練](sql-database-disaster-recovery-drills.md)。
