...
---
# <a name="when-should-an-elastic-pool-be-used"></a>何時使用彈性集區？
根據資料庫使用模式和彈性集區與單一資料庫之間的定價差異，評估使用彈性集區是否具成本效益。 也提供其他指引，以協助判斷一組現有的 SQL 資料庫所需的目前集區大小。  

* 如需集區的概觀，請參閱 [SQL Database 彈性集區](sql-database-elastic-pool.md)。

> [!NOTE]
> 彈性集區已在所有 Azure 區域中正式運作 (GA)，但印度西部除外，此區域目前提供預覽版。 我們將儘速在這些區域提供彈性集區的 GA。
>
>
