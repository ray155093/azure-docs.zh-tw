<properties
	pageTitle="教學課程：在 PowerApps 和邏輯流程中使用 ASP.Net Web 應用程式建立自訂 API | Microsoft Azure"
...
2. 在下一個畫面上，選取 Web API 範本，然後選取 [沒有驗證]：![](./media/powerapps-web-api-tutorial/noauth.png "沒有授權")
	...
	>[AZURE.IMPORTANT] 請確定您將驗證設為 [沒有驗證]。

3. 專案建立之後，您必須為您的資源建置 Web API。本教學課程中，我們不會深入探究建置 Web API 的相關資訊。

4. 接下來，為 Web API 產生 Swagger 檔案。您可以開啟 [套件管理器主控台]，然後安裝 __Swashbuckle__ 來輕鬆地進行此動作：![](./media/powerapps-web-api-tutorial/swashbuckle-console.png "Swashbuckle 主控台")
...
7. 瀏覽至 ***https://\<azure-webapp-url>/swagger/docs/v1***，解壓縮 Swagger 的 json。
	...
	> [AZURE.IMPORTANT] 具有重複之 operationid 的 Swagger 文件是無效的。如果您使用範例 C# 範本，作業識別碼 "Values\_Get" 會重複兩次。將一個執行個體變更為 "Value\_Get"。
...
>[AZURE.IMPORTANT] 這兩個應用程式必須在相同目錄中。
