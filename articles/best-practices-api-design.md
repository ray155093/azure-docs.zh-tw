...
> [!TIP]
> 支持 Web API 的資料來源不一定非得是資料存放區，它可以是另一個服務或企業營運系統應用程式，或甚至是在組織內部執行的內部部署應用程式。
> 
> 
...
提供 */orders/ordervalue\_greater\_than\_n* (其中 *n* 代表訂單價格) 之類的 URI 配置可能是其中一個解決方案，但除了有限數目的價格以外，這個方法並不實用。此外，如果您需要依據其他準則查詢訂單，可能會面臨提供一長串的 URI 和可能非直覺式名稱的窘境。
