...
Cosmos DB 設計用來取得可預測的效能。 當您建立容器時，可以根據每秒的[要求單位](../documentdb/documentdb-request-units.md) (RU) 來為每分的 RU 預留輸送量。 每項要求都會指派有與系統資源 (例如作業所使用的 CPU、記憶體和 IO) 數量成正比的要求單位費用。 讀取 1 KB 具有工作階段一致性的文件，會使用 1 個要求單位。 不論儲存的項目數或同時執行的並行要求數，讀取一次都是 1 個 RU。 根據大小之不同，較大的項目需要較高的要求單位。 如果您知道實體大小以及支援您應用程式所需的讀取次數，則可以佈建應用程式讀取需求確實需要的輸送量。 
...
下列範例示範如何使用 DocumentDB API 建立容器 (集合)。 您可以在[使用 DocumentDB API 進行資料分割](../documentdb/documentdb-partition-data.md)中找到更多詳細資料。
...
* 了解 [Azure Cosmos DB 中的全域分散](../documentdb/documentdb-distribute-data-globally.md)
