...
本快速入門會示範如何使用 Azure 入口網站建立適用於圖形 API (預覽) 的 Azure Cosmos DB 帳戶、資料庫和圖形。 您會接著使用 OSS [Gremlin Java](https://mvnrepository.com/artifact/com.tinkerpop.gremlin/gremlin-java) 驅動程式來建置和執行主控台應用程式。  
...
* 已從您稍早設定之 `src/remote-secure.yaml` 中的組態初始化 Gremlin `Client`。
...
1. 在 [Azure 入口網站](http://portal.azure.com/)中，於您 Azure Cosmos DB 帳戶的左側瀏覽區中，按一下 [金鑰]，然後按一下 [讀寫金鑰]。 在下一個步驟中，您會使用畫面右側的複製按鈕，將 URI 和主要金鑰複製到 `Program.java` 檔案。

    ![在 Azure 入口網站的 [金鑰] 刀鋒視窗中檢視並複製存取金鑰](./media/create-documentdb-dotnet/keys.png)

2. 開啟 `src/remote-secure.yaml` 檔案。 

3. 在 `src/remote-secure.yaml` 檔案中填入您的 host、port、username、password、connectionPool 和 serializer 組態︰
