...
除了基本類型，許多應用程式需要 [DateTime](https://msdn.microsoft.com/library/system.datetime(v=vs.110).aspx) 類型來表示日期和時間戳記。 本文說明開發人員如何使用.NET SDK 來儲存、擷取、查詢 Cosmos DB 中的日期。
...
或者，您可以將 DateTimes 儲存為 Unix 時間戳記，也就是用一個數字來表示自 1970 年 1 月 1 日之後經過的秒數。 Cosmos DB 的內部時間戳記 (`_ts`) 屬性便是使用此方法。 您可以使用 [UnixDateTimeConverter](https://msdn.microsoft.com/library/azure/microsoft.azure.documents.unixdatetimeconverter.aspx) 類別將 DateTime 序列化為數字。 
...
您可以參閱 [Cosmos DB 索引編製原則](documentdb-indexing-policies.md)，來深入了解如何設定索引編製原則。
...
您可以參閱[查詢 Cosmos DB](documentdb-sql-query.md)，來深入了解 Cosmos DB 的 SQL 查詢語言和 LINQ 提供者。

在本文中，我們已經了解如何進行 Cosmos DB 中 DateTime 的儲存、索引編製及查詢。
