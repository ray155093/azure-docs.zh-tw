...
## <a id="GettingStarted"></a>開始使用 DocumentDB 中的 SQL 命令
...
## <a id="Indexing"></a>DocumentDB 索引編製
...
## <a id="Basics"></a>DocumentDB SQL 查詢的基本概念
...
下表顯示 DocumentDB 中支援的彙總函式清單。 `SUM` 和 `AVG` 是對數值執行，而 `COUNT`、`MIN`和 `MAX` 則可對數字、字串、布林值和 null 執行。 
