...
4. [儲存體容量] 預設會設定為 [250 GB] 來處理已分割的集合。
...
6. 在 [分割區索引鍵] 方塊中，輸入集合的分割區索引鍵。 您必須為已分割的集合輸入此索引鍵，但針對單一分割區集合則可以視需要輸入。 選取正確的資料分割索引鍵對於建立高效能集合來說很重要。 如需選取資料分割索引鍵的詳細資訊，請參閱 [資料分割設計](documentdb-partition-data.md#designing-for-partitioning)。
