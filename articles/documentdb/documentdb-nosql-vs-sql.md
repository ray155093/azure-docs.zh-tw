---
title: "NoSQL 與 SQL 的使用時機 | Microsoft Docs"
...
現在您可以使用類似 SQL Server 的關聯式解決方案來儲存資料及使用聯結來查詢它，因為 SQL 支援將動態資料[格式化為 JSON](https://msdn.microsoft.com/library/dn921897.aspx) - 但是針對此特定案例，還有另一個選項，可簡化方法的 NoSQL 選項。 藉由將類似以下的單一文件，並且將其儲存在 DocumentDB (Azure NoSQL 文件資料庫服務) 中，您可以增加效能以及使用一個查詢且沒有任何聯結來擷取整個文章。 這個方法更簡單、更直接了當，並且會產生更好的效能。
