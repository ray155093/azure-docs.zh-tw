...
每個效能等級都有相關聯的[要求單位 (RU)](http://go.microsoft.com/fwlink/?LinkId=735027) 費率限制。這是依以效能等級為基礎保留給集合的輸送量，而且是該集合所專用。
...
> [AZURE.NOTE] 當您的應用程式超過一個或多個集合的效能等級時，要求將會依每個集合為基礎降低。這表示某些應用程式的要求可能會成功，而其他的可能受到節流控制。建議新增
...
Azure 入口網站是您在管理集合的效能層級的其中一個選項。請遵循下列步驟，在 Azure 入口網站中從使用預先定義的效能層級變更為使用者定義的效能層級，或觀看 75 秒的 [Channel 9 影片](https://channel9.msdn.com/Blogs/AzureDocumentDB/ChangeDocumentDBCollectionPerformance)。如需價格選項變更的詳細資訊，請參閱部落格文章 [DocumentDB: Everything you need to know about using the new pricing options](https://azure.microsoft.com/blog/documentdb-use-the-new-pricing-options-on-your-existing-collections/)。
...
7. 在 [管理集合] 刀鋒視窗中，按一下 [定價層]。

    ![Azure DocumentDB [管理集合] 和 [選擇定價層] 刀鋒視窗的螢幕擷取畫面，其中顯示可供變更集合定價層的位置][1]

8. 在 [選擇定價層] 刀鋒視窗中，按一下 [標準]。

9. 在 [選擇定價層] 刀鋒視窗中，按一下 [選取]。

10. 回到 [管理集合] 刀鋒視窗中，[定價層] 已變更為 [標準] 並且會顯示 [輸送量 (RU/秒)]。

    將 [輸送量] 方塊中的值變更為介於 400 到 10000 [要求單位](documentdb-request-units.md)/秒 (RU/秒) 的值。頁面底部的 [價格摘要] 會自動更新以提供每月成本估計。

    ![[管理集合] 刀鋒視窗的螢幕擷取畫面，其中顯示可供變更集合輸送量值的位置][2]

9. 在 [管理集合] 刀鋒視窗中，按一下 [確定] 將您的集合更新為使用者定義的效能。
