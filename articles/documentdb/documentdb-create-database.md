...
1. 在 [DocumentDB (NoSQL)] 刀鋒視窗中，選取要在其中新增 DocumentDB NoSQL 資料庫的帳戶。如果您沒有任何列出的帳戶，您需要[建立 DocumentDB 帳戶](documentdb-create-account.md)。
2. 在 [DocumentDB 帳戶] 刀鋒視窗中，按一下 [新增資料庫]。
   
    ![示範如何建立資料庫，並反白顯示 [新增資料庫] 按鈕、[識別碼] 方塊及 [確定] 按鈕的螢幕擷取畫面](./media/documentdb-create-database/docdb-database-creation-3-5.png)
3. 在 [**新增資料庫**] 刀鋒視窗中，輸入您的新資料庫識別碼。驗證名稱時，[識別碼] 方塊中會出現綠色的核取記號。然後按一下 [確定]。
   
    ![示範如何建立資料庫，並反白顯示 [新增資料庫] 按鈕、[識別碼] 方塊及 [確定] 按鈕的螢幕擷取畫面](./media/documentdb-create-database/docdb-database-creation-4.png)
4. 新的資料庫現在便會出現在 [**DocumentDB 帳戶**] 刀鋒視窗上的 [**資料庫**] 鏡頭中。
