...
---
# <a name="automatic-regional-failovers-for-business-continuity-in-azure-cosmos-db"></a>Azure Cosmos DB 中商務持續性的自動區域性容錯移轉
Azure Cosmos DB 會簡化資料的全域散發作業，方法是提供多個可完全管理的[多重地區資料庫帳戶](documentdb-distribute-data-globally.md)，在一致性、可用性和效能之間進行明確取捨，這一切全都倚靠相對應的保證來完成。 Cosmos DB 帳戶具備下列優點：高可用性、個位數的毫秒延遲、[定義完善的一致性層級](documentdb-consistency-levels.md)、利用多路連接 API 透明進行的區域性容錯移轉，以及全球輸送量及儲存體的靈活調整能力。 
...
![使用 Cosmos DB 部署全域分散式應用程式](./media/documentdb-regional-failovers/app-deployment.png)
...
![Azure Cosmos DB 中的讀取區域失敗](./media/documentdb-regional-failovers/read-region-failures.png)
...
![Azure Cosmos DB 的容錯移轉優先順序](./media/documentdb-regional-failovers/failover-priorities.png)

在自動容錯移轉期間，Cosmos DB 會根據 Azure Cosmos DB 帳戶指定的優先順序，自動選擇下一個寫入區域。 

![Azure Cosmos DB 中的寫入區域失敗](./media/documentdb-regional-failovers/write-region-failures.png)
...
![Azure Cosmos DB 中的手動容錯移轉](./media/documentdb-regional-failovers/manual-failovers.png)
...
* 了解 Cosmos DB 如何支援[全域散發](documentdb-distribute-data-globally.md)
