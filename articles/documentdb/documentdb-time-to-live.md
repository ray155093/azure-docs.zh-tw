...
利用「存留時間」或 TTL，Microsoft Azure DocumentDB 提供了在一段時間後從系統自動清除文件的功能。預設存留時間可以在集合層級設定，並且在每份文件上覆寫。一旦設定 TTL (做為集合預設值或在文件層級)，DocumentDB 就會自動移除自上次修改後存在一段時間 (以秒為單位) 的文件。
