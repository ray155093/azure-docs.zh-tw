...
> [AZURE.NOTE] 諸如**匯出/匯入註冊**、**透過 API 存取遙測**等數種遙測相關功能僅適用於 Standard 階層。如果您使用 Free 或 Basic 階層但嘗試使用以上功能，系統會顯示與此效果相關的例外狀況訊息 (使用 SDK 時) 和 HTTP 403 (禁止) (直接從 REST API 使用時)。請務必透過 Azure 傳統入口網站上移至 Standard 階層。
