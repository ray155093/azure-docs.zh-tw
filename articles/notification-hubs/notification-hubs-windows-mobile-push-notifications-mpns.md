<properties
	pageTitle="在 Windows Phone 上使用 Azure 通知中樞傳送推播通知 | Microsoft Azure"
...
5. 在 App.xaml.cs 中 **Application\_Launching** 方法的頂端新增下列程式碼：
    ...
    >[AZURE.NOTE] MyPushChannel 值是一個索引，用來查閱 [HttpNotificationChannel](https://msdn.microsoft.com/library/windows/apps/microsoft.phone.notification.httpnotificationchannel.aspx) 集合中的現有通道。如果集合中沒有任何項目，請以該名稱建立新的項目。
...
8. 關閉應用程式。
   ...
   >[AZURE.NOTE] 若要接收快顯推播通知，應用程式不得在前景執行。
...
如需使用 [REST API](https://msdn.microsoft.com/library/azure/dn223264.aspx) 傳送推播通知的範例，請查看[如何從 Java 使用通知中樞](./notification-hubs-java-push-notification-tutorial.md)和[如何從 PHP 使用通知中樞](./notification-hubs-php-push-notification-tutorial.md)。

1. 以滑鼠右鍵按一下方案，選取 [加入] 和 [新增專案...]，然後按一下 [Visual C#] 底下的 [Windows] 和 [主控台應用程式]，再按一下 [確定]。

   	![Visual Studio - 新增專案 - 主控台應用程式][6]
...
6. 開啟 `Program.cs` 檔案，並加入下列 `using` 陳述式：
...
6. 在 `Program` 類別中，新增下列方法：
	...
	>[AZURE.NOTE]請確定您使用的連接字串具有 [**完整**] 存取權，而非 [**接聽**] 存取權。接聽存取權的字串沒有傳送推播通知的權限。
