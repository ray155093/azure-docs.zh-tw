...
7. 輸入應用程式名稱，然後按一下 [创建] \(**建立**)。
      ...
      ![][15]
8. 成功建立百度雲推送專案後，您會看到包含 **AppID**、**API 金鑰**與**秘密金鑰**的頁面。 記下 API 金鑰和秘密金鑰，我們會在稍後使用。
...
6. 下載並解壓縮[百度推播 Android SDK]，開啟 **libs** 資料夾，然後將 **pushservice-x.y.z** jar 檔案以及 **armeabi** & **mips** 資料夾複製到 Android 應用程式的 **libs** 資料夾。
...
11. 對其新增下列程式碼：
    ...
    使用您先前從百度雲端專案所擷取的金鑰來設定 **API_KEY** 的值，使用 Azure 傳統入口網站中的通知中樞名稱來設定 **NotificationHubName**，並使用 Azure 傳統入口網站中的 DefaultListenSharedAccessSignature 來設定 **NotificationHubConnectionString**。
12. 新增名為 **DemoApplication.java**的類別，並在其中新增下列程式碼：
...
13. 新增另一個名為 **MyPushMessageReceiver.java**的類別，並在其中新增下列程式碼。 此類別會處理接收自百度推播伺服器的推播通知。
...
14. 開啟 **MainActivity.java**，並將下列程式碼新增至 **onCreate** 方法：
    
            PushManager.startWork(getApplicationContext(),
                    PushConstants.LOGIN_TYPE_API_KEY, ConfigurationSettings.API_KEY);
15. 在頂端開啟下列 import 陳述式：
