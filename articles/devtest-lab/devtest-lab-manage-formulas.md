<properties
	pageTitle="管理研測實驗室公式來建立 VM | Microsoft Azure"
...
公式 (如[自訂映像](./devtest-lab-create-template.md)和 [Marketplace 映像](./devtest-lab-configure-marketplace-images.md)) 提供進行快速 VM 佈建的機制。研測實驗室中的公式是用來建立實驗室 VM 的預設屬性值清單。透過公式建立 VM 時，可以依現況使用預設值，或修改預設值。
...
> [AZURE.NOTE] 公式與[自訂映像](./devtest-lab-create-template.md)的相似處，在於每個都可讓您透過用來佈建 VM 的 VHD 建立基本映像。若要協助決定哪一個適用於您的特定環境，請參閱文章：[比較研測實驗室中的自訂映像和公式](./devtest-lab-comparing-vm-base-image-types.md)。

## 建立新的公式
具有研測實驗室「使用者」權限的所有使用者都可以使用公式作為基礎以在實驗室中建立 VM。有兩種方式可以建立公式：
...
1. 點選 [建立] 建立公式。入口網站建立公式之後，該公式會列在 [Lab formulas] (實驗室公式) 刀鋒視窗上。
...
1. 在 [建立公式] 刀鋒視窗上，輸入新公式的 [名稱] 和 [描述]，或點選 [確定]。入口網站建立公式之後，該公式會列在 [Lab formulas] (實驗室公式) 刀鋒視窗上。
...
## 後續步驟
建立要在建立 VM 時使用的公式之後，下一個步驟就是[將 VM 加入研測實驗室](./devtest-lab-add-vm-with-artifacts.md)。
