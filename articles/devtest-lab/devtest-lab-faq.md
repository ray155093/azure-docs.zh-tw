<properties
	pageTitle="DevTest Labs 常見問題集 | Microsoft Azure"
	description="尋找 DevTest Labs 常見問題的解答"
...
- [如何使用 DevTest Labs？](#how-can-i-use-devtest-labs)
- [DevTest Labs 會收取多少費用？](#how-will-i-be-charged-for-devtest-labs)
 
## 安全性 
- [DevTest Labs 中有哪些不同的安全性層級？](#what-are-the-different-security-levels-in-devtest-labs)
- [如何建立讓使用者只能執行單一工作的特定角色？](#how-do-i-create-a-specific-role-to-allow-users-to-perform-only-a-single-task)
 
## CI/CD 整合與自動化 
 
- [DevTest Labs 是否會與我的 CI/CD 工具鏈整合？](#does-devtest-labs-integrate-with-my-cicd-toolchain)
 
## 虛擬機器 
 
- [為什麼我在 [Azure 虛擬機器] 刀鋒視窗中沒看到可在 DevTest Labs 看到的某些 VM？](#why-cant-i-see-certain-vms-in-the-azure-virtual-machines-blade-that-i-see-within-devtest-labs)
...
- [如何將現有 Azure VM 移到 DevTest Labs 實驗室？](#how-do-i-move-my-existing-azure-vms-into-my-devtest-labs-lab)
...
- [如何從 Azure Resource Manager 範本建立實驗室？](#how-do-i-create-a-lab-from-an-arm-template)
- [為何我所有的 VM 會建立在具有任意名稱的不同資源群組中？ 我是否可以重新命名或修改這些資源群組？](#why-are-all-of-my-vms-created-in-different-resource-groups-with-arbitrary-names-can-i-rename-or-modify-these-resource-groups)
...
### 為何應使用 DevTest Labs？ 
DevTest Labs 可讓您的小組節省時間和金錢。開發人員可以使用許多不同的基底物件建立自己的環境，並使用構件快速部署和設定應用程式。透過自訂映像和公式，就可以將虛擬機器儲存為範本並輕易予以重現。此外，實驗室還提供數個可設定的原則 (例如自動關閉、成本臨界值、每位使用者的 VM 數上限，VM 大小上限)，可讓實驗室系統管理員減少浪費並管理小組的環境。如需更深入的 DevTest Labs 說明，請閱讀[概觀](devtest-lab-overview.md)或觀看[簡介影片](/documentation/videos/videos/what-is-azure-devtest-labs)。

### 「零煩惱的自助方式」是什麼意思？
「零煩惱的自助方式」表示開發人員和測試人員可以視需要建立自己的環境，而且系統管理員可獲得保障，知道 DevTest Labs 會協助他們把浪費和管控成本降到最低。系統管理員可以指定允許的 VM 大小、VM 數目上限，以及何時啟動和關閉 VM。DevTest Labs 也可讓成本監視和警示設定動作變得簡單，以便能夠隨時注意實驗室資源的使用情形。

### 如何使用 DevTest Labs？ 
每當您需要開發或測試環境，而且想要加以快速重現和 (或) 以節省成本的原則進行管理時，DevTest Labs 便能派上用場。
...
### DevTest Labs 會收取多少費用？ 
DevTest Labs 是一項免費服務，這表示建立實驗室和設定原則、範本與構件都是免費的。您只需要針對在實驗室內使用到的 Azure 資源 (例如虛擬機器、儲存體帳戶和虛擬網路) 支付費用。如需實驗室資源成本的詳細資訊，請閱讀 [Azure DevTest Labs 定價](https://azure.microsoft.com/pricing/details/devtest-lab/)。

### DevTest Labs 中有哪些不同的安全性層級？  
安全性存取權是由 [Azure 角色型存取控制 (RBAC)](../active-directory/role-based-access-built-in-roles.md) 所決定。若要了解存取權的運作方式，了解 RBAC 所定義的權限、角色和範圍之間的差異將有所幫助。權限是針對特定動作所定義的存取權 (例如，針對所有虛擬機器的讀取權限)。角色是一組可加以群組並指派給使用者的權限 (例如，「訂用帳戶擁有者」具有訂用帳戶內所有資源的存取權)。範圍是 Azure 資源階層的其中一個層級 (例如，資源群組、單一實驗室或整個訂用帳戶)。
 
在 DevTest Labs 的範圍內有兩種可用來定義使用者權限的角色類型︰實驗室擁有者和實驗室使用者。

- 實驗室擁有者 - 實驗室擁有者可以存取實驗室內的任何資源；因此，他們可以修改原則、讀取和寫入任何 VM、變更虛擬網路，以及進行其他工作。
- 實驗室使用者 - 實驗室使用者可以檢視所有實驗室資源，例如 VM、原則和虛擬網路，但他們不能修改原則或任何由其他使用者所建立的 VM。您也可以在 DevTest Labs 建立自訂角色，若要了解如何進行，請參閱[將特定實驗室原則的權限授與使用者](devtest-lab-grant-user-permissions-to-specific-lab-policies.md)一文。
...
### DevTest Labs 是否會與我的 CI/CD 工具鏈整合？ 
如果您使用 VSTS，[Azure DevTest Labs 工作擴充功能](https://marketplace.visualstudio.com/items?itemName=ms-azuredevtestlabs.tasks)可讓您在 DevTest Labs 自動執行發行管線。此擴充功能的部分用法包括︰
...
至於其他 CI/CD 工具鏈，所有可透過 VSTS 工作擴充功能實現的上述案例，皆可經由類似方式，透過使用 [Azure PowerShell Cmdlet](../resource-group-template-deploy.md) 和 [.NET SDK](https://www.nuget.org/packages/Microsoft.Azure.Management.DevTestLabs/) 部署 [Azure Resource Manager 範本](https://github.com/Azure/azure-devtestlab/tree/master/ARMTemplates)來實現。您也可以使用[適用於 DevTest Labs 的 REST API](http://aka.ms/dtlrestapis) 來與您的工具鏈整合。

### 為什麼我在 [Azure 虛擬機器] 刀鋒視窗中沒看到可在 DevTest Labs 看到的某些 VM？
當您在 DevTest Labs 建立 VM 時，您會獲得該 VM 的存取權限，然後您就可以同時在 [實驗室] 刀鋒視窗和 [虛擬機器] 刀鋒視窗中看到它。如果您是 DevTest Labs 使用者，這可讓您透過實驗室的 [所有虛擬機器] 刀鋒視窗看到實驗室中建立的所有虛擬機器。不過，身為 DevTest Labs 使用者，您並不會自動獲得其他人所建立之 VM 資源的讀取權限，因此這些資源不會顯示在 [虛擬機器] 刀鋒視窗中。
...
### 如何從相同範本一次建立多個 VM？ 
您可以使用 [VSTS 工作擴充功能](https://marketplace.visualstudio.com/items?itemName=ms-azuredevtestlabs.tasks)，或在建立 VM 時[產生 Azure Resource Manager 範本](devtest-lab-add-vm-with-artifacts.md#save-arm-template)，然後[從 Windows PowerShell 部署 Azure Resource Manager 範本](../resource-group-template-deploy.md)。
 
### 如何將現有 Azure VM 移到 DevTest Labs 實驗室？ 
我們正在設計可直接將 VM 移至 DevTest Labs 的解決方案，但目前來說，您可以如下所示將現有 VM 複製到 DevTest Labs︰

1. 使用此 [Windows PowerShell 指令碼](https://github.com/Azure/azure-devtestlab/blob/master/Scripts/CopyVHDFromVMToLab.ps1)複製現有 VM 的 VHD 檔案
1. 在 DevTest Labs 實驗室內[建立自訂映像](devtest-lab-create-template.md)。
1. 在實驗室中從自訂映像建立 VM
 
### 是否可以在 VM 連接多個磁碟？ 
在 VM 連接多個磁碟有受到支援。
 
### 如何自動執行上傳 VHD 檔案的程序以建立自訂映像？ 
有兩個選項：
...
1. 選取左側面板中的 [資源群組]。
1. 找出並選取與您的實驗室相關聯的資源群組。
1. 在 [概觀] 刀鋒視窗中，選取其中一個儲存體帳戶。
1. 選取 [Blob]。
1. 在清單中尋找要上傳的項目。如果沒有任何一個存在，請返回步驟 4，然後嘗試另一個儲存體帳戶。
...
### 每個實驗室可以建立幾個 VM？ 
每個實驗室可以建立的 VM 數目並無特定限制，但其目前可支援的 VM 數目為：標準儲存體只支援同時執行大約 40 個 VM，進階儲存體只支援並行執行大約 25 個 VM。我們目前正在努力增加這些限制值。
