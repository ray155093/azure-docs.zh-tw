---
title: "將具有構件的 VM 加入 Azure DevTest Labs 中的實驗室 | Microsoft Docs"
...
---
# <a name="add-a-vm-with-artifacts-to-a-lab-in-azure-devtest-labs"></a>將具有構件的 VM 加入 Azure DevTest Labs 中的實驗室
...
DevTest Labs「構件」可讓您指定在建立 VM 時所執行的「動作」。 
...
## <a name="add-a-vm-with-artifacts"></a>加入具有構件的 VM
1. 登入 [Azure 入口網站](http://go.microsoft.com/fwlink/p/?LinkID=525040)。
2. 選取 [更多服務]，然後從清單中選取 [DevTest Labs]。
3. 從實驗室清單中，選取您想要在其中建立 VM 的實驗室。  
4. 在實驗室的 [概觀] 刀鋒視窗中，選取 [+ 虛擬機器]。  
    ![加入 VM 按鈕](./media/devtest-lab-add-vm-with-artifacts/devtestlab-home-blade-add-vm.png)
5. 在 [選擇基底]  刀鋒視窗中，選取 VM 的基底。
6. 在 [虛擬機器] 刀鋒視窗的 [虛擬機器名稱] 文字方塊中，輸入新虛擬機器的名稱。
   
    ![實驗室 VM 刀鋒視窗](./media/devtest-lab-add-vm-with-artifacts/devtestlab-lab-vm-blade.png)
7. 輸入 **使用者名稱** ，此名稱將被授與虛擬機器上的系統管理員權限。  
8. 如果您想使用儲存在「密碼存放區」中的密碼，請選取 [使用密碼存放區中的密碼]，並指定與您密碼對應的金鑰值。 否則，請在標示為 [輸入值] 的文字欄位中輸入密碼。
9. 選取 [虛擬機器大小]  ，然後選取其中一個預先定義的項目，這些項目可以指定處理器核心、RAM 大小，以及要建立的 VM 的硬碟大小。
10. 選取 [虛擬網路]  ，然後選取所需的虛擬網路。
11. 選取 [子網路]  ，然後選取子網路。
12. 如果實驗室原則是設定為允許所選子網路的公用 IP 位址，請選取 [是] 或 [否]，來指定是否要讓 IP 位址成為公用位址。 否則，這個選項會停用並選取為 [否] 。 
13. 選取 [構件]，然後從構件清單中，選取並設定您想要新增到基本映像中的構件。 
    **附註：**如果您對 DevTest Labs 或設定成品並不熟悉，請先跳到[將現有的構件加入至 VM](#add-an-existing-artifact-to-a-vm) 一節，完成該節之後再返回此處。
14. 如果您想要檢視或複製 Azure Resource Manager 範本，請跳至 [儲存 Azure Resource Manager 範本](#save-azure-resource-manager-template) 一節，然後在完成時回到這裡。
15. 選取 [建立]  ，將指定的 VM 加入實驗室。
16. 實驗室刀鋒視窗會顯示 VM 的建立狀態，其狀態會先是 [正在建立]，在啟動該 VM 之後才會變成 [正在執行]。
17. 請移至 [後續步驟](#next-steps) 一節。 
...
2. 在 [加入構件]  刀鋒視窗中，選取所需的構件。  
   
    ![加入構件刀鋒視窗](./media/devtest-lab-add-vm-with-artifacts/devtestlab-add-artifact-blade.png)
3. 輸入必要的參數值以及任何您所需的選用參數。  
4. 選取 [加入] 以加入構件，然後返回 [加入構件] 刀鋒視窗。
5. 視需要繼續為您的 VM 加入構件。
6. 加入構件之後，您可以 [變更構件的執行順序](#change-the-order-in-which-artifacts-are-run)。 您也可以返回來 [檢視或修改構件](#view-or-modify-an-artifact)。
...
2. 若要指定構件執行的順序，請將構件拖放到預期的順序。 **附註︰** 如果您在拖曳構件時發生問題，請確定您是從構件左側進行拖曳。 
3. 完成時選取 [確定]  。  
...
1. 在 [加入構件]  刀鋒視窗頂端，選取會指出已加入 VM 的構件數目的連結。
    ...
    ![新增至 VM 的構件數目](./media/devtest-lab-add-vm-with-artifacts/devtestlab-add-artifacts-blade-selected-artifacts.png)
2. 在 [選取的構件]  刀鋒視窗中，選取您想要檢視或編輯的構件。  
3. 在 [加入構件] 刀鋒視窗中，進行任何所需的變更，然後選取 [確定] 以關閉 [加入構件] 刀鋒視窗 。
4. 選取 [確定] 以關閉 [選取的構件] 刀鋒視窗。
