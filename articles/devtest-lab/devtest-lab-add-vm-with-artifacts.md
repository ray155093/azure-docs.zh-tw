...
4. 在實驗室的 [概觀] 刀鋒視窗中，選取 [+ 虛擬機器]。![加入 VM 按鈕](./media/devtest-lab-add-vm-with-artifacts/devtestlab-home-blade-add-vm.png)
...
8. 如果您想使用儲存在「密碼存放區」的密碼，請選取 [使用密碼存放區中的密碼]，並指定對應您的密碼的金鑰值。否則，請在標示為 [輸入值] 的文字欄位中輸入密碼。
9. 選取 [虛擬機器大小]，然後選取其中一個預先定義的項目，這些項目可以指定處理器核心、RAM 大小，以及要建立的 VM 的硬碟大小。
10. 選取 [虛擬網路]，然後選取所需的虛擬網路。
11. 選取 [子網路]，然後選取子網路。
12. 如果將實驗室原則設定為允許所選子網路的公用 IP 位址，請選取 [是] 或 [否]，來指定您是否想要讓 IP 位址成為公用的位址。否則，這個選項會停用並選取為 [否]。
13. 選取 [構件]，從構件清單中選取並設定您想要加入至基本映像的構件。**附註：**如果您剛接觸到研發/測試實驗室或設定構件，請先跳到[將現有的構件加入至 VM](#add-an-existing-artifact-to-a-vm) 一節，完成該節之後再返回此處。
14. 如果您想要檢視或複製 Azure Resource Manager 範本，請跳至[儲存 Azure Resource Manager 範本](#save-arm-template)一節，然後在完成時回到這裡。
15. 選取 [建立]，將指定的 VM 加入實驗室。
16. [實驗室] 刀鋒視窗會顯示 VM 的建立狀態，其狀態會先是 [正在建立]，在啟動該 VM 之後才會變成 [正在執行]。
