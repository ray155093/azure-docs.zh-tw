...
1. 在實驗室的 [設定] 刀鋒視窗的 [VM 原則] 下，選取 [允許的 VM 大小]。
...
1. 在實驗室的 [設定] 刀鋒視窗的 [VM 原則] 下，選取 [每位使用者的 VM 數目上限]。

	![Settings](./media/devtest-lab-set-lab-policy/max-vms-per-user.png)

1. 選取 [開啟] 來啟用此原則，以及選取 [關閉] 來停用它。

1. 如果啟用這個原則，可在 [每位使用者所允許的 VM 數目上限] 文字方塊中輸入數值，指出使用者可建立的 VM 數目上限。如果您輸入的數字無效，UI 將會顯示此欄位允許的最大數目。
...
1. 在實驗室的 [設定] 刀鋒視窗的 [VM 原則] 下，選取 [允許的 VM 總數]。

	![Settings](./media/devtest-lab-set-lab-policy/total-vms-allowed.png)

1. 選取 [開啟] 來啟用此原則，以及選取 [關閉] 來停用它。

1. 如果啟用這個原則，可在 [這個實驗室允許的 VM 總數] 文字方塊中輸入數值，指出目前實驗室可以建立的 VM 數目上限。如果您輸入的數字無效，UI 將會顯示此欄位允許的最大數目。
...
1. 在實驗室的 [設定] 刀鋒視窗的 [VM 原則] 下，選取 [自動關機]。
...
1. 在實驗室的 [設定] 刀鋒視窗的 [VM 原則] 下，選取 [自動啟動]。
