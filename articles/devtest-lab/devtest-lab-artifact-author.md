...
2. 取得範例 artifactfile.json - 在我們的 [GitHub 儲存機制](https://github.com/Azure/azure-devtestlab)中查看 Azure 研發/測試實驗室小組所建立的構件，我們在其中建立了豐富的構件庫，可協助您建立自己的構件。下載構件定義檔，並對它進行變更以建立您自己的構件。
...
5. 將構件儲存機制加入實驗室 - 請參閱[將 Git 構件儲存機制加入實驗室](devtest-lab-add-artifact-repo.md)一文。
