<properties
	pageTitle="從 VHD 檔案建立 DevTest Labs 自訂映像 | Microsoft Azure"
	description="了解如何從 VHD 檔案建立自訂映像，接著可使用該映像在研發/測試實驗室中建立 VM"
...
當您[建立實驗室](devtest-lab-create-lab.md)之後，就可以[將虛擬機器 (VM) 加入該實驗室](devtest-lab-add-vm-with-artifacts.md)。當您建立 VM 時，您會指定「基底」，這可以是「自訂映像」或「Marketplace 映像」。在本文中，您將看到如何從 VHD 檔案建立自訂映像的方式。請注意，您必須能夠存取有效的 VHD 檔案，才能執行本文中的所有步驟。
