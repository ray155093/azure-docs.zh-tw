...
Kubernetes 是受歡迎的開放原始碼生產等級容器 Orchestrator 工具，可用於自動化容器部署、擴充和管理容器化應用程式。 由於它是開放原始碼解決方案，而且是由開放原始碼社群所推動，因此可在 Azure 容器服務上順利執行，並可應用於 Azure Container Service 上大規模的容器部署。

![設定來使用 Kubernetes 以顯示代理程式與主機的 Azure Container Service。](media/acs-intro/kubernetes.png)
