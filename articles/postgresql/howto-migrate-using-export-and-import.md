...
若要將內部部署或 VM 中的現有 PostgreSQL 資料庫匯出至 sql 指令碼檔案，請執行下列命令︰
```bash
...
```
在此範例中，我們將使用 psql 和上一個步驟中名為 **testdb.sql** 的指令碼檔案，將資料匯入目標伺服器 **mypgserver-20170401.postgres.database.azure.com** 上的資料庫 **mypgsqldb**。
```bash
