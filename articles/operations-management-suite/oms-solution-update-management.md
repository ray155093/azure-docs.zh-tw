...
如果代理程式無法與 OMS 服務進行通訊，並已設定為透過防火牆或 Proxy 伺服器來與網際網路通訊，請藉由檢閱[在 Log Analytics 中設定 Proxy 和防火牆設定](../log-analytics/log-analytics-proxy-firewall.md)，確認防火牆或 Proxy 伺服器設定正確。
...
| 查詢 | 說明 |
| --- | --- |
|以 Windows 為基礎且需要更新的伺服器電腦 |`Type:Update OSType!=Linux UpdateState=Needed Optional=false Approved!=false | measure count() by Computer` |
|需要更新的 Linux 伺服器 | `Type:Update OSType=Linux UpdateState!="Not needed" | measure count() by Computer` |
...
| 特定電腦的遺漏更新 (以您自己的電腦名稱取代此值) |`Type=Update UpdateState=Needed Optional=false Computer="COMPUTER01.contoso.com" | select Computer,Title,KBID,Product,UpdateSeverity,PublishedDate` |
...
| 以 Windows 為基礎且在更新回合中更新失敗的伺服器電腦 | `Type:UpdateRunProgress InstallationStatus=failed | measure count() by Computer, Title, UpdateRunName` |
| 在更新回合中更新失敗的 Linux 伺服器 |`Type:UpdateRunProgress InstallationStatus=failed | measure count() by Computer, Product, UpdateRunName` |
...
| 列出哪些更新部署已修改過電腦 |`Type:UpdateRunProgress | measure Count() by UpdateRunName` |
|在此更新回合中更新的電腦 (以您的更新部署名稱取代此值) |`Type:UpdateRunProgress UpdateRunName="DeploymentName" | measure Count() by Computer` |
