...
4. 選取您希望從邊緣節點清除的資產。  如果您希望清除所有資產，請按一下 [清除]  核取方塊。  或者，在 [路徑] 文字方塊中輸入每個您想要清除之資產的完整路徑 (例如，`/pictures/kitten.png`)。
   ...
   > 
   > 路徑必須是符合下列[規則運算式](https://msdn.microsoft.com/library/az24scfc.aspx)的相對 URL：`^\/(?:[a-zA-Z0-9-_.\u0020]+\/)*\*$";`。  若為**來自 Verizon 的 Azure CDN** (標準和進階)，可使用星號 (\*) 做為萬用字元 (例如 `/music/*`)。  萬用字元和**全部清除**不能搭配**來自 Akamai 的 Azure CDN** 使用。
   > 
   > 
5. 按一下 [清除]  按鈕。
