...
---
# <a name="replicate-hyper-v-vms-in-vmm-clouds-to-a-secondary-site-with-azure-site-recovery-using-san"></a>使用 SAN 搭配 Azure Site Recovery 將 Hyper-V VM (位於 VMM 雲端中) 複寫至次要網站


如果您想要部署 [Site Recovery](site-recovery-overview.md)以管理到次要 VMM 網站的 HYPER-V VM 複寫，請閱讀本文。


本文包含案例概觀、在 VMM 中設定 SAN 儲存體及在 Site Recovery 入口網站中設定複寫的指示。 最後它會測試容錯移轉，藉此確定一切如預期般運作。

請在底下留下您的意見。 請至 [Azure Recovery Services Forum (Azure 復原服務論壇)](https://social.msdn.microsoft.com/forums/azure/home?forum=hypervrecovmgr) 尋找技術問題的解答。
...
* 您將利用現有的 SAN 基礎結構，來保護部署在 Hyper-V 叢集中的關鍵任務應用程式。 VM 可以群組方式複寫，使多層式架構應用程式可以一致的方式容錯移轉。
...
- 不支援從 SAN 複寫至 Azure。
- 您無法複寫共用 VHDX 或是透過 iSCSI 或光纖通道直接連接到 VM 的邏輯單元 (LUN)。 可以複寫客體叢集。
...
- **Azure**︰您在 Azure 入口網站中設定 Site Recovery 保存庫。
- **SAN 存放裝置**︰在 VMM 網狀架構中管理 SAN 存放裝置。 您新增儲存體提供者、建立儲存體分類，並設定存放集區。
- **VMM 與 Hyper-V**：我們建議一個網站一部 VMM 伺服器。 您設定 VMM 私人雲端，並將 HYPER-V 叢集放在這些雲端中。 部署期間，Azure Site Recovery 提供者是安裝在每部 VMM 伺服器上，伺服器則註冊於保存庫中。 提供者會與 Site Recovery 服務通訊以管理複寫、容錯移轉和容錯回復。
...
- **容錯回復**︰若要容錯回復，您可以啟用反向複寫，將差異變更從次要網站傳送到主要網站。 反向複寫完成後，則執行從次要到主要的計劃性容錯移轉。 此計劃性容錯移轉會停止次要網站上的複本 VM，並在主要網站上啟動它們。
...
**必要條件** | **詳細資料**
--- | ---
**Azure** |您需要 [Microsoft Azure](https://azure.microsoft.com/) 帳戶。 您可以從 [免費試用](https://azure.microsoft.com/pricing/free-trial/)開始。 [深入了解](https://azure.microsoft.com/pricing/details/site-recovery/) Site Recovery 價格。 您建立 Azure Site Recovery 保存庫來設定和管理複寫和容錯移轉。
**VMM** | 您可以使用單一 VMM 伺服器，並在不同的雲端之間進行複寫，但我們建議在主要網站和次要網站中各部署一個 VMM。 VMM 可部署為實體或虛擬獨立伺服器，或是一個叢集。 <br/><br/>VMM 伺服器應至少執行附帶最新累計更新的 System Center 2012 R2<br/><br/> 您需要至少有一個雲端設定在您想要保護的主要 VMM 伺服器上，一個雲端要設定在您想要用於容錯移轉的次要 VMM 伺服器上。<br/><br/> 來源雲端必須包含一或多個 VMM 主機群組。<br/><br/> 所有的 VMM 雲端都必須設定 Hyper-V 容量設定檔。<br/><br/> 深入了解設定 VMM 雲端 (https://technet.microsoft.com/en-us/system-center-docs/vmm/scenario/cloud-overview)。
**Hyper-V** | 在主要和次要 VMM 雲端中，您需要一或多個 Hyper-V 叢集。<br/><br/> 來源 Hyper-V 叢集應包含一或多部 VM。<br/><br/> 主要和次要網站中的 VMM 主機群組應包含至少一個 HYPER-V 叢集。<br/><br/>主機和目標 Hyper-V 伺服器必須至少執行 Hyper-V 角色的 Windows Server 2012，並且安裝最新更新。<br/><br/> 如果您在叢集中執行 Hyper-V，請注意，如果您具有靜態 IP 位址叢集，並不會自動建立叢集代理。 您必須手動進行設定。 在 Aidan Finn 的部落格中[深入了解](https://www.petri.com/use-hyper-v-replica-broker-prepare-host-clusters)。
...
**網路對應** | 您可以設定網路對應，以確保在容錯移轉後，會以最佳方式將已複寫的虛擬機器置於次要 Hyper-V 主機伺服器上，使這些虛擬機器可以連線到適當的 VM 網路。 如果您沒有設定網路對應，複本 VM 將不會在容錯移轉之後連接到任何網路。<br/><br/> 您必須確定 VMM 網路設定正確，才能在 Site Recovery 部署期間設定網路對應。 在 VMM 中，來源 Hyper-V 主機上的 VM 應連接到 VMM VM 網路。 該網路應該連結到與雲端相關聯的邏輯網路。<br/><br/> 目標雲端應該設定相對應的 VM 網路，而該網路應該連結到與目標雲端相關聯的相對應邏輯網路。<br/><br/>[深入了解](site-recovery-network-mapping.md)網路對應。
...
1. 在 VMM 主控台，按一下 [網狀架構]  >  [儲存體]  >  [新增資源]  >  [存放裝置]。
2. 在 [新增存放裝置精靈] > [選取存放裝置提供者類型] 中，選取 [由 SMI-S 提供者探索到及管理的 SAN 和 NAS 裝置]。
...
請根據您需要保護的應用程式、容量需求及要一起複寫的項目，建立 LUN。
...
1. 存放裝置出現在 VMM 網狀架構之後，您可以[佈建 LUN](https://technet.microsoft.com/en-us/system-center-docs/vmm/manage/manage-storage-host-groups#create-a-lun-in-vmm)。
     ...
     > [!NOTE]
     > 請勿將已啟用複寫的 VM 的 VHD 新增到 LUN。 (如果這些 LUN 不在 Site Recovery 複寫群組中。) 如果您這樣做，Site Recovery 將偵測不到它們。
     >
     
2. 接著，配置儲存容量給 Hyper-V 主機叢集，讓 VMM 可以將虛擬機器資料部署到已佈建的存放裝置上：

   * 將存放裝置配置給叢集之前，您必須先將其配置給叢集所在的 VMM 主機群組。 深入了解如何[配置存放裝置邏輯單元](https://technet.microsoft.com/library/gg610686.aspx)和[存放集區](https://technet.microsoft.com/library/gg610635.aspx)至主機群組。</a>
...
3. 在 [請指定儲存 SMI-S 提供者的通訊協定與位址]中，選取 [SMI-S CIMXML]。 指定連接到提供者的設定。 您只能針對 SMI-S CIMXML 使用 SSL 連線。
    ...
    ![提供者連接](./media/site-recovery-vmm-san/connect-settings.png)
4. 在 [執行身分帳戶]  中指定一個可以存取提供者的 VMM 執行身分帳戶，或建立一個新帳戶。
5. 在 [收集資訊]  中，VMM 會自動嘗試探索並匯入存放裝置資訊。 若需要重試，請按一下 [掃描提供者]。 當探索程序成功時，頁面上就會列出存放裝置陣列、存放集區、製造商、型號及容量。

    ![探索儲存體](./media/site-recovery-vmm-san/discover.png)
7. 在 [選取要列入管理的存放集區並指派分類] 中，選取 VMM 將管理的存放集區，並為它們指派一個分類。 LUN 資訊將會從存放集區匯入。
...
1. 從您想要在保存庫中註冊的 VMM 伺服器登入 [管理入口網站](https://portal.azure.com) 。
...
9. 在 [網際網路連線] 中，指定提供者連接到網際網路的方式。 如果您想使用伺服器上的預設網際網路連線設定，請選取 [使用預設的系統 Proxy 設定]。
   ...
   * 如果您使用的是自訂 proxy，則會使用指定的 proxy 認證自動建立 VMM RunAs 帳戶 (DRAProxyAccount)。 設定 Proxy 伺服器，讓此帳戶可以進行驗證。 您可以修改 VMM 主控台中的 RunAs 帳戶設定 ([設定]  >  [安全性]  >  [執行身分帳戶]  >  [DRAProxyAccount])。 您需要重新啟動 VMM 服務，變更才會生效。
...
您可以對應主要和次要陣列，以指定哪一個次要存放集區要從主要集區接收複寫資料。 請在設定複寫之前先對應存放裝置，因為在您為複寫群組啟用保護時，會使用此對應資訊。
...
註冊 VMM 伺服器之後，您就可以設定雲端保護設定。

1. 在 [快速啟動] 頁面上，按一下 [設定 VMM 雲端的保護] 。
2. 在 [受保護的項目] 索引標籤上，選取 [雲端] > [組態]。
3. 在 [目標] 中，選取 [VMM]。
...
儲存設定之後，會建立一個工作，並可在 [工作] 索引標籤上進行監視。 在 [設定] 索引標籤上可修改設定。 如果您要修改目標位置或目標雲端，則必須移除雲端組態，然後重新設定雲端。

## <a name="step-5-enable-network-mapping"></a>步驟 5：啟用網路對應

1. 在 [快速入門] 頁面上，按一下 [對應網路] 。
2. 選取來源 VMM 伺服器，然後選取對應網路的目標 VMM 伺服器。 隨即會顯示來源網路的清單和與其相關聯的目標網路。 對於未對應的網路，將顯示空白值。 按一下來源和目標網路名稱旁邊的資訊圖示，以檢視每個網路的子網路。
...
6. 按一下打勾記號完成對應程序。 隨即會啟動一個追蹤進度的工作。 您可以在 [工作]  索引標籤上檢視它。

## <a name="step-6-enable-replication-for-replication-groups"></a>步驟 6：為複寫群組啟用複寫

您必須先為存放裝置複寫群組啟用複寫，才能為虛擬機器啟用保護。

1. 在 Site Recovery 入口網站之主要雲端的內容頁面中，開啟 [虛擬機器] 索引標籤，然後按一下 [新增複寫群組]。
...
在存放裝置群組複寫之後，請使用下列其中一個方法在 VMM 主控台中為 VM 啟用保護：

* **新增虛擬機器** — 當您建立新的 VM 時，啟用複寫並將該 VM 與複寫群組建立關聯。
  使用此選項時，VMM 會使用智慧型放置，以最佳方式將 VM 存放裝置放置在複寫群組的 LUN 上。 Site Recovery 會在次要網站上協調建立影子 VM 並配置容量，以便能夠在容錯移轉後啟動複本 VM。
...
VM 啟用複寫後，就會顯示在 Site Recovery 主控台。 您可以檢視 VM 內容、追蹤狀態，以及讓包含多個 VM 的複寫群組進行容錯移轉。 在 SAN 複寫中，與複寫群組關聯的所有 VM 必須都一起進行容錯移轉。 這是因為容錯移轉會先發生在儲存層。 請務必正確地組成您的複寫群組，只將相關聯的 VM 放在一起。

> [!NOTE]
> 在啟用了 VM 的複寫之後，您不應該將它的 VHD 新增至不在 Site Recovery 複寫群組的 LUN。 如果您這樣做，Site Recovery 將偵測不到它們。
>
>

您可以在 [工作] 索引標籤中追蹤進度，包括初始複寫。 執行「完成保護」工作之後，虛擬機器即準備好進行容錯移轉。
...
2. 指定復原方案的名稱，以及來源和目標 VMM 伺服器。 來源伺服器必須有已啟用容錯移轉和復原功能的 VM。 選取 [SAN]  ，僅檢視為 SAN 複寫設定的雲端。

    ![建立復原計畫](./media/site-recovery-vmm-san/r-plan.png)

3. 在 [選取虛擬機器] 中，選取複寫群組。 與群組相關聯的所有 VM 都會加入復原方案。 這些 VM 將新增到復原方案的預設群組 (群組 1)。 您可以視需要新增更多群組。 複寫之後，VM 將會根據復原方案群組的順序來啟動。

    ![新增虛擬機器](./media/site-recovery-vmm-san/r-plan-vm.png)
4. 建立復原方案之後，它會出現在 [復原方案] 索引標籤上的清單中。 選取方案，然後按一下 [測試容錯移轉]。
5. 在 [確認測試容錯移轉] 頁面上，選取 [無]。 啟用此選項時，容錯移轉的複本 VM 將不會連線到任何網路。 這會測試 VM 容錯移轉是否如預期般運作，但不會測試網路環境。 [深入了解](site-recovery-failover.md#run-a-test-failover)其他網路功能選項。

    ![選取測試網路](./media/site-recovery-vmm-san/test-fail1.png)


1. 測試虛擬機器將建立在複本虛擬機器所在的相同主機上。 它並不會新增至複本虛擬機器所在的雲端。
6. 測試 VM 會建立在複本 VM 所在的相同主機上。 它不會新增至複本 VM 所在的雲端。
2. 複寫之後，複本 VM 的 IP 位址會不同於主要虛擬機器的 IP 位址。 如果您是從 DHCP 發出位址，便會自動更新位址。 如果您不是使用 DHCP 而想要相同位址，您將需要執行數個指令碼。
3. 執行此指令碼以擷取 IP 位址。
