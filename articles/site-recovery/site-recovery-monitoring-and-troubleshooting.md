---
title: 監視和疑難排解虛擬機器與實體伺服器的保護 | Microsoft Docs
...
### <a name="vmware/physical-site-deployment-for-replication-between-on-premises-and-azure."></a>用於在內部部署和 Azure 之間複寫的 VMware/實體網站部署。
...
### <a name="vmm-site-deployment-for-replication-between-on-premises-site."></a>用於在內部部署網站之間複寫的 VMM 網站部署。
...
### <a name="vmm-site-deployment-for-replication-between-on-premises-&-azure."></a>用於在內部部署和 Azure 之間複寫的 VMM 網站部署。
在內部部署和 Azure 之間設定 DR 的一部分；Azure Site Recovery 提供者必須下載並安裝在 VMM 伺服器上，連同 Azure 復原服務代理程式，它必須安裝在每個 Hyper-V 主機上。 如需詳細資訊，請參閱 [了解網站至 Azure 保護](site-recovery-understanding-site-to-azure-protection.md) 。

![用於在內部部署和 Azure 之間複寫的 VMM 網站部署。](media/site-recovery-monitoring-and-troubleshooting/image2.png)

### <a name="hyper-v-site-deployment-for-replication-between-on-premises-&-azure"></a>用於在內部部署和 Azure 之間複寫的 Hyper-V 網站部署。
與 VMM 部署相同 – 不同的是提供者和代理程式都安裝在 HYPER-V 主機本身。 如需詳細資訊，請參閱 [了解網站至 Azure 保護](site-recovery-understanding-site-to-azure-protection.md) 。
...
## <a name="monitor-replication-health-for-virtual-machine"></a>監視虛擬機器的複寫健全狀況
...
與 Hyper-V 複本有關的所有事件都會記錄在位於 **Applications and Services Logs\\Microsoft\\Windows** 底下的 Hyper-V-VMMS\\Admin。 此外，可以針對 Hyper-V-VMMS 啟用分析記錄檔。 若要啟用此記錄檔，先讓 [分析] 與 [偵錯] 記錄檔可在 [事件檢視器] 中檢視。 開啟 [事件檢視器]，然後在 [檢視] 功能表中，按一下 [顯示分析與偵錯記錄檔]。
...
## <a name="reaching-out-for-microsoft-support"></a>連絡 Microsoft 支援
...
## <a name="common-asr-errors-and-their-resolutions"></a>常見的 ASR 錯誤及其解決方案
...
### <a name="configuration-server,-process-server,-master-target"></a>設定伺服器、處理序伺服器、主要目標
...
如果入口網站中的 [連接] 按鈕呈現灰色，而且您未透過 Express Route 或網站間 VPN 連接來連接到 Azure，您就必須先建立 VM 並為其指派公用 IP 位址，才能使用 RDP/SSH。 遵循下列步驟以將公用 IP 新增到虛擬機器的網路介面上。  
