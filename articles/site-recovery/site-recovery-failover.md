...
6. 完成之後，在容錯移轉到達 [完成測試] 階段時，請按一下 [完成測試] 來完成。
7. 在 [記事]  中，記錄並儲存關於測試容錯移轉的任何觀察。
8. 按一下 [測試容錯移轉已完成]  ，即可自動清除測試環境。 完成此動作之後，測試容錯移轉將會顯示 [完成] 狀態。

> [!NOTE]
> 如果測試容錯移轉持續兩週以上，系統即會強制完成該測試容錯移轉。 任何在測試容錯移轉期間自動建立的項目或虛擬機器都將刪除。
> 
> 
...
5. 完成之後，請確認虛擬機器已成功啟動。
6. 確認虛擬機器成功啟動之後，完成測試容錯移轉以清除隔離的環境。 如果您選擇自動建立 VM 網路，清除功能會刪除所有的測試虛擬機器和測試網路。
