---
title: "將 VMWare 虛擬機器和實體伺服器容錯回復至內部部署網站 | Microsoft Docs"
...
5. 如果您已選取 [傳統部署]  網路 - 系統會要求您透過 Azure 資源庫建立新的 VM，並在其中安裝處理序伺服器。
   ...
   2. 根據 [這裡提供](site-recovery-vmware-to-azure-classic.md#step-5-install-the-management-server)
...
> [!NOTE]
> 如果您想要在 Linux 上安裝主要目標伺服器，請遵循下一個程序中的指示。 針對 MT OS，請確定您只使用 CentOS 6.6 最基本作業系統。
> 
> 
...
3. 如果所要保護之來源機器的根目錄或開機裝置是執行搭配 Reiser 的 Linux，或是 XFS 檔案系統，則應該下載並安裝下列額外封裝：
   ...
   * # <a name="yum-install-device-mapper-multipath---this-is-required-to-enable-multipath-packages-on-the-mt-server"></a>yum install device-mapper-multipath   這是在 MT 伺服器上啟用「多重路徑」套件的必要做法。
...
5. 「保留磁碟機」是在將 VM 複寫回內部部署時，用來停止時間點。 保留磁碟機的某些準則如下，如果沒有這些準則，磁碟機將不會針對主要目標伺服器列出。
   ...
   a. 磁碟區不能使用於其他用途 (做為複寫目標等)

   b. 磁碟區不能處於鎖定模式。

   c. 磁碟區不能是快取磁碟區。 (該磁碟區上不能存在 MT 安裝。 PS+MT 自訂安裝磁碟區不適合做為保留磁碟區。 這裡已安裝的 PS+MT 磁碟區是 MT 的快取磁碟區。)

   d. 磁碟區檔案系統類型不能是 FAT 和 FAT32。
