...
1. 在 [保存庫] > [設定] > [站台復原基礎結構](位於 \[管理\] 標題下方.md) > [組態伺服器](位於 \[適用於 VMware 與實體機器\] 標題下方.md) 中，選取組態伺服器。 按一下於下列範例螢幕擷取畫面中以黃色醒目提示的 [+ 處理序伺服器]。
...
3. 如果 **disk.EnableUUID** 資料列存在，請確定其值已設定為 [True](有區分大小寫.md)。 如果已如此設定，則可加以取消，然後在機器開機之後，於客體作業系統內部測試 SCSI 命令。 
