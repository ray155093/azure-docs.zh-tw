...
5. [安裝行動服務](site-recovery-vmware-to-azure.md#step-6-replicate-applications)。 所有要保護的 VM 都需要安裝行動服務。 這項服務會將資料傳送至處理序伺服器。 行動服務可以手動方式安裝，或在為 VM 啟用保護時由處理序伺服器自動推入安裝。 您所要移轉 VM 上的防火牆規則應該設定為允許推入安裝這項服務。
