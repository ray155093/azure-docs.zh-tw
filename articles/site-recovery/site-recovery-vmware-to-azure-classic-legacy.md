...
6. 如果您在執行 Linux：
   ...
   4. 將檔案從 Gzip 安裝程式解壓縮，方法是執行：**tar –xvzf Microsoft-ASR\_UA\_8.4.0.0\_RHEL6-64***  
...
> [!NOTE]
> 註冊完成之後，最多需要 15 分鐘，入口網站才會列出主要目標伺服器。若要立刻更新，請在 [組態伺服器] 頁面上按一下 [重新整理]。
> 
> 
...
5. 請注意，您可能需要重新啟動伺服器才能完成安裝。在 [組態伺服器] > [伺服器詳細資料] 中，檢查已出現處理序伺服器，並在保存庫中成功註冊。
...
2. 在容錯移轉和 Azure 虛擬機器大小調整之後，您可以修改要提供給 Azure 中機器的名稱。您也可以選取機器要在容錯移轉之後要連結的 Azure 網路。
   ...
   > [!NOTE]
   > [Migration of networks](../resource-group-move-resources.md) 對於用於部署 Site Recovery 的網路，不支援跨相同訂用帳戶內的資源群組或跨訂用帳戶。
   > 
   > 
