...
![Azure 虛擬機器的可用性設定組](./media/resiliency-technical-guidance-recovery-local-failures/partitioning-2.png)
...
>[AZURE.NOTE] Azure SQL Database 不提供與 SQL Server 的一對一功能對應。其目的是要滿足一組唯一適用於雲端應用程式的不同需求 (彈性調整、資料庫即服務以降低維護成本等等)。如需詳細資訊，請參閱[選擇雲端 SQL Server 選項：Azure SQL Database (PaaS) 或 Azure VM 上的 SQL Server (IaaS)](../sql-database/data-management-azure-sql-database-and-sql-server-iaas.md)。
