...
使用 Microsoft Azure 為您的內部部署、混合式和公用雲端應用程式達成商務持續性和恢復的詳細概念和最佳作法清單。

###[建置在 Microsoft Azure 上之應用程式的災害復原和高可用性](./resiliency-disaster-recovery-high-availability-azure-applications.md)
可用性和災害復原的詳細概觀。它涵蓋手動複寫參考資料和交易資料的挑戰。最後幾個小節摘要說明可跨越 Azure 區域以獲得最高層級可用性之不同類型的災害復原拓撲。

###[概觀：雲端商務持續性和 SQL Database 的資料庫災害復原](../sql-database/sql-database-business-continuity.md)
獨家著重於 Azure 的 Azure SQL Database 技術可用性，主要著重於備份與還原策略。如果您在雲端服務中使用 Azure SQL Database，應該檢閱本文和其相關的資源。

###[Azure 虛擬機器中的 SQL Server 高可用性和災害復原](../virtual-machines/virtual-machines-windows-sql-high-availability-dr.md)
討論當您使用基礎結構即服務 (IaaS) 來主控資料庫服務時，您可以使用的可用性選項。它討論 AlwaysOn 可用性群組、資料庫鏡像、記錄傳送，以及備份/還原。請注意，相同小節中另外還有數個教學課程示範如何使用這些技術。
...
著重於開發高度可調整的雲端架構。您用來改善可用性的許多技術也有助於改善可調整性。此外，如果您的應用程式在負載增加時無法調整，可調整性就變成可用性問題。

###[Azure 虛擬機器中的 SQL Server 備份和還原](../virtual-machines/virtual-machines-windows-sql-backup-recovery.md)
如何備份和還原 Azure 虛擬機器上執行的 Microsoft SQL Server 的技術指引。

###[Failsafe︰恢復雲端架構指引](https://channel9.msdn.com/Series/FailSafe)
建置恢復雲端架構的指引、用於採用 Microsoft 技術實作這些架構的指引，以及針對特定案例實作這些架構的訣竅。

##後續步驟
這篇文章是一系列文章的一部分，著重在 [Azure 復原技術指導](./resiliency-technical-guidance.md)。如果您有興趣閱讀本系列的其他文章，請從標題為[從本機失敗復原](./resiliency-technical-guidance-recovery-local-failures.md)的下一篇文章開始。
