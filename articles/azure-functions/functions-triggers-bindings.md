...
這篇文章說明如何在 Azure Functions 中設定和編碼觸發程序及繫結。透過 Azure 入口網站的**整合** UI 來輕鬆管理大部分繫結，但入口網站並未說明每個繫結的所有功能和選項。
...
您可以使用 HTTP 或 WebHook 觸發程序來呼叫函式以回應 HTTP 要求。要求必須包含 API 金鑰，但目前只有在 Azure 入口網站 UI 中提供。
...
WebHook 觸發程序是具有以下專為 Webhook 設計之功能的 HTTP 觸發程序︰

* 對於特定 WebHook 提供者 (目前支援 GitHub 和 Slack)，Functions 執行階段會驗證提供者的簽章。
* 對於 Node.js 函式，Functions 執行階段提供要求主體而非要求物件。沒有任何 C# 函式的特殊處理，因為您可以指定參數類型來控制提供的內容。如果指定 `HttpRequestMessage`，會取得要求物件。如果指定 POCO 類型，Functions 執行階段會嘗試剖析要求主體中的 JSON 物件以填入物件屬性。
...
根據預設，API 金鑰必須包含 HTTP 要求，以觸發 HTTP 或 WebHook 函式。金鑰可以包含在名為 `code` 的查詢字串變數中，或包含在 `x-functions-key` HTTP 標頭。對於非 WebHook 函式，您可以在 function.json 檔案中將 `authLevel` 屬性設為 [anonymous]，以指定 API 金鑰為非必要。

您可以在函式應用程式的檔案系統中，於 D:\\home\\data\\Functions\\secrets 中找到 API 金鑰值。主要金鑰和函式金鑰是在 host.json 檔案中設定，如此範例所示。
...
如果 secrets 資料夾包含與函式同名的 JSON 檔案，也可使用該檔案中的 `key` 屬性來觸發函式，且此金鑰只會使用其參考的函式。例如，名為 `HttpTrigger` 之函式的 API 金鑰是在 secrets 資料夾的 HttpTrigger.json 中指定。下列是一個範例：
...
排程運算式可以是包含 6 個欄位的 [CRON 運算式](http://en.wikipedia.org/wiki/Cron#CRON_expression)：{秒} {分鐘} {小時} {日} {月} {星期幾}。您在線上找到的許多 cron 運算式會省略 {秒} 欄位，因此如果從其中之一複製，必須調整額外的欄位。
...
* [function.json 中的 Azure 儲存體連接屬性](#storageconnection)
...
### <a id="storageconnection"></a>function.json 中的 Azure 儲存體連接屬性

對於所有 Azure 儲存體觸發程序和繫結，function.json 檔案包含 `connection` 屬性。例如：
...
如果您留空 `connection`，觸發程序或繫結會使用函式應用程式的預設儲存體帳戶。如果想要觸發程序或繫結使用不同的儲存體帳戶，請在指向要使用的儲存體帳戶的函式應用程式中建立應用程式設定，並將 `connection` 設為應用程式設定名稱。若要加入應用程式設定，請遵循下列步驟︰

1. 在 Azure 入口網站的 [函式應用程式] 刀鋒視窗中，按一下 [函式應用程式設定] > [移至 App Service 設定]。

2. 按一下 [設定] 刀鋒視窗中的 [應用程式設定]。

3. 向下捲動至 [應用程式設定] 區段，並將項目加入**金鑰** = {您選擇的一些唯一值} 和**值** = 儲存體帳戶的連接字串。

### <a id="storagequeuetrigger"></a>Azure 儲存體佇列觸發程序

function.json 檔案會提供要輪詢的佇列名稱和佇列訊息的變數名稱。例如：
...
function.json 檔案會提供輸出佇列的名稱和訊息內容的變數名稱。這個範例會使用佇列觸發程序及撰寫佇列訊息。
...
* `string` (函式結束時，如果參數值非 Null，就會建立佇列訊息)
* `byte[]` (像字串般運作) 
* `CloudQueueMessage` (像字串般運作) 
* JSON 物件 (當函式結束時，如果參數為 Null，就會使用 Null 物件建立訊息)
...
function.json 提供路徑，指定要監視的容器，及選擇性的 blob 名稱模式。此範例會觸發新增至範例工作項目容器的任何 blob。
...
> [AZURE.NOTE] 如果觸發程序正在監視的 blob 容器包含超過 10,000 個 blob，Functions 執行階段會掃描記錄檔以監看新增或變更的 blob。此程序的速度很慢；可能直到建立 Blob 之後數分鐘或更久，才會觸發函數。此外，[儲存體記錄檔建立在「竭盡所能」](https://msdn.microsoft.com/library/azure/hh343262.aspx)的基礎上；並不保證會擷取所有事件。在某些情況下可能會遺失記錄檔。如果您的應用程式無法接受大容器的 blob 觸發程序的速度和可靠性限制，建議的方法是當您建立 blob 時建立佇列訊息，並使用佇列觸發程序而非 blob 觸發程序來處理 blob。
...
這個 C# 程式碼範例會將每個新增的 blob 內容記錄到容器。
...
您可以指定包含副檔名之固定值的模式，以限制觸發函式的 blob 類型。如果您將 `path` 設為 samples/{name}.png，只有 samples 容器中的 .png blob 會觸發函式。
...
Azure Functions 可確保不會針對相同的全新或更新 blob 多次呼叫 blob 觸發程序函式。它的運作方式是藉由維護 *Blob 回條*來判斷指定的 Blob 版本是否已處理過。
...
function.json 提供 blob 名稱和內容的容器名稱和變數名稱。這個範例會使用佇列觸發程序來複製 blob：
...
`blob` 繫結可序列化或還原序列化下列類型︰

* `Stream`
* `TextReader`
* `TextWriter`
* `string` (適用於輸出 blob；當函式傳回時，如果字串參數非 Null，就只會建立 blob)
* JSON 物件 (適用於輸出 blob︰當函式結束時，如果參數值為 null，會建立 blob 作為 null 物件)
* `CloudBlobStream` (僅輸出)
* `ICloudBlob`
* `CloudBlockBlob` 
* `CloudPageBlob` 
...
儲存體資料表的 function.json 提供數個屬性：

* `name` - 用於資料表繫結的程式碼中使用的變數名稱。
* `tableName`
...
## Azure Mobile Apps 簡單資料表繫結

Azure App Service Mobile Apps 可讓您將資料表端點資料公開至行動用戶端。此相同的表格式資料可用於 Azure Functions 的輸入和輸出繫結中。當您有 Node.js 後端行動應用程式時，您可以使用簡單資料表，在 Azure 入口網站中使用表格式資料。簡單資料表支援動態結構描述，會自動加入資料表以符合正在插入的資料狀，簡化結構描述開發。動態結構描述預設會啟用，而且應該在生產行動應用程式中停用。如需 Mobile Apps 中簡單資料表的詳細資訊，請參閱[如何：在 Azure 入口網站中使用簡單資料表](../app-service-mobile/app-service-mobile-node-backend-how-to-use-server-sdk.md#in-portal-editing)。請注意，入口網站中的簡單資料表目前不受 .NET 後端行動應用程式支援。您仍然可以使用.NET 後端行動應用程式資料表端點函式繫結，但動態結構描述不是支援的 .NET 後端行動應用程式。

本節包含下列小節：

* [Azure Mobile Apps 簡單資料表 API 金鑰](#easytablesapikey)
* [Azure Mobile Apps 簡單資料表輸入繫結](#easytablesinput)
* [Azure Mobile Apps 簡單資料表輸出繫結](#easytablesoutput)

### <a id="easytablesapikey"></a>使用 API 金鑰來保護 Mobile Apps 簡單資料表端點的存取。

Azure Functions 目前無法存取 App Service 驗證所保護的端點。這表示在您的函式中與簡單資料表繫結一起使用的任何 Mobile Apps 端點都必須允許匿名存取，這是預設值。簡單資料表繫結可讓您指定 API 金鑰，也就是可用來防止您的函式以外的應用程式存取共用的密碼。Mobile Apps 並沒有內建支援的 API 金鑰驗證。不過，您可以遵循 [Azure App Service Mobile Apps 後端實作 API 金鑰](https://github.com/Azure/azure-mobile-apps-node/tree/master/samples/api-key)的範例，在 Node.js 後端行動應用程式中實作 API 金鑰。

>[AZURE.IMPORTANT] 此 API 金鑰不可與您的行動應用程式用戶端一起散佈，它應該只安全地散佈給服務端用戶端，如 Azure Functions。

### <a id="easytablesinput"></a>Azure Mobile Apps 簡單資料表輸入繫結

輸入繫結可從 Mobile Apps 資料表端點載入記錄，並將它直接傳遞至繫結。您可以根據叫用該函式的觸發程序來判斷記錄識別碼。在 C# 函式中，當函式成功結束時，會將記錄所做的任何變更自動傳回資料表。

function.json 檔案支援下列屬性，可與 Mobile Apps 簡單資料表輸入繫結搭配使用︰

- `name`︰函式程式碼中用於新記錄的變數名稱。
- `type` ︰飛過類型必須設為 easyTable。
...
#### C# 佇列觸發程序的 Azure Mobile Apps 簡單資料表程式碼範例

根據上述的 function.json 範例，輸入繫結會擷取記錄 (包含符合佇列訊息字串的識別碼)，並將它傳遞至 record 參數。找不到記錄時，參數為 null。當函式結束時，記錄隨後會以新的 Text 值更新。
...
#### Node.js 佇列觸發程序的 Azure Mobile Apps 簡單資料表程式碼範例

根據上述的 function.json 範例，輸入繫結會擷取記錄 (包含符合佇列訊息字串的識別碼)，並將它傳遞至 record 參數。在 Node.js 函式中，更新的記錄不會傳回至資料表。這個程式碼範例會將擷取的記錄寫入記錄檔。
...
### <a id="easytablesoutput"></a>Azure Mobile Apps 簡單資料表輸出繫結

您的函式可以使用簡單資料表輸出繫結，將記錄寫入 Mobile Apps 資料表端點。

function.json 檔案支援下列屬性，可與簡單資料表輸出繫結搭配使用︰

- `name`︰函式程式碼中用於新記錄的變數名稱。
- `type` ︰必須設為 easyTable 的繫結類型。
- `tableName`︰其中將建立新記錄的資料表。
- `apiKey`︰字串，為指定行動應用程式的選擇性 API 金鑰之應用程式設定。當您的行動應用程式使用 API 金鑰來限制用戶端存取時，這是必要的選項。
- `connection`︰字串，為指定行動應用程式 URI 的應用程式設定。
...
這個 C# 程式碼範例會將包含 Text 屬性的新記錄，插入上述繫結中指定的資料表。
...
這個 Node.js 程式碼範例會將包含 text 屬性的新記錄，插入上述繫結中指定的資料表。
