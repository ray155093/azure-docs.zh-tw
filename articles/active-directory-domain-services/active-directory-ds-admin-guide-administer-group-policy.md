...
3. 按一下 [Forest: contoso100.com] 和 [Domains] 節點加以展開，查看您的受管理網域的群組原則。 有兩個內建群組原則物件 (GPO)，各自用於受管理網域中的「AADDC 電腦」和「AADDC 使用者」容器。

    ![內建 GPO](./media/active-directory-domain-services-admin-guide/builtin-gpos.png)
4. 您可以自訂這些內建 GPO，來設定受管理網域上的群組原則。 以滑鼠右鍵按一下 GPO，然後按一下 [編輯...] 以自訂內建 GPO。 [群組原則設定編輯器] 工具可讓您自訂 GPO。

    ![編輯內建 GPO](./media/active-directory-domain-services-admin-guide/edit-builtin-gpo.png)
5. 現在您可以使用 [群組原則管理編輯器] 主控台來編輯內建的 GPO。 例如，下列螢幕擷取畫面示範如何自訂內建「AADDC 電腦」的 GPO。
