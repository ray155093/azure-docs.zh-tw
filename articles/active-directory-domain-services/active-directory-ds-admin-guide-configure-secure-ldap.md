...
1. **信任的簽發者** - 憑證必須由需要使用安全的 LDAP 連線到網域的電腦所信任的授權單位來發行。這可能是您組織的企業憑證授權單位或這些電腦所信任的公共憑證授權單位。

2. **存留期** - 憑證必須至少在接下來的 3 至 6 個月內保持有效。這可確保當憑證過期時，還能對受管理的網域進行安全的 LDAP 存取。

3. **主體名稱** - 在受管理的網域中，憑證的主體名稱必須是萬用字元。比方說，如果您的網域名稱為 'contoso100.com'，則憑證的主體名稱必須是 '*.contoso100.com'。DNS 名稱 (主體替代名稱) 也必須設定為此萬用字元名稱。

3. **金鑰使用方法** - 必須將憑證設定為下列用途 - 數位簽章和金鑰編密。

4. **憑證目的** - 憑證必須適用於進行 SSL 伺服器驗證。
...
在要求憑證時，請確保您有遵循[安全 LDAP 憑證的需求](./active-directory-ds-admin-guide-configure-secure-ldap.md/#requirements-for-the-secure-ldap-certificate)中所述的需求。
...
在您的 Windows 電腦上以**系統管理員**的身分開啟新的 PowerShell 視窗並輸入下列命令，以建立新的自我簽署憑證。
...
2. 在 [使用者帳戶控制] 提示中按一下 [是]，以系統管理員身分啟動 MMC (Microsoft Management Console)。
...
8. 在 MMC 視窗中，按一下以展開 [主控台根目錄]。您應該會看到已載入 [憑證] 嵌入式管理單元。按一下 [憑證 (本機電腦)] 以便展開。按一下以依序展開 [個人] 節點和 [憑證] 節點。
...
13. 在 [匯出檔案格式] 頁面上，選取 [個人資訊交換 - PKCS #12 (.PFX)] 做為匯出憑證的檔案格式。
...
14. 在 [安全性] 頁面上選取 [密碼] 選項，然後輸入密碼來保護 .PFX 檔案。請記住此密碼，因為下一個工作會用到它。按 [下一步] 繼續進行。
	...
	> [AZURE.NOTE] 記下此密碼。在[工作 3 - 為受管理的網域啟用安全 LDAP](./active-directory-ds-admin-guide-configure-secure-ldap.md/#task-3---enable-secure-ldap-for-the-managed-domain)中為此受管理網域啟用安全 LDAP 時將需要此密碼。

15. 在 [要匯出的檔案] 頁面上，指定檔案名稱以及您想要將憑證匯出到的位置。
...
5. 向下捲動到標題為 [網域服務] 的區段。您應該會看到標題為 [安全 LDAP (LDAPS)] 的選項，如以下螢幕擷取畫面所示。
...
7. 按一下 [含有憑證的 PFX 檔案] 下面的資料夾圖示，指定要用於對受管理網域進行安全 LDAP 存取之憑證所在的 PFX 檔案。此外，也請輸入將憑證匯出到 PFX 檔案時所指定的密碼。完成時，請按一下底部的完成按鈕。

    ![指定安全 LDAP 的 PFX 檔案和密碼](./media/active-directory-domain-services-admin-guide/secure-ldap-specify-pfx.png)

8. [設定] 索引標籤的 [網域服務] 區段應該會變成灰色，而且會有幾分鐘的時間處在 [擱置中...] 狀態。在此期間，LDAPS 憑證會進行驗證以確認是否正確，並為受管理的網域設定安全 LDAP。
...
9. 在為受管理的網域成功啟用安全 LDAP 後，[擱置中...] 訊息應該會消失。您應該會看到已顯示憑證的指紋。
...
3. [設定] 索引標籤的 [網域服務] 區段應該會變成灰色，而且會有幾分鐘的時間處在 [擱置中...] 狀態。在此期間，將會啟用透過安全 LDAP 對受管理網域的網際網路存取。
...
4. 在成功啟用透過網際網路對受管理網域的安全 LDAP 存取後，[擱置中...] 訊息應該會消失。您應該會在 [LDAPS 存取的外部 IP 位址] 欄位中看到可用來透過 LDAPS 存取您的目錄的外部 IP 位址。
