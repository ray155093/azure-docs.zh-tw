<properties
	pageTitle="Azure Active Directory Domain Services：疑難排解指南 | Microsoft Azure"
...
> [AZURE.NOTE] 建議使用 UPN 格式來登入 Azure AD 網域服務受管理網域。
...
- **過長的 UPN 前置詞：**確保 Azure AD 租用戶中受影響使用者帳戶的 UPN 前置詞 (UPN 的第一個部分) 長度少於 20 個字元。例如，'joereallylongnameuser@contoso.com' UPN 的前置詞 ('joereallylongnameuser') 超過 20 個字元，因而無法在受管理的網域中無法使用此帳戶。

- **重複的 UPN 前置詞：**確保 Azure AD 租用戶中沒有其他使用者帳戶與受影響使用者帳戶具有相同的 UPN 前置詞 (UPN 的第一個部分)。比方說，如果您有兩個使用者帳戶 'joeuser@finance.contoso.com' 和 'joeuser@engineering.contoso.com'，這兩個使用者在登入受管理網域時都會遇到問題。其中一個使用者帳戶是外部帳戶 (例如 'joeuser@live.com') 時也有可能發生此問題。我們正在努力修正這個問題。
