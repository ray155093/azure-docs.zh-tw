...
Data Factory 是雲端架構資料整合服務，用來**自動移動和轉換資料**。就像製造廠運轉設備來取得原物料並將之轉換成成品一樣，Data Factory 會協調現有服務的流程來收集原始資料，並將之轉換成隨時可用的資訊。
 
Data Factory 服務可讓您建立資料驅動的工作流程，在內部部署和雲端資料存放區之間移動資料，以及使用例如 Azure HDInsight 和 Azure Data Lake Analytics 的計算服務來處理/轉換資料。建立管線以執行您需要的動作之後，您可以排程定期執行 (每小時、每天、每週等...)。
...
請參閱 [Azure 訂用帳戶和服務限制、配額及條件約束](../azure-subscription-service-limits.md#data-factory-limits)中的 **Azure Data Factory 限制**一節。
...
如果您有另一個 Data Factory，其管線會接受 dataset4 (由 data factory 1 中的 pipeline 2 產生)，您就需要把 dataset4 標示為外部資料集，因為該資料集是由不同的 Data Factory (DataFactory1，而非 DataFactory2) 產生。
