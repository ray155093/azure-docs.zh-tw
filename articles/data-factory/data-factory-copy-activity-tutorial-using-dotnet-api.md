<properties 
	pageTitle="教學課程：使用 .NET API 建立具有複製活動的管線 | Microsoft Azure" 
...
3. 執行下列命令以選取您要使用的訂用帳戶。以您的 Azure 訂用帳戶名稱取代 **&lt;NameOfAzureSubscription**&gt;。
	...
	> [AZURE.IMPORTANT] 請記下此命令輸出中的 **SubscriptionId** 和 **TenantId**。
...
5. 建立 Azure Active Directory 應用程式。
...
8. 將下列會建立 **Azure 儲存體連結服務**的程式碼加入 **Main** 方法中。

	> [AZURE.IMPORTANT] 以 Azure 儲存體帳戶的名稱和金鑰取代 **storageaccountname** 和 **accountkey**。
...
9. 將下列會建立 **Azure SQL 連結服務**的程式碼加入 **Main** 方法中。
 
	> [AZURE.IMPORTANT] 以您的 Azure SQL 伺服器名稱、資料庫名稱、使用者和密碼取代 **servername**、**databasename**、**username** 和 **password**。
...
10. 將下列會建立**輸入和輸出資料集**的程式碼加入 **Main** 方法中。
...
13. 將 **Main** 方法所使用的下列 Helper 方法加入 **Program** 類別中。
