...
| 預設容器 |隨選叢集用於儲存及處理資料的 Azure 儲存體連結服務。 |是 |
...
| 屬性 | 說明 | 必要 |
|:--- |:--- |:--- |
| headNodeSize |指定前端節點的大小。 預設值為：Standard_D3。 如需詳細資料，請參閱以下 **指定節點大小** 一節。 |否 |
...
| 預設容器 |此 HDInsight 叢集所使用之 Blob 儲存體的連結服務名稱。 |是 |
