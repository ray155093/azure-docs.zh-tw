...
### <a name="create-a-linked-service-for-the-azure-storage-account"></a>建立 Azure 儲存體帳戶的連結服務
...
3. 現在，按一下 [資料集]  磚。
4. 在 [資料集] 刀鋒視窗中，按一下 [InputDataset]。 此資料集是 **ADFTutorialPipeline** 的輸入資料集。
...
13. (選擇性) 在 **ADFTutorialDataFactory** 的起始刀鋒視窗上按一下 [管線]，再按一下 [管線] 刀鋒視窗中的 **ADFTutorialPipeline**，然後深入檢視輸入資料表 (**已取用**) 或輸出資料表 (**已產生**)。
