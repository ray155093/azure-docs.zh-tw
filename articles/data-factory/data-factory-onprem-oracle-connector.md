...
藉由在資料管理閘道電腦上安裝 Oracle 資料提供者 .NET 12.1，您可以使用此 Oracle 連接器，將資料往返複製到下列版本的 Oracle 執行個體。 如需如何設定的詳細資料，請參閱[安裝](#installation)小節。
...
您可能需要如下列範例所示變更查詢 (使用 to_date 函式)：
...
2. 如果您即使在安裝提供者之後還是會收到錯誤訊息，請執行下列步驟：
   ...
   2. 搜尋 **Oracle Data Provider for .NET**，而您應該能夠找到類似下列範例所示的項目：“der **system.data** -> **DbProviderFactories**:
...
4. 執行 `gacutil /i [provider path]`，將 “<ODP.NET 安裝路徑>\11.2.0\client_1\odp.net\bin\4\Oracle.DataAccess.dll” 安裝到全域組件快取 (GAC)。
