<properties
	pageTitle="建置您的第一個 Data Factory (PowerShell) |Microsoft Azure"
...
1. 在 C:\\ADFGetStarted 資料夾中，使用以下內容建立名為 StorageLinkedService.json 的 JSON 檔案。如果不存在，請建立 ADFGetStarted 資料夾。
	...
	使用您的 Azure 儲存體帳戶名稱取代帳戶名稱，並使用 Azure 儲存體帳戶的存取金鑰取代帳戶金鑰。若要了解如何取得儲存體存取金鑰，請參閱[檢視、複製和重新產生儲存體存取金鑰](https://azure.microsoft.com/documentation/articles/storage-create-storage-account/#view-copy-and-regenerate-storage-access-keys)。
...
3. 執行 **Get-AzureRmDataFactoryRun**，以取得特定配量的活動執行詳細資料。
	...
	您可以繼續執行此 Cmdlet 直到您看到配量處於**就緒**狀態或**失敗**狀態。當配量處於就緒狀態時，檢查您 Blob 儲存體中 **adfgetstarted** 容器內 **partitioneddata** 資料夾的輸出資料。建立隨選 HDInsight 叢集通常需要一些時間。
