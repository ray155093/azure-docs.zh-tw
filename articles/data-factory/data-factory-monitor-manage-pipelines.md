...
Data Factory 服務提供一個可靠且完整的儲存、處理和資料移動服務檢視。 此服務可提供您監視儀表板說明供您執行下列工作︰ 
...
4. 您應該會看到 Data Factory 的首頁 ([Data Factory] 刀鋒視窗)。
...
### <a name="view-state-of-each-activity-inside-a-pipeline"></a>管線中每個活動的檢視狀態
...
<tr>
<td>Skipped</td><td>None</td><td>未處理配量。</td>
</tr>
<tr>
...
Data Factory 內的資料集狀態轉換流程：等候中 -> 進行中/進行中 (驗證中) -> 就緒/失敗
...
#### <a name="use-azure-portal-to-debug-an-error"></a>使用 Azure 入口網站偵錯錯誤：
...
#### <a name="use-the-powershell-to-debug-an-error"></a>使用 PowerShell 偵錯錯誤
...
#### <a name="specifying-an-alert-definition"></a>指定警示定義：
...
#### <a name="deploying-the-alert"></a>部署警示
...
#### <a name="troubleshooting-user-events"></a>使用者事件疑難排解
...
#### <a name="enabling-metrics"></a>啟用度量：
...
### <a name="setting-up-alert-on-metrics"></a>設定度量警示：
按一下 [Data Factory 度量] 刀鋒視窗︰ 
...
![Data Factory 度量刀鋒視窗 - 新增警示](./media/data-factory-monitor-manage-pipelines/add-an-alert-rule.png)

成功新增警示規則後，刀鋒視窗會隨即關閉，而您會在 [度量] 頁面上看到新的警示。 

![Data Factory 度量刀鋒視窗 - 新增警示](./media/data-factory-monitor-manage-pipelines/failed-alert-in-metric-blade.png)

您也應該會在 [警示] 圖格上看到警示數目。 按一下 [警示] 圖格。

![Data Factory 度量刀鋒視窗 - 新增規則](./media/data-factory-monitor-manage-pipelines/alert-rules-tile-rules.png)

在 [警示] 刀鋒視窗中，您會看到任何現有的警示。 若要新增警示，請按一下工具列上的 [新增警示]。
...
### <a name="deploying-alerts-using-powershell"></a>使用 PowerShell 部署警示
...
## <a name="move-data-factory-to-a-different-resource-group-or-subscription"></a>將 Data Factory 移至另一個資源群組或訂用帳戶
