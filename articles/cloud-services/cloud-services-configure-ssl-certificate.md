...
> [AZURE.NOTE] 此工作的程序適用於 Azure 雲端服務，若為應用程式服務，請參閱[這篇](../app-service-web/web-sites-configure-ssl-certificate.md)文章。
...
2.  在網頁瀏覽器中，將連結修改為使用 **https** 而非 **http**，然後造訪網頁。
    ...
    >[AZURE.NOTE] 如果使用自我簽署憑證，則當您在瀏覽器中瀏覽至與該自我簽署憑證相關聯的 HTTPS 端點時，您會看見憑證錯誤。 使用信任的憑證授權單位所簽署的憑證，則不會有此問題；因此，您可以忽略該錯誤。 (另一個選項為新增自我簽署憑證至使用者的受信任憑證授權單位憑證存放區。)
