...
>[AZURE.NOTE]請注意，您完全無法控制這個程序，而且它只有在整個資料中心服務中斷時才會發生。因此，您也必須依賴其他的應用程式特定備份策略，以達到最高層級的可用性。如需詳細資訊，請參閱關於[災害復原的資料策略](https://aka.ms/drtechguide#DSDR)的章節。如果您希望能夠影響您自己的容錯移轉，就可能想要考慮使用[讀取存取異地備援儲存體 (RA-GRS)](../storage/storage-redundancy.md#read-access-geo-redundant-storage)，在其他區域中建立資料的唯讀複本。
...
若要開發雲端平台功能的詳細技術知識，請參閱 [Azure 商務持續性技術指引](https://aka.ms/bctechguide)。
