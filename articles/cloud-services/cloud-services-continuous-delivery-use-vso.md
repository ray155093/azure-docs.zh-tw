---
title: 在 Azure 中使用 Visual Studio Team Services 來連續傳遞 | Microsoft Docs
...
14. 在 [Azure 傳統入口網站](http://go.microsoft.com/fwlink/?LinkID=213885)中，選取預備環境之後，您可以在 [部署] 索引標籤上檢視相關聯的部署。
     ...
     ![][30]
15. 瀏覽至網站的 URL。若是 Web 應用程式，請按一下命令列上的 [瀏覽] 按鈕。若是雲端服務，請在 [**儀表板**] 頁面的 [**快速概覽**] 區段中選擇 URL，以顯示雲端服務的預備環境。依預設，來自雲端服務連續整合的部署會發行至預備環境。您可以將 [替代雲端服務環境] 屬性設為 [生產] 來變更此設定。此擷取畫面顯示網站 URL 在雲端服務儀表板頁面上的位置。
