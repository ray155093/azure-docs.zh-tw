---
title: "SQL Azure 搭配 Azure RemoteApp | Microsoft Docs"
...
首先 [將 SQL Server Database 移轉到 Azure SQL Database](../sql-database/sql-database-cloud-migrate.md)。 
...
4. 執行 Sysprep 並關閉 VM 擷取 VM 作為映像以便用於 Azure。 **注意：**您需要確定應用程式能夠透過 sysprep 程序來保留資料庫連線資訊。 如果應用程式無法保留資料庫連線資訊，您可以詢問應用程式的廠商，查看如何指定連接字串。
