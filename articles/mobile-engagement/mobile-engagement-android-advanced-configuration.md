<properties
	pageTitle="使用 Android Mobile Engagement SDK 的進階組態"
	description="使用 Azure Mobile Engagement SDK 的 Android 進階組態選項"
...
# 使用 Android Mobile Engagement SDK 的進階組態

> [AZURE.SELECTOR]
- [Android](mobile-engagement-android-logging.md)

此程序描述如何設定 Engagement Android 應用程式的各種進階組態選項。
...
## 權限需求
有許多選項需要特定權限，全部列在這裡方便參考，也內嵌在特定功能中。將這些權限加入至專案的 AndroidManifest.xml 中，而且要緊接在 `<application>` 標記之前或之後。

權限程式碼必須看起來如下所示，請根據下表填入適當的權限。
...
| ---------- | --------- |
| 網際網路 | 基本報告 |
| ACCESS\_NETWORK\_STATE" | 基本報告 |
| WRITE\_EXTERNAL\_STORAGE | 基本報告 |
| RECEIVE\_BOOT\_COMPLETED | 基本報告 |
| VIBRATE | 基本報告 |
| DOWNLOAD\_WITHOUT\_NOTIFICATION | 重點通知 |
| WAKE\_LOCK | 使用 wifi 或螢幕關閉時收集統計資料 |
| RECEIVE\_BOOT\_COMPLETED | 啟用背景報告 |
| ACCESS\_COARSE\_LOCATION | 即時位置報告 |
| ACCESS\_FINE\_LOCATION | GPS 式報告 |
...
根據預設，Engagement 會即時報告記錄檔。如果應用程式報告記錄檔的頻率很高，建議您緩衝記錄檔，以固定時段一次報告全部的記錄檔 (此稱為「高載模式」)。若要這樣做，請在 `<application>` 和 `</application>` 標籤之間加入此程式碼：
