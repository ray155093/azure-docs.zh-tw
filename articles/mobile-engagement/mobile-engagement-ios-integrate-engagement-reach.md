...
> [AZURE.NOTE] 每次使用者按一下通知以取得 "my\_category" 類別的宣告時，會將您已經註冊的檢視控制器 (在此情況下為 `initWithAnnouncement:`) 透過呼叫 `MyCustomAnnouncementViewController` 方法來初始化，而且會將檢視新增到目前的應用程式視窗。
