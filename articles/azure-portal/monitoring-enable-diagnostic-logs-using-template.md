<properties
	pageTitle="使用 Resource Manager 範本自動啟用診斷設定 | Microsoft Azure"
...
# 使用 Resource Manager 範本在建立資源時自動啟用診斷設定
在本文中，我們示範如何在建立資源時使用 [Azure Resource Manager 範本](../resource-group-authoring-templates.md)設定診斷設定。這可讓您在建立資源時，自動開始將您的診斷記錄檔和度量串流至事件中樞，或將它們封存在儲存體帳戶中。
...
1. 將參數加入儲存體帳戶名稱和服務匯流排規則識別碼的參數 blob (可啟用儲存體帳戶中診斷記錄檔的封存，和/或串流記錄檔至事件中樞)。
...
> [AZURE.WARNING] 這最後一個步驟要做得正確可能會有點棘手。[請參閱這篇文章](../virtual-machines/virtual-machines-windows-extensions-diagnostics-template.md#diagnostics-configuration-variables)的範例，將診斷組態結構描述分割成正確逸出和格式正確的變數。
