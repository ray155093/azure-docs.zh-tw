---
title: "事件中樞傳訊例外狀況 | Microsoft Docs"
...
本文列出一些 Azure 服務匯流排傳訊 API 產生的例外狀況。 這個 API 集包含事件中樞。 此參考可能有所變更，請不定期查看更新。
...
| [MessagingCommunicationException](https://msdn.microsoft.com/library/azure/microsoft.servicebus.messaging.messagingcommunicationexception.aspx) |用戶端無法建立事件中樞連線。 |確定提供的主機名稱正確，且主機可以連線。 |如果有間歇性的連線問題，重試也許有幫助。 |
...
| [QuotaExceededException](https://msdn.microsoft.com/library/azure/microsoft.servicebus.messaging.quotaexceededexception.aspx) |傳訊實體已達到允許的大小上限。 如果在個別取用者群組層級開啟的接收者數目已經達到上限 (5)，便可能發生此情況。 |從實體或其子佇列接收訊息，在實體中建立空間。 |如果在此同時已移除訊息，重試可能會有幫助。 |
|  | | | |
