<properties
    pageTitle="使用範本部署配置有 IPv6 的網際網路面向負載平衡器解決方案 | Microsoft Azure"
...
# 使用範本部署配置有 IPv6 的網際網路面向負載平衡器解決方案
...
這份文件參考 [Azure 快速入門範本](https://azure.microsoft.com/documentation/templates/)資源庫中發佈的範本。您可以從資源庫下載範本，或在 Azure 中直接從資源庫啟動部署。本文假設您已下載範本至本機電腦。
...
5. 按一下 [編輯範本]。 刪除現有的內容，複製/貼上範本檔案的整個內容 (包括開始和結束的 {})，然後按一下 [儲存]。
    ...
    > [AZURE.NOTE] 如果您使用 Microsoft Internet Explorer，當您貼上內容時，會看到對話方塊，詢問是否要允許存取 Windows 剪貼簿。按一下 [允許存取]。
...
8. 在 [資源群組] 刀鋒視窗上，按一下您在步驟 6 中指定的資源群組名稱。您會看到所有已部署資源的清單。如果一切順利，[上次部署] 下應該會顯示 [成功]。 如果沒有，請確定您使用的帳戶具有建立所需資源的權限。
    ...
    > [AZURE.NOTE] 如果在完成步驟 6 之後立即瀏覽資源群組，[上次部署] 會在資源部署期間顯示部署的狀態。
