...
10. **服務的流量恢復** - 流量管理員傳送 GET 並在 10 秒鐘內收到 200 OK。然後，它開始在 DNS 伺服器要求更新時送出雲端服務的 DNS 名稱。當傳回其他端點的快取 DNS 回應過期時，以及當其他端點的現有連線終止時，流量將會再次回到端點。

>[AZURE.NOTE] 由於流量管理員是在 DNS 層級上運作，所以無法影響任何端點的現有連線。在容錯回復期間，流量管理員可能會將新的連線導向至主要端點，而次要端點會繼續透過現有的連線接收流量，直到這些工作階段終止為止。如果需要快速容錯回復，應用程式應該限制次要端點上的工作階段持續時間。
