...
# <a name="add-disable-enable-or-delete-endpoints"></a>加入、停用、啟用或刪除端點
...
> [!NOTE]
> 您無法使用 Azure 傳統入口網站將外部位置或流量管理員設定檔加入為端點。 您必須使用 REST API [建立定義](http://go.microsoft.com/fwlink/p/?LinkId=400772)或 Windows PowerShell [Add-AzureTrafficManagerEndpoint](http://go.microsoft.com/fwlink/p/?LinkId=400774)。
...
> [!NOTE]
> 如果您打算使用 [ *容錯移轉* ] 流量路由方法，在新增或移除端點之後，請務必調整組態頁面上的容錯移轉優先權清單，以反映您要用於組態的容錯移轉順序。 如需詳細資訊，請參閱 [設定容錯移轉流量路由](traffic-manager-configure-failover-routing-method.md)。
...
> [!NOTE]
> 您無法使用 Azure 傳統入口網站刪除做為端點的外部位置或流量管理員設定檔。 您必須使用 Windows PowerShell。 如需詳細資訊，請參閱 [Remove-AzureTrafficManagerEndpoint](https://msdn.microsoft.com/library/dn690251.aspx)。
