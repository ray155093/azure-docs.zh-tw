...
| [Tempdb][] | Tempdb 的大小上限 | 每 DW100 各 399 GB。因此，DWU1000 Tempdb 大小為 3.99TB |
...
| 類別 | 說明 | 最大值 |
| :---------------- | :------------------------------------------- | :----------------- |
| 資料庫 | 大小上限 | 在磁碟上壓縮後 240 TB<br/><br/>此空間與 tempdb 或記錄檔空間無關，因此此空間為供永久資料表專用。叢集資料行存放區壓縮估計為 5 倍，這表示當所有資料表都是叢集資料行存放區 (預設的資料表類型) 時，未壓縮的資料庫大小可能成長到大約 1 PB。|
| 資料表 | 大小上限 | 磁碟上壓縮後 60 TB |
| 資料表 | 每個資料庫的資料表 | 20 億 |
| 資料表 | 每個資料表的資料行 | 1024 個資料行 |
| 資料表 | 每個資料行的位元組 | 8000 個位元組 |
| 資料表 | 每個資料列的位元組，已定義的大小 | 8060 個位元組<br/><br/>每個資料列的位元組數目計算方式和已啟用頁面壓縮的 SQL Server 所使用的方式相同。就像 SQL Server，SQL 資料倉儲支援資料列溢位儲存，讓可變長度的資料行能發送至超出資料列。發送超出資料列的可變長度資料行在主資料列只有儲存 24 位元的根。如需詳細資訊，請參閱《SQL Server 線上叢書》中的[超過 8 KB 的資料列溢位資料][]主題。<br/><br/>如需 SQL 資料倉儲資料類型大小的清單，請參閱 [CREATE TABLE (Azure SQL 資料倉儲)][]。 |
...
| 索引 | 叢集資料行存放區索引的並行組建。 | 32<br/><br/>適用於叢集資料行存放區索引全部都是在不同資料表上建置時。每個資料表只允許有 1 個叢集資料行存放區索引建置。其他要求會在佇列中等候。|
