...
SQL 資料倉儲最多允許 1,024 個並行存取連線。這 1,024 個連線全都可以並行提交查詢。不過，為了達到最佳輸送量，SQL 資料倉儲可能會將某些查詢排入佇列，以確保每個查詢能夠獲得最少的記憶體授與。到達並行存取限制時，就會在執行查詢時發生佇列作業。藉由在到達並行存取限制時將查詢排入佇列，SQL 資料倉儲就能夠確保作用中查詢能夠取得急需的記憶體資源存取權，以增加總輸送量。

並行存取限制受到兩個概念所控制，分別是**並行查詢**和**並行存取插槽**。若要讓查詢執行，它必須在並行存取限制和並行存取插槽配置下執行。

- **並行查詢**就是在同一時間執行的查詢數目。SQL 資料倉儲在較大的 DW 大小 (DW1000 以上) 支援高達 32 個**並行查詢**。不過，由於並行查詢數目會因 DWU 數量而異，我們提供以下資料表來顯示依 DWU 的限制。
- **並行存取插槽**是更動態的概念。每個同時執行的查詢都會取用一或多個並行存取插槽。查詢取用的確切插槽數目取決於 SQL 資料倉儲的大小和查詢的[資源類別](#resource-classes)。
...
當符合上述其中一個臨界值時，新的查詢就會排入佇列。當查詢完成時，已排入佇列的查詢會以先進先出的基本原則執行，然後查詢與插槽數目就會落到限制之下。

> [AZURE.NOTE]  只針對動態管理檢視 (DMV) 和目錄檢視執行的 SELECT 查詢**不是**由資源類別所控管。這可讓使用者即使在所有並行存取插槽都處於使用中時，仍可監視系統。

## 資源類別

資源類別是 SQL 資料倉儲工作負載管理中不可或缺的一部分，因為它們允許將更多記憶體或 CPU 週期配置給由指定使用者執行的查詢。共有四種資源類別，每種都是**資料庫角色**形式。這四種資源類別分別是 **smallrc、mediumrc、largerc 和 xlargerc**。smallrc 中的使用者會獲得較小量的記憶體，因此有較高的並行存取能力。相反地，指派給 xlargerc 的使用者會獲得大量記憶體，因此可以並行執行的查詢較少。

有幾種查詢不會受益於較大的記憶體配置，因此請忽略其資源類別配置，並改為在小型資源類別中執行。強制這些查詢一律在小型資源類別中執行，可讓這些查詢在並行存取插槽承受壓力時執行，防止這些查詢耗用超過所需數量的插槽。這些[資源類別例外狀況](#resource-class-exceptions)會在本文稍後說明。
...
提升使用者的資源類別有利有弊。雖然提升使用者的資源類別可能表示其查詢可以取得更多記憶體，因而可以更快速地執行，但也會減少可以執行的並行查詢數目。這是配置大量記憶體給單一查詢，和允許其他也需要配置記憶體的並行查詢執行兩者間的取捨。如果給予一位使用者更多記憶體來用於查詢，其他使用者就沒有記憶體可供他們執行查詢。

下表依 DWU 和資源類別來對應所配置的記憶體與每個散發套件。在 SQL 資料倉儲中，每個資料庫有 60 個散發套件，因此，在 DW2000 上以 xlarge 資源類別執行的查詢可以在 60 個資料庫內各取得 6,400 MB。
...
使用上述相同範例，在全系統以 xlarge 資源類別執行於 DW2000 上的查詢，會配置 375 GB 的總記憶體 (6,400 MB * 60 個散發套件 / 1,024 以轉換為 GB)。
...
如上所述，資源類別越高，授與的記憶體越多。因為記憶體是固定的資源，每個查詢所配置的記憶體越多，可支援的並行存取就越少。下表重述依 DWU 的可用並行存取插槽數目，以及每個資源類別所使用的插槽。
...
| **配置** | | | | | | | | | | | | |
| 並行查詢上限 | 32 | 32 | 32 | 32 | 32 | 32 | 32 | 32 | 32 | 32 | 32 | 32 |
| 並行存取插槽上限 | 4 | 8 | 12 | 16 | 20 | 24 | 40 | 48 | 60 | 80 | 80 | 80 |
| **插槽耗用量** | | | | | | | | | | | | |
...
您可以從這個資料表中看到，以 DW100 形式執行的 SQL 資料倉儲允許 4 個並行存取 smallrc 查詢或 2 個並行存取 largerc 查詢。

## 查詢的重要性

在幕後，共有八個工作負載群組會控制資源類別的行為。不過，八個工作負載群組中只有四個可在任何給定的 DWU 中使用。這很合理，因為每個工作負載群組都會指派給 smallrc、mediumrc、largerc 或 xlargerc。了解這些幕後工作負載群組的重要性在於其中的某些工作負載群組會設定為較高的**重要性**。重要性可用於 CPU 排程。以高重要性執行的查詢會取得比中度重要性的查詢高 3 倍的 CPU 週期。因此，並行存取插槽對應也會判斷 CPU 重要性。當查詢取用 16 個以上的插槽時，就會以高重要性執行。
