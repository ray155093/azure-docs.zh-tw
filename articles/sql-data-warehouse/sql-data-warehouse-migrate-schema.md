...
在移轉您的資料表時，您會想要熟悉 SQL 資料倉儲資料表的資料表功能。[資料表概觀][]是很好的起點。本文將介紹建立資料表時最重要的考量事項，例如資料表統計資料、散發、分割和編製索引。它也包含一些 [不支援的資料表功能][]和因應措施。

SQL 資料倉儲支援常見的商務資料類型。請參閱[][]一文，了解支援和[不支援的資料類型][]清單。[資料類型][]一文也包含識別[不支援的資料類型][]的查詢。轉換資料類型時，請務必查看[資料類型的最佳作法][]。
...
若要深入了解 SQL 資料倉儲最佳做法，請參閱[最佳做法][]一文。
