...
目前，我們的 MPP 架構將您的資料分散於 60 個不共用的儲存和處理單元。資料會儲存在進階本地備援儲存體中，並連結至計算節點以便執行查詢。使用這個架構，SQL 資料倉儲可以採用分治法來執行複雜的 T-SQL 查詢。在處理時，控制節點會最佳化每個查詢，然後將工作傳遞至計算節點，其中的每個節點會平行「佔領」其資料部分。
...
- 當您不需要執行查詢時，或許是晚上或週末，請停止查詢，然後暫停計算資源，以免在不需要這些資源時還付費使用 DWU。

- 當系統要求不高時，請考慮將 DWU 減為小型大小，以便讓使用者可以存取資料，但又能節省大量成本。
...
- SQL 資料倉儲仍處於開發階段，較不常用於資料倉儲工作負載或是比 SQL Server 還新的特定 TSQL 語言元素目前可能無法使用。請參閱我們的[移轉文件][]以取得與此問題相關的詳細資訊。
...
**以雲端為基礎的工具**：SQL 資料倉儲可以與 Azure 中的數個新工具一起使用，並且已與 Azure 的 Data Factory、串流分析、機器學習服務和 Power BI 緊密整合。請參閱[整合式工具概觀][]，以取得我們所整合的 Azure 工具清單。

**協力廠商工具**：許多協力廠商工具提供者與 SQL 資料倉儲整合的工具都已經過認證。請參閱 [SQL 資料倉儲解決方案合作夥伴][]，以取得完整的合作夥伴清單。
