...
3. 在 [SQL 資料倉儲]  刀鋒視窗中，填入所需的資訊，然後按 [建立] 來建立。
   ...
   * **效能**：我們建議從 400 [DWU][DWU] 開始。 您可以將滑桿向左邊或向右移動，以調整資料倉儲的效能，在建立之後相應增加或相應減少。  若要深入了解 DWU，請參閱[調整](sql-data-warehouse-manage-compute-overview.md)相關文件或我們的[價格頁面][SQL 資料倉儲價格]。 
   * **訂用帳戶**：選取此 SQL 資料倉儲將會計費的 [訂用帳戶] 。
   * **資源群組**： [資源群組s][資源群組] 是為了協助您管理 Azure 資源集合而設計的容器。 深入了解[資源群組](../azure-resource-manager/resource-group-overview.md)。
