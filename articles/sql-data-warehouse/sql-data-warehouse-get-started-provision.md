...
* **Azure SQL Server**︰如需詳細資訊，請參閱[使用 Azure 入口網站建立 Azure SQL Database 邏輯伺服器][使用 Azure 入口網站建立 Azure SQL Database 邏輯伺服器]。

> [!NOTE]
> 建立 SQL 資料倉儲可能會導致新的可計費服務。如需詳細資訊，請參閱 [SQL 資料倉儲價格][SQL 資料倉儲價格]。
> 
> 
...
> [!NOTE]
> SQL 資料倉儲的預設定序為 SQL\_Latin1\_General\_CP1\_CI\_AS。如果需要不同的定序，可以使用 [T-SQL][T-SQL] 來建立具有不同定序的資料庫。
> 
> 
...
[!INCLUDE [SQL Database 建立伺服器](../../includes/sql-database-create-new-server-firewall-portal.md)]
...
也可以使用 Transact-SQL 來設定防火牆規則。如需詳細資訊，請參閱 [sp\_set\_firewall\_rule][sp\_set\_firewall\_rule] 和 [sp\_set\_database\_firewall\_rule][sp\_set\_database\_firewall\_rule]。

查看我們的[最佳作法][最佳作法]也是不錯的辦法。
