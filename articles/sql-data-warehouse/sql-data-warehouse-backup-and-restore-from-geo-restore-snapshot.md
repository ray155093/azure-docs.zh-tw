...
異地還原可從異地備援備份還原資料庫。您可以在任何 Azure 區域中的任何伺服器上建立還原的資料庫。異地還原使用異地備援備份作為其來源，因此即使因為中斷而無法存取資料庫，也能用來復原資料庫。除了從中斷復原之外，異地還原還可以用於其他案例，例如，將資料庫移轉或複製到不同的伺服器或區域。
...
## 使用異地還原進行復原
復原資料庫可從最新的異地備援備份建立新的資料庫。請務必確定您要還原到的伺服器有足夠的 DTU 容量供新的資料庫使用。請參閱此部落格文章以深入了解[如何檢視和增加 DTU 配額][]。
...
[列出可復原的資料庫]: https://msdn.microsoft.com/library/azure/dn800984.aspx
[取得可復原的資料庫]: https://msdn.microsoft.com/library/azure/dn800985.aspx
[建立資料庫復原要求]: https://msdn.microsoft.com/library/azure/dn800986.aspx
