...
若要提升計算能力，您可以使用 Azure 傳統入口網站中的縮放滑桿，將更多 DWU 加入服務。您也可以透過 T-SQL、REST API 或 Azure Powershell Cmdlet 加入 DWU。雖然相應增加和減少計算能力會取消所有執行中或已進入佇列的活動，但此動作可在幾秒內完成，讓您以較多或較少的計算能力繼續進行。
...
您也可以匯入 AzureRM.Sql 模組並使用下列程式碼，透過 Azure Powershell 達成相同結果：
...
如要暫停和繼續執行計算能力，可經由 [Azure 傳統入口網站][]、透過 REST API 或 Azure PowerShell 來完成。暫停會取消所有執行中或已進入佇列的活動；當您恢復使用時，可在幾秒內繼續執行計算資源。

若要使用 Azure PowerShell 來暫停和繼續服務，您需要先匯入 AzureRM.Sql 模組，如下所示︰

```Powershell
Import-Module AzureRM.Sql
```

下列程式碼示範如何使用 Azure PowerShell 執行暫停：

```Powershell
Suspend-AzureRmSqlDatabase –ResourceGroupName "ResourceGroup11" –ServerName "Server01" –DatabaseName "Database02"
```

Azure PowerShell 也可輕易地繼續執行服務：

```Powershell
Resume-AzureRmSqlDatabase –ResourceGroupName "ResourceGroup11" –ServerName "Server01" –DatabaseName "Database02"
```

如需如何使用 Azure PowerShell 的詳細資訊，請參閱[透過 SQL 資料倉儲使用 PowerShell Cmdlet 和 REST API][]。
