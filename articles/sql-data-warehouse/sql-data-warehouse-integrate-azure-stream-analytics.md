...
Azure 串流分析是完全受管理的服務，可用來對雲端中的串流資料進行低延遲、高可用性、可延展的複雜事件處理。如需基本概念，請參閱 [Azure 串流分析簡介][Azure 串流分析簡介]。您可以接著依照[開始使用 Azure 資料流分析][開始使用 Azure 資料流分析]教學課程，了解如何使用資料流分析建立端對端解決方案。
