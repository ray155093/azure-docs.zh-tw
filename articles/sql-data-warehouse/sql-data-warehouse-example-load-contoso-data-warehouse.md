<properties
   pageTitle="將 Contoso 零售資料倉儲載入 SQL 資料倉儲中 | Microsoft Azure"
   description="了解如何使用 PolyBase 將 Contoso 零售資料倉儲載入 SQL 資料倉儲中以獲得最大效能"
...
在本文中，您將學習如何快速地將資料載入 Contoso 零售資料倉儲結構描述。

此資料集提供可用於了解 Azure SQL 資料倉儲的替代資料來源。

Contoso 資料集的大小大約是 GB。未經處理的資料會保留在 Azure Blob 儲存體中，我們將直接存取該儲存體以將資料載入 SQL 資料倉儲。

在本教學課程中，您將：

1. 設定 PolyBase 以從公用 Blob 儲存體容器載入
2. 使用 [CTAS][] 匯入資料
3. [REBUILD][] 資料表，確保最佳化資料行存放區索引
4. 建立索引鍵資料行的統計資料

## 設定 PolyBase

載入資料時的第一個步驟是設定 PolyBase 先決條件指令碼︰

### 建立外部資料來源

將資料保留在公用容器時，第一個工作是建立外部資料來源︰
...
在此情況下，不需要建立主要金鑰或資料庫範圍認證，因為 Blob 儲存體容器設定為「公用」，這表示任何人都可以讀取並列舉此容器。

> [AZURE.IMPORTANT] 如果您選擇將 Azure Blob 儲存體容器設為公用，請記住，資料離開您將負責的資料中心時，會發生資料流出費用。

### 建立外部檔案格式物件

檔案格式提供剖析 Blob 儲存體中所保留文字檔所需的資訊。
...
在我們的案例中，資料會進行壓縮且以管線分隔。

## 建立外部資料表

現在，已設定與 Azure Blob 儲存體的連線，我們可以建立外部資料表。
...
現在，我們可以開始建立外部資料表。請記住，在這裡只需要提供資料類型的對應；將此定義繫結至中繼資料即可找到檔案。
...
若要載入資料，最好利用我們剛剛建立的強類型外部資料表。因此，簡單 [CTAS][] 陳述式就已足夠。
...
為了載入資料，我們使用 [CTAS][] 陳述式。請注意，一個資料表有一個 [CTAS][]。
...
您可以使用 `[sys].[dm_pdw_exec_requests]` 動態管理檢視 (DMV) 來追蹤載入進度。上述 CTAS 陳述式有 `label` 時，您也可以使用 `label` 來追蹤進度。如需詳細資訊，請參閱[標籤][]一文。

## 重建索引

最後，為了確保資料表是壓縮資料行存放區格式，我們可以對資料表執行重建作業。這會將任何已開啟或關閉資料列群組轉換成壓縮資料行存放區格式。
...
若要找出如何檢查叢集資料行存放區狀態的詳細資訊，請參閱 [管理資料行存放區索引][] 文章。

## 建立一些統計資料物件

若要建立所有資料行的統計資料，您可以利用[統計資料][]一文中所述的預存程序程式碼 `prc_sqldw_create_stats`。

以下是完整涵蓋維度以及 Contoso 零售資料倉儲中所有聯結資料行的輸出子集。這是不錯的起點。

您稍後可以自行將更多統計資料加入其他事實資料行中。
...
您現在已將所有資料載入 Azure SQL 資料倉儲中。太棒了！
...
## 後續步驟
如需載入的概觀，請參閱[將資料載入 SQL 資料倉儲][]。若要深入了解尋找資料行存放區索引，請參閱 [管理資料行存放區索引][]。
...
<!--MSDN references-->
[sys.dm_pdw_exec_requests]: https://msdn.microsoft.com/zh-TW/library/mt203887.aspx
[REBUILD]: https://msdn.microsoft.com/zh-TW/library/ms188388.aspx
