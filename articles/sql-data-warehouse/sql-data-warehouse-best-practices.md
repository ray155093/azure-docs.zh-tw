...
另請參閱[資料載入][資料載入]、[使用 PolyBase 指南][使用 PolyBase 指南]、[Azure SQL 資料倉儲載入模式和策略][Azure SQL 資料倉儲載入模式和策略]、[使用 Azure Data Factory 載入資料][使用 Azure Data Factory 載入資料]、[使用 Azure Data Factory 移動資料][使用 Azure Data Factory 移動資料]、[CREATE EXTERNAL FILE FORMAT][CREATE EXTERNAL FILE FORMAT]、[如選取建立資料表 (CTAS)][如選取建立資料表 (CTAS)]
...
另請參閱[使用 PolyBase 指南][使用 PolyBase 指南]
...
另請參閱[資料表概觀][資料表概觀]、[資料表發佈][資料表發佈]、[選取資料表發佈][選取資料表發佈]、[CREATE TABLE][CREATE TABLE]、[CREATE TABLE AS SELECT][CREATE TABLE AS SELECT]

## <a name="do-not-overpartition"></a>不要過度執行資料分割
...
另請參閱[了解交易][了解交易]、[最佳化交易][最佳化交易]、[資料表分割][資料表分割]、[TRUNCATE TABLE][TRUNCATE TABLE]、[ALTER TABLE][ALTER TABLE]、[如選取建立資料表 (CTAS)][如選取建立資料表 (CTAS)]
...
另請參閱[資料表概觀][資料表概觀]、[資料表的資料型別][資料表的資料型別]、[CREATE TABLE][CREATE TABLE]
...
另請參閱[並行存取和工作負載管理][並行存取和工作負載管理][sys.dm_pdw_waits][sys.dm_pdw_waits]
...
另請參閱[使用 DMV 監視您的工作負載][使用 DMV 監視您的工作負載]、[LABEL][LABEL]、[OPTION][OPTION]、[sys.dm_exec_sessions][sys.dm_exec_sessions]、[sys.dm_pdw_exec_requests][sys.dm_pdw_exec_requests]、[sys.dm_pdw_request_steps][sys.dm_pdw_request_steps]、[sys.dm_pdw_sql_requests][sys.dm_pdw_sql_requests]、[sys.dm_pdw_dms_workers]、[DBCC PDW_SHOWEXECUTIONPLAN][DBCC PDW_SHOWEXECUTIONPLAN]、[sys.dm_pdw_waits][sys.dm_pdw_waits]
...
[如選取建立資料表 (CTAS)]: ./sql-data-warehouse-develop-ctas.md
[資料表概觀]: ./sql-data-warehouse-tables-overview.md
[資料表的資料型別]: ./sql-data-warehouse-tables-data-types.md
[資料表發佈]: ./sql-data-warehouse-tables-distribute.md
...
[使用 PolyBase 指南]: ./sql-data-warehouse-load-polybase-guide.md
[資料載入]: ./sql-data-warehouse-overview-load.md
...
[使用 DMV 監視您的工作負載]: ./sql-data-warehouse-manage-monitor.md
...
[選取資料表發佈]: https://blogs.msdn.microsoft.com/sqlcat/2015/08/11/choosing-hash-distributed-table-vs-round-robin-distributed-table-in-azure-sql-dw-service/
