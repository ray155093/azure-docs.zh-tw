...
5. 按 CTRL+F5 執行應用程式，再按一下 [**CM Demo**] 連結；或瀏覽 https://localhost:(port#)/Cm。
...
10. 使用 *user1@contoso.com* 的身分和密碼 "P\_assw0rd1" ("word" 中的 "0" 是數字零) 登入。系統隨即將您重新導向到先前選取的編輯頁面。
...
1. 在 Visual Studio 的 [方案總管] 中以滑鼠右鍵按一下專案，再選取內容功能表中的 [發行]。
    ...
    此時會開啟 [發行 Web] 精靈。
2. 按一下 [發行 Web] 對話方塊左側的 [設定] 索引標籤。
3. 在 [ApplicationDbContext]下，選取您建立專案時所建立的資料庫。
4. 在 [ContactManagerContext] 下選取 [Execute Code First Migrations]。
   
    ![settings](./media/web-sites-dotnet-deploy-aspnet-mvc-app-membership-oauth-sql-database/rrc2.png)
5. 按一下 [發佈]。
6. 以 *user1@contoso.com* (使用密碼 "P\_assw0rd1") 身分登入，並確認您可以編輯資料。
7. 登出。
...
9. 使用 Google 或 Facebook 登入。如此將使 Google 或 Facebook 帳戶新增至 **canEdit** 角色。如果您收到 HTTP 400 錯誤訊息： *要求中的重新導向 URI：https://contactmanager{my https://contactmanager{my version}.azurewebsites.net/signin-google 與已註冊的重新導向 URI 不符。* ，您必須等到您所做的變更都已傳播為止。如果您是在超過數分鐘之後收到此錯誤，請確認 URI 是正確的。
...
12. 按一下 [編輯] 連結。
     ...
     系統會將您重新導向至 [登入] 頁面。
13. 在 [Use another service to log in] 下按一下 [Google] 或 [Facebook]，並以您先前註冊的帳戶登入。(如果您的動作夠快，同時工作階段 Cookie 未逾時，將使用您先前所使用的 Google 或 Facebook 帳戶為您自動登入。)
14. 確認您在登入該帳戶後可編輯資料。
