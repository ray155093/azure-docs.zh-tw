---
title: 在 Eclipse 中建立 Azure Hello World Web 應用程式
...
2. 基於本教學課程的目的，將專案命名為 **MyHelloWorld** 您的畫面將出現，如下所示：
   
    ![][02]
3. 按一下 [完成]。
4. 在 Eclipse 的 [專案總管] 檢視中，展開 [MyHelloWorld]。在 **WebContent** 上按一下滑鼠右鍵、按一下 [新增]，然後按一下 [JSP File]。
5. 在 [新增 JSP 檔案] 對話方塊中，將檔案命名為 **index.jsp**。將父資料夾保留為 **MyHelloWorld/WebContent**。
6. 在 [選取 JSP 範本] 對話方塊中，基於本教學課程的目的，選取 [新增 JSP 檔案 (html)]，然後按一下 [完成]。
7. 當 index.jsp 檔案在 Eclipse 中開啟時，於現有的 `<body>` 元素中加入文字以動態顯示 **Hello World!**。您已更新的 `<body>` 內容看起來應該與下列範例類似：
...
6. 如果您之前尚未建立 Azure Web 應用程式容器，或您想要將應用程式發佈到新的容器中，請使用下列步驟。否則，請選取現有的 Web 應用程式容器，並跳至以下的步驟 7。
   ...
   6. 在 [資源群組] 下拉式功能表中，選取您要與 Web 應用程式相關聯的資源群組。
      ...
      * 按一下 [確定]。
   7. [App Service 方案] 下拉式功能表會列出與您選取之資源群組相關聯的應用程式服務方案。
