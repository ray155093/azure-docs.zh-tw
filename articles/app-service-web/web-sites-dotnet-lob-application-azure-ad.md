...
8. 按一下 [設定] 以設定此應用程式。
9. 向下捲動至 [金鑰] 區段，並選取持續時間來新增金鑰。然後按一下 [委派的權限] 並選取 [讀取目錄資料]。按一下 [儲存]。
...
13. 尋找應用程式的驗證設定，其位置在訂用帳戶 > &lt;subscriptionname> > [resourceGroups] > &lt;resourcegroupname> > [提供者] > [Microsoft.Web] > [網站] > &lt;appname> > [設定] > [authsettings]。
14. 按一下 [**編輯**]。
...
13. 開啟 ~\\App\_Start\\BundleConfig.cs，然後進行下列醒目提示的變更︰
...
14. 最後，在 ~\\Global.asax 中，於 `Application_Start()` 方法內新增下面這一行程式碼。對每個命名解析錯誤按一下 `Ctrl`+`.` 來加以修正。
