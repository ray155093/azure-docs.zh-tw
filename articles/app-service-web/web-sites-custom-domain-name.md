...
> [!IMPORTANT]
> 請勿建立根網域的 CNAME 記錄 (也就是「根記錄」)。 如需詳細資訊，請參閱 [為什麼無法在根網域使用 CNAME 記錄](http://serverfault.com/questions/613829/why-cant-a-cname-record-be-used-at-the-apex-aka-root-of-a-domain)。
> 若要將根網域對應至 Azure 應用程式，請改用 A 記錄。
> 
> 
...
## <a name="step-3-enable-the-custom-domain-name-for-your-app"></a>步驟 3. 為您的應用程式啟用自訂網域名稱
...
3. 按一下您的應用程式，然後按一下 [自訂網域]  > 中將自訂網域名稱對應至您的 Web 應用程式、行動裝置應用程式後端或 API 應用程式。
...
## <a name="verify-dns-propagation"></a>確認 DNS 傳播
