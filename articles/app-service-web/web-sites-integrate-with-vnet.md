...
這篇文章並未說明在 V2 VNET 中放置 ASE。尚未支援，因此這篇文章未提及。這篇文章是關於啟用您的應用程式以在 V1 或 V2 VNET 中使用資源。
...
- 需要標準或進階價格方案 
- 將使用 V1(Classic) 或 V2(Resource Manager) VNET 
- 支援 TCP 和 UDP
- 使用 Web、行動及 API 應用程式
...
- VNET 整合僅會使用標準或進階價格方案中的應用程式。如果您啟用此功能，然後將您的 App Service 方案調整為不支援的價格方案，則您的應用程式將會失去與其正在使用之 VNET 的連接。  
...
VNET 整合 UI 可讓您從 VNET 的清單中選取。V1 VNET 會在 VNET 名稱旁邊的括號中以「傳統」文字指出它們屬於這一類。清單會以先列出 V2 VNET 的方式儲存。在下面顯示的影像中，您可以看到只有一個 VNET 可選取。有多個原因，導致 VNET 呈現灰色，包括：
...
##### 在 V1 VNET 中啟用點對站 #####
如果您的 VNET 沒有閘道器，也沒有點對站，則您首先必須設定它。若要針對 V1 VNET 執行此動作，請移至 [Azure 入口網站][AzurePortal]，並引出虛擬網路 (傳統) 的清單。從這裡按一下您想要整合的網路，並按一下稱為 VPN 連線之基本功能下的大型方塊。從這裡您可以建立點對站 VPN，甚至讓它建立閘道器。在您使用閘道器建立體驗完成點對站之後，大約 30 分鐘即可使用它。
...
若要透過 VNET 整合 UI 建立 V2 VNET，只需選取 [建立新的虛擬網路]，然後提供：
...
>[AZURE.NOTE] 使用閘道器建立 V2 VNET 大約需要 30 分鐘，目前無法整合 VNET 與您的應用程式。使用閘道器建立 VNET 之後，您必須回到您的應用程式 VNET 整合 UI，並選取新的 VNET。
...
>[AZURE.NOTE] 儘管 VNET 整合功能將使用站對站 VPN 存取內部部署資源，但是它目前不會使用 ExpressRoute VPN 執行相同的動作。與 V1 或 V2 VNET 整合時，也是如此。如果您需要透過 ExpressRoute VPN 存取資源，則您可以使用可在 VNET 中執行的 ASE。
