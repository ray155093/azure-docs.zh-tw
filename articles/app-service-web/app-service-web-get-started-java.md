...
## <a name="deploy-a-java-app-to-your-web-app"></a>將 Java 應用程式部署到您的 Web 應用程式
...
8. 在 FTP/FTPS 用戶端中，瀏覽到遠端站台中的 **/site/wwwroot/webapps** ，並將下載到您本機電腦的 WAR 檔案拖放到該遠端目錄。
   ...
   > [!NOTE]
   > 根據 Tomcat 的預設行為，/site/wwwroot/webapps 中的檔案名稱 **ROOT.war** 會提供根 Web 應用程式 (http://*&lt;appname>*.azurewebsites.net)，而檔案名稱 ***&lt;anyname>*.war** 會提供已命名的 Web 應用程式 (http://*&lt;appname>*.azurewebsites.net/*&lt;anyname>*)。
   > 
   > 

就這麼簡單！ 您的 Java 應用程式現在正在 Azure 中即時執行。 在瀏覽器中，瀏覽至 http://&lt;>.azurewebsites.net 來查看其實際運作。 
