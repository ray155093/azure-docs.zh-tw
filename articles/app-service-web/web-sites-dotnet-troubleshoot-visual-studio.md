...
4. 在 `ViewBag.Message` 字行中[設定中斷點](http://www.visualstudio.com/get-started/debug-your-app-vs.aspx)。
...
7. 在 [伺服器總管] 中，依序展開 [Azure] > [App Service] > 您的 Web 應用程式 > [WebJob] > [連續]，然後使用滑鼠右鍵按一下 [ContosoAdsWebJob]。
8. 按一下 [連結偵錯工具]。
...
4. 按 CTRL+F5 執行應用程式。
5. 在瀏覽器視窗的網址列中，將 *trace.axd* 新增至 URL，然後按 Enter (此 URL 會類似於 http://localhost:53370/trace.axd)。
