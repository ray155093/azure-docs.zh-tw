...
6. 若要從憑證存放區匯出 SSL 憑證，請按 `Win`+`R` 並執行 **certmgr.msc** 以啟動憑證管理員。選取 [個人] > [憑證]。在 [簽發給] 資料行中，您應該會看到一個含有您自訂網域名稱的項目，[簽發者] 資料行中則是您用來產生憑證的 CA。
...
3. 若要從憑證存放區匯出憑證，請按 `Win`+`R` 並執行 **certmgr.msc** 以啟動憑證管理員。選取 [個人] > [憑證]。在 [簽發給] 資料行中，您應該會看到一個含有您自訂網域名稱的項目，[簽發者] 資料行中則是您用來產生憑證的 CA。
