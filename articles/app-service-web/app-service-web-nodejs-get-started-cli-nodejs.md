...
- [Azure CLI 2.0](app-service-web-nodejs-get-started.md) - 適用於資源管理部署模型的新一代 CLI
...
4. 變更為 ASM 模式並登入 Azure (您需要 [Azure CLI](#prereq))：
...
5. 確定您仍在應用程式的根目錄中，然後使用下一個命令，以唯一的應用程式名稱在 Azure 中建立 App Service 應用程式資源。 例如︰http://{應用程式名稱}.azurewebsites.net
   
        azure site create --git {appname}
   
    依照提示來選取要部署的目標 Azure 區域。 
...
8. 儲存變更，然後使用 git 將您的應用程式部署至 Azure。 出現提示時，請使用您稍早建立的使用者認證。
