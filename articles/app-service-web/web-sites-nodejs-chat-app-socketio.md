...
> [AZURE.NOTE] 此工作中的程序適用於 [App Service Web Apps]。對於雲端服務，請參閱[在 Azure 雲端服務上使用 Socket.IO 建立 Node.js 聊天應用程式]。
...
1.  將此封存解壓縮並將 **examples\\chat** 目錄複製到新的位置。例如，**\\node\\chat**。
...
1. 開啟 **package.json** 檔案，並在 `dependencies` 下方加入對 socket.io 的參考，如下所示：
...
1. 安裝 Azure 命令列介面 (Azure CLI)，並連線到您的 Azure 訂用帳戶。請參閱[安裝和設定 Azure CLI](../xplat-cli-install.md)。

1. 如果這是您第一次在 Azure 中設定儲存機制，就需要建立登入認證。從 Azure CLI 中，輸入下列命令：
...
1. 使用下列命令，將現有的檔案認可到本機儲存機制：

		git add .
		git commit -m "Initial commit"

1. 使用下列命令，將檔案推送至 Azure Web Apps 儲存機制：
...
1. 若要在 Azure 上檢視 Web 應用程式，請使用下列命令來啟動網頁瀏覽器，並瀏覽至裝載的 Web 應用程式：
...
Socket.IO 應用程式可以使用__配接器__來橫向擴充，以在多個應用程式執行個體之間散佈訊息和事件。由於市面上已有數種配接器，因此可以輕鬆使用 [socket.io-redis] 配接器來搭配 Azure Redis 快取功能。

> [AZURE.NOTE] 橫向擴充 Socket.IO 解決方案的額外需求是支援黏性工作階段。Azure Web Apps 的黏性工作階段預設是透過 Azure 要求路由來啟用。如需詳細資訊，請參閱 [Azure 網站的執行個體同質性]。
...
1. 修改 __app.js__ 檔案以在 `var io = require('socket.io')(server);` 之後緊接下列行
...
>[AZURE.NOTE] 如果您想在註冊 Azure 帳戶前開始使用 Azure App Service，請移至[試用 App Service]，即可在 App Service 中立即建立短期入門 Web 應用程式。不需要信用卡；沒有承諾。
