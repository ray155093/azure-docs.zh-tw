---
title: "為 Web 應用程式有效地使用 DevOps 環境"
...
本文將說明如何針對應用程式的多個版本，例如開發、預備、品管和生產來設定和管理 Web 應用程式部署。 您的應用程式的每個版本可以視為您的部署程序內的特定需求的開發環境。 例如品管環境可以由您的開發人員小組用來測試應用程式的品質，之後才將變更推入至生產環境。
設定多個開發環境可能是具挑戰性的工作，因為您需要追蹤、管理資源 (計算、Web 應用程式、資料庫、快取等) 和跨環境部署程式碼。

## <a name="setting-up-a-non-production-environment-stagedevqa"></a>設定非生產環境 (預備、開發、品管)
...
若要設定預備部署位置，請參閱[針對 Azure App Service 中的 Web 應用程式設定預備環境](web-sites-staged-publishing.md)。 每個環境應該包含其自己的一組資源，例如，如果您的 Web 應用程式使用資料庫，則生產和預備 Web 應用程式應該使用不同的資料庫。 加入預備開發環境資源，例如資料庫、儲存體或快取，用於設定您的預備開發環境。
...
在 Web 應用程式根目錄下建立名為 `config` 的資料夾，並加入二個檔案：`wp-config.azure.php` 和 `wp-config.local.php`，分別代表 Azure 和本機環境。
...
設定上述的安全性金鑰可以協助防止您的 Web 應用程式受到駭客攻擊，因此請使用唯一值。 如果您需要為先前所述的安全性金鑰產生字串，您可透過此 [連結](https://api.wordpress.org/secret-key/1.1/salt)
...
最後一件事是將 WordPress 應用程式設定為使用相對路徑。 WordPress 會在資料庫中儲存 URL 資訊。 這會使得在不同環境間移動內容變得困難，因為每次從本機移至預備環境或從預備環境移至生產環境時，都需要更新資料庫。 若要減少每次在不同環境間部署資料庫時會造成問題的風險，請使用[相對根連結外掛程式](https://wordpress.org/plugins/root-relative-urls/)，它可以使用 WordPress 管理員儀表板來安裝或從手動從[這裡](https://downloads.wordpress.org/plugin/root-relative-urls.zip)下載。
...
任何 WordPress 核心更新並不會影響您的 `wp-config.php`、`wp-config.azure.php` 和 `wp-config.local.php` 檔案。 在最後，`wp-config.php` 檔案看起來會像這樣
...
假設您已經在 Azure 網站上執行您的 WordPress Web 應用程式，請登入 [Azure 管理預覽入口網站](http://portal.azure.com)並移至 WordPress Web 應用程式。 如果沒有，您可以從 Marketplace 建立一個。 若要深入了解，請[按一下這裡](web-sites-php-web-site-gallery.md)。
按一下 [設定] -> [部署位置] -> [新增] 以建立具有 stage 名稱的部署位置。部署位置是與以上建立的主要的 Web 應用程式共用相同的資源的另一個 Web 應用程式。
...
下面定義的這個程序在執行時很有用，因為它同時包含 WordPress 應用程式的檔案變更和資料庫變更：
...
取消核取 WP_ENV 以外的所有設定參數的 [位置設定] 核取方塊。 這將會交換 Web 應用程式的組態，以及檔案內容和資料庫。 如果 [位置設定] **已勾選**，執行交換操作時，Web 應用程式的應用程式設定、連接字串組態將「不會」跨環境移動，因此如果有任何資料庫變更，這將不會中斷您的生產 Web 應用程式。
...
> [!NOTE]
> 如果您有只需要推送檔案的案例 (沒有資料庫更新)，那麼請在執行交換之前，於 Azure Preview 入口網站的 Web 應用程式設定刀鋒視窗中**勾選**所有資料庫相關的 [應用程式設定] 和 [連接字串設定] 的 [位置設定]。 在此情況下，DB_NAME、DB_HOST、DB_PASSWORD、DB_USER 預設連接字串設定在執行**交換**的時候應該不會顯示在預覽變更中。 在此時，當您完成**交換**作業，WordPress Web 應用程式將**只**會有更新檔案。
> 
> 
...
1. 使用 Visual Studio 建立 Umbraco Web 應用程式，請[按一下這裡](https://our.umbraco.org/documentation/Installation/install-umbraco-with-nuget)。
2. 若要使用 WebMatrix 建立 Umbraco Web 應用程式，請[按一下這裡](http://umbraco.com/help-and-support/video-tutorials/getting-started/working-with-webmatrix)。
...
利用 [Courier2](http://umbraco.com/products/more-add-ons/courier-2) 模組，利用按一下滑鼠右鍵即可以將內容、樣式表、開發模組等從預備 Web 應用程式推送至生產 Web 應用程式，以獲得更不麻煩的部署，並降低在部署更新時中斷您的生產 Web 應用程式的風險。
為網域 `*.azurewebsites.net` 和您的自訂網域 (假設是 ) 購買 Courier2 授權。購買授權之後，將下載的授權 (.LIC 檔) 放在 `bin` 資料夾中。
...
在 `<repositories>`底下，輸入生產網站 URL 和使用者資訊。 如果您使用預設 Umbraco 成員資格提供者，則請在 <user> 區段中新增系統管理使用者的識別碼。 如果您使用自訂 Umbraco 成員資格提供者，請使用 Courier2 模組 `<login>`、`<password>` 的知識來連接到生產網站。 如需詳細資訊，請檢閱 Courier 模組的 [文件](http://umbraco.com/help-and-support/customer-area/courier-2-support-and-download/developer-documentation) 。
...
如需有關如何在本機升級 Web 應用程式的詳細資訊，請遵循[這裡](https://our.umbraco.org/documentation/getting-started/set up/upgrading/general)提及的指導方針。
...
3. 可讓您使用 **在生產環境中測試** 功能執行 [A/B 測試](https://azure.microsoft.com/documentation/videos/introduction-to-azure-websites-testing-in-production-with-galin-iliev/)
