---
title: "針對 Azure App Service 中的 Web 應用程式設定預備環境"
...
---
# <a name="set-up-staging-environments-for-web-apps-in-azure-app-service"></a>針對 Azure App Service 中的 Web 應用程式設定預備環境
<a name="Overview"></a>
...
## <a name="add-a-deployment-slot-to-a-web-app"></a>將部署位置新增至 Web 應用程式
...
2. 按一下 [設定]，然後按一下 [部署位置]。 然後，在 [部署位置] 刀鋒視窗中，按一下 [加入位置]。
   ...
   > 
   > 
3. 在 [新增位置]  刀鋒視窗中，指定位置名稱，然後選取是否要複製其他現有部署位置的 Web 應用程式設定。 按一下打勾記號繼續。
...
4. 在 [部署位置] 刀鋒視窗中，按一下部署位置以開啟該位置的刀鋒視窗，其中含有一組度量和設定，就像任何其他 Web 應用程式一樣。 **your-web-app-name-deployment-slot-name** 將出現在刀鋒視窗頂端，提醒您正在檢視該部署位置。
...
## <a name="to-swap-deployment-slots"></a>交換部署位置
> [!IMPORTANT]
> 在您將 Web 應用程式從部署位置交換到生產位置之前，請確定所有非位置特定的設定已完全依照您想要在交換目標中擁有它的方式明確地加以設定。
> 
> 
...
2. 按一下 [確定]  來完成操作。 當操作完成時，部署位置就已交換完畢。

## <a name="configure-auto-swap-for-your-web-app"></a>設定 Web 應用程式的自動交換
...
2. 按一下 [ **應用程式設定**]。 針對 [自動交換] 選取 [開啟]、在 [自動交換位置] 中選取所需的目標位置，然後按一下命令列中的 [儲存]。 確定此位置的組態設定完全適用於目標位置的組態設定。
   ...
   > [!NOTE]
   > 若要針對您的 Web 應用程式測試自動交換，可在 [自動交換位置] 中選取非生產的目標位置，以便先熟悉這個功能。  
   > 
   > 
...
## <a name="use-multi-phase-swap-for-your-web-app"></a>針對 Web 應用程式使用多階段交換
...
- - -
### <a name="initiate-multi-phase-swap-and-apply-target-slot-configuration-to-source-slot"></a>起始多階段交換和將目標位置組態套用至來源位置
```
...
- - -
### <a name="revert-the-first-phase-of-multi-phase-swap-and-restore-source-slot-configuration"></a>還原多階段交換的第一個階段和還原來源位置組態
```
