...
使用 [New-AzureRmKeyVault](https://msdn.microsoft.com/library/azure/mt603736.aspx) Cmdlet 來建立金鑰保存庫。這個 Cmdlet 包含三個必要參數：**資源群組名稱**、**金鑰保存庫名稱**和**地理位置**。
...
>[AZURE.NOTE]  當您嘗試建立新的金鑰保存庫時，如果看到「訂用帳戶未登錄要使用命名空間 'Microsoft.KeyVault'」錯誤，請執行 `Register-AzureRmResourceProvider -ProviderNamespace "Microsoft.KeyVault"`，然後重新執行 New-AzureRmKeyVault 命令。如需詳細資訊，請參閱 [Register-AzureRmProvider](https://msdn.microsoft.com/library/mt679020.aspx)。
>
...
若要授權應用程式存取保存庫中的金鑰或密碼，請使用 [Set-AzureRmKeyVaultAccessPolicy](https://msdn.microsoft.com/library/azure/mt603625.aspx) Cmdlet。
...
如果您不再需要金鑰保存庫及其所包含的金鑰或密碼，您可以使用 [Remove-AzureRmKeyVault](https://msdn.microsoft.com/library/azure/mt619485.aspx) Cmdlet 來刪除金鑰保存庫：
...
如需最新 Azure 金鑰保存庫的 Azure PowerShell Cmdlet 清單，請參閱 [Azure 金鑰保存庫 Cmdlet](https://msdn.microsoft.com/library/azure/dn868052.aspx)。
