...
1. 登入您的帳戶。
        ...
        azure login
2. 建立應用程式的服務主體。提供顯示名稱、可描述應用程式之頁面的 URI、可識別應用程式的 URI，以及應用程式身分識別的密碼。此命令會同時建立 AD 應用程式和服務主體。
...
3. 授與服務主體對您訂用帳戶的權限。在此範例中，您會將服務主體新增至 [讀取者] 角色，以授與讀取訂用帳戶中所有資源的權限。若為其他角色，請參閱 [RBAC︰內建角色](active-directory/role-based-access-built-in-roles.md)。針對 **ServicePrincipalName** 參數，提供您在建立應用程式時所使用的 **ObjectId**。
...
4. 登入您的帳戶。
        ...
        azure config mode arm
        azure login
5. 建立應用程式的服務主體。提供顯示名稱、可描述應用程式之頁面的 URI、可識別應用程式的 URI，以及您複製的憑證資料。此命令會同時建立 AD 應用程式和服務主體。
     ...
     傳回新的服務主體。授與權限時，需要物件識別碼。
        ...
        data:                      https://www.contoso.org/example
        info:    ad sp create command OK
6. 授與服務主體對您訂用帳戶的權限。在此範例中，您會將服務主體新增至 [讀取者] 角色，以授與讀取訂用帳戶中所有資源的權限。若為其他角色，請參閱 [RBAC︰內建角色](active-directory/role-based-access-built-in-roles.md)。針對 **ServicePrincipalName** 參數，提供您在建立應用程式時所使用的 **ObjectId**。
