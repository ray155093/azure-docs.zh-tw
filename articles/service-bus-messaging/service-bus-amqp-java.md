...
Java 訊息服務 (JMS) 是在 Java 平台上搭配訊息導向中介軟體一起使用的標準 API。 我們已根據 Apache Qpid 專案開發的 JMS 用戶端程式庫，使用 AMQP 1.0 測試過 Microsoft Azure 服務匯流排。 此程式庫支援完整的 JMS 1.1 API，可以搭配 AMQP 1.0 相容的任何訊息服務一起使用。 在 [Windows Server 服務匯流排](https://msdn.microsoft.com/library/dn282144.aspx) (內部部署服務匯流排) 中也支援這種情況。 如需詳細資訊，請參閱 [Windows Server 服務匯流排中的 AMQP][]。
...
| `[username]`  | 從 [Azure 入口網站][]取得的服務匯流排 SAS 金鑰名稱。                    |   |   |   |   |
| `[password]`  | 從 [Azure 入口網站][]取得的服務匯流排 SAS 金鑰，為 URL 編碼格式。 |   |   |   |   |
...
在服務匯流排 .NET API 中，訊息應用程式屬性包含在 [BrokeredMessage][] 的 **Properties** 集合中。 下列程式碼示範如何讀取從 JMS 用戶端接收之訊息的應用程式屬性。
