...
本指南說明如何使用服務匯流排主題和訂閱。 相關範例是以 Java 撰寫並使用 [Azure SDK for Java][]。 涵蓋的案例包括**建立主題和訂用帳戶**、**建立訂用帳戶篩選器**、**傳送訊息至主題**、**接收訂用帳戶的訊息**，以及**刪除主題和訂用帳戶**。
...
以下範例將建立名為 `HighMessages` 的訂用帳戶，而且所含的 [SqlFilter][] 物件只會選取自訂 **MessageNumber** 屬性大於 3 的訊息：
...
同樣地，下列範例將建立名為 `LowMessages` 的訂用帳戶，而且所含的 [SqlFilter][] 物件只會選取 **MessageNumber** 屬性小於或等於 3 的訊息：
...
傳送至服務匯流排主題的訊息是 [BrokeredMessage][] 類別的執行個體。 [BrokeredMessage][]* 物件具有一組標準方法 (例如 **setLabel** 和 **TimeToLive**)、一個用來保存自訂應用程式特定屬性的字典，以及任意應用程式資料的主體。應用程式可設定訊息主體，方法是將任何可序列化物件傳遞到 [BrokeredMessage][] 的建構函式，接著系統便會使用適當的 **DataContractSerializer** 來序列化物件。或者，也可以提供 **java.io.InputStream**。
