---
title: 使用適用於 Visual Studio 的資料湖工具開發 U-SQL 指令碼 | Microsoft Docs
...
6. 在 [方案總管] 中，用滑鼠右鍵按一下 **Script.usql**，然後按一下 [建置指令碼]。確認 [輸出] 窗格中的結果。
7. 在 [方案總管] 中，用滑鼠右鍵按一下 **Script.usql**，然後按一下 [提交指令碼]。(選擇性) 您也可以從 Script.usql 窗格按一下 [提交]。請參閱上一個螢幕擷取畫面。按一下 [提交] 按鈕旁的向下箭號，使用進階選項提交：
8. 指定 [工作名稱]、驗證 [分析帳戶]，然後按一下 [提交]。提交作業完成時，[適用於 Visual Studio 的資料湖工具結果] 視窗中便會出現提交結果和工作連結。
...
9. 您必須按一下 [重新整理] 按鈕，才能查看最新的工作狀態並重新整理畫面。當工作成功時，它將會顯示**工作圖形**、**中繼資料作業**、**狀態記錄**、**診斷**：
