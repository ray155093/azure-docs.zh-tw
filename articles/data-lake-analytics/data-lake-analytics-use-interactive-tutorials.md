---
title: 使用 Azure 入口網站互動式教學課程了解資料湖分析和 U-SQL | Microsoft Docs
...
> [!NOTE]
> 如果您想要使用 Visual Studio 完成相同的教學課程，請參閱[使用資料湖分析來分析網站記錄](data-lake-analytics-analyze-weblogs.md)。更加互動式的教學課程會新增至入口網站。
> 
> 
...
3. 按一下 **Marketplace** 磚。
4. 在 [所有項目] 刀鋒視窗的搜尋方塊中輸入 [Azure 資料湖分析]，然後按 **ENTER**。您應該會在清單中看到 [Azure 資料湖分析]。
...
9. 按一下 [建立]。它會帶領您前往入口網站「開始面板」。新的磚會新增至首頁，具有顯示「部署 Azure 資料湖分析」的標籤。建立資料湖分析帳戶需要一些時間。當建立帳戶之後，入口網站就會在新的刀鋒視窗上開啟帳戶。
