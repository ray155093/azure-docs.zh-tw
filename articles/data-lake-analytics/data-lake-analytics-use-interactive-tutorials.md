...
7. 輸入或選取下列項目：
   ...
   * **資源群組**。 選取現有的 Azure 資源群組，或建立一個新的群組。 應用程式通常由許多元件組成，例如，Web 應用程式、資料庫、資料庫伺服器、儲存體和協力廠商服務。 Azure 資源管理員 (ARM) 可讓您將應用程式中的資源做為群組使用，稱為 Azure 資源群組。 您可以透過單一、協調的作業來部署、更新、監視或刪除應用程式的所有資源。 您會使用部署的範本，且該範本可以用於不同的環境，例如測試、預備和生產環境。 您可以檢視整個群組的彙總成本，為您的組織釐清計費。 如需詳細資訊，請參閱 [Azure 資源管理員概觀](../azure-resource-manager/resource-group-overview.md)。 
   * **位置**。 為資料湖分析帳戶選取 Azure 資料中心。 
8. 選取 [ **釘選到「開始面板」**]。 這是遵循本教學課程的必要步驟。
9. 按一下 [建立] 。 它會帶領您前往入口網站「開始面板」。 新的磚會新增至首頁，具有顯示「部署 Azure 資料湖分析」的標籤。 建立 Data Lake Analytics 帳戶需要一些時間。 當建立帳戶之後，入口網站就會在新的刀鋒視窗上開啟帳戶。
