...
| N/A |資源群組 |***新增資料行。*** 部署的資源正在其中執行的資源群組。請參閱 [Azure Resource Manager 概觀](resource-group-overview.md) |
| 元件 |執行個體識別碼 |執行中資源的識別碼。識別碼包含在建立時為資源指定的名稱。 |
| N/A |標籤 |***新增資料行。*** Azure 中的新資源類型可讓您標記資源。請參閱[使用標籤來組織 Azure 資源](http://azure.microsoft.com/updates/organize-your-azure-resources-with-tags/) |
