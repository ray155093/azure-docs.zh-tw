...
2. 將下列新屬性新增至 **TodoItem** 類別：
   ...
   > [!NOTE]
   > 使用預設資料庫初始設定式時，當 Entity Framework 在 Code First 定義中偵測到資料模型變更時，就會捨棄並重新建立資料庫。若要進行此資料模型變更，並保有資料庫的現有資料，必須使用 Code First Migrations。無法針對 Azure 中的 SQL Database 使用預設的初始設定式。如需詳細資訊，請參閱[如何使用 Code First Migrations 更新資料模型](../articles/mobile-services/mobile-services-dotnet-backend-how-to-use-code-first-migrations.md) (英文)。
   > 
   > 
...
1. 如果您尚未建立儲存體帳戶，請參閱[如何建立儲存體帳戶]。
2. 在[ Azure 傳統入口網站中，按一下 [儲存體]、[儲存體帳戶]和 [管理金鑰](https://manage.windowsazure.com/)。
...
5. 在 Visual Studio 的 [方案總管] 中，為行動服務專案開啟 Web.config 檔案，並新增下列新應用程式設定，以您剛在入口網站中設定的儲存體帳戶名稱和存取金鑰來取代預留位置：
...
6. 在 Controllers 資料夾中，開啟 TodoItemController.cs 檔案並新增下列 **using** 指示詞：
   
       using System;
       using Microsoft.WindowsAzure.Storage.Auth;
       using Microsoft.WindowsAzure.Storage.Blob;
7. 使用下列程式碼來取代現有的 **PostTodoItem** 方法：
   ...
   > [!NOTE]
   > 此程式碼會為個別 BLOB 建立 SAS。如果您需要使用相同的 SAS 將多個 Blob 上傳至容器，您可以改用空白的 Blob 資源名稱呼叫 <a href="http://go.microsoft.com/fwlink/?LinkId=390455" target="_blank">generateSharedAccessSignature 方法</a>，如下所示：<pre><code>blobService.generateSharedAccessSignature(containerName, '', sharedAccessPolicy);</code></pre>
   > 
   > 
