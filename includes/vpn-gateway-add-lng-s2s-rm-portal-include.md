...
2. 在 [一切] 刀鋒視窗搜尋方塊中，輸入**區域網路閘道**，然後按一下以搜尋。 這會傳回清單。 按一下 [區域網路閘道] 以開啟刀鋒視窗，然後按一下 [建立] 開啟 [建立區域網路閘道] 刀鋒視窗。

  ![建立區域網路閘道](./media/vpn-gateway-add-lng-s2s-rm-portal-include/createlng.png)

3. 在 [建立區域網路閘道] 刀鋒視窗中，指定區域網路閘道的值。
...
4. 當您完成值的指定時，按一下刀鋒視窗底部的 [建立]，以建立區域網路閘道。