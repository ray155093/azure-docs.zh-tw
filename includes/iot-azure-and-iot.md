...
在此 IoT 解決方案架構中，裝置會將遙測 (例如幫浦站的感應器讀數) 傳送給雲端端點，以進行儲存和處理。 在預測性維護案例中，解決方案後端可能會使用感應器資料流，來判斷特定幫浦何時需要維護。 裝置也可以透過讀取來自雲端端點的訊息，以接收和回應雲端到裝置訊息。 比方說，在預測性維護案例中，解決方案後端可能會傳送訊息給幫浦站的其他幫浦，以在維護應開始之前先重新路由流量，這樣一來維護工程師到場時即可開始工作。
