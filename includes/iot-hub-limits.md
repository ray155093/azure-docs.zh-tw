...
> [!NOTE]
> 若您預期單一 S1、S2 或 S3 層的中樞會使用超過 200 個單位，請連絡 Microsoft 支援。
> 
> 
...
> [!NOTE]
> 如果您在 Azure 訂用帳戶中需要超過 10 個付費 IoT 中樞，請連絡 Microsoft 支援服務。
> 
> 
...
| 檔案上傳作業 |5000 個檔案上傳通知/最小值/單位 (適用於 S3)，100 個檔案上傳通知/最小值/單位 (適用於 S1 和 S2)。 <br/> 10000 個 SAS URI 可以讓 Azure 儲存體帳戶一次用盡。<br/> 10 個 SAS URI/裝置可以一次用盡。 |
...
| 作業的操作 <br/> (建立、更新、列出、刪除) | 5000/秒/單位 (適用於 S3)、100/秒/單位 (適用於 S2)、100/秒/單位 (適用於 S1) |
