...
4. 在 `Program` 類別中，新增或取代 (如果方法已存在) 下列方法：
   ...
   > [!NOTE]
   > 此後端程式碼支援 Windows 市集、Windows Phone、iOS 和 Android 用戶端。當尚未針對特定用戶端平台設定通知中樞時，傳送方法會傳回錯誤回應。
   > 
   > 
5. 在上述程式碼中，請使用您的通知中心名稱及先前取得的 *DefaultFullSharedAccessSignature* 連接字串，來取代 `<hub name>` 和 `<connection string with full access>` 預留位置。
6. 在 **[主要]** 方法中新增下列命令列。
