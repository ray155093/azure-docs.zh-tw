...
> [!NOTE]
> 儲存體模擬器所支援的驗證金鑰僅可用來測試用戶端驗證碼的功能。它不提供任何安全性用途。您無法將生產儲存體帳戶和金鑰與儲存體模擬器搭配使用。也請注意，您不應該將開發帳戶與生產資料搭配使用。
> 
> 請注意，儲存體模擬器只透過 HTTP 支援連線。不過，HTTPS 是建議的通訊協定，用於存取 Azure 生產儲存體帳戶中的資源。
> 
> 
