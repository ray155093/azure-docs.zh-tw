...
9. 在 [端點] 頁面上，指定您要提供給您的虛擬機器使用者使用的公用端點。例如，除了預設啟用的遠端桌面和 PowerShell 端點以外，您可以選擇啟用 HTTP (連接埠 80)。若要新增端點，請選擇在 [連接埠名稱] 下拉式清單方塊中選擇一個端點，然後選擇 [加入] 按鈕。若要移除端點，請選擇端點清單中名稱旁邊的紅色 **X**。
