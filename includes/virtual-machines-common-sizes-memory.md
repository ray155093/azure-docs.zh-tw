...
* M 系列可在雲端提供最高 CPU 計數 (最多 128 個 vCPU) 和最大記憶體 (最多 2.0 TiB) 的 VM。  它很適合用於受益於高 CPU 計數和大量記憶體的極大型資料庫或其他應用程式。
...
| 大小             | vCPU | 記憶體：GiB | 本機 SSD: GiB | 最大資料磁碟 | 最大快取和本機磁碟輸送量︰IOPS / MBps (以 GiB 為單位的快取大小) | 最大取消快取的磁碟輸送量︰IOPS / MBps | 最大 NIC/預期的網路效能 (Mbps) |
|------------------|-----------|-------------|----------------|----------------|-----------------------------------------------------------------------|-------------------------------------------|------------------------------------------------|
...
| Standard_E32_v3 | 32        | 256         | 512            | 32             | 64,000 / 512 (800)                                                    | 51,200 / 768                              | 8 / 極高                             |
| Standard_E64_v3 | 64        | 432         | 864            | 32             | 128,000/1024 (1600)                                                   | 80,000 / 1200                             | 8 / 極高                             |
...
| 大小 | CPU 核心 | 記憶體：GiB | 本機 SSD: GiB | 最大資料磁碟 | 最大快取和本機磁碟輸送量︰IOPS / MBps (以 GiB 為單位的快取大小) | 最大取消快取的磁碟輸送量︰IOPS / MBps | 最大 NIC/預期的網路效能 (Mbps) |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
...
| 大小         | CPU 核心 | 記憶體：GiB | 本機 SSD: GiB | 最大本機磁碟輸送量：IOPS / 讀取 MBps / 寫入 MBps | 最大資料磁碟 / 輸送量︰IOPS | 最大 NIC/預期的網路效能 (Mbps) |
|--------------|-----------|-------------|----------------|----------------------------------------------------------|-----------------------------------|------------------------------|
