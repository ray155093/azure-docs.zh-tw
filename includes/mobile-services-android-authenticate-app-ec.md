...
2. 將下列方法加入至 **ToDoActivity** 類別：
    ...
    private void authenticate() { // 使用 Google 提供者登入。
...
1. 在 **onCreate** 方法中，在具現化 `MobileServiceClient` 物件的程式碼後面新增下列這一行程式碼。
   
        authenticate();
   
    此呼叫會啟動驗證程序。
2. 將 **onCreate** 方法中 `authenticate();` 後面的其餘程式碼移至新的 **createTable** 方法，如下所示：
...
3. 從 [執行] 功能表，按一下 [執行] 來啟動應用程式，並以您選擇的身分識別提供者登入。
