...
3. 在 Microsoft Update Catalog 的 [搜尋] 方塊中，輸入您要下載的 Hotfix 的知識庫 (KB) 編號，例如 **3121901**，然後按一下 [搜尋]。
...
5. 搜尋上表中所列的其他任何 Hotfix (**3121900**、**3080728**、**3090322** 和 **3121899**)，然後新增每個購物籃。
6. 按一下 [**檢視購物籃**]。
...
3. 使用 `Get-HcsUpdateStatus` 命令來監視安裝進度。當 `RunInProgress` 變成 `False` 時，即代表更新完成。
4. 安裝完成之後，維護模式 Hotfix 安裝所在的控制器將會重新開機。以具有完整存取權的選項 1 登入，並驗證磁碟韌體版本。輸入：
   ...
   下方顯示一項範例輸出。
    ...
    請在第二個控制器上執行 `Get-HcsFirmwareVersion` 命令，來驗證軟體版本已經更新。然後您就可以結束維護模式。若要這麼做，請針對每個裝置控制器輸入以下命令：
   
   `Exit-HcsMaintenanceMode`
5. 當您離開維護模式時，控制器便會重新啟動。在磁碟韌體更新已成功套用且裝置已結束維護模式後，返回 Azure 傳統入口網站。請注意，入口網站可能不會顯示您已安裝維護模式更新 24 小時。
