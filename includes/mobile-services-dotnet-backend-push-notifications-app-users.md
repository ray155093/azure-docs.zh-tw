...
3. 以滑鼠右鍵按一下專案、按一下 [新增]，然後按一下 [類別...]。
4. 將新的空類別命名為 `PushRegistrationHandler`，然後按一下 [新增]。
5. 在程式碼頁面頂端，新增下列 **using** 陳述式：
...
6. 使用下列程式碼來取代現有的 **PushRegistrationHandler** 類別：
...
7. 展開 Controllers 資料夾、開啟 TodoItemController.cs 專案檔、找到 **PostTodoItem** 方法，然後使用下列程式碼來取代呼叫 **SendAsync** 的程式碼行：
