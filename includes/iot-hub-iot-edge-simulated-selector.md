...
Simulated Device Cloud Upload 範例示範如何使用 IoT Edge 建立閘道，以將遙測從模擬裝置傳送到 IoT 中樞。 模擬裝置無法直接連接到 IoT 中樞，因為︰

* 裝置不會使用 IoT 中樞所了解的通訊協定。
* 裝置不夠聰明到記住 IoT 中樞指派給它們的身分識別。

IoT Edge 閘道會使用下列方式來解決模擬裝置的這些問題︰
...
> [!NOTE]
> 模組不會彼此直接傳遞訊息。 模組會將訊息發佈到內部訊息代理程式，而內部代理程式會使用訂用帳戶機制將訊息傳遞到其他模組 (如下圖所示)。 如需詳細資訊，請參閱[開始使用 Azure IoT Edge][lnk-gw-getstarted]。
> 
> 
