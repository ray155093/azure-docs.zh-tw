## <a name="receive-messages-with-apache-storm"></a>使用 Apache Storm 接收訊息
...
如需事件中樞接收模式的詳細資訊，請參閱[事件中樞概觀][事件中樞概觀]。

本教學課程使用 [HDInsight Storm][HDInsight Storm] 安裝，其包含在已可使用的事件中樞 Spout 中。

1. 請遵循 [HDInsight Storm - 入門](../articles/hdinsight/hdinsight-storm-overview.md) 程序來建立新的 HDInsight 叢集，並透過遠端桌面與其連線。
