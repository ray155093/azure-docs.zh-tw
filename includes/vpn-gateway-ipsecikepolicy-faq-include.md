...
Azure 的**標準**和 **HighPerformance** VPN 閘道可支援自訂 IPsec/IKE 原則。 **基本** SKU。
...
| 流量選取器 | UsePolicyBasedTrafficSelectors** ($True/$False)                             |
|                  |                                                                             |
...
是，若連線指定了自訂原則，Azure VPN 閘道只會使用該連線的原則，來同時作為 IKE 啟動器和 IKE 回應程式。
