### <a name="a-nameserver-authahow-to-authenticate-with-a-provider-server-flow"></a><a name="server-auth"></a>作法：向提供者驗證 (伺服器流程)
若要讓 Mobile Apps 管理應用程式中的驗證程序，您必須向身分識別提供者註冊應用程式。 接著在您的 Azure App Service 中，您必須設定提供者所提供的應用程式識別碼和密碼。
如需詳細資訊，請參閱 [將驗證新增至您的應用程式](../articles/app-service-mobile/app-service-mobile-ios-get-started-users.md)教學課程。
...
###<a name="a-nameclient-authahow-to-authenticate-with-a-provider-client-flow"></a><a name="client-auth"></a>作法：向提供者驗證 (用戶端流程)
...
###<a name="a-nameauth-getinfoahow-to-obtain-information-about-the-authenticated-user"></a><a name="auth-getinfo"></a>作法：取得已驗證使用者的相關資訊
