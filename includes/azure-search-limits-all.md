...
索引：每個設定檔的函數數量上限|8|8|8|8 
索引子：每個叫用的索引負載上限|10,000 份文件|只受文件數量上限的限制|只受文件數量上限的限制|只受文件數量上限的限制 
索引子：執行時間上限|3 分鐘|24 小時|24 小時|24 小時 
Blob 索引子：Blob 大小上限，MB|16|16|128|256 Blob 索引子：從 Blob 擷取的內容字元上限|32,000|64,000|400 萬|400 萬 
每秒查詢數量 (QPS) <sup>2</sup>|無|每個複本 ~3 個|每個複本 ~15 個|每個複本 ~60 個 相應放大：搜尋單位 (SU) 數量上限 <sup>3</sup>|無|最多 3 個單位 (3 個複本及 1 個分割區)|36 個單位|36 個單位 
定價 <sup>4</sup>|無|每個月中的每個 SU 美金 $75 |每個月中的每個 SU 美金 $250 |每個月中的每個 SU 美金 $1000
