...
1. 根據預設，此解決方案入口網站會顯示儀表板。 使用頁面左側的功能表，瀏覽至解決方案入口網站的其他區域。
...
1. 選擇 [讓我定義自己的裝置識別碼]。 輸入 [裝置識別碼]，例如 **device01**，按一下 [檢查識別碼] 以確認您尚未在解決方案中使用此名稱，然後按一下 [建立] 來佈建裝置。

   ![新增裝置識別碼][3]

1. 記下裝置認證 ([裝置識別碼]、[IoT 中樞主機名稱] 及 [裝置金鑰])。 Intel NUC 上的用戶端應用程式需要這些值，才能連線到遠端監視解決方案。 然後按一下 [完成]。
