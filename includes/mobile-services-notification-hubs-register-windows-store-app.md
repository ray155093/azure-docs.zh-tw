...
12. 按一下 [推播] 索引標籤，輸入您從 WNS 取得的 [用戶端密碼] 和 [封裝 SID] 值，然後按一下 [儲存]。
    ...
    > [!NOTE]
    > 當您使用舊版行動服務完成這個教學課程時，可能會在 [推送] 索引標籤底部看見名為 [啟用進階推送] 的連結。立即按一下這個連結，即可升級您的行動服務來與通知中樞整合。這個變更無法回復。如需如何在生產行動服務中啟用進階推播通知的詳細資訊，請參閱<a href="http://go.microsoft.com/fwlink/p/?LinkId=391951">此指引</a>。
    > 
    > 
    ...
    > [!NOTE]
    > 當您在入口網站的 [推播] 索引標籤中設定進階推播通知的 WNS 認證時，這些認證將會與通知中樞共用，以設定您應用程式適用的通知中樞。
    > 
    > 
