...
4. 以下列程式碼取代 **DataTemplate** 中的 **StackPanel** 元素：
...
5. 在 Windows Phone 應用程式專案中，開啟 MainPage.xaml 檔案，然後以下列程式碼取代 **ButtonSave** 元素：
...
6. 以下列程式碼取代 **DataTemplate** 中的 **StackPanel** 元素：
...
7. 在共用的 DataModel 資料夾中，開啟 TodoItem.cs project 檔案，然後將下列屬性新增至 TodoItem 類別：
...
8. 開啟共用的 MainPage.cs 專案檔案並新增下列 **using** 陳述式：
...
10. 以下列程式碼取代現有 `InsertTodoItem` 方法：
...
7. 點選預覽以拍照，然後按一下 [**上傳**] 以插入新項目及上傳影像。
