...
4. 回到 [建立行動服務] 對話方塊，選取您的 [訂閱]、[執行階段] 中的 [JavaScript] 後端，以及您行動服務的 [區域]，然後輸入行動服務的 [名稱]。
   
   > [!NOTE]
   > 行動服務名稱必須是唯一的。當您提供的名稱無法使用時，**[名稱]** 旁邊便會顯示一個紅色 X。
   > 
   > 
5. 在 [資料庫] 中選取 [&lt;建立免費的 SQL Database&gt;]，提供 [伺服器使用者名稱]、[伺服器密碼] 和 [伺服器密碼確認]，然後按一下 [建立]。
...
1. 建立行動服務後，請從 [服務管理員] 中的清單選取新建立的行動服務，然後按一下 [確定]。
