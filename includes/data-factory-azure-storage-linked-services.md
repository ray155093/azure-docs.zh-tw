...
請參閱下列文章的步驟來檢視/複製 Azure 儲存體帳戶金鑰：[檢視、複製和重新產生儲存體存取金鑰](../articles/storage/storage-create-storage-account.md#view-copy-and-regenerate-storage-access-keys)。
...
* 「到期時間」必須適當地設定。請確定 Azure 儲存體物件的存取權不會在管線的作用中期間內過期。
